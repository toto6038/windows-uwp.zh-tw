...
如需在 Windows 應用程式中以不同方式使用語音互動的詳細資訊，請參閱[語音互動](/windows/uwp/design/input/speech-interactions)。
