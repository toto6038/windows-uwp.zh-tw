...
建立原生 Android 應用程式的最簡單方式是使用 Android Studio 搭配 [JAVA 或 Kotlin](#java-or-kotlin)，不過，如果您有特定用途，也可以 [使用 c 或 c + + 進行 Android 開發](#use-c-or-c-for-android-game-development) 。 Android Studio SDK 工具會將您的程式碼、資料和資源檔編譯為封存 Android 套件 apk 檔案。 其中一個 APK 檔案包含 Android 應用程式的所有內容，而且是 Android 裝置用來安裝應用程式的檔案。
...
遵循 Android Studio 中的安裝程式，並安裝所建議的任何 SDK 套件。 當新的工具和其他 api 可供使用時，Android Studio 會透過快顯視窗通知您，**或選取 [** 說明檢查] 以查看是否有更新  >  ** **。
...
- 深入瞭解如何選取 Android Studio 檔中的 [活動範本](https://developer.android.com/studio/projects/templates#SelectTemplate) 。
...
**Kotlin** 已先宣佈為新的開放原始碼語言，JetBrains 于2011中，並已納入為 Android Studio 自2017起的 JAVA 替代方案。 在5月2019日，Google 宣佈 Kotlin 是 Android 應用程式開發人員慣用的語言，因此即使是較新的語言，它也有強大的支援小組，並已識別為最快速成長的程式設計語言之一。 Kotlin 是跨平臺、靜態型別，而且是設計來與 JAVA 完全互通。
...
![Android Studio API 選取範圍最小值畫面](../images/android-minimum-api-selection.png)

選取 [ **協助我選擇** ] 連結以開啟比較圖表，其中顯示與平臺版本發行相關聯的裝置支援分佈和主要功能。

![Android Studio API 比較最小值畫面](../images/android-minimum-api-selection-2.png)
...
Android Studio 的 [ **專案** ] 視窗包含下列檔案 (請確定已從下拉式功能表選取 [Android 視圖]) ：
...
定義活動使用者介面配置的 XML 檔案 (UI) 。 它包含文字為 "Hello World" 的 TextView 元素
...
### <a name="fluent-design-system-for-android"></a>適用于 Android 的 Fluent Design 系統

Microsoft 也提供設計指引，目的是要在整個 Microsoft 行動應用程式組合中提供順暢的體驗。

[適用于 android 的 Fluent Design 系統](https://www.microsoft.com/design/fluent/#/android) 設計，並建立原生 Android 的自訂應用程式，但仍可獨特地流暢。
...
- [Android 消費者介面指導方針](https://developer.android.com/design/)
