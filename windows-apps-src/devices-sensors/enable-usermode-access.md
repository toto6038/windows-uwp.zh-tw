...
Windows 上的低層級匯流排的使用者模式存取是透過現有的和架構進行檢測 `GpioClx` `SpbCx` 。 新的驅動程式稱為*RhProxy*（可在 Windows IoT 核心版和 windows Enterprise 上取得），會 `GpioClx` `SpbCx` 向使用者模式公開和資源。 若要啟用 Api，必須在您的 ACPI 資料表中宣告 rhproxy 的裝置節點，其中包含應該向使用者模式公開的每個 GPIO 和 SPB 資源。 本文件會逐步解說關於編寫與驗證 ASL 的資訊。
...
若要將 `SerCx` 樣式的 UART 公開至使用者模式，請 `UARTSerialBus` 如下所示宣告資源。
...
- 針腳多工處理伺服器 – 這些是控制針腳多工處理控制區塊的驅動程式。 Pin muxing 伺服器會透過要求來接收來自用戶端的 pin muxing 要求，以保留 muxing 資源（透過*IRP_MJ_CREATE*）要求，以及要求切換 pin 的功能（透過 * IOCTL_GPIO_COMMIT_FUNCTION_CONFIG_PINS 要求）。 由於多工處理區塊有時為 GPIO 區塊的一部分，因此針腳多工處理伺服器通常為 GPIO 驅動程式。 即使多工處理區塊為個別周邊裝置，GPIO 驅動程式亦是放置多工處理功能的邏輯位置。
...
本節說明如何提供多工處理資源至用戶端驅動程式。 請注意，您必須具有 Microsoft ASL 編譯器建置 14327 或更新版本，才能編譯包含 `MsftFunctionConfig()` 資源的表格。 `MsftFunctionConfig()`系統會提供資源來將 muxing 用戶端作為硬體資源來釘選。 `MsftFunctionConfig()`資源應該提供給需要 pin muxing 變更的驅動程式（通常是 SPB 和串列控制器驅動程式），但不應該提供給 SPB 和串列週邊驅動程式，因為控制器驅動程式會處理 muxing 設定。
...
`GpioClx`具有內建的 pin muxing 支援。 GpioClx 迷你連接埠驅動程式 (亦稱為「GpioClx 用戶端驅動程式」) 驅動 GPIO 控制器硬體。 自 Windows 10 組建 14327 開始，GpioClx 迷你連接埠驅動程式可透過實作兩個新 DDI，新增針腳多工處理支援：
...
例如，假設針腳的預設多工處理設定為 UART，且針腳亦可用作 GPIO。 當呼叫 CLIENT_ConnectIoPins 以連線 GPIO 針腳時，它應會將針腳多工處理至 GPIO，且在 CLIENT_DisconnectIoPins 中應會將針腳多工回復為 UART。 一般而言，中斷連線常式應會復原 Connect 常式所完成的作業。
...
`SpbCx`在叫用控制器驅動程式的[EvtSpbTargetDisconnect （）](https://docs.microsoft.com/windows-hardware/drivers/ddi/content/spbcx/nc-spbcx-evt_spb_target_disconnect)回呼之後，就會在其*IRP_MJ_CLOSE*處理常式中還原 pin muxing 設定。 最後，針腳會在周邊裝置驅動程式開啟 SPB 控制器驅動程式控制代碼時多工處理至 SPB 功能，並在周邊裝置驅動程式關閉控制代碼時結束多工處理。

`SerCx`行為類似。 `SerCx`會 `MsftFunctionConfig()` 在叫用控制器驅動程式的[EvtSerCx2FileOpen （）](https://docs.microsoft.com/windows-hardware/drivers/ddi/content/sercx/nc-sercx-evt_sercx2_fileopen)回呼之前，取得其*IRP_MJ_CREATE*處理常式中的所有資源，並在叫用控制器驅動程式的[EvtSerCx2FileClose](https://docs.microsoft.com/windows-hardware/drivers/ddi/content/sercx/nc-sercx-evt_sercx2_fileclose)回呼之後釋放其 IRP_MJ_CLOSE 處理常式中的所有資源。
...
1. 使用編譯和載入您的 rhproxy 節點`ACPITABL.dat`
1. 確認 `rhproxy` 裝置節點存在
1. 確認 `rhproxy` 載入並啟動
1. 確認預期的裝置已公開至使用者模式
1. 確認您可以從命令列與每部裝置互動
...
2. 下載[WDK](https://docs.microsoft.com/windows-hardware/drivers/download-the-wdk) `asl.exe` 並尋找`C:\Program Files (x86)\Windows Kits\10\Tools\x64\ACPIVerify`
...
複製存放 [https://github.com/ms-iot/samples](https://github.com/ms-iot/samples) 庫，並建立 `GpioTestTool` 、 `I2cTestTool` 、 `SpiTestTool` 和 `Mincomm` 範例。 複製工具到進行測試的裝置，並使用下列命令列舉裝置。
...
- [MinComm (序列)](https://github.com/microsoft/Windows-iotcore-samples/tree/6e473075bbe616e4d9ce90e67c6412fba661c337/BusTools/MinComm)
...
另請參閱[Raspberry Pi 2 & 3 Pin](/windows/iot-core/learn-about-hardware/pinmappings/pinmappingsrpi)對應
...
另請參閱[MinnowBoard 最大 Pin](/windows/iot-core/learn-about-hardware/pinmappings/pinmappingsmbm)對應
