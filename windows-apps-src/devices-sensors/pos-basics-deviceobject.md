...
您可以使用 \<DeviceObject\>，偵測物件是否因任何原因而關閉。關閉的事件處理常式會接著記下裝置識別碼，以在未來重新建立連接。   或者，您可能想要在「App 暫停」通知上處理這個，以保留裝置 ID 用於在「App 繼續執行」通知上重新建立裝置連接。  請確定您不會在 \<DeviceObject\>的事件處理常式和裝置物件的重複動作上加倍。已關閉和應用程式暫停。

> [!TIP]
> 有關 Windows 10 通用 Windows 平台 (UWP) 應用程式週期的詳細資訊，請參閱下列主題：
> - [Windows 10 通用 Windows 平臺（UWP）應用程式生命週期](../launch-resume/app-lifecycle.md)
> - [處理應用程式暫停](../launch-resume/suspend-an-app.md)
