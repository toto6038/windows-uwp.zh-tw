...
服務點、銷售點或 POS 裝置是用來協助零售交易的電腦週邊設備。 例如，電子收銀機、條碼掃描器、磁條讀取器和收據印表機便是 POS 裝置。

您將從本文了解使用通用 Windows 平台 (UWP) POS API 銜接 POS 裝置的基本概念。 我們會討論裝置列舉、檢查裝置功能、宣告裝置和裝置共用。 我們使用條碼掃描器裝置做為範例，但幾乎這裡所有的指引都適用於任何 UWP 相容 POS 裝置  (如需所支援裝置的清單，請參閱 [POS 裝置支援](pos-device-support.md))。

## <a name="finding-and-connecting-to-pos-peripherals"></a>找出和連接至 POS 週邊設備

POS 裝置必須先與執行應用程式的電腦進行配對，應用程式才能使用該裝置。 有幾個方式可以連接至 POS 裝置，若不是藉由程式設計的方式，就是透過 [設定] App。

### <a name="connecting-to-devices-by-using-the-settings-app"></a>使用 [設定] App 連接至裝置
條碼掃描器等 POS 裝置接上電腦時，該裝置就像任何其他裝置一樣地出現其中。 您可以在 \[設定\] App 的 **\[裝置\] > \[藍牙與其他裝置\]** 區段中找到。 您可以在那裡選取 **\[新增藍牙或其他裝置\]**，與 POS 裝置進行配對。

有些 POS 裝置必須使用 POS API，以程式設計方式來列舉，否則這些裝置無法在 [設定] App 中出現。

### <a name="getting-a-single-pos-device-with-getdefaultasync"></a>使用 GetDefaultAsync 取得單一 POS 裝置
在簡單的使用案例中，可能只有一部 POS 週邊設備連接至 App 執行所在的電腦，您想要盡快加以設定。 若要這樣做，請使用 **GetDefaultAsync** 方法來擷取預設裝置，如下所示。
...
> [!NOTE] 
> 如果有多個 POS 裝置連接至電腦，**GetDefaultAsync** 會傳回找到的第一個裝置。 因此，除非您確定應用程式只有顯示一個 POS 裝置，否則請使用 **FindAllAsync**。
...
連接至裝置時，您可能會想要將搜尋限制在 App 可存取之 POS 週邊設備的子集。 您可以使用 **GetDeviceSelector** 方法，將選取範圍限定為僅擷取透過特定方法 (藍牙、USB 等) 連接的裝置。 您可以建立選取器，搜尋透過 **\[藍牙\]**、**\[IP\]**、**\[本機\]** 或 **\[所有連接類型\]** 連接的裝置。 這相當實用，因為相較於本機 (有線) 探索，無線裝置探索需要很長的時間。 您可以將 **FindAllAsync** 限制為 **\[本機\]** 連接類型，確保本機裝置連線的等待時間可確定。 例如，此程式碼會擷取所有可透過本機連線存取的條碼掃描器。 
...
## <a name="checking-the-capabilities-of-a-pos-device"></a>檢查 POS 裝置的功能
...
**Device.Capabilities** 類別是所有 POS 裝置類別的屬性，可用來取得關於每個裝置的一般資訊。 例如，此範例會判斷裝置是否支援統計資料報告，若不支援，則擷取任何受支援類型的統計資料。
...
## <a name="claiming-a-pos-device"></a>宣告 POS 裝置
在 POS 裝置可供作用中輸入或輸出使用之前，您必須宣告該裝置，並授與應用程式對其許多功能的獨佔存取權。 此範例示範如何在您使用上述其中一個方法找到裝置後，宣告條碼掃描器裝置。
...
透過網路或藍牙連線使用 POS 裝置，您可能希望與網路上的其他應用程式共用該裝置  (如需詳細資訊，請參閱＜共用裝置＞[link])。有時候，您可能想要佔住裝置長時間使用。 此範例示範如何在其他應用程式要求釋放已宣告的條碼掃描器之後保留該裝置。
...
POS 裝置經常用於多個 App 必須在一小段時間內存取這些裝置的案例。  在本機 (USB 或其他有線連線) 或透過藍牙或 IP 網路連接至多個 App 時，可以共用裝置。 視每個 App 的需求而定，某一個處理程序可能需要在裝置上處置其宣告。 這個程式碼會處置我們宣告的條碼掃描器裝置，並允許其他 App 宣告和使用該裝置。
...
> [!NOTE]
> 已宣告和未宣告的 POS 裝置類別都會實作 [IClosable 介面](https://docs.microsoft.com/en-us/uwp/api/windows.foundation.iclosable)。 如果裝置透過網路或藍牙應用程式連接至 App，則必須先處置已宣告及未宣告的物件，其他的應用程式才能進行連接。
