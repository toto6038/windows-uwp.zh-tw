...
| **近似** | 此資料對部分應用程式而言已經足夠準確。 如果虛擬實境 app 只需了解使用者是否已將裝置往上/下或往左/右移動，則該 app 不需要校正也能繼續執行。 需要絕對方向的 app (例如導航 app，該 app 需要知道您正在行駛的方向，才能為您提供方向) 就需要提出校正的要求。 |
