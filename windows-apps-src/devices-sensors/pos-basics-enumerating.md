...
<br/>
顯示裝置選擇器 UI，並讓使用者選擇連接的裝置。 這個方法會處理更新清單，當裝置連接並移除，並會更簡單、 更安全，比其他方法。
...
使用**GetDeviceSelector**方法來接受[PosConnectionTypes](https://docs.microsoft.com/uwp/api/windows.devices.pointofservice.posconnectiontypes)值做為參數，您可以限制您的選取器列舉本機、 網路或附加的藍牙 POS 裝置，減少完成查詢所花的時間。  下列範例顯示使用的這個方法，以定義選取器支援只會在本機連接的 POS 印表機。
