...
**方法 1:**[使用的裝置選擇器](#method-1:-use-a-device-picker)
<br/>
顯示的裝置選擇器 UI，並讓使用者選擇連接的裝置。 這個方法會處理更新清單，當裝置連接並移除，並會更簡單、 更安全比其他方法。
...
**方法 3:**[裝置的快照](#Method-2:-Snapshot-of-devices)<br />列舉的時間出現在某個指定點的系統的服務點裝置的快照。 您想要建置自己的 UI，或者需要在不向使用者顯示 UI 的情況下列舉裝置時，這非常實用。 [FindAllAsync](https://docs.microsoft.com/uwp/api/windows.devices.enumeration.deviceinformation.findallasync)會保留回結果直到整個列舉完成。
...
**GetDeviceSelector**方法，針對不同的裝置類型如下：
...
使用**GetDeviceSelector**方法來接受[PosConnectionTypes](https://docs.microsoft.com/uwp/api/windows.devices.pointofservice.posconnectiontypes)值做為參數，您可以限制您的選取器列舉本機、 網路或藍牙連接的 POS 裝置，減少完成查詢所需的時間。  下列範例顯示的這個方法，以定義只會在本機支援選取器使用連接的 POS 印表機。
...
若要取得的服務點裝置的最簡單方式是使用**GetDefaultAsync**取得的服務點裝置類別中的第一個可用裝置。 

下列範例說明[GetDefaultAsync](https://docs.microsoft.com/uwp/api/windows.devices.pointofservice.barcodescanner.getdefaultasync#Windows_Devices_PointOfService_BarcodeScanner_GetDefaultAsync)適用於[BarcodeScanner](https://docs.microsoft.com/uwp/api/windows.devices.pointofservice.barcodescanner)的使用。 編碼模式都相似所有服務點裝置類別。
...
> [!CAUTION]
> 因為它可能會從一個工作階段將不同的裝置，前往下一**GetDefaultAsync**必須小心使用。 許多事件可能影響這個列舉，導致第一個可用裝置不同，包括： 
> - 變更連接到您電腦的相機 
