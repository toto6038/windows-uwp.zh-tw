...
> [!NOTE]  
> .3mf 檔案類型提供大量的功能，但本教學課程無法涵蓋。 若要深入了解 3MF 及它提供給 3D 產品生產者與消費者的功能，請參閱 [3MF 規格](http://3mf.io/what-is-3mf/3mf-specification/)。 若要了解如何以 Windows 10 運用這些功能，請參閱[產生 3MF 套件](https://msdn.microsoft.com/windows/uwp/devices-sensors/generate-3mf)教學課程。

幸運的是，[3D Builder](https://www.microsoft.com/store/apps/3d-builder/9wzdncrfj3t6) app 可以開啟大部分熱門的 3D 格式，並另存為 .3mf 檔案。 在這個範例中，當檔案類型有所不同時，一種非常簡單的解決方案是開啟 3D Builder 並提示使用者將匯入的資料另存為 .3mf 檔案，然後重新載入。
...
**Printing3DModel** 物件現在已經修復且可供列印。 當建立類別時，使用 [**SaveModelToPackageAsync**](https://msdn.microsoft.com/library/windows/apps/windows.graphics.printing3d.printing3d3mfpackage.savemodeltopackageasync) 將模型指派到您建立類別時宣告的 **Printing3D3MFPackage** 物件。
