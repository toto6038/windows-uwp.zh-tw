...
使用 Windows 10 的企業應該強烈考慮使用[Microsoft Passport 和 Windows Hello](microsoft-passport.md)，這會啟用高度安全的雙因素驗證。 如果您無法使用 Microsoft Passport，[智慧卡](smart-cards.md)和[指紋生物特徵辨識技術](fingerprint-biometrics.md)也能提供額外的安全性。
...
<tr><td><a href="companion-device-unlock.md">使用隨附 IoT 裝置的 Windows 解除鎖定</a></td><td>隨附裝置是可與您的 Windows 10 Desktop 搭配使用，以增強使用者驗證體驗的裝置。 使用隨附的裝置架構，即使 Windows Hello 無法使用（例如，如果 Windows 10 桌上型電腦缺少臉部驗證或指紋辨識器裝置的相機），隨附的裝置也可以為 Microsoft Passport 提供豐富的體驗。範例）。</td></tr>
