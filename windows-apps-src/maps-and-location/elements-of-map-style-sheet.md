...
## <a name="entries"></a>項目
此表格使用 ">" 字元代表項目階層層級。  它也會顯示哪些版本的 Windows 支援的每個項目，其中忽略它。
...
| >> area                      | [MapElement](#mapelement) |  ✔   |  ✔   |  ✔   |  ✔   | 利用描述土地的區域。  這些不應該與實體建築物這是在結構項目下混淆。 |
...
| >>> address                  | [PointStyle](#pointstyle) |      |      |  ✔   |  ✔   | 解決數字的標籤。 |
...
| >>>>> entertainmentPoint     | [PointStyle](#pointstyle) |      |  ✔   |  ✔   |  ✔   | 代表娛樂場合例如劇場、 cinemas 等等的圖示。 |
...
| rasterRegionsVisible         | Bool    |  ✔   |  ✔   |  ✔   |  ✔   | 旗標，表示要繪製點陣區域他們有更好的表示法，比向量 （日本及韓國）。 |
