...
本文章示範如何列舉 MIDI (樂器數位介面) 裝置，並且從通用 Windows app 傳送及接收 MIDI 訊息。 Windows 10 支援透過 USB (符合類別規範和大部分專屬驅動程式) 、透過藍牙的 MIDI (Windows 10 周年版和更新版本，以及透過乙太網路和路由式 MIDI 等可自由使用的協力廠商產品。
...
[**Windows.Devices.Enumeration**](/uwp/api/Windows.Devices.Enumeration) 命名空間提供 [**DeviceWatcher**](/uwp/api/Windows.Devices.Enumeration.DeviceWatcher)，它可以在系統中新增或移除裝置時，或裝置的資訊更新時，通知您的 app。 因為已啟用 MIDI 的 app 通常會想要輸入和輸出裝置，這個範例會建立實作 **DeviceWatcher** 的協助程式類別，以便相同的程式碼可以用於 MIDI 輸入和 MIDI 輸出裝置，而不需要重複。

將新的類別新增至您的專案做為裝置監控程式。 在此範例中，類別名為 **MyMidiDeviceWatcher**。 在本節中的其餘程式碼是用來實作協助程式類別。

將部分成員變數新增至類別：

-   [**DeviceWatcher**](/uwp/api/Windows.Devices.Enumeration.DeviceWatcher) 物件，該物件會監視裝置變更。
...
呼叫 [**DeviceInformation.CreateWatcher**](/uwp/api/windows.devices.enumeration.deviceinformation.createwatcher) 以建立 **DeviceWatcher** 類別的新執行個體，傳入 MIDI 裝置選取器字串。
...
-   [**Added**](/uwp/api/windows.devices.enumeration.devicewatcher.added) - 在新的裝置新增至系統時引發。
-   [**Removed**](/uwp/api/windows.devices.enumeration.devicewatcher.removed) - 在從系統移除裝置時引發。
-   [**Updated**](/uwp/api/windows.devices.enumeration.devicewatcher.updated) - 在與現有裝置相關聯的資訊更新時引發。
-   [**EnumerationCompleted**](/uwp/api/windows.devices.enumeration.devicewatcher.enumerationcompleted) - 在監控程式已完成其要求裝置類型的列舉時引發。
...
建立 [**MidiInPort**](/uwp/api/Windows.Devices.Midi.MidiInPort) 物件，代表選取的輸入裝置，方法是呼叫 [**MidiInPort.FromIdAsync**](/uwp/api/windows.devices.midi.midiinport.fromidasync)，傳入選取的裝置的 [**Id**](/uwp/api/windows.devices.enumeration.deviceinformation.id) 屬性。
...
1.  在**方案總管**的專案底下，以滑鼠右鍵按一下 [**參考**]，然後選取 [**加入參考**]。
2.  展開 [ **通用 Windows** ] 節點。
3.  選取 [擴充功能]。
