---
author: normesta
Description: "為您現有的 Windows Forms、WPF 或 Win32 應用程式或遊戲建立現代化 Windows 應用程式套件。 新增適用於 Windows 10 使用者的現代化體驗以及簡化部署和獲利。"
...
運用現有的傳統型應用程式，新增適用於 Windows 10 使用者的現代化體驗。 接著，透過 Windows 市集散佈您的應用程式，達到跨國際市場的更大覆蓋率。 您可以利用市集內建的功能，以更簡單的方式從您的應用程式獲利。 當然，您不一定需要使用市集。 歡迎使用您現有的通道。
<div style="float: left; padding: 10px">
    ![傳統型轉 UWP 橋接器影像](images/desktop-to-uwp/desktop-bridge-4.png)
</div>
傳統型轉 UWP 橋接器是我們平台內建的基礎結構，可讓您使用現代化 Windows 應用程式套件，有效率地散佈您的 Windows Forms、WPF 或 Win32 傳統型應用程式或遊戲。
...
**簡化部署**。 使用橋接器的應用程式和遊戲具備了良好的部署體驗。 此種體驗可讓使用者充滿信心的安裝和更新應用程式。 如果使用者選擇解除安裝應用程式，系統將會完全移除它，且不會留下任何痕跡。 這能縮短編寫安裝體驗的時間，並讓使用者擁有最新的更新。

**自動更新和授權**。 您的 App 可以參與 Windows 市集的內建授權和自動更新功能。 由於自動更新只會下載檔案已變更的部分，因此是一項非常可靠又有效率的機制。

**增加觸達對象並簡化營利**。 選擇透過 Windows 市集來散布，可讓您的觸達對象擴展至數百萬的 Windows10 使用者，而他們可以利用當地的付款選項來取得應用程式、遊戲，並進行應用程式內購買。
...
您計畫要將您的應用程式發行至 [Windows 市集](https://www.microsoft.com/store/apps)嗎？ 若是如此，請先填寫[此表單](https://developer.microsoft.com/windows/projects/campaigns/desktop-bridge)。 Microsoft 將會與您連絡以啟動上架程序。 作為這個程序的一部分，您將會在市集中保留一個名稱，並取得您需要用來建立 Windows 應用程式套件的資訊。
...
Desktop App Converter 也會為您完成其他幾個項目。 以下是一些範例。
...
請參閱[使用 Desktop App Converter 封裝應用程式 (傳統型轉 UWP 橋接器)](desktop-to-uwp-run-desktop-app-converter.md)

### <a name="manual-packaging"></a>手動封裝

若您喜歡細微控制您的轉換，您可以建立一個資訊清單檔，然後執行 **MakeAppx.exe** 工具來建立您的 Windows 應用程式套件。

此方法在您對您安裝程式所做出的系統變更非常熟悉的情況下，或是您沒有安裝程式，且安裝您的應用程式基本上可透過如 **xcopy** 等指令將檔案直接複製到一個資料夾位置完成的情況下，將會比較合理。 但我們建議您：不要因為沒有安裝程式就直接選擇使用手動方式封裝您的應用程式。 即使您沒有安裝程式，您還是可以使用 Desktop App Converter 封裝您的應用程式。

請參閱[手動封裝應用程式 (傳統型轉 UWP 橋接器)](desktop-to-uwp-manual-conversion.md)。

### <a name="visual-studio"></a>Visual Studio

此選項與上述手動轉換選項相似，但 Visual Studio 會為您完成幾項工作，例如：產生您應用程式的應用程式套件和視覺資產。 建議您將 Visual Studio 視為可利用幾項便利措施協助您手動封裝應用程式的工具。

請參閱[使用 Visual Studio 封裝 .NET 應用程式 (傳統型轉 UWP 橋接器)](desktop-to-uwp-packaging-dot-net.md)

### <a name="third-party-installer"></a>協力廠商安裝程式

 一些受歡迎的協力廠商產品及安裝程式現在也支援傳統型轉 UWP 橋接器。 您可以使用他們來產生 MSI 安裝程式，或是透過只按幾下來完成應用程式套件。 雖然我們不會提供關於這些工具使用方式的說明文件，您還是可以瀏覽他們的網站以深入了解。
...
<div style="float: left; padding: 10px; width: 20%">
     ![Advanced Installer 標誌](images/desktop-to-uwp/Advanced_Installer_Vertical.png)
</div>
...
對於其企業營運應用程式不相容於 Windows 10 和 Windows 10 S 的企業客戶，Cloudhouse Compatibility Containers 可將 Windows XP 和 Windows 7 應用程式轉換成 UWP，然後透過商務用 Windows 市集或 Microsoft Intune 提供，而無須更動原始碼。
<div style="float: left; padding: 10px; width: 20%">
     ![Cloudhouse-Compatibility-Containers](images/desktop-to-uwp/container.png)
</div>
Cloudhouse 提供 Auto Packager，適用於任何應用程式並可在其目前執行的系統 (例如 Windows XP) 上封裝應用程式，藉以建立 Compatibility Container。 接著可將容器整合至 Desktop Bridge Converter 工具以建立 Windows 應用程式套件，然後轉換成新的 UWP 格式。

Auto Packager 使用安裝/擷取和執行階段分析來建立應用程式的容器，其中包括應用程式檔案及登錄，以及能讓應用程式在 Windows 10 上執行的相容性及重新導向引擎。 加上您也可以包含、以及隔離執行應用程式所需的任何執行階段或必要條件，因此不會影響或與其他已經安裝的應用程式或執行階段產生衝突。


請查看公告支援通用 Windows app 和商務用 Windows 市集的[部落格](http://www.cloudhouse.com/resources/release-solution-to-get-any-line-of-business-app-to-uwp)文章。
...
<div style="float: left; padding: 10px; width: 20%">
     ![Advanced Installer 標誌](images/desktop-to-uwp/FG3rdPartyLogo.png)
</div>
...
<div style="float: left; padding: 10px; width: 20%">
    ![FireGiant 標誌](images/desktop-to-uwp/installaware.png)
</div>
...
[進階 APPX 建立選項](https://www.installaware.com/mhtml5/desktop/appx.htm)可協助您針對 Windows 市集提交，或產生已簽署的 Windows 應用程式套件二進位檔以便側載散佈給使用者。 您甚至可以透過單一來源建立 **WSA** (Windows Server 應用程式) 安裝程式套件並鎖定部署至 **Nano Server**，以及除了 GUI 以外，還能獲得[命令列自動化](https://www.installaware.com/scripting-automation-interface.htm)的完整支援。
...
<div style="float: left; padding: 10px; width: 20%">
    ![InstallShield 標誌](images/desktop-to-uwp/InstallShield-logo.jpg)
</div>
數秒內即可掃描您的 InstallShield 專案，自動找出的應用程式和 UWP 與 WSA 套件之間潛在的相容性問題，節省數小時的調查工作。

從現有的 InstallShield 專案建置 UWP 應用程式套件，替 Windows 市集做好準備並簡化您軟體在 Windows 10 上的安裝體驗。 可同時建置 Windows Installer 和 UWP 應用程式套件，支援您客戶想要的全部部署案例。 從您現有的 InstallShield 專案建置 WSA 套件，支援 Nano Server 與 Windows Server 2016 部署。

在模組中開發安裝，讓部署及維護更加輕鬆，然後在建置階段將元件和相依性合併至適用於 Windows 市集的單一 UWP 應用程式套件。 對於市集外的直接散佈，使用 Suite/Advanced UI 安裝程式將您的 UWP 應用程式套件和其他相依性一起組合起來。
...
您可以逐漸將舊的程式碼移轉到 UWP，同時仍保留在 Windows 桌面上執行與發佈應用程式的能力。 一旦您完全移轉至 UWP (且您的 App 不再包含 WPF/Win32 元件)，您就可以觸達所有 Windows 裝置，包括手機、Xbox One 和 HoloLens。
...
>[!IMPORTANT]
> 如果您計劃將您的應用程式發行至 Windows 市集，請確定您的應用程式可在執行 Windows 10 S 的裝置上正確運作這是市集需求。 請參閱[針對 Windows 10 S 測試您的 Windows 應用程式](desktop-to-uwp-test-windows-s.md)。

## <a name="validate"></a>驗證

為了讓您的應用程式能順利在 Windows 市集上發行或成為 [Windows 認證](http://go.microsoft.com/fwlink/p/?LinkID=309666)，請在送出以進行認證之前，先在本機進行驗證和測試。

若您正在使用 DAC 封裝您的應用程式，您可以使用新的 ``-Verify`` 旗標來驗證您的套件是否違反傳統型橋接器和市集的需求。 請參閱[封裝應用程式，簽署應用程式，並準備提交市集](desktop-to-uwp-run-desktop-app-converter.md#optional-parameters)。

若您正在使用 Visual Studio，您可以從 **\[建立應用程式套件\]** 精靈中驗證您的應用程式。 請參閱[建立應用程式套件](../packaging/packaging-uwp-apps.md#create-an-app-package)。
...
您可以透過將您的應用程式發行至我們的 Windows 市集，或是側載至其他系統來散佈您的應用程式。
...
| [傳統型橋接器的幕後作業 (傳統型橋接器)](desktop-to-uwp-behind-the-scenes.md) | 深入了解傳統型轉 UWP 橋接器背後的運作方式。 |
| [已知問題 (傳統型橋接器)](desktop-to-uwp-known-issues.md) | 列出傳統型轉 UWP 橋接器的已知問題。 |
