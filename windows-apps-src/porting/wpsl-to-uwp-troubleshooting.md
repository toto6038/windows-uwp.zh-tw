...
| _XamlCompiler 錯誤 WMC0055:無法將指派的文字值 '&lt;資料流幾何&gt;' 屬性的型別 'RectangleGeometry' 的 ' 剪輯'_ | 在 UWP 中，為 [Microsoft DirectX](https://msdn.microsoft.com/library/windows/desktop/ee663274) 和 XAML C++ UWP App 的類型。 |
| _XamlCompiler 錯誤 WMC0001:未知的類型 'RadialGradientBrush' 在 XML 命名空間 [...]_ | UWP 沒有 **RadialGradientBrush** 類型。 請從標記中移除 **RadialGradientBrush**，並使用一些其他類型的 [Microsoft DirectX](https://msdn.microsoft.com/library/windows/desktop/ee663274) 和 XAML C++ UWP App。 |
| _XamlCompiler 錯誤 WMC0011:未知的成員 'OpacityMask' 元素 '&lt;UIElement 型別&gt;'_ | UWP [Microsoft DirectX](https://msdn.microsoft.com/library/windows/desktop/ee663274) 和 XAML C++ UWP App。 |
| _類型 'System.Runtime.InteropServices.COMException' 的第一個可能發生例外狀況發生在系統中。NI。DLL。其他資訊：應用程式會針對不同的執行緒呼叫已封送處理的介面。(發生例外狀況於 HRESULT:0X8001010E (RPC_E_WRONG_THREAD))。_ | 您正在進行的工作必須在 UI 執行緒上完成。 呼叫 [**CoreWindow.GetForCurrentThread**](https://msdn.microsoft.com/library/windows/apps/hh701589))。 |
