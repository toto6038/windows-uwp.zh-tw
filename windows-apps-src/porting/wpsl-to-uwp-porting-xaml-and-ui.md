...
有時候，修正命令式程式碼就像變更參數類型一樣，只是小事一樁。 其他的時間，您必須使用 UWP Api，而不是 Windows Runtime.NET Api 的 8.x 應用程式。 若要找出支援哪些 Api、 與[.NET Windows Runtime 8.x 應用程式概觀 （英文）](https://msdn.microsoft.com/library/windows/apps/xaml/br230302.aspx)與[Windows Runtime 參考](https://msdn.microsoft.com/library/windows/apps/br211377)結合使用本移植指南的其餘部分。
...
| Panorama | Windows Phone Silverlight Panorama 控制項對應至[Windows Runtime 8.x 應用程式內的集線器控制項的指導方針](https://msdn.microsoft.com/library/windows/apps/dn449149)和指導方針 hub 控制項。 <br/> 請注意，Panorama 控制項會將最後一個區段迴繞到第一個區段，且其背景影像會以和區段相對的視差方式移動。 [Hub](https://msdn.microsoft.com/library/windows/apps/dn251843) 區段不會迴繞，且不會使用視差。 |
...
如果點陣圖的淺色和深色形式有差異，有一個適合此情況的更普遍方法，就是使用兩個影像資產：一個具有深色前景 (用於淺色佈景主題)，另一個具有淺色前景 (用於深色佈景主題)。 如需如何命名本系列點陣圖資產的進一步詳細資訊，請參閱[量身訂作語言、 比例和其他辨識符號的資源](../app-resources/tailor-resources-lang-scale-contrast.md)。 正確命名一組影像檔之後，您便可以在摘要中使用它們的根名稱來參考它們，就像這樣：
