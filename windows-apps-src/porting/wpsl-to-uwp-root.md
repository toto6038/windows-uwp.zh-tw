---
description: 如果您是具備 WindowsPhone Silverlight 應用程式的開發人員，然後您可以讓充分利用您的技能組合與原始程式碼中移到 windows 10。
...
ms.locfileid: "9045211"
---
#  <a name="move-from-windowsphone-silverlight-to-uwp"></a>從 WindowsPhone Silverlight 移動到 UWP


如果您是具備 WindowsPhone Silverlight 應用程式的開發人員，然後您可以讓充分利用您的技能組合與原始程式碼中移到 windows 10。 在 windows 10 中，您可以建立通用 Windows 平台 (UWP) 應用程式，也就是可供客戶安裝至各種類型裝置的單一應用程式套件。 如詳細背景資訊 windows 10，UWP app，以及調適型程式碼與我們將在此移植指南中，提及之調適型 UI 概念的請參閱 「[通用 Windows 平台 (UWP) app 指南](https://msdn.microsoft.com/library/windows/apps/dn894631)。

當您移植您的 windows 10 應用程式的 WindowsPhone Silverlight 應用程式時，您將能夠跟上的行動裝置功能已[在 Windows Phone 8.1 中引進](https://msdn.microsoft.com/library/windows/apps/dn632424)，最新和遠超出他們使用其應用程式模型與 UI 架構的通用 Windows 平台 (UWP)是跨所有 windows 10 裝置通用的。 這使得以一個程式碼為基底，以及單一應用程式套件，來支援電腦、平板電腦、手機與大量其他種類裝置成為可能。 而這將讓您 app 的潛在對象倍增，並藉由共用資料、購買的消費性產品等，創造新的可能性。 如需新功能的詳細資訊，請參閱[開發人員的 windows 10 的新的功能](https://dev.windows.com/getstarted/whats-new-windows-10)。

如果您選擇，您的應用程式的 WindowsPhone Silverlight 版本與 windows 10 版本都可供客戶在同一時間。

**注意：** 本指南的設計可協助您以手動方式移植您的 windows 10 WindowsPhone Silverlight 應用程式。 除了使用本指南中的資訊移植您的 app 外，您還可嘗試 **Mobilize.NET 的 Silverlight Bridge** 的開發人員預覽以協助移植程序自動化。 此工具會分析您的應用程式原始程式碼，並參考 WindowsPhone Silverlight 控制項和 Api 轉換成其 UWP 對應項目。 因為此工具仍在開發人員預覽中，尚無法處理所有轉換案例。 不過，大部分的開發人員應可藉由開始使用此工具而節省一些時間和精力。 若要嘗試開發人員預覽，請瀏覽 [Mobilize.NET 的網站](https://go.microsoft.com/fwlink/p/?LinkId=624546)。

## <a name="xaml-and-net-or-html"></a>XAML 與 .NET 或 HTML？

WindowsPhone Silverlight 具有以 Silverlight 4.0 與您只需要針對一個.NET Framework 版本與小的 UWP Api 子集上為基礎的 XAML UI 架構。 因為您可以使用 Extensible Application Markup Language (XAML) WindowsPhone Silverlight 應用程式中，很可能的 XAML 會是您選擇您的 windows 10 版本，因為大部分的知識與經驗，都會一併轉移以及大部分的原始程式碼和所使用的軟體模式。 甚至您的 UI 標記與設計也可以輕易地移植過去。 您將發現 Managed API、XAML 標記、UI 架構與工具全都令人熟悉不已，而且您可以在 UWP app 中使用 C++、C# 或 Visual Basic 來搭配 XAML。 您可能會訝異此程序相對上有多麼容易，即使過程中有一兩項挑戰。

請參閱[使用 C# 或 Visual Basic 建立通用 Windows 平台 (UWP) app 的藍圖](https://msdn.microsoft.com/library/windows/apps/br229583)。

**注意：** windows 10 支援更多的.NET Framework 比 Windows Phone 市集應用程式。 例如，windows 10 具有數個 System.ServiceModel.\* 命名空間，以及 System.Net、 System.Net.NetworkInformation 與 System.Net.Sockets。 因此，現在是移植您 WindowsPhone Silverlight，讓您.NET 程式碼直接編譯並在新的平台上運作的絕佳時機。 請參閱[命名空間與類別對應](wpsl-to-uwp-namespace-and-class-mappings.md)。
另一個現有.NET 原始程式碼重新編譯成 windows 10 應用程式的更好理由是，您將可以從.NET 原生，其將 MSIL 轉換成原生可執行機器碼的事先編譯技術。 .NET 原生 app 比其對應的 MSIL 啟動更快、使用更少的記憶體，而且消耗較少的電池電力。
...
在功能層級，好消息是 UWP 中很少有不支援的項目。 您的技能組合與原始程式碼大部分都能非常好地轉譯到 UWP app，您將在此移植指南的剩餘部分閱讀到相關資訊。 但是，以下是您可能已經使用幾個 WindowsPhone Silverlight 功能適用於這沒有任何 UWP 對等項目。

| 沒有 UWP 對等功能的功能 | WindowsPhone Silverlight 功能的文件 |
|----------------------------------------------|---------------------------------------------------------|
| Microsoft XNA。 一般而言，使用 C++ 的 [Microsoft DirectX](https://msdn.microsoft.com/library/windows/desktop/ee663274) 可以做為替代。 請參閱[開發遊戲](https://msdn.microsoft.com/library/windows/apps/hh452744)和 [DirectX 與 XAML 互通性](https://msdn.microsoft.com/library/windows/apps/hh825871)。 | [XNA Framework 類別庫](https://msdn.microsoft.com/library/bb203940.aspx) | 
|鏡頭應用程式 | [Windows Phone 8 的鏡頭](https://msdn.microsoft.com/library/windows/apps/jj206990.aspx) |
...
| [移植專案](wpsl-to-uwp-porting-to-a-uwp-project.md) | 您在 Visual Studio 中建立新的 windows 10 專案，並將您的檔案複製到此開始移植程序。 |
...
| [移植 XAML 與 UI](wpsl-to-uwp-porting-xaml-and-ui.md) | UWP 應用程式的宣告式 XAML 標記形式定義 UI 的做法可以極順利地從 WindowsPhone Silverlight 中轉譯。 您會發現在更新系統資源索引鍵參考、變更某些元素類型名稱，以及將 "clr-namespace" 變更為 "using" 之後，大部分的標記都相容。 |
...
|[案例研究：Bookstore1](wpsl-to-uwp-case-study-bookstore1.md) | 本主題提供的案例研究移植到 Windows10UWP 應用程式非常簡單 WindowsPhone Silverlight 應用程式。 在 windows 10，您可以建立單一應用程式套件，可供客戶安裝至各種裝置，而這就是我們將在這個案例研究中執行。 |
| [案例研究：Bookstore2](wpsl-to-uwp-case-study-bookstore2.md) | 這個案例研究 — 根據[Bookstore1](wpsl-to-uwp-case-study-bookstore1.md)中的資訊來建置 — 是從 WindowsPhone Silverlight 應用程式顯示分組資料在**LongListSelector**中的開始。 在檢視模型中，每個 **Author** 類別執行個體都代表該作者所著之書籍的群組，而在 **LongListSelector** 中，我們可以檢視依作者分組的書籍清單，或是縮小來查看作者的捷徑清單。 |

## <a name="related-topics"></a>相關主題

**文件**
* [在 windows 10 中的開發人員的新功能](https://dev.windows.com/getstarted/whats-new-windows-10)
...
**雜誌文章**
* [Visual Studio 雜誌：Windows Phone 8.1：Convergence 的一大躍進](https://go.microsoft.com/fwlink/p/?LinkID=398541)

**簡報**
* [將 Nokia 音樂從 Windows Phone 移到 [Windows8 的到本文](https://go.microsoft.com/fwlink/p/?LinkId=321521)
 
