...
ARM 上的 Windows 10 會從 Microsoft Store 執行 ARM32 和 ARM64 [UWP 應用程式](../get-started/universal-application-platform-guide.md)的所有 x86。 沒有任何的模擬功能，原生執行的 ARM32 和 ARM64 應用程式時在模擬下執行應用程式的 x86。 如果您是 UWP 開發人員，請務必為您的應用程式提交 ARM 套件，這將會為裝置提供最佳的使用者體驗。 如需詳細資訊，請參閱[應用程式套件架構](../packaging/device-architecture.md)。
...
除了 UWP 應用程式，ARM 上的 Windows 10 也可以執行您的 x86 Win32 應用程式 （例如 Adobe Reader) 未修改狀態，提供良好的效能與順暢的使用者體驗，就像任何電腦。 這些 x86 Win32 應用程式不需要為 ARM 重新編譯甚至不知道它們 ARM 處理器上執行。 請注意，不支援 64 位元 x64 Win32 應用程式，但大部分的所有應用程式都有 x86 應用程式版本，所以從使用者觀點，只要選擇 x86 32 位元安裝程式就能在 Windows-on-ARM 電腦上執行。
...
|[疑難排解 ARM 上的 ARM 應用程式](apps-on-arm-troubleshooting-arm32.md)|一般 ARM32 和 ARM64 應用程式在 ARM，以及如何修正這些問題上執行時的問題。 |
