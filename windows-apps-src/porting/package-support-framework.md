...
ms.locfileid: "3665682"
---
# <a name="apply-runtime-fixes-to-an-msix-package-by-using-the-package-support-framework"></a>使用套件支援架構來執行階段的修正程式套用到 MSIX 套件

套件支援架構是開放原始碼套件，可協助您將套用修正程式現有的 win32 應用程式時您不需要的存取權的原始程式碼，讓它可以在 MSIX 容器中執行。 套件支援的架構，可協助您的應用程式遵循現代化的執行階段環境中的最佳做法。
...
此架構開放原始碼、 輕量，且您可以使用它來解決應用程式問題快速。 它也會提供您有機會洽詢，擁有遍佈全球社群，並在其他的投資上建置。

## <a name="a-quick-look-inside-of-the-package-support-framework"></a>在套件支援架構快速瀏覽
...
當使用者啟動您的應用程式時，填充碼啟動程式會執行的第一個可執行檔。 它會讀取您的設定檔，並會執行階段 fix(s) 和執行階段 manager DLL 插入的應用程式的程序。

![套件支援架構 DLL 插入](images/desktop-to-uwp/package-support-framework-2.png)

當您需要在 MSIX 容器中執行的應用程式時，執行階段管理員適用於修正程式。
...
首先，建立您的應用程式的套件。 然後，將它安裝、 執行，並觀察其行為。 您可能會收到錯誤訊息，可協助您找出相容性問題。 您也可以使用[處理程序監視器](https://docs.microsoft.com/en-us/sysinternals/downloads/procmon)，以找出問題。  常見的問題與應用程式假設有關的工作目錄及程式路徑權限。
...
如果您懷疑檔案系統權限失敗，搜尋失敗事件 System32/SysWOW64 或是套件檔案路徑。 篩選器也可以協助在這裡，太。 啟動底部的這份清單，然後向上捲動。 最近發生失敗，底部的這份清單會顯示。 留意大部分的錯誤，包含字串，例如 「 拒絕存取 」 和 「 路徑/名稱找不到 」，並略過不尋找可疑的項目。 [PSFSample](https://github.com/Microsoft/MSIX-PackageSupportFramework/blob/master/samples/PSFSample/)有兩個問題。 您可以看到下列影像中出現的清單中的這些問題。

![ProcMon Config.txt](images/desktop-to-uwp/procmon_config_txt.png)

中會出現在此映像的第一個問題，應用程式無法讀取，而且位於 「 C:\Windows\SysWOW64 「 路徑的 「 Config.txt"檔案中。 您不太應用程式嘗試直接參考該路徑。 最有可能，它嘗試使用相對路徑，讀取該檔案，並根據預設，「 System32/SysWOW64 」 是應用程式的工作目錄。 這會建議應用程式預期其目前的工作目錄將它設定為某個位置在套件中。 尋找 appx 內，我們可以看到該檔案位於與可執行檔相同的目錄。
...
您可以使用[檔案重新導向填充碼](https://github.com/Microsoft/MSIX-PackageSupportFramework/tree/develop/FileRedirectionShim)重新導向嘗試寫入或讀取不是從在 MSIX 容器中執行的應用程式存取目錄中的資料。
...
請務必先檢閱社群比重被到我們的[GitHub](https://github.com/Microsoft/MSIX-PackageSupportFramework/tree/develop)頁面。 它是可能的其他開發人員已解決類似於您的問題，並有共用的執行階段修正。
...
如果您沒有.appx 檔案開始使用，您可以從頭開始建立的套件資料夾和檔案。

### <a name="get-the-package-support-framework-files"></a>取得套件支援的架構檔案

您可以透過使用 Visual Studio 中取得 PSF Nuget 套件。 您也可以取得它使用獨立 Nuget 命令列工具。

#### <a name="get-the-package-by-using-visual-studio"></a>透過使用 Visual Studio，以取得套件

在 Visual Studio 中，您的方案或專案節點上按一下滑鼠右鍵並選擇其中一項管理 Nuget 套件命令。  搜尋**Microsoft.PackageSupportFramework**或**PSF** Nuget.org 在尋找套件。然後，將它安裝。
...
在文字編輯器中開啟您的封裝資訊清單，然後設定`Executable`屬性`Application`項目填充碼啟動程式可執行檔的名稱。  如果您知道您目標的應用程式的架構，選取適當的版本，ShimLauncher32.exe 或 ShimLauncher64.exe。  如果沒有，ShimLauncher32.exe 能在所有情況下。  範例如下。
...
| applications | workingDirectory | （選擇性）與套件相對的路徑做為工作目錄的應用程式的啟動。 如果您未設定此值，作業系統會使用`System32`目錄做為應用程式的工作目錄。 |
| 處理程序 | 可執行檔 | 在大部分情況下，這將會是名稱`executable`上述移除的路徑和檔案的擴充功能與設定。 |
| 相容性修正 | dll | 載入填充碼.appx 套件相對路徑。 |
| 相容性修正 | 設定 | （選擇性）控制填充碼 dl 的運作方式。 此值的確切格式而有所不同以填充碼填充碼藉由為基礎如想時，每個填充碼可以解譯此 「 blob 」。 |

`applications`， `processes`，並`shims`鍵是陣列。 這表示，您可以使用 config.json 檔案來指定多個應用程式、 處理程序，以及填充碼 DLL。
...
執行應用程式，並使用執行階段修正套用觀察的行為。  重複執行診斷與封裝為所需的步驟。

### <a name="use-the-trace-shim"></a>使用追蹤填充碼

若要用來診斷已封裝的應用程式相容性問題的替代技術是使用追蹤填充碼。 這個 DLL 隨附於 PSF，並提供詳細的診斷檢視的應用程式的行為，類似於處理程序監視器。  此外，它是特別設計顯色應用程式相容性問題。  使用追蹤填充碼，將 DLL 新增到 「 套件 」，將下列片段新增到您 config.json，然後將封裝和安裝和您的應用程式。
...
根據預設，追蹤填充碼會篩選掉失敗可能會被視為 「 必須 」。  例如，應用程式可能會嘗試無條件刪除檔案，而不檢查以確認是否已經存在，請略過結果。 這有某些非預期的錯誤可能會篩選出來，不幸結果，因此在上述範例中，我們選擇從檔案系統函式會收到所有失敗。 因為我們了解從之前，先嘗試讀取 Config.txt 檔案中會失敗，且訊息 「 找不到檔案 」，我們可以這麼做。 這是經常觀察到與一般並不會假設為非預期的失敗。 實際上它是最有可能出篩選，只以非預期的失敗，以及然後後援的所有失敗，仍無法識別的問題是否啟動。

根據預設，追蹤填充碼的輸出傳送到在附加偵錯工具。 針對此範例，我們將不是移至附加在偵錯工具，並將會改為使用 SysInternals [DebugView](https://docs.microsoft.com/en-us/sysinternals/downloads/debugview)程式來檢視它的輸出。 在之後執行該應用程式，我們可以看到相同的失敗像之前一樣，這會我們指向相同的執行階段修正程式。
...
您可以使用 Visual Studio 以偵錯的執行階段修正，延伸執行階段修正程式，或建立從頭租用戶。 您將需要執行下列動作，才能成功。
...
### <a name="add-a-packaging-project"></a>將封裝專案加入

如果您還沒有**Windows 應用程式封裝專案**，請建立租用戶，並將它新增至您的方案。
...
在 [**方案總管]** 中，封裝專案上按一下滑鼠右鍵，選取 [**編輯**，並從中新增這到底部的專案檔案：
...
該專案中，以滑鼠右鍵按一下，然後在操作功能表中，選擇 [**管理 Nuget 套件**的選項。 請確定 [**封裝來源**選項將**所有**或**nuget.org**。
...
將**PSF** Nuget 套件新增到此專案中，使用相同上一節中所述的指導方針。

開啟專案，然後在**一般**的設定 \] 頁面的屬性頁面**目標名稱**將屬性設定為``ShimLauncher32``或``ShimLauncher64``取決於您的應用程式的架構。
...
新增一個名為檔案``config.json``到您的封裝專案，然後，將複製並貼到的檔案的下列 json 文字。 設定為 [**內容**的 [**封裝動作**] 屬性。
...
在 Visual Studio 中，按 F5 來啟動偵錯工具。  啟動第一件事是填充碼啟動程式應用程式，接著，會啟動傳統型應用程式目標。  若要偵錯目標的傳統型應用程式，您必須手動附加的傳統型應用程式處理程序選擇 [**偵錯**->**附加至處理序**，並選取應用程式處理程序。 若要允許偵錯的原生執行階段修正 DLL 的.NET 應用程式，請選取 （混合的模式偵錯） 的 managed 和原生程式碼類型。  

一旦您已將它設定好，您可以在傳統型應用程式程式碼和執行階段修正專案中設定中斷點，旁邊幾行程式碼。 如果您不需要您的應用程式的原始碼，您將無法在執行階段修正專案中設定中斷點只旁邊幾行程式碼。

>[!NOTE]
> Visual Studio 可讓您在最簡單的開發和偵錯經驗，有一些限制，而稍後在本指南中，我們會討論其他偵錯技術，您可以套用。
...
宣告``SHIM_DEFINE_EXPORTS``巨集，然後將包含的陳述式新增`shim_framework.h`頂端的每個。您想要將您的執行階段修正程式的函式的位置 CPP 檔案。
...
建立具有相同的簽章的函式的函式人員有您想要修改的行為。 以下是範例函式，以取代`MessageBoxW`函式。
...
呼叫`DECLARE_SHIM`對應`MessageBoxW`函式，以您新的取代函式。 您的應用程式嘗試呼叫`MessageBoxW`函式，它會呼叫取代函式改為。

#### <a name="protect-against-recursive-calls-to-functions-in-runtime-fixes"></a>防止遞迴呼叫函式在執行階段的修正程式

您可以選擇性地將套用`reentrancy_guard`抵禦遞迴呼叫函式在執行階段的修正程式函式的類型。

例如，您可能會產生取代的函式`CreateFile`函式。 您的實作可能會呼叫`CopyFile`函式，但的實作`CopyFile`函式可能會呼叫`CreateFile`函式。 這可能會導致遞迴無限循環，呼叫的`CreateFile`函式。

如需詳細資訊`reentrancy_guard`請[authoring.md](https://github.com/Microsoft/MSIX-PackageSupportFramework/blob/master/Authoring.md)
...
第一次，F5 偵錯執行應用程式部署的套件配置資料夾路徑的鬆散檔案，而非.appx 套件安裝。  配置資料夾通常沒有相同的安全性限制為已安裝的套件資料夾。 如此一來，它不可能要重新產生套件之前套用的執行階段修正路徑存取拒絕錯誤。

若要解決此問題，使用.appx 封裝部署，而非 F5 鬆散檔案部署。  若要建立.appx 套件檔案，請使用[MakeAppx](https://docs.microsoft.com/en-us/windows/desktop/appxpkg/make-appx-package--makeappx-exe-)公用程式於 Windows SDK，從，如上文所述。 或者，從 Visual Studio 中，您的應用程式的專案節點上按一下滑鼠右鍵並選取**市集**->**建立應用程式套件**。
