...
套件支援架構是開放原始碼套件，可協助您將套用修正程式現有的 win32 應用程式時您不需要的存取權的原始程式碼，讓它可以 MSIX 容器中執行。 套件支援的架構，可協助您的應用程式遵循現代化的執行階段環境的最佳做法。
...
[處理程序監視器](https://docs.microsoft.com/en-us/sysinternals/downloads/procmon)是功能強大的公用程式，適用於觀察應用程式的檔案和登錄作業，以及它們的結果。  這可協助您了解應用程式相容性問題。  開啟處理程序監視器之後, 新增要包含只從應用程式可執行檔的事件篩選器 (篩選 > 篩選...)。

![ProcMon 應用程式的篩選器](images/desktop-to-uwp/procmon_app_filter.png)

會顯示的事件清單。 針對許多這些事件，word**成功**會出現在 [**結果**] 欄。
...
中會出現在這個影像中的第一個問題，應用程式無法讀取位於 「 C:\Windows\SysWOW64 「 路徑 」 Config.txt 」 檔案中。 您不太應用程式嘗試直接參考該路徑。 最有可能，它嘗試讀取該檔案，藉由使用相對路徑，並根據預設，「 System32/SysWOW64 」 是應用程式的工作目錄。 這會建議應用程式預期其目前的工作目錄某處設定套件中。 尋找 appx 內，我們可以看到檔案存在，可執行檔相同的目錄中。
...
例如，如果您的應用程式寫入的記錄檔是位於與您的應用程式可執行檔相同的目錄，然後您可以使用[檔案重新導向修復](https://github.com/Microsoft/MSIX-PackageSupportFramework/tree/master/fixups/FileRedirectionFixup)另一個位置，例如本機 app 資料存放區中建立該記錄檔。

### <a name="runtime-fixes-from-the-community"></a>來自社群的執行階段修正

請務必檢閱社群比重被到我們的[GitHub](https://github.com/Microsoft/MSIX-PackageSupportFramework)頁面。 它是可能的其他開發人員已解決類似於您的問題，並有共用的執行階段修正。
...
#### <a name="get-the-package-by-using-visual-studio"></a>取得套件，使用 Visual Studio

在 Visual Studio 中，您的方案或專案節點上按一下滑鼠右鍵，並選擇其中一個管理 Nuget 套件命令。  搜尋**Microsoft.PackageSupportFramework**或**PSF**尋找 Nuget.org 套件。然後，安裝它。
...
在文字編輯器中，開啟您的套件資訊清單，然後設定`Executable`屬性`Application`元素 PSF 啟動程式可執行檔的名稱。  如果您知道您的目標應用程式的架構，選取適當的版本，PSFLauncher32.exe 或 PSFLauncher64.exe。  如果沒有，PSFLauncher32.exe 將在所有情況下運作。  這裡提供一個範例。
...
建立檔案名稱``config.json``，並將該檔案儲存到您的套件的根資料夾。 修改宣告的應用程式之檔案的識別碼 config.json 指向您只是取代的可執行檔。 使用您所獲得使用處理程序監視器知識，您可以也設定工作目錄，以及使用檔案重新導向修復重新導向到.log 目錄底下的檔案套件相對 」 PSFSampleApp 」 的讀取/寫入。
...
| 處理程序 | 可執行檔 | 在大部分情況下，這將會是名稱`executable`副檔名的路徑和檔案移除設定上方。 |
...
另一個技術來診斷已封裝的應用程式相容性問題是使用追蹤修復。 這個 DLL 隨附於 PSF，並提供應用程式的行為，類似於處理程序監視器診斷的詳細的檢視。  它被專為顯色應用程式相容性問題。  使用追蹤修復、 將 DLL 新增到 「 套件 」，將下列片段新增到您 config.json，然後將封裝和安裝和您的應用程式。
...
根據預設，追蹤修復會篩選掉失敗可能會被視為 「 必須是 」。  例如，應用程式可能會嘗試無條件刪除檔案，而不需檢查以查看是否已經存在，請略過結果。 這會有某些非預期的失敗可能會篩選出來，不幸後果，因此在上述範例中，我們選擇從檔案系統函式會收到所有失敗。 因為我們知道從之前，嘗試從 Config.txt 檔案讀取會失敗訊息 「 找不到檔案 」，我們可以這麼做。 這是很頻繁觀察到，且通常不會假設為非預期的失敗。 實際上它是最有可能出篩選，只會以預期的失敗，以及然後後援的所有失敗，仍無法識別的問題是否啟動。
...
該專案，以滑鼠右鍵按一下，然後在操作功能表中，選擇 [**管理 Nuget 套件**的選項。 確定**所有**或**nuget.org****套件來源**選項設定。

按一下設定圖示接下來該欄位。

搜尋*PSF** Nuget 套件，然後再將它安裝此專案。

![nuget 套件](images/desktop-to-uwp/psf-package.png)

如果您想要偵錯，或將現有的執行階段修正延伸，新增執行階段修正檔案取得使用本指南中[找到的執行階段修正](#find)所述的指導方針。
...
| 修復 | 組態 | （選擇性）控制修復 DLL 的行為。 因為每個修復可解譯此 「 blob 」 它想要修復的修復為基礎而異確切此值的格式。 |
...
在 Visual Studio 中，按 F5 來啟動偵錯工具。  啟動第一件事是 PSF 啟動程式應用程式，接著，會啟動傳統型應用程式目標。  若要偵錯目標的傳統型應用程式，您必須手動附加到傳統型應用程式的程序選擇 [**偵錯**->**附加至處理序**，並選取應用程式處理程序。 若要允許偵錯的原生執行階段修正 DLL 的.NET 應用程式，請選取 managed 和原生程式碼類型 （混合的模式偵錯）。  

一旦您已將它設定好，您可以設定中斷點，旁邊幾行程式碼中的傳統型應用程式程式碼和執行階段修正專案。 如果您不需要原始碼，您的應用程式，您將無法在執行階段修正專案中設定中斷點，只旁邊幾行程式碼。
...
如果沒有尚未執行階段修正問題，您想要解決，您可以透過撰寫取代函式，並包括任何設定資料來建立新的執行階段修正這很合理。 讓我們看看每個部分。

### <a name="replacement-functions"></a>取代函式

首先，找出哪些 MSIX 容器中執行的應用程式時，呼叫會失敗的函式。 然後，您可以建立您想要改為呼叫，執行階段管理員的取代函式。 這會讓您有機會，將函式的實作取代符合的現代化的執行階段環境的規則的行為。
...
您可以選擇性地將套用`reentrancy_guard`抵禦遞迴呼叫函式在執行階段修正程式函式的類型。
...
如果您想要將設定資料新增到您的執行階段修正程式，請考慮新增至``config.json``。 如此一來，您可以使用`FixupQueryCurrentDllConfig`輕鬆地分析該資料。 這個範例會剖析來自該設定檔的字串和布林值。
...
>[!NOTE]
> [PLMDebug](https://docs.microsoft.com/en-us/windows-hardware/drivers/debugger/plmdebug)也可用來偵錯工具附加到應用程式在啟動，時，也會包含在[Windows 偵錯工具](https://docs.microsoft.com/en-us/windows-hardware/drivers/debugger/index)。  不過，它會更複雜的直接存取的支援所 WinDbg 現在提供比使用。
