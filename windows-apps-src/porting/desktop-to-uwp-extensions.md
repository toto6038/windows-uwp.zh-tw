...
|Name |通訊協定的名稱。 |
|Parameters |之參數與值，以啟動應用程式時，將傳遞至您的應用程式，做為事件引數的清單。 若變數包含了檔案路徑，請以引號包圍參數。 這將可避免路徑中包含空格時可能發生的任何問題。 |
...
啟動工作能讓您的應用程式，每次使用者登入時，自動執行可執行檔。
...
當使用者將裝置連接至電腦時，「 自動播放 」 會顯示您的應用程式成為一個選項。
...
到目前為止，您的應用程式有 30 秒的時間可以關閉它本身的處理程序，否則平台會強制終止。
...
|Parameters |使您的應用程式正常處理要求的任何必要參數。 |
