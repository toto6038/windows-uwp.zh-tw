...
ms.locfileid: "57652543"
---
# <a name="package-a-desktop-application-using-the-desktop-app-converter"></a>封裝使用 Desktop App Converter 的桌面應用程式

[取得傳統型應用程式轉換器](https://aka.ms/converter)

Desktop App Converter (DAC) 會建立將與最新的 Windows 功能，整合的桌面應用程式的套件，包括散發和服務透過 Microsoft Store。 這包括 Win32 應用程式，以及您使用 .NET 4.6.1 建立的應用程式。

![DAC 圖示](images/desktop-to-uwp/dac.png)

雖然此工具的名稱中出現「Converter」(轉換器) 這個詞，但它並不會轉換您的應用程式。 您的應用程式維持不變。 不過，此工具會產生含有套件識別資料並可呼叫大範圍 WinRT API 的 Windows 應用程式套件。
...
:heavy_check_mark:註冊您的 COM 伺服器。

**Windows 10 年度更新版或更新版本**

:heavy_check_mark:自動簽署您的套件，讓您可以測試您的應用程式。

:heavy_check_mark:驗證您的應用程式，針對已封裝應用程式和 Microsoft Store 的需求。
...
2. 下載適合的 [Desktop App Converter 基本映像檔](https://aka.ms/converterimages)。
   ...
   >[!IMPORTANT]
   > 如果您使用組建編號**15063**，且該組建的次要版本為等於或大於 **.483** (例如：**15063.540**)，請務必下載**BaseImage-15063-UPDATE.wim**檔案。 如果該組建的次要版本小於 **.483**，請下載 **BaseImage-15063.wim** 檔案。 如果您已經安裝不相容的基本檔案版本，您可以進行修正。 此[部落格文章](https://blogs.msdn.microsoft.com/appconsult/2017/08/04/desktop-app-converter-fails-on-windows-10-15063-483-and-later-how-to-solve-it/)說明如何執行。
...
您會使用參數來指定應用程式封裝名稱、 發行者和版本號碼。

> [!NOTE]
> 如果您已保留在 Microsoft Store 應用程式名稱，您可以使用取得的套件和發行者名稱[合作夥伴中心](https://partner.microsoft.com/dashboard)。 若您想要在其他系統上側載您的應用程式，您可以為他們提供您自己的名稱，只要其發行者名稱與您選擇用來簽署您應用程式之憑證上的名稱相同即可。
...
* [封裝具有的 installer (.msi) 檔案的應用程式](#installer-conversion)
* [封裝具有安裝程式的可執行檔的應用程式](#setup-conversion)
* [沒有安裝程式將應用程式封裝](#no-installer-conversion)
* [將應用程式封裝，簽署應用程式，並準備好儲存提交](#optional-parameters)

<a id="installer-conversion" />

#### <a name="package-an-application-that-has-an-installer-msi-file"></a>封裝具有的 installer (.msi) 檔案的應用程式
...
> [!IMPORTANT]
> 以下是應該牢記的兩件重要的事： 首先，確定您的安裝程式位於一個獨立的資料夾中，且相同的資料夾中只有其他與安裝程式相關的檔案。 轉換器會將該資料夾中的所有內容複製到隔離 Windows 環境中。 <br> 其次，如果合作夥伴中心會將身分識別指派給您以數字開頭的套件，請確定，您也可傳入<i>-AppId</i>參數，並使用只有字串尾碼 （之後的句號分隔符號） 做為該參數的值。  
...
#### <a name="package-an-application-that-has-a-setup-executable-file"></a>封裝具有安裝程式的可執行檔的應用程式
...
```
>[!IMPORTANT]
>如果合作夥伴中心會將身分識別指派給您以數字開頭的套件，請確定，您也可傳入<i>-AppId</i>參數，並使用只有字串尾碼 （之後的句號分隔符號） 做為該參數的值。

``InstallerArguments`` 是一項選擇性參數。 不過，因為 Desktop App Converter 需要您在自動安裝模式中執行的安裝程式，您可能必須使用它，如果您的應用程式必須要以無訊息模式執行的無訊息旗標。 ``/S`` 旗標是一個相當常見的無訊息旗標，但您使用的旗幟可能會因您所使用的安裝技術而有所不同。
...
#### <a name="package-an-application-that-doesnt-have-an-installer"></a>沒有安裝程式將應用程式封裝

在此範例中，使用``Installer``參數指向您的應用程式檔案的根資料夾。
...
這個範例是類似於第一個，但它會顯示如何登入您的應用程式進行本機測試，並驗證您的應用程式，針對已封裝應用程式和 Microsoft Store 的需求。
...
``Sign``參數會產生憑證，再登入您的應用程式使用它。 若要執行您的應用程式，您必須先安裝該產生的憑證。 若要了解如何執行，請參閱本文中[執行封裝後的應用程式](#run-app)一節。
...
||||
|-------------|-----------|-------------|
|<a id="setup-params" /> <strong>安裝程式參數</strong>  ||
...
|-NatSubnetPrefix &lt;String&gt; |選擇性 |可供 Nat 執行個體使用的首碼值。 通常，只有在您的主機電腦附加到與轉換器的 NetNat 相同的子網路範圍時，您才會想要變更此值。 您可以使用 **Get-NetNat** Cmdlet 來查詢目前的轉換器 NetNat 組態。 |
|-NoRestart [&lt;SwitchParameter&gt;] |必要 |執行安裝時不顯示重新開機的提示 (需要重新開機才能啟用容器功能)。 |
|<a id="conversion-params" /> <strong>轉換參數</strong>|||
|-AppInstallPath &lt;String&gt;  |選擇性 |您的應用程式在安裝之後，其已安裝檔案根資料夾的完整路徑 (例如 "C:\Program Files (x86)\MyApp")。|
...
|-PackageName &lt;String&gt; |必要 |通用 Windows app 套件的名稱。 如果合作夥伴中心會將身分識別指派給您以數字開頭的套件，請確定，您也可傳入<i>-AppId</i>參數，並使用只有字串尾碼 （之後的句號分隔符號） 做為該參數的值。 |
|-Publisher &lt;String&gt; |必要 |通用 Windows 應用程式套件的發行者 |
|-Version &lt;Version&gt; |必要 |通用 Windows 應用程式套件的版本號碼 |
...
|-AppId &lt;String&gt; |選擇性 |在 Windows 應用程式套件資訊清單中指定要為應用程式識別碼所設定的值。 如果未指定，它將會設定為針對 */packagename* 傳入的值。 在很多時候，使用*PackageName*即可。 不過，如果合作夥伴中心會將身分識別指派給您以數字開頭的套件，請確定，您也可傳入<i>-AppId</i>參數，並使用只有字串尾碼 （之後的句號分隔符號） 做為該參數的值。 |
...
|<a id="cleanup-params" /><strong>清除參數</strong>|||
...
|<a id="architecture-params" /><strong>封裝架構參數</strong>|||
...
|-ExpandedBaseImage &lt;String&gt;  |選擇性 |已經展開之基礎映像的完整路徑。|
|-LogFile &lt;String&gt;  |選擇性 |指定記錄檔。 如果省略，則會建立記錄檔暫時位置。 |
| -Sign [&lt;SwitchParameter&gt;] |選擇性 |告訴此指令碼使用為測試用途產生的憑證簽署輸出的 Windows 應用程式套件。 這個參數應該出現在參數 ```-MakeAppx``` 旁邊。 |
|&lt;一般參數&gt; |必要 |此 Cmdlet 支援一般參數：*Verbose*，*偵錯*， *ErrorAction*， *ErrorVariable*， *WarningAction*， *WarningVariable*， *outBuffer*， *PipelineVariable*，和*OutVariable*。 如需詳細資訊，請參閱 [about_CommonParameters](https://go.microsoft.com/fwlink/?LinkID=113216)。 |
| -Verify [&lt;SwitchParameter&gt;] |選擇性 |這個參數，存在時，會指示要驗證封裝的應用程式和 Microsoft Store 需求的應用程式套件的 DAC。 結果會是驗證報告「VerifyReport.xml」，這在瀏覽器中呈現最佳視覺效果。 這個參數應該出現在參數 `-MakeAppx` 旁邊。 |
|-PublishComRegistrations| 選擇性| 掃描所有由您的安裝程式建立的公用 COM 註冊，並發行資訊清單中有效的項目。 只有當您想要讓其他應用程式使用這些註冊時，才使用此旗標。 若只有您的應用程式才會使用到這些註冊，則您就不需要使用此旗標。 <br><br>請參閱[本文](https://blogs.windows.com/buildingapps/2017/04/13/com-server-ole-document-support-desktop-bridge/#lDg5gSFxJ2TDlpC6.97)以確定您的 COM 註冊在您封裝應用程式之後，其行為會跟預期中的一樣。
...
其中一個方式是開啟 PowerShell 命令提示字元，並輸入此命令︰```Add-AppxPackage –Register AppxManifest.xml```。 它可能是最簡單的方式執行您的應用程式，因為您不需要進行簽署。

另一個方法是使用憑證簽署應用程式。 如果您使用```sign```參數，Desktop App Converter 會為您產生一個，然後登入您的應用程式使用它。 該憑證的檔名為 **auto-generated.cer**，您可以在封裝後應用程式的根資料夾中找到該檔案。
...
您可能會解決的 bug、 新增視覺效果資產，或加強您的應用程式，例如動態磚的新式體驗已封裝應用程式進行變更。

在您完成變更之後，您不需要再執行一次轉換器。 在大部分情況下，您可以使用 MakeAppx 工具只是重新封裝應用程式，並 appxmanifest.xml 檔案 DAC 產生應用程式。 請參閱[產生 Windows 應用程式套件](desktop-to-uwp-manual-conversion.md#make-appx)。
...
傳統型應用程式轉換器可能會收集您及您軟體使用方式的相關資訊，並將這項資訊傳送給 Microsoft。 您可以在產品文件和 [Microsoft 隱私權聲明](https://go.microsoft.com/fwlink/?LinkId=521839)中深入了解 Microsoft 的資料收集及使用方式。 　貴用戶玆同意遵守 Microsoft 隱私權聲明之所有適用條款。
