...
|-AppId &lt;String&gt; |選擇性 |在 Windows 應用程式套件資訊清單中指定要為應用程式識別碼所設定的值。 如果未指定，它將會設定為針對 *PackageName* 傳入的值。 在很多時候，使用*PackageName*即可。 不過，如果開發人員中心為您套件指定以數字開頭的身分識別，請確定您也傳入 <i>-AppId</i> 參數，並只使用字串尾碼（句點分隔符號之後）為參數值。 |
