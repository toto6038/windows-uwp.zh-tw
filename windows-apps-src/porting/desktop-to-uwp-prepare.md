---
author: normesta
Description: "本文列出使用傳統型橋接器封裝應用程式之前，您需要知道的事項。 您可能不需要針對應用程式的封裝程序做太多準備。"
...
本文列出封裝傳統型應用程式之前，您需要知道的事項。 您可能不需要針對應用程式的封裝程序做太多準備，但如果以下任何項目適用於您的應用程式，您就需要先處理之後才能封裝。 請記住，Windows 市集會為您處理授權和自動更新，因此您可以從程式碼基底中移除與那些工作有關的任何功能。

+ __您的 App 是使用 4.6.1 之前的 .NET 版本__。 僅支援 .NET 4.6.1。 您必須將您的應用程式重定為 .NET 4.6.1 才能進行封裝。
...
+ __您的應用程式會執行公用程式以執行工作__。 避免啟動命令公用程式，例如 PowerShell 和 Cmd.exe。 事實上，若使用者將您的應用程式安裝至執行 Windows 10 S 的系統上，您的應用程式將會完全無法啟動他們。 這可能會使得您的應用程式無法提交至 Windows 市集，因為提交到 Windows 市集的所有應用程式都必須與 Windows 10 S 相容
...
+ __您的應用程式會產生程式碼__。 您的應用程式可以產生可從記憶體中取用的程式碼，但請避免將產生的程式碼寫入磁碟，因為 Windows 應用程式認證程序無法在應用程式提交之前驗證該程式碼。 同時，將程式碼寫入磁碟的應用程式將不會在執行 Windows 10 S 的系統上正常運作。這可能會使得您的應用程式無法提交至 Windows 市集，因為提交到 Windows 市集的所有應用程式都必須與 Windows 10 S 相容。

+ __您的應用程式使用 MAPI API__。 傳統型橋接器應用程式目前並不支援 [Outlook MAPI API](https://msdn.microsoft.com/library/office/cc765775.aspx(d=robot))。
