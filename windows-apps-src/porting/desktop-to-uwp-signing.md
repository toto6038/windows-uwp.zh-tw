...
translationtype: HT
---
# <a name="desktop-to-uwp-bridge-sign"></a>傳統型轉 UWP 橋接器：簽署

此文章說明如何簽署已轉換至通用 Windows 平台 (UWP) 的傳統型應用程式。 部署 Windows 應用程式套件之前，您必須以憑證簽署它。

## <a name="automatically-sign-using-the-desktop-app-converter-dac"></a>使用 Desktop App Converter (DAC) 自動簽署

執行 DAC 以自動簽署您的 Windows 應用程式套件時，請使用 ```-Sign``` 旗標。 如需詳細資訊，請參閱 [Desktop App Converter 預覽](desktop-to-uwp-run-desktop-app-converter.md)。
...
```
最後，使用 SignTool.exe 來使用憑證簽署您的 Windows 應用程式套件。
