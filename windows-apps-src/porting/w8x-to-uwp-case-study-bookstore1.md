...
description: 本主題提供的簡單 Universal 8.1 應用程式移植到 Windows 10 通用 Windows 平台 (UWP) 應用程式的個案研究。
ms.date: 02/08/2017
ms.topic: article
...
**附註**  時開啟 Bookstore1Universal\_10，以 Visual Studio 中，如果您看到訊息 「 Visual Studio 需要更新 」，然後依照中的步驟[TargetPlatformVersion](w8x-to-uwp-troubleshooting.md)。

## <a name="downloads"></a>下載

[下載 Bookstore1\_81 Universal 8.1 應用程式](https://go.microsoft.com/fwlink/?linkid=532946)。
...
以下是哪些 Bookstore1\_81 — 我們將連接埠的應用程式，看起來像。 它是一個垂直捲動的書籍清單方塊，位於 app 名稱的標題和網頁標題下方：

![如何 bookstore1\-81 看起來在 windows 上](images/w8x-to-uwp-case-studies/c01-01-win81-how-the-app-looks.png)

Bookstore1\_在 Windows 上的 81

![如何 bookstore1\-81 看起來在 windows phone 上](images/w8x-to-uwp-case-studies/c01-02-wp81-how-the-app-looks.png)

Bookstore1\_Windows Phone 上的 81

##  <a name="porting-to-a-windows10-project"></a>移植到 Windows 10 專案

Bookstore1\_81 解決方案是 8.1 通用應用程式專案，並包含這些專案。

-   Bookstore1\_81.Windows。 這是為 Windows 8.1 中建置應用程式封裝專案。
...
在這個案例研究中，我們擁有[如果您有通用 8.1 App](w8x-to-uwp-root.md)中所述之與支援哪些裝置有關的常見選項。 這裡的決策是一個簡單： 此應用程式具有相同的功能，且大部分都與相同的程式碼，以其 Windows 8.1 和 Windows Phone 8.1 形式。 因此，我們將為 Windows 10 為目標的通用裝置系列 （其中您可以將它安裝到廣泛的裝置） 連接埠共用專案 （和任何其他項目我們需要從其他專案） 的內容。

它是非常快速的工作，以透過建立新的專案在 Visual Studio 中，將檔案複製到它從 Bookstore1\_81，並將複製的檔案包含在新的專案。 一開始先建立新的空白應用程式 (Windows 通用) 專案。 它命名為 Bookstore1Universal\_10。 這些是檔案，可透過複製從 Bookstore1\_Bookstore1Universal 的 81\_10。
...
編輯您剛才複製的來源的程式碼和標記檔案，並變更任何參考 Bookstore1\_Bookstore1Universal 81 命名空間\_10。 執行此作業的快速方法是使用 [**檔案中取代**] 功能。 在檢視模型中，或在任何其他非常重要的程式碼中，都不需要變更任何程式碼。 但是，為了讓您更輕鬆地查看哪些版本的應用程式是否正在執行，變更所傳回的值**Bookstore1Universal\_10.BookstoreViewModel.AppName**屬性從"BOOKSTORE1\_81"到"BOOKSTORE1UNIVERSAL\_10"。

您可以立即建置並執行。 以下是我們新的 UWP 應用程式之後需要完成任何明確的工作，尚未將其移轉至 Windows 10 的外觀。

![已變更初始來源程式碼的 Windows 10 應用程式](images/w8x-to-uwp-case-studies/c01-03-desk10-initial-source-code-changes.png)

Windows 10 應用程式與桌面裝置上執行的初始來源的程式碼變更

![已變更初始來源程式碼的 Windows 10 應用程式](images/w8x-to-uwp-case-studies/c01-04-mob10-initial-source-code-changes.png)

Windows 10 應用程式與行動裝置上執行的初始來源的程式碼變更
...
Bookstore1\_81 應用程式使用兩個不同的資源字典 (BookstoreStyles.xaml) 來調整其樣式至 Windows 8.1 和 Windows Phone 8.1 的作業系統。 這兩個 BookstoreStyles.xaml 檔案都不包含完全將我們的 Windows 10 應用程式所需的樣式。 但是，好消息是我們需要的實際上比它們任何一個都更簡單。 所以接下來的步驟大部分將會涉及移除及簡化我們的專案檔案和標記。 步驟如下。 而且您可以使用此主題頂端的連結來下載專案，並查看這裡和案例研究結束時所有變更的結果。

-   若要盡可能縮短項目之間的間距，請找出 MainPage.xaml 中的 `BookTemplate` 資料範本，並將 `Margin="0,0,0,8"` 從根目錄 **Grid** 中刪除。
-   此外，在 `BookTemplate` 中，有 `BookTemplateTitleTextBlockStyle` 和 `BookTemplateAuthorTextBlockStyle` 的參考。 Bookstore1\_81 會使用間接取值為這些金鑰，以便單一索引鍵的兩個應用程式中有不同的實作。 我們已經不再需要該間接取值，我們可以直接參照系統樣式。 因此請個別使用 `TitleTextBlockStyle` 和 `SubtitleTextBlockStyle` 取代那些參照。
...
-   在 `TitlePanel` 中，將對 `TitleTextBlockStyle` (現在已經有一點太大) 的參照變更成參照 `CaptionTextBlockStyle`。 `PageTitleTextBlockStyle` 是另一個 Bookstore1\_81 沒事取這麼長，我們不需要的間接取值。 請將它變更成參照 `HeaderTextBlockStyle`。
