...
針對通用8.1 應用程式，Windows 執行階段8.x 應用程式和 Windows Phone Store 應用程式對您所顯示的 UI 和您處理 [上一頁] 按鈕的事件有不同的方法。 但是，對於 Windows 10 應用程式，您可以在應用程式中使用單一方法。 在行動裝置上，按鈕會在裝置上以電容式按鈕的方式，或以殼層中的按鈕的方式提供您使用。 在傳統型裝置上，您可以在只要能夠於應用程式中進行反向瀏覽時，就在應用程式組建區塊中加入一個按鈕，而且這會顯示在視窗化應用程式的標題列中，或平板電腦模式的工作列中。 返回按鈕事件是所有裝置系列的一個通用概念，且以硬體或軟體方式實作的按鈕都可以引發相同的 [**BackRequested**](/uwp/api/windows.ui.core.systemnavigationmanager.backrequested) 事件。
...
在 Windows 10 上執行的通用 8.1 應用程式將保留與控制項相關的 8.1 外觀和行為。 但是，當您將應用程式移植到 Windows 10 應用程式時，會在外觀和行為上產生一些需注意的差異。 針對 Windows 10 應用程式，控制項的架構和設計本質上是不變的，因此變更大多是圍繞設計語言、簡化和可用性的改進。

**注意**   PointerOver 視覺狀態與 Windows 10 apps 和 Windows 執行階段8.x 應用程式中的自訂樣式/範本相關，但無法在 Windows Phone Store 應用程式中。 基於這個理由 (和因為 Windows 10 應用程式) 支援的系統資源金鑰，建議您在將應用程式移植到 Windows 10 時，從 Windows 執行階段8.x 應用程式重複使用自訂樣式/範本。
...
| [**CommandBar**](/uwp/api/Windows.UI.Xaml.Controls.AppBar) | 在 Windows 執行階段8.x 應用程式中，命令 [**欄**](/uwp/api/Windows.UI.Xaml.Controls.AppBar) 的次要命令一律為可見。 在 Windows Phone 市集應用程式以及 Windows 10 應用程式中，不會顯示這些命令，直到命令列開啟為止。 |
...
| [**TextBox**](/uwp/api/Windows.UI.Xaml.Controls.TextBox) | 在 Windows 執行階段8.x 應用程式中，預設會關閉 [**TextBox**](/uwp/api/Windows.UI.Xaml.Controls.TextBox)的拼寫檢查。 在 Windows Phone 市集應用程式和 Windows 10 應用程式中，它預設為開啟。 |
...
| 各種類型 | 輔色適用于 Windows Phone Store 應用程式和 Windows 10 應用程式，但不適用於 Windows 執行階段8.x 應用程式。  |

如需 UWP app 控制項的詳細資訊，請參閱[依功能分類的控制項](../design/controls-and-patterns/controls-by-function.md)、[控制項清單](../design/controls-and-patterns/index.md)，以及[控制項的指導方針](../design/controls-and-patterns/index.md)。
...
不建議您嘗試支援所有縮放比例，但 Windows 10 應用程式的完整縮放比例清單為 100%、125%、150%、200%、250%、300% 及 400%。 若有提供，市集將會為每個裝置挑選大小正確的資產，同時只會下載那些資產。 市集會根據裝置的 DPI 來選取要下載的資產。 您可以從 Windows 執行階段8.x 應用程式的規模（例如140% 和220%）重複使用資產，但您的應用程式會在其中一個新的縮放因數中執行，如此一來，就無法避免某些點陣圖調整。 在各種裝置測試您的應用程式，看看您是否對結果滿意。

您可能會在標記中使用常值維度值的 Windows 執行階段8.x 應用程式中重複使用 XAML 標記 (可能會調整圖形或其他元素的大小，或許是針對印刷樣式) 。 但在某些情況下，較大的縮放比例會用於 Windows 10 應用程式適用的裝置，而非通用 8.1 應用程式適用的裝置 (例如，先前使用 140%，現在會使用 150%，而先前使用 180%，現在會使用 200%)。 因此，如果您發現這些常值目前在 Windows 10 上太大，則請嘗試將它們乘以 0.8。 如需詳細資訊，請參閱[適用於 UWP App 的回應式設計入門](../design/layout/screen-sizes-and-breakpoints-for-responsive-design.md)。
...
-   [**IsSwipeEnabled**](/uwp/api/windows.ui.xaml.controls.listviewbase.isswipeenabled)屬性 (Windows 執行階段8.x 應用程式僅適用于 Windows 10 應用程式) 。 API 仍然存在，但設定它不會產生任何作用。 支援所有先前的選取手勢，但向下撥動 (不支援此手勢，因為資料會顯示它是無法探索的) 和按一下滑鼠右鍵 (其保留來顯示操作功能表) 除外。
...
在 Windows Phone 市集應用程式中，縮小檢視會展開至螢幕的大小。 在 Windows 執行階段8.x 應用程式中，以及在 Windows 10 應用程式中，放大視圖的大小會限制為 **SemanticZoom** 控制項的界限。

在 Windows Phone 市集應用程式中，如果縮小檢視在其背景具有任何透明度，則縮小檢視 (z 順序) 背後的內容會穿透顯示。 在 Windows 執行階段8.x 應用程式中，以及在 Windows 10 應用程式中，放大視圖之後不會顯示任何專案。

在 Windows 執行階段8.x 應用程式中，當應用程式停用並重新啟動時，會關閉放大視圖， (如果顯示) 並改為顯示放大的視圖。 在 Windows Phone 市集應用程式和 Windows 10 應用程式中，縮小檢視將會維持在顯示狀態。

在 Windows Phone 市集應用程式和 Windows 10 應用程式中，按下返回按鈕時，會關閉縮小檢視。 針對 Windows 執行階段8.x 應用程式，並沒有內建的 [上一頁] 按鈕處理，所以問題不適用。
...
在 Windows 執行階段8.x 應用程式和 Windows Phone Store 應用程式中，預設字型系列是全域消費者介面。 在 Windows 10 應用程式中，預設字型系列為 Segoe UI。 因此，您應用程式中的字型標準可能會看起來不一樣。 如果您想要重新產生 8.1 文字的外觀，您可以使用像是 [**LineHeight**](/uwp/api/windows.ui.xaml.controls.textblock.lineheight) 和 [**LineStackingStrategy**](/uwp/api/windows.ui.xaml.controls.textblock.linestackingstrategy) 這樣的屬性設定您自己的標準。

在 Windows 執行階段8.x 應用程式和 Windows Phone Store 應用程式中，會將文字的預設語言設定為組建的語言，或設定為 en-us。 在 Windows 10 應用程式中，預設語言設為常見的應用程式語言 (字型遞補)。 您可以明確地設定 [**FrameworkElement.Language**](/uwp/api/windows.ui.xaml.frameworkelement.language)，但是如果您未設定該屬性的值，將盡情享受更好的字型遞補行為。
