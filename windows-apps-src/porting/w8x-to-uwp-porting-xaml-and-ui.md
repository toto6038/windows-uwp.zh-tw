...
| 控制項名稱 | 變更 |
|--------------|--------|
| **AppBar**   | 如果您使用**AppBar**控制項 ([**CommandBar** ](https://docs.microsoft.com/uwp/api/Windows.UI.Xaml.Controls.AppBar)建議改用)，然後在 Windows 10 應用程式中的預設並未隱藏。 您可以使用 [**AppBar.ClosedDisplayMode**](https://docs.microsoft.com/uwp/api/windows.ui.xaml.controls.appbar.closeddisplaymode) 屬性加以控制。 |
| **AppBar**、[**CommandBar**](https://docs.microsoft.com/uwp/api/Windows.UI.Xaml.Controls.AppBar) | 在 Windows 10 應用程式中， **AppBar**並[ **CommandBar** ](https://docs.microsoft.com/uwp/api/Windows.UI.Xaml.Controls.AppBar)有**查看更多**（省略符號）按鈕。 |
| [**CommandBar**](https://docs.microsoft.com/uwp/api/Windows.UI.Xaml.Controls.AppBar) | 在 Windows 執行階段 8.x 應用程式中的第二個命令[ **CommandBar** ](https://docs.microsoft.com/uwp/api/Windows.UI.Xaml.Controls.AppBar)永遠會顯示。 在 Windows Phone 市集應用程式，並在 Windows 10 應用程式中，直到命令列會開啟不會出現。 |
| [**CommandBar**](https://docs.microsoft.com/uwp/api/Windows.UI.Xaml.Controls.AppBar) | 針對 Windows Phone 市集應用程式，[**CommandBar.IsSticky**](https://docs.microsoft.com/uwp/api/windows.ui.xaml.controls.appbar.issticky) 的值不會影響命令列是否會消失關閉。 Windows 10 應用程式中，如果**IsSticky**已設為 true，然後**CommandBar**會忽略淺解除筆勢。 |
| [**CommandBar**](https://docs.microsoft.com/uwp/api/Windows.UI.Xaml.Controls.AppBar) | 在 Windows 10 應用程式中， [ **CommandBar** ](https://docs.microsoft.com/uwp/api/Windows.UI.Xaml.Controls.AppBar)不會處理[ **EdgeGesture.Completed** ](https://docs.microsoft.com/uwp/api/windows.ui.input.edgegesture.completed)也[ **UIElement.RightTapped** ](https://docs.microsoft.com/uwp/api/windows.ui.xaml.uielement.righttapped)事件。 它不會回應點選也不會回應向上撥動。 您仍然可以選擇處理這些事件，並設定 [**IsOpen**](https://docs.microsoft.com/uwp/api/windows.ui.xaml.controls.appbar.isopen)。 |
| [**DatePicker**](https://docs.microsoft.com/uwp/api/Windows.UI.Xaml.Controls.DatePicker)， [ **TimePicker**](https://docs.microsoft.com/uwp/api/Windows.UI.Xaml.Controls.TimePicker) | 檢閱在視覺變更為 [**DatePicker**](https://docs.microsoft.com/uwp/api/Windows.UI.Xaml.Controls.DatePicker) 和 [**TimePicker**](https://docs.microsoft.com/uwp/api/Windows.UI.Xaml.Controls.TimePicker) 之後您應用程式的外觀。 Windows 10 應用程式中的行動裝置上執行，這些控制項不會再瀏覽至 [選取] 頁面上，而是使用 light 可關閉快顯視窗。 |
| [**DatePicker**](https://docs.microsoft.com/uwp/api/Windows.UI.Xaml.Controls.DatePicker)， [ **TimePicker**](https://docs.microsoft.com/uwp/api/Windows.UI.Xaml.Controls.TimePicker) | 在 Windows 10 應用程式中，您無法暫停[ **DatePicker** ](https://docs.microsoft.com/uwp/api/Windows.UI.Xaml.Controls.DatePicker)或是[ **TimePicker** ](https://docs.microsoft.com/uwp/api/Windows.UI.Xaml.Controls.TimePicker)飛出視窗內。如果您希望在快顯類型控制項中顯示這些控制項，則您可以使用 [**DatePickerFlyout**](https://docs.microsoft.com/uwp/api/Windows.UI.Xaml.Controls.DatePickerFlyout) 和 [**TimePickerFlyout**](https://docs.microsoft.com/uwp/api/Windows.UI.Xaml.Controls.TimePickerFlyout)。 |
| **GridView**、**ListView** | 針對 **GridView**/**ListView**，請參閱 [GridView 和 ListView 變更](#gridview-and-listview-changes)。 |
| [**Hub**](https://docs.microsoft.com/uwp/api/Windows.UI.Xaml.Controls.Hub) | 在 Windows Phone 市集應用程式中，[**Hub**](https://docs.microsoft.com/uwp/api/Windows.UI.Xaml.Controls.Hub) 控制項會從最後一個區段迴繞到第一個區段。 在 Windows 執行階段 8.x 應用程式，和 Windows 10 應用程式中，中樞區段不換行周圍。 |
| [**Hub**](https://docs.microsoft.com/uwp/api/Windows.UI.Xaml.Controls.Hub) | 在 Windows Phone 市集應用程式中，[**Hub**](https://docs.microsoft.com/uwp/api/Windows.UI.Xaml.Controls.Hub) 控制項的背景影像不會以相對於中樞區段的視差移動。 在 Windows 執行階段 8.x 應用程式，和 Windows 10 應用程式中，將不會使用視差。 |
...
| **MessageDialog** | 如果您使用的是 **MessageDialog**，則請考慮改用更有彈性的 [**ContentDialog**](https://docs.microsoft.com/uwp/api/Windows.UI.Xaml.Controls.ContentDialog)。 另請參閱 [XAML UI 基本知識](https://go.microsoft.com/fwlink/p/?linkid=619992)範例。 |
...
中的 Api [ **Windows.Media.PlayTo** ](https://docs.microsoft.com/uwp/api/Windows.Media.PlayTo)命名空間不再支援 Windows 10 應用程式的益處[ **Windows.Media.Casting** ](https://docs.microsoft.com/uwp/api/Windows.Media.Casting)Api。
...
對於仍然支援的索引鍵，設計語言的變更表示某些樣式設定的屬性已經變更。 例如，`TitleTextBlockStyle`設定**FontSize**要在 Windows 執行階段 8.x 應用程式和 Windows Phone 市集應用程式中的 18.14px 14.667px。 但是，相同的樣式集**FontSize**到較大 24px 中 Windows 10 應用程式。 檢閱您的設計和配置，並在適當的位置使用適當的樣式。 如需詳細資訊，請參閱[字型的指導方針](https://docs.microsoft.com/windows/uwp/controls-and-patterns/fonts)和[設計 UWP app](https://developer.microsoft.com/en-us/windows/apps/design)。
...
[**AutoSuggestBox** ](https://docs.microsoft.com/uwp/api/Windows.UI.Xaml.Controls.AutoSuggestBox)也具有輸入的法編輯器 (Ime) 的支援。 而且，如果您想要顯示「尋找」圖示，您也可以這麼做 (與該圖示互動將會引發 **QuerySubmitted** 事件)。
...
在 Windows 執行階段 8.x 應用程式和 Windows Phone 市集應用程式中，預設字型家族是全域的使用者介面。 在 Windows 10 應用程式中，預設字型家族是 Segoe UI。 因此，您應用程式中的字型標準可能會看起來不一樣。 如果您想要重新產生 8.1 文字的外觀，您可以使用像是 [**LineHeight**](https://docs.microsoft.com/uwp/api/windows.ui.xaml.controls.textblock.lineheight) 和 [**LineStackingStrategy**](https://docs.microsoft.com/uwp/api/windows.ui.xaml.controls.textblock.linestackingstrategy) 這樣的屬性設定您自己的標準。

在 Windows 執行階段 8.x 應用程式和 Windows Phone 市集應用程式中，文字的預設語言設定為語言的組建，或設為 en-us-我們。 在 Windows 10 應用程式中的預設語言設為最上層的應用程式語言 （字型後援）。 您可以明確地設定 [**FrameworkElement.Language**](https://docs.microsoft.com/uwp/api/windows.ui.xaml.frameworkelement.language)，但是如果您未設定該屬性的值，將盡情享受更好的字型遞補行為。

如需詳細資訊，請參閱[字型的指導方針](https://docs.microsoft.com/windows/uwp/controls-and-patterns/fonts)和[設計 UWP app](https://go.microsoft.com/fwlink/p/?LinkID=533896)。 另請參閱上述的[控制項](#controls-and-control-styles-and-templates)一節，以了解文字控制項的變更。
...
磚與快顯通知，您目前使用的範本會繼續在您的 Windows 10 應用程式中運作。 但是有全新適合的範本可供您使用，在[通知、磚、快顯通知以及徽章](https://docs.microsoft.com/windows/uwp/controls-and-patterns/tiles-badges-notifications)中有詳細說明。
