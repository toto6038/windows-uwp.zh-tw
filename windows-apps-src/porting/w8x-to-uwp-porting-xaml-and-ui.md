...
針對通用8.1 應用程式，Windows 執行階段8.x 應用程式和 Windows Phone 存放區應用程式會對您所顯示的 UI 和您處理 [上一頁] 按鈕的事件有不同的方法。 但對於 Windows 10 應用程式，您可以在應用程式中使用單一方法。 在行動裝置上，按鈕會在裝置上以電容式按鈕的方式，或以殼層中的按鈕的方式提供您使用。 在傳統型裝置上，您可以在只要能夠於應用程式中進行反向瀏覽時，就在應用程式組建區塊中加入一個按鈕，而且這會顯示在視窗化應用程式的標題列中，或平板電腦模式的工作列中。 返回按鈕事件是所有裝置系列的一個通用概念，且以硬體或軟體方式實作的按鈕都可以引發相同的 [**BackRequested**](https://docs.microsoft.com/uwp/api/windows.ui.core.systemnavigationmanager.backrequested) 事件。
...
您不需要變更任何與常用鍵整合的程式碼，但您需要將一些 UI 新增至您的應用程式，以取代常用鍵列，這不是 Windows 10 shell 的一部分。 在 Windows 10 上執行的通用8.1 應用程式具有應用程式標題列中系統轉譯之 chrome 所提供的專屬替代 UI。

## <a name="controls-and-control-styles-and-templates"></a>控制項，以及控制項樣式和範本

在 Windows 10 上執行的通用8.1 應用程式將會保留與控制項相關的8.1 外觀和行為。 但是，當您將該應用程式移植到 Windows 10 應用程式時，外觀和行為會有一些差異需要注意。 在 Windows 10 應用程式中，控制項的架構和設計基本上是不變的，因此，這些變更大多是圍繞設計語言、簡化和使用性改善。

**請注意**   PointerOver 視覺狀態與 Windows 10 應用程式和 Windows 執行階段8.x 應用程式中的自訂樣式/範本有關，但不適用於 Windows Phone Store 應用程式。 基於這個理由（以及因為 Windows 10 應用程式支援的系統資源金鑰），建議您在將應用程式移植到 Windows 10 時，從 Windows 執行階段的 8. x 應用程式重新使用自訂樣式/範本。
如果您想要確定您的自訂樣式/範本使用的是一組最新的視覺狀態，而且是從預設樣式/範本的效能改進受益而來，則請編輯新的 Windows 10 預設範本的複本，並將您的自訂重新套用至該元件。 效能改進的其中一個範例是，已移除先前括住 **ContentPresenter** 的任何 **Border** 或面板，而且子元素現在會呈現框線。

以下是控制項變更的一些更具體的範例。

| 控制項名稱 | 變更 |
|--------------|--------|
| **AppBar**   | 如果您使用**AppBar**控制項（建議改用[**CommandBar**](https://docs.microsoft.com/uwp/api/Windows.UI.Xaml.Controls.AppBar) ），則 Windows 10 應用程式預設不會將它隱藏。 您可以使用 [**AppBar.ClosedDisplayMode**](https://docs.microsoft.com/uwp/api/windows.ui.xaml.controls.appbar.closeddisplaymode) 屬性加以控制。 |
| **AppBar**、[**CommandBar**](https://docs.microsoft.com/uwp/api/Windows.UI.Xaml.Controls.AppBar) | 在 Windows 10 應用程式中， **AppBar**和[**CommandBar**](https://docs.microsoft.com/uwp/api/Windows.UI.Xaml.Controls.AppBar)會有 [**查看更多**] 按鈕（省略號）。 |
| [**CommandBar**](https://docs.microsoft.com/uwp/api/Windows.UI.Xaml.Controls.AppBar) | 在 Windows 執行階段 8. x 應用程式中， [**CommandBar**](https://docs.microsoft.com/uwp/api/Windows.UI.Xaml.Controls.AppBar)的次要命令一律會是可見的。 在 Windows Phone 存放區應用程式和 Windows 10 應用程式中，除非命令列開啟，否則不會出現。 |
| [**CommandBar**](https://docs.microsoft.com/uwp/api/Windows.UI.Xaml.Controls.AppBar) | 針對 Windows Phone 市集應用程式，[**CommandBar.IsSticky**](https://docs.microsoft.com/uwp/api/windows.ui.xaml.controls.appbar.issticky) 的值不會影響命令列是否會消失關閉。 針對 Windows 10 應用程式，如果**IsSticky**設定為 true，則**CommandBar**會略過淺的關閉手勢。 |
| [**CommandBar**](https://docs.microsoft.com/uwp/api/Windows.UI.Xaml.Controls.AppBar) | 在 Windows 10 應用程式中， [**CommandBar**](https://docs.microsoft.com/uwp/api/Windows.UI.Xaml.Controls.AppBar)不會處理[**EdgeGesture**](https://docs.microsoft.com/uwp/api/windows.ui.input.edgegesture.completed)和[**RightTapped**](https://docs.microsoft.com/uwp/api/windows.ui.xaml.uielement.righttapped)事件。 它不會回應點選也不會回應向上撥動。 您仍然可以選擇處理這些事件，並設定 [**IsOpen**](https://docs.microsoft.com/uwp/api/windows.ui.xaml.controls.appbar.isopen)。 |
| [**DatePicker**](https://docs.microsoft.com/uwp/api/Windows.UI.Xaml.Controls.DatePicker)、 [ **TimePicker**](https://docs.microsoft.com/uwp/api/Windows.UI.Xaml.Controls.TimePicker) | 檢閱在視覺變更為 [**DatePicker**](https://docs.microsoft.com/uwp/api/Windows.UI.Xaml.Controls.DatePicker) 和 [**TimePicker**](https://docs.microsoft.com/uwp/api/Windows.UI.Xaml.Controls.TimePicker) 之後您應用程式的外觀。 對於在行動裝置上執行的 Windows 10 應用程式，這些控制項不會再流覽至選取頁面，而是使用可解除快顯視窗。 |
| [**DatePicker**](https://docs.microsoft.com/uwp/api/Windows.UI.Xaml.Controls.DatePicker)、 [ **TimePicker**](https://docs.microsoft.com/uwp/api/Windows.UI.Xaml.Controls.TimePicker) | 在 Windows 10 應用程式中，您無法將[**DatePicker**](https://docs.microsoft.com/uwp/api/Windows.UI.Xaml.Controls.DatePicker)或[**TimePicker**](https://docs.microsoft.com/uwp/api/Windows.UI.Xaml.Controls.TimePicker)放在飛出視窗內。如果您想要在快顯類型控制項中顯示這些控制項，您可以使用[**DatePickerFlyout**](https://docs.microsoft.com/uwp/api/Windows.UI.Xaml.Controls.DatePickerFlyout)和[**TimePickerFlyout**](https://docs.microsoft.com/uwp/api/Windows.UI.Xaml.Controls.TimePickerFlyout)。 |
| **GridView**、**ListView** | 針對 **GridView**/**ListView**，請參閱 [GridView 和 ListView 變更](#gridview-and-listview-changes)。 |
| [**中樞**](https://docs.microsoft.com/uwp/api/Windows.UI.Xaml.Controls.Hub) | 在 Windows Phone 市集應用程式中，[**Hub**](https://docs.microsoft.com/uwp/api/Windows.UI.Xaml.Controls.Hub) 控制項會從最後一個區段迴繞到第一個區段。 在 Windows 執行階段 8. x 應用程式和 Windows 10 應用程式中，中樞區段不會換行。 |
| [**中樞**](https://docs.microsoft.com/uwp/api/Windows.UI.Xaml.Controls.Hub) | 在 Windows Phone 市集應用程式中，[**Hub**](https://docs.microsoft.com/uwp/api/Windows.UI.Xaml.Controls.Hub) 控制項的背景影像不會以相對於中樞區段的視差移動。 在 Windows 執行階段 8. x 應用程式和 Windows 10 應用程式中，不會使用視差。 |
| [**中樞**](https://docs.microsoft.com/uwp/api/Windows.UI.Xaml.Controls.Hub)  | 在通用 8.1 app 中，[**HubSection.IsHeaderInteractive**](https://docs.microsoft.com/uwp/api/windows.ui.xaml.controls.hubsection.isheaderinteractive) 屬性會讓區段標頭—和旁邊呈現的 ＞ 形箭號字符—變成互動式。 在 Windows 10 應用程式中，標頭旁會有互動式的「查看更多」 affordance，但標頭本身不是互動式的。 **IsHeaderInteractive** 仍然會決定互動是否會引發 [**Hub.SectionHeaderClick**](https://docs.microsoft.com/uwp/api/windows.ui.xaml.controls.hub.sectionheaderclick) 事件。 |
...
| **ListPickerFlyout**、**PickerFlyout**  | Windows 10 應用程式的**ListPickerFlyout**和**PickerFlyout**已被取代。 如需單一選擇飛出視窗，請使用 [**MenuFlyout**](https://docs.microsoft.com/uwp/api/Windows.UI.Xaml.Controls.MenuFlyout)；如需更複雜的體驗，請使用 [**Flyout**](https://docs.microsoft.com/uwp/api/Windows.UI.Xaml.Controls.Flyout)。 |
| [**PasswordBox**](https://docs.microsoft.com/uwp/api/Windows.UI.Xaml.Controls.PasswordBox) | [**PasswordBox. IsPasswordRevealButtonEnabled**](https://docs.microsoft.com/uwp/api/windows.ui.xaml.controls.passwordbox.ispasswordrevealbuttonenabled)屬性在 Windows 10 應用程式中已被取代，且設定不會有任何作用。 請改用[**PasswordBox**](https://docs.microsoft.com/uwp/api/windows.ui.xaml.controls.passwordbox.passwordrevealmode) ，其預設為 [**查看**] （會顯示眼睛圖像，如同在 Windows 執行階段的 8. x 應用程式中）。 另請參閱[密碼方塊的指導方針](https://docs.microsoft.com/windows/uwp/controls-and-patterns/password-box)。 |
...
| [**搜尋方塊**](https://docs.microsoft.com/uwp/api/Windows.UI.Xaml.Controls.SearchBox) | 雖然 [**SearchBox**](https://docs.microsoft.com/uwp/api/windows.ui.xaml.controls.searchbox) 可在通用裝置系列中實作，但在行動裝置上無法使用所有功能。 請參閱 [SearchBox 已取代為 AutoSuggestBox](#searchbox-deprecated-in-favor-of-autosuggestbox)。 |
| **SemanticZoom** | 針對 **SemanticZoom**，請參閱 [SemanticZoom 變更](#semanticzoom-changes)。 |
| [**ScrollViewer**](https://docs.microsoft.com/uwp/api/Windows.UI.Xaml.Controls.ScrollViewer)  | [  **ScrollViewer**](https://docs.microsoft.com/uwp/api/Windows.UI.Xaml.Controls.ScrollViewer) 的某些預設屬性已經變更。 [**HorizontalScrollMode**](https://docs.microsoft.com/uwp/api/windows.ui.xaml.controls.scrollviewer.horizontalscrollmode)是**auto**、 [**VerticalScrollMode**](https://docs.microsoft.com/uwp/api/windows.ui.xaml.controls.scrollviewer.verticalscrollmode)是**auto**，而且[**ZoomMode**](https://docs.microsoft.com/uwp/api/windows.ui.xaml.controls.scrollviewer.zoommode)已**停用**。 如果新的預設值不適用於您的 app，則您可以在樣式上進行變更，或當做控制項本身的本機值變更。  |
| [**工具箱**](https://docs.microsoft.com/uwp/api/Windows.UI.Xaml.Controls.TextBox) | 在 Windows 執行階段 8. x 應用程式中， [**TextBox**](https://docs.microsoft.com/uwp/api/Windows.UI.Xaml.Controls.TextBox)預設會關閉拼寫檢查。 在 Windows Phone 存放區應用程式中，以及在 Windows 10 應用程式中，預設為開啟。 |
| [**工具箱**](https://docs.microsoft.com/uwp/api/Windows.UI.Xaml.Controls.TextBox) | [  **TextBox**](https://docs.microsoft.com/uwp/api/Windows.UI.Xaml.Controls.TextBox) 的預設字型大小已從 11 變更為 15。 |
| [**工具箱**](https://docs.microsoft.com/uwp/api/Windows.UI.Xaml.Controls.TextBox) | [  **TextBox.TextReadingOrder**](https://docs.microsoft.com/uwp/api/windows.ui.xaml.controls.textblock.textreadingorder) 的預設值已從 **Default** 變更為 **DetectFromContent**。 如果不適當，則使用 **UseFlowDirection**。 **Default** 已過時。 |
| 各種 | 輔色適用于 Windows Phone Store 應用程式和 Windows 10 應用程式，但不適用於 Windows 執行階段8.x 應用程式。  |
...
我們不建議您嘗試支援所有的規模因素，但 Windows 10 應用程式的擴展因素完整清單是100%、125%、150%、200%、250%、300% 和400%。 若有提供，市集將會為每個裝置挑選大小正確的資產，同時只會下載那些資產。 市集會根據裝置的 DPI 來選取要下載的資產。 您可以從 Windows 執行階段的 8. x 應用程式重新使用資產（例如140% 和220%），但您的應用程式將會在其中一個新的縮放比例中執行，因此某些點陣圖調整將無法避免。 在各種裝置測試您的應用程式，看看您是否對結果滿意。

您可以從 Windows 執行階段的 8. x 應用程式重新使用 XAML 標記，其中常值維度值用於標記中（可能是針對印刷樣式，可能會調整圖形或其他元素的大小）。 但在某些情況下，Windows 10 應用程式的裝置上會使用較大的縮放比例，而不是通用8.1 應用程式（例如，在140% 之前使用150%，而在 180% was 使用200%）。 因此，如果您發現這些常值在 Windows 10 上現在太大，請嘗試將它們乘以0.8。 如需詳細資訊，請參閱[適用於 UWP App 的回應式設計入門](https://docs.microsoft.com/windows/uwp/layout/screen-sizes-and-breakpoints-for-responsive-design)。
...
Windows 10 應用程式的[**windows.media.playto**](https://docs.microsoft.com/uwp/api/Windows.Media.PlayTo)命名空間中的 api 已被取代，以改用[**windows. 媒體. 轉換**](https://docs.microsoft.com/uwp/api/Windows.Media.Casting)api。

## <a name="resource-keys-and-textblock-style-sizes"></a>資源索引鍵和 TextBlock 樣式大小

設計語言已針對 Windows 10 演變，因此某些系統樣式已經變更。 在某些情況下，您會想要重新瀏覽檢視的視覺設計，使其與變更的樣式屬性彼此協調。
...
在 Windows Phone 存放區應用程式中，相應放大的視圖會展開至螢幕的大小。 在 Windows 執行階段 8. x 應用程式和 Windows 10 應用程式中，相應放大視圖的大小會限制為**SemanticZoom**控制項的界限。

在 Windows Phone 市集應用程式中，如果縮小檢視在其背景具有任何透明度，則縮小檢視 (z 順序) 背後的內容會穿透顯示。 在 Windows 執行階段 8. x 應用程式和 Windows 10 應用程式中，在縮小視圖之後看不到任何內容。

在 Windows 執行階段的 8. x 應用程式中，當應用程式停用並重新啟動時，會關閉相應放大的視圖（如果已顯示），而會改為顯示相應放大的視圖。 在 Windows Phone 存放區應用程式中，以及在 Windows 10 應用程式中，相應放大的視圖會繼續顯示是否顯示。
...
文字 (或印刷樣式) 是 UWP app 中的一個重要層面，在移植時，您可以重新檢閱檢視的視覺設計，以確保它們不會與新的設計語言產生違和感。 請使用這些插圖說明來找出可用的通用 Windows 平台 (UWP) **TextBlock** 系統樣式。 尋找對應到您所使用之 Windows Phone Silverlight 樣式的範本。 或者，您也可以建立自己的通用樣式，並將 Windows Phone Silverlight 系統樣式的屬性複製到其中。

![適用於 Windows 10 app 的系統 textblock 樣式](images/label-uwp10stylegallery.png) <br/>適用于 Windows 10 應用程式的系統 TextBlock 樣式

在 Windows 執行階段8.x 應用程式和 Windows Phone 存放區應用程式中，預設字型系列為全域使用者介面。 在 Windows 10 應用程式中，預設的字型系列是 Segoe UI。 因此，您應用程式中的字型標準可能會看起來不一樣。 如果您想要重新產生 8.1 文字的外觀，您可以使用像是 [**LineHeight**](https://docs.microsoft.com/uwp/api/windows.ui.xaml.controls.textblock.lineheight) 和 [**LineStackingStrategy**](https://docs.microsoft.com/uwp/api/windows.ui.xaml.controls.textblock.linestackingstrategy) 這樣的屬性設定您自己的標準。

在 Windows 執行階段8.x 應用程式和 Windows Phone 存放區應用程式中，文字的預設語言會設定為組建的語言，或 en-us。 在 Windows 10 應用程式中，預設語言會設為最上層應用程式語言（字型回復）。 您可以明確地設定 [**FrameworkElement.Language**](https://docs.microsoft.com/uwp/api/windows.ui.xaml.frameworkelement.language)，但是如果您未設定該屬性的值，將盡情享受更好的字型遞補行為。
...
針對通用 8.1 應用程式，預設佈景主題預設是深色。 對於 Windows 10 裝置，預設主題已變更，但您可以在 app.xaml 中宣告所要求的主題，以控制所使用的主題。 例如，若要在所有裝置上使用深色佈景主題，可將 `RequestedTheme="Dark"` 新增到根應用程式元素。

## <a name="tiles-and-toasts"></a>磚和快顯通知

針對磚和快顯通知，您目前使用的範本將會在您的 Windows 10 應用程式中繼續工作。 但是有全新適合的範本可供您使用，在[通知、磚、快顯通知以及徽章](https://docs.microsoft.com/windows/uwp/controls-and-patterns/tiles-badges-notifications)中有詳細說明。
