...
# 將 Windows Runtime 8.x 專案移植到 UWP 專案
...
5.  您在這個階段可能會看到一些建置錯誤。 但如果您知道需要變更什麼，就可以使用 Visual Studio **Find and Replace** 命令針對程式碼進行大量變更，還可以在 Visual Studio 的命令式程式碼編輯器中使用內容功能表上的 **Resolve** 和 **Organize Usings** 命令，對更多目標項目做出變更。
...
-   有項可能很有用的類似技術，就是在別無他法時，為標記檔案或 **ResourceDictionary** 檔案 (或包含該檔案的資料夾) 指定特殊名稱，如此一來，只有在特定裝置系列執行您的應用程式時，才會於執行階段自動載入。 此技術會在 [Bookstore1](w8x-to-uwp-case-study-bookstore1.md#an-optional-adjustment) 案例研究中說明。
