...
description: 此案例研究，建置 Bookstore 所提供的資訊，開始使用 Windows Phone Silverlight 應用程式會顯示群組 LongListSelector 中的資料。
title: Windows Phone Silverlight UWP 案例研究，Bookstore2
ms.date: 02/08/2017
ms.topic: article
...
本案例研究-這是根據所提供的資訊[Bookstore1](wpsl-to-uwp-case-study-bookstore1.md)— 與 Windows Phone Silverlight 應用程式來顯示群組中的資料開始**LongListSelector**。 在檢視模型中，每個 **Author** 類別執行個體都代表該作者所著之書籍的群組，而在 **LongListSelector** 中，我們可以檢視依作者分組的書籍清單，或是縮小來查看作者的捷徑清單。 與捲動書籍清單相比，捷徑清單可提供更快速的瀏覽。 我們會逐步的應用程式移植到 Windows 10 通用 Windows 平台 (UWP) 應用程式的步驟。

**附註**  時開啟 Bookstore2Universal\_10，以 Visual Studio 中，如果您看到訊息 「 Visual Studio 需要更新 」，然後遵循的步驟中設定目標平台版本[TargetPlatformVersion](w8x-to-uwp-troubleshooting.md)。

## <a name="downloads"></a>下載

[下載 Bookstore2WPSL8 Windows Phone Silverlight 應用程式](https://go.microsoft.com/fwlink/p/?linkid=522601)。
...
下圖示範 Bookstore2WPSL8 (我們即將移植的 app 看起來的樣子)。 它是依作者分組的書籍垂直捲動 **LongListSelector**。 您可以縮小至捷徑清單，然後從該處往回瀏覽至任何群組。 此應用程式有兩個主要部分：提供分組資料來源的檢視模型，以及繫結至該檢視模型的使用者介面。 如我們所見，這兩個組件的連接埠輕鬆地從 Windows Phone Silverlight 技術到通用 Windows 平台 (UWP)。
...
![桌面裝置，縮小檢視，兩種大小的視窗上執行移轉的 windows 10 應用程式](images/w8x-to-uwp-case-studies/c02-08-desk10-zo-ported.png)
...
與前一個案例研究相比，這個案例研究涉及更酷炫的使用者介面。 所有的設施及 Windows Phone Silverlight 的概念 **LongListSelector**— 和更多功能 — 找不到可供 UWP 應用程式的形式**SemanticZoom**， **ListView**， **GridView**，以及**CollectionViewSource**。 我們示範了如何同時重複使用 (或複製和編輯) UWP app 中的命令式程式碼和標記，以完成自訂符合最窄到最寬的 Windows 裝置尺寸和之間所有大小的功能、UI 和互動。
