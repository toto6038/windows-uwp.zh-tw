...
如果您的應用程式會終止所有您知道已處理的例外狀況已擲回在 XAML 標記剖析期間，然後，可能是遺失資源 （也就是索引鍵存在 Universal 8.1 應用程式，但不適用於 Windows 10 應用程式的參考例如某些系統**TextBlock**樣式索引鍵)。 或者，可能是在 **UserControl**、自訂控制項或自訂配置面板內擲回例外狀況。
...
| 在裝置上執行時，應用程式就會終止，或從 Visual Studio 啟動時，您會看到錯誤 「 無法啟動 Windows 執行階段 8.x 應用程式\[...\]. 啟用要求失敗，錯誤為「Windows 無法與目標應用程式通訊。 這通常表示目標應用程式的處理序已中止。 \[...\]"。 | 問題可能出在初始化期間您自己「頁面」中或繫結屬性 (或其他類型) 中執行的命令式程式碼。 或者，也可能是在應用程式終止時，正在剖析即將顯示的 XAML 檔案 (如果是從 Visual Studio 啟動，那將會是啟動頁面) 的情況下發生。 尋找無效的資源索引鍵及 (或) 嘗試本主題＜追蹤問題＞一節中的一些指導方針。|
