...
ms.locfileid: "57637093"
---
# <a name="known-issues-with-packaged-desktop-applications"></a>封裝的桌面應用程式的已知的問題

本文包含當您建立 Windows 應用程式套件的桌面應用程式時可能發生的已知的問題。
...
### <a name="new-containernetwork-the-object-already-exists-error"></a>New-containernetwork:此物件已經存在的錯誤
...
### <a name="your-net-application-is-compiled-with-the-anycpu-build-option-and-fails-to-install"></a>您的.NET 應用程式會使用"AnyCPU"組建選項編譯和安裝失敗
...
### <a name="publishing-public-side-by-side-fusion-assemblies-wont-work"></a>發行公用的並列 Fusion 組件無法運作
 ...
 在安裝期間，應用程式可以發行公用的並列 Fusion 組件，可存取任何其他處理序。 在處理序啟用內容建立期間，這些組件是透過名為 CSRSS.exe 的系統處理序來擷取。 針對已轉換的處理序完成此動作之後，這些組件的啟用內容建立和模組載入將會失敗。 並列 Fusion 組件會登錄於下列位置︰
...
若您應用程式中包含了副檔名為大寫 **.EXE** 的可執行檔，就有可能發生這個錯誤。 雖然您的應用程式執行時，應該不會影響此延伸模組的大小寫，這可能會造成產生此錯誤的 DAC。

若要解決此問題，請嘗試指定 **-AppExecutable**旗標，當您封裝，並使用小寫".exe"做為副檔名的主要可執行檔 (例如：MYAPP.exe)。    或者您可以變更您從小寫為大寫的應用程式的所有可執行檔的大小寫 (例如： 從。EXE.exe)。
...
安裝或啟動從 Microsoft Store 的特定應用程式時，您的電腦可能會非預期地重新開機後發生錯誤：**0x139 (核心\_安全性\_檢查\_失敗)**。
...
如果您是開發人員，建議您避免在不包含此更新的 Windows 版本上安裝已封裝應用程式。 請注意，如此您的應用程式將無法使用未安裝更新的使用者。 若要限制您已安裝此更新的使用者的應用程式的可用性，請修改 AppxManifest.xml 檔案如下所示：
...
### <a name="publisher-and-cert-mismatch-causes-signtool-error-error-signersign-failed--21470248850x8007000b"></a>發行者和憑證不相符會導致 Signtool 錯誤 」 錯誤：SignerSign() Failed" (-2147024885/0x8007000b)
...
執行**certutil**從命令列上的 PFX 檔案，並複製*主旨*輸出欄位。
...
這種情形時您的套件包含已損毀的憑證的二進位檔。 以下是一些發生此情況的原因：

* 憑證的開頭不是結尾的映像。  

* 憑證的大小不是正數。

* 憑證開始之後，不`IMAGE_NT_HEADERS32`32 位元的可執行檔或之後的結構`IMAGE_NT_HEADERS64`64 位元的可執行檔的結構。

* 憑證指標未正確對齊 WIN_CERTIFICATE 結構。

若要尋找包含不正確的 PE 憑證的檔案，請開啟**命令提示字元**，並設定環境變數，名為`APPXSIP_LOG`設為 1 的值。

```
set APPXSIP_LOG=1
```

接著，從**命令提示字元**，登入您的應用程式一次。 例如：

```
signtool.exe sign /a /v /fd SHA256 /f APPX_TEST_0.pfx C:\Users\Contoso\Desktop\pe\VLC.appx
```

包含錯誤的 PE 憑證檔的相關資訊會出現在**主控台視窗**。 例如：
