...
title: 將 Windows Phone Silverlight 移植到 UWP 的外型規格和 UX
...
ms.locfileid: "75684629"
---
#  <a name="porting-windowsphone-silverlight-to-uwp-for-form-factor-and-ux"></a>將 Windows Phone Silverlight 移植到 UWP 的外型規格和 UX


前一個主題是[移植商務與資料層](wpsl-to-uwp-business-and-data.md)。

Windows app 在電腦、行動裝置與任何其他類型的裝置，都有相同的外觀及操作方式。 使用者介面、輸入及互動模式皆非常相似，而在裝置之間移動的使用者會欣然感受到熟悉的體驗。 Windows 10 轉譯通用 Windows 平臺（UWP）應用程式的方式，像是實體大小、預設方向和有效圖元解析因素等裝置之間的差異。 好消息是系統會使用智慧型概念 (例如有效像素)，為您處理大部分的繁重工作。
...
除了 [清單視圖] 和 [格線] 視圖以外的新控制項之外，Windows Phone Silverlight 所建立的大部分配置類型在通用 Windows 平臺（UWP）中都有對等專案。 例如 [**Canvas**](https://docs.microsoft.com/uwp/api/Windows.UI.Xaml.Controls.Canvas)、[**Grid**](https://docs.microsoft.com/uwp/api/Windows.UI.Xaml.Controls.Grid) 與 [**StackPanel**](https://docs.microsoft.com/uwp/api/Windows.UI.Xaml.Controls.StackPanel)。 移植使用這些類型的大部分 UI 應該相當簡單，但是請一律尋找可利用這些版面配置面板之動態配置功能的方法，以自動在不同大小的裝置上調整大小和重新配置。

除了內建在系統控制項和版面配置面板中的動態配置之外，我們還可以使用新的 Windows 10 功能，稱為「[自我調整視覺狀態管理員](wpsl-to-uwp-porting-xaml-and-ui.md)」。

## <a name="input-modalities"></a>輸入形式

Windows Phone 的 Silverlight 介面是觸控特有的。 因此，所移植之應用程式的介面也應該理所當然地支援觸控，但是您可以選擇另外支援其他輸入形式，例如滑鼠與鍵盤。 在 UWP 中，滑鼠、手寫筆與觸控輸入已整合為 *「指標輸入」* 。 如需詳細資訊，請參閱[處理指標輸入](https://docs.microsoft.com/windows/uwp/input-and-devices/handle-pointer-input)和[鍵盤互動](https://docs.microsoft.com/windows/uwp/input-and-devices/keyboard-interactions)。
...
-   [設計 UWP 應用程式](https://developer.microsoft.com/windows/apps/design)
-   [字型方針](https://docs.microsoft.com/windows/uwp/controls-and-patterns/fonts)
-   [針對不同的外型規格進行規劃](https://docs.microsoft.com/windows/uwp/layout/screen-sizes-and-breakpoints-for-responsive-design)
