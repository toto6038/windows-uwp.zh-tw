...
title: 板型規格和 UX 移植到 UWP 的 Windows Phone Silverlight
...
Windows app 在電腦、行動裝置與任何其他類型的裝置，都有相同的外觀及操作方式。 使用者介面、輸入及互動模式皆非常相似，而在裝置之間移動的使用者會欣然感受到熟悉的體驗。 實體的大小、 預設的方向和有效的像素解析度比例到 Windows 10 的通用 Windows 平台 (UWP) 應用程式呈現的方式等裝置之間的差異。 好消息是系統會使用智慧型概念 (例如有效像素)，為您處理大部分的繁重工作。
