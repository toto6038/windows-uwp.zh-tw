...
Xbox One 遊戲台配備方向鍵；**A**、**B**、**X**、**Y** 按鍵及**檢視**和**功能表**按鈕；左搖桿、右搖桿、LB 鍵、RB 鍵、LT 鍵和 RT 鍵；以及共四個震動馬達。 兩個搖桿都提供 X 和 Y 軸中的雙類比讀數，同時在往內按時當成按鈕。 每個發射鍵都提供可代表拉回多遠的類比讀數。

> **注意**    Xbox Elite 無線控制器在其下方配備四個額外**按板**按鈕。 這些可以用來為很難同時使用的遊戲命令 (例如，同時使用右搖桿與 **A**、**B**、**X** 或 **Y** 按鍵其中任何一項) 提供備援存取，或為額外命令提供專用存取。

> **注意**    `Windows.Gaming.Input.Gamepad` 也支援 Xbox 360 遊戲台，這種遊戲台的控制項配置與標準 Xbox One 遊戲台相同。

### <a name="vibration-and-impulse-triggers"></a>震動和脈衝發射鍵

Xbox One 遊戲台提供兩個獨立馬達來進行強烈和輕微遊戲台震動，以及兩個專用馬達來提供每個發射鍵的劇烈震動 (這個特殊功能是 Xbox One 遊戲台發射鍵稱為「脈衝發射鍵」__的原因)。

> **注意**    Xbox 360 遊戲台未配備「脈衝發射鍵」__。

如需詳細資訊，請參閱[震動和脈衝發射鍵概觀](#vibration-and-impulse-triggers-overview)。

### <a name="thumbstick-deadzones"></a>搖桿靜止區域

理論上，中心位置的靜止搖桿每次產生的 X 和 Y 軸中性讀數應相同。 不過，基於搖桿的機械力和敏感度，中心位置的實際讀數僅大約是理想中性值，而且後續讀數可能會不同。 基於此原因，您必須套用小型「靜止區域」__ (接近理想中心位置的可忽略值範圍)，以補償製造差異、機械損耗或其他遊戲台問題。
...
[Gamepad][] 類別提供靜態屬性 [Gamepad][]，這個屬性是目前已連接遊戲台的唯讀清單。 因為您可能只對一些已連接的遊戲台感興趣，所以建議您維護您的專屬集合，而非透過 `Gamepads` 屬性進行存取。

下列範例會將所有已連接的遊戲台複製到新集合。
...
新增或移除遊戲台時，將會引發 [GamepadAdded][] 和 [GamepadRemoved][] 事件。 您可以註冊這些事件的處理常式來追蹤目前已連接的遊戲台。
...
在您找出感興趣的遊戲台之後，即可收集其輸入。 不過，遊戲台不是透過引發事件來溝通狀態變更，這與您可能習慣使用的一些其他類型的輸入不同。 相反的，您可以進行「輪詢」__來定期讀取其目前狀態。
...
每個搖桿都提供 X 和 Y 軸中介於 -1.0 與 +1.0 之間的類比讀數。 在 X 軸中，-1.0 的值對應到最左邊的搖桿位置；+1.0 的值對應到最右邊的搖桿位置。 在 Y 軸中，-1.0 的值對應到最底端的搖桿位置；+1.0 的值對應到最頂端的搖桿位置。 在這兩個軸中，搖桿處於中心位置時，值大約會是 0.0，但其精確值通常會不同，即使在後續讀取時也是一樣；本節稍後會討論如何減少這項差異。
...
讀取搖桿值時，如果搖桿靜止在中心位置，您會注意到搖桿值不會確實地產生中性讀數 0.0；相反地，每次移動搖桿並回到中心位置時，它們都會產生接近 0.0 的不同值。 若要減少這些變化，您可以實作小型「靜止區域」__，這是指接近理想中心位置的可忽略值範圍。 實作靜止區域的一種方法是判定搖桿與中心的距離，若讀數比您選擇的距離值更接近中心則加以忽略。 只要使用畢式定理，就可以約略計算出距離；因為搖桿讀數基本上是極性 (非平面) 值，所以距離不是確切值。 所產生的會是一個放射狀靜止區域。
...
每個遊戲台按鈕 (方向鍵的四個方向、LB 鍵、RB 鍵、左搖桿按下、右搖桿按下、**A**、**B**、**X**、**Y**、**檢視**和**功能表**) 都提供表示按下 (向下) 還是放開 (向上) 的數位讀數。 為了更有效率，按鈕讀數並非作為個別布林值呈現，而是壓縮成由 [GamepadButtons][] 列舉所呈現的單一位元欄位。

> **注意**    Xbox Elite 無線控制器在其下方配備四個額外**按板**按鈕。 這些按鈕也會以 `GamepadButtons` 列舉呈現，而且讀取其值的方式與讀取標準遊戲台按鈕的值相同。
...
[GamepadUWP 範例 _(github)_](https://github.com/Microsoft/Xbox-ATG-Samples/tree/master/Samples/System/GamepadUWP) 示範如何連接到遊戲台，以及如何讀取其狀態。
...
Xbox One 遊戲台共配備四個獨立震動馬達。 其中兩個為位在遊戲台主體的大型馬達；左馬達提供激烈、大幅度的震動，右馬達則提供較溫和、較輕微的震動。 另外兩個是小型馬達，每個發射鍵內各有一個，其提供使用者發射手指的直接劇烈大量的震動；Xbox One 遊戲台的這項獨特功能是其發射鍵稱為「脈衝發射鍵」__的原因。 將這些馬達組合在一起，可以產生多種不同的觸覺。
...
雖然可以直接修改 `Gamepad.Vibration` 屬性的成員，但是建議先將不同的 `GamepadVibration` 執行個體初始化成您要的值，然後將它複製到 `Gamepad.Vibration` 屬性，以同時變更實際馬達強度。
...
請記住，這兩個馬達不會完全相同，因此，將這些屬性設定成相同值並不會讓兩個馬達產生相同的震動。 針對任何值，如果值相同，左馬達與右馬達相較之下會以較低的頻率產生更強的震動，右馬達則會以較高的頻率產生較溫和的震動。 即使是最大值，左馬達還是無法產生右馬達的高頻率，右馬達也無法產生左馬達的高力道。 然而，因為馬達與遊戲台主體緊密連接，即使馬達具有不同特性並反應不同強度的震動，玩家不會完整感覺到單一馬達的震動。 這種方式所產生的觸覺會比馬達完全相同時更多樣、更複雜。
