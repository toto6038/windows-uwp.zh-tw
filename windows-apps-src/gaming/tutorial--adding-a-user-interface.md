...
ms.locfileid: "4964461"
---
# <a name="add-a-user-interface"></a>新增使用者介面


現在，我們的遊戲有現成的其 3D 視覺效果，它會將焦點放在新增一些 2D 元素，讓遊戲可以將遊戲狀態的相關意見反應提供給玩家的時間。 這可以透過新增簡易選單選項以及平視顯示器元件之上的 3d 圖形管線輸出。
...
使用 Direct2D，將數個使用者介面圖形和行為加入至我們 UWP DirectX 遊戲包括：
...
雖然有許多方式可以在 DirectX 遊戲中顯示文字和使用者介面元素，我們將焦點在使用[Direct2D](https://msdn.microsoft.com/library/windows/apps/dd370990.aspx)。 我們也會使用[DirectWrite](https://msdn.microsoft.com/library/windows/desktop/dd368038)文字元素。
...
Direct2D 不被專為使用者介面或像 HTML 和 XAML 的版面配置。 它不會提供使用者介面元件，例如清單、 方塊或按鈕。 它也不會提供像是 table、 或格線版面配置元件。
...
中斷方法向下進一步，這段[**Gamehud**](https://github.com/Microsoft/Windows-universal-samples/blob/5f0d0912214afc1c2a7c7470203933ddb46f7c89/Samples/Simple3DGameDX/cpp/GameHud.cpp#L320-L358)方法繪製我們的移動和射擊矩形以[**ID2D1RenderTarget::DrawRectangle**](https://msdn.microsoft.com/library/windows/desktop/dd371902)，並使用兩個呼叫[**ID2D1RenderTarget::DrawLine**](https://msdn.microsoft.com/library/windows/desktop/dd371895)交叉線。
...
**Gamehud**方法中，我們儲存遊戲視窗中的邏輯大小`windowBounds`變數。 這會使用[`GetLogicalSize`](https://github.com/Microsoft/Windows-universal-samples/blob/5f0d0912214afc1c2a7c7470203933ddb46f7c89/Samples/Simple3DGameDX/cpp/Common/DeviceResources.h#L41) **DeviceResources**類別的方法。 
```cpp
auto windowBounds = m_deviceResources->GetLogicalSize();
```

 取得遊戲視窗的大小是很重要的 UI 程式設計。 視窗的大小會獲得 dip （裝置獨立像素），其中 DIP 定義為每英吋的 1/96 度量單位。 Direct2D 會繪圖單位縮放為實際像素繪圖時，如此一來使用 Windows 每英吋點數 (DPI) 設定。 同樣地，當您繪製文字時，使用[**DirectWrite**](https://msdn.microsoft.com/en-us/library/windows/desktop/dd368038)，指定 Dip，而不是點大小的字型。 DIP 以浮點數表示。

 

### <a name="displaying-game-state-info"></a>顯示遊戲狀態資訊

除了平視顯示器，遊戲範例還有一個重疊，表示六個遊戲狀態。 所有狀態的都功能讓玩家閱讀的文字與大型黑色矩形基本類型。 因為它們不是使用這些狀態中，將不會繪製的移動視角控制器矩形和交叉線。
...
重疊會分成兩個區段：**狀態**和**動作**。 **狀態**secton 進一步細分成**標題**和**內文**矩形。 [**動作**] 區段中只需要一個矩形。 每個矩形會有不同的用途。

-   `titleRectangle` 包含的標題文字。
-   `bodyRectangle` 包含內文文字。
-   `actionRectangle` 包含通知玩家採取特定動作的文字。

遊戲的可設定的六個狀態。 使用重疊**狀態**部分來傳達遊戲的狀態。 使用的一些下列狀態與對應的方法也會更新**狀態**矩形。
...
根據遊戲的**狀態**與**動作**] 區段中將什麼事會調整文字欄位。
我們來看看如何初始化和繪製重疊的這些六個狀態。

### <a name="initializing-and-drawing-the-overlay"></a>初始化並繪製重疊

**六個狀態**有一些共同點，如此一來資源，方法所需非常類似。
...
[**GameInfoOverlay::GameInfoOverlay**](https://github.com/Microsoft/Windows-universal-samples/blob/5f0d0912214afc1c2a7c7470203933ddb46f7c89/Samples/Simple3DGameDX/cpp/GameInfoOverlay.cpp#L30-L78)建構函式會初始化覆疊，維護點陣圖表面，我們將使用上顯示給玩家的資訊。 建構函式會從傳遞給它，它會使用來建立重疊物件自己可以繪製到[**ID2D1DeviceContext**](https://msdn.microsoft.com/library/windows/desktop/hh404479) [**ID2D1Device**](https://msdn.microsoft.com/library/windows/desktop/hh404478)物件取得 factory。 [IDWriteFactory::CreateTextFormat](https://msdn.microsoft.com/en-us/library/windows/desktop/dd368203) 
...
[**DeviceResources::SetDpi**](https://github.com/Microsoft/Windows-universal-samples/blob/5f0d0912214afc1c2a7c7470203933ddb46f7c89/Samples/Simple3DGameDX/cpp/Common/DeviceResources.cpp#L514-L527)方法設定視窗的 dpi。 DPI 已變更，且必須是時呼叫這個方法，取得這時發生調整遊戲視窗的大小進行調整。 更新之後 DPI，這個方法也會呼叫[**deviceresources:: Createwindowsizedependentresources**](https://github.com/Microsoft/Windows-universal-samples/blob/5f0d0912214afc1c2a7c7470203933ddb46f7c89/Samples/Simple3DGameDX/cpp/Common/DeviceResources.cpp#L214-L487)以確定每次調整視窗大小會重新建立所需的資源。


#### <a name="gameinfooverlaycreatewindowssizedependentresources"></a>GameInfoOverlay::CreateWindowsSizeDependentResources
[**GameInfoOverlay::CreateWindowsSizeDependentResources**](https://github.com/Microsoft/Windows-universal-samples/blob/5f0d0912214afc1c2a7c7470203933ddb46f7c89/Samples/Simple3DGameDX/cpp/GameInfoOverlay.cpp#L108-L225)方法就是我們所有的繪圖會發生的位置。 以下是方法的步驟的概略說明。
...
- 點陣圖會建立名為`m_levelBitmap`，目前的 DPI 列入使用**CreateBitmap**帳戶。
...
- 點陣圖已清除進行每個像素的黑色使用[**ID2D1RenderTarget::Clear**](https://msdn.microsoft.com/en-us/library/windows/desktop/dd371772)。
...
- [**ID2D1RenderTarget::EndDraw**](ID2D1RenderTarget::EndDraw)稱為停止上的所有繪圖操作`m_levelBitmap`。
- 另一個點陣圖會建立使用**CreateBitmap**名為`m_tooSmallBitmap`做為後援，只顯示的顯示設定是否太小，該遊戲中使用。
- 重複上繪製的程序`m_levelBitmap`的`m_tooSmallBitmap`，一次只繪製字串`Paused`本文中。




現在，我們只需要是六種方法，以填滿我們的六個重疊狀態的文字 ！
...
遊戲狀態 | 狀態 set 方法 | 狀態欄位
:----- | :------- | :---------
正在載入 | [GameInfoOverlay::SetGameLoading](https://github.com/Microsoft/Windows-universal-samples/blob/5f0d0912214afc1c2a7c7470203933ddb46f7c89/Samples/Simple3DGameDX/cpp/GameInfoOverlay.cpp#L254-L306) |**Title**</br>載入資源 </br>**Body**</br> 以遞增方式列印 」。 」 來表示載入活動。
...
暫停遊戲 | [GameInfoOverlay::SetPause](https://github.com/Microsoft/Windows-universal-samples/blob/5f0d0912214afc1c2a7c7470203933ddb46f7c89/Samples/Simple3DGameDX/cpp/GameInfoOverlay.cpp#L475-L502) |**Title**</br>暫停遊戲</br>**Body**</br>無
...
動作矩形會更新[**Directxapp**](https://github.com/Microsoft/Windows-universal-samples/blob/5f0d0912214afc1c2a7c7470203933ddb46f7c89/Samples/Simple3DGameDX/cpp/GameInfoOverlay.cpp#L522-L564)後續呼叫**GameMain**物件，以提供**Directxapp**判斷正確的訊息，以所需的遊戲狀態資訊的方法玩家，例如 「 點選以繼續 」。
...
遊戲現在已可根據遊戲的狀態，玩家文字資訊的方式，我們有一種切換顯示的內容給他們在整個遊戲的方式。
