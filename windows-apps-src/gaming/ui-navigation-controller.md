...
|    Left | XY 焦點向左瀏覽
|   Right | XY 焦點向右瀏覽
|    檢視 | 顯示遊戲資訊 (計分板、遊戲統計資料、目標、世界或區域地圖)__
|    功能表 | 主要功能表 / 暫停 (設定、狀態、設備、存貨、暫停)__
|  接受 | 肯定回應 (接受、進階、確認、開始、是)__
|  取消 | 否定回應 (拒絕、反向、婉拒、停止、否)__
...
|               檢視 | 檢視按鈕                         | 檢視按鈕        | 檢視按鈕        |
|               功能表 | 功能表按鈕                         | 功能表按鈕        | 功能表按鈕        |
|             接受 | A 按鍵                            | 動作 1 按鈕    | A 按鍵           |
|             取消 | B 按鍵                            | 動作 2 按鈕    | B 按鍵           |

下表為瀏覽命令「選用集」__ 對應至各種輸入裝置的方式摘要。

| 導覽命令 | 遊戲台輸入          | 電動搖桿輸入 | 競速方向盤輸入    |
| ------------------:| ---------------------- | ------------------ | --------------------- |
|             PageUp | LT鍵           | _不支援_    | _不同_              |
|           PageDown | RT 鍵          | _不支援_    | _不同_              |
|           PageLeft | LB 鍵            | _不支援_    | _不同_              |
|          PageRight | RB 鍵           | _不支援_    | _不同_              |
|           ScrollUp | 右搖桿向上    | _不支援_    | _不同_              |
|         ScrollDown | 右搖桿向下  | _不支援_    | _不同_              |
|         ScrollLeft | 右搖桿向左  | _不支援_    | _不同_              |
|        ScrollRight | 右搖桿向右 | _不支援_    | _不同_              |
|           Context1 | X 按鈕               | _不支援_    | X 按鈕 (常用__) |
|           Context2 | Y 按鈕               | _不支援_    | _一般_) 的 Y 按鈕 ( |
|           Context3 | 左搖桿按下  | _不支援_    | _不同_              |
|           Context4 | 右搖桿按下 | _不支援_    | _不同_              |
...
每個瀏覽裝置都可以與一個使用者帳戶建立關聯，以便將使用者的身分識別連結到他們的輸入，並且可以連接耳機來協助語音交談或瀏覽功能。 如需深入了解如何處理使用者和耳機，請參閱[追蹤使用者和其裝置](input-practices-for-games.md#tracking-users-and-their-devices)以及[耳機](headset.md)。

## <a name="reading-the-ui-navigation-controller"></a>閱讀 UI 瀏覽控制器

在您識別到感興趣的 UI 瀏覽裝置之後，即可收集其輸入。 不過，瀏覽裝置不是透過引發事件來溝通狀態變更，這與您可能習慣使用的一些其他類型的輸入不同。 相反的，您可以進行_輪詢_來定期讀取其目前狀態。
...
屬於「必要集」__ 的按鈕會從 `RequiredButtons` 屬性讀取 (該屬性屬於 [UINavigationReading][] 結構)，而屬於「選用集」__ 的按鈕會從 `OptionalButtons` 屬性讀取。 因為這些屬性為位元欄位，所以使用位元遮罩來隔離感興趣按鈕的值。 設定對應位元時，即按下 (向下) 按鈕；否則為放開 (向上)。
