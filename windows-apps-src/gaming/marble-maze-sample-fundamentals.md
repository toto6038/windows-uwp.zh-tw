...
description: 本文件說明大理石迷宮專案; 的基本特性例如，如何在 Windows 執行階段環境中使用 Visual c + +、 如何建立與結構化，它和建置的方式。
...
-   幫助探索您的應用程式中的錯誤與程式碼分析工具搭配使用[SAL 附註](https://docs.microsoft.com/visualstudio/code-quality/using-sal-annotations-to-reduce-c-cpp-code-defects)。

## <a name="creating-the-visual-studio-project"></a>建立 Visual Studio 專案


如果您已下載並解壓縮範例，您可以在 Visual Studio 中開啟**MarbleMaze_VS2017.sln**檔案 （在**c + +** 資料夾） 並需要在您前方碼。
...
1. 在 Visual Studio 2017、 選取**檔 > 新增 > Project...**

2. 在 [**新增專案**] 視窗左提要欄位中，選取 [ **Installed > 範本 > Visual c + +**。

3. 在中間] 清單中選取**DirectX 11 應用程式 (通用 Windows)**。 如果您沒有看見這個選項，您可能沒有安裝必要的元件&mdash;查看[新增或移除工作負載與元件的修改 Visual Studio 2017](https://docs.microsoft.com/visualstudio/install/modify-visual-studio)如需如何安裝其他元件的資訊。

4. 授與您的專案**名稱**、**位置**檔案要儲存和**解決方案名稱**，並按一下 [**確定]**。
...
> **注意**   **/ZW** 選項與如 **/clr** 等選項不相容。 如果使用 **/clr**，這表示您無法在同一個 Visual C++ 專案中，同時以 .NET Framework 與 Windows 執行階段為目標。

 

取得 Microsoft 存放區從每個 UWP app 有的應用程式套件。 App 套件包含套件資訊清單，內含 App 的相關資訊。 例如，您可以指定應用程式的功能 (也就是對受保護系統資源或使用者資料的必要存取權)。 如果您認為應用程式需要特定的功能，請使用封裝資訊清單來宣告所需的功能。 資訊清單也可讓您指定專案屬性，例如支援的裝置旋轉、影像填滿和啟動顯示畫面。 您可以在您的專案中開啟 **Package.appxmanifest** 來編輯資訊清單。 如需有關應用程式套件的詳細資訊，請參閱[封裝應用程式](https://msdn.microsoft.com/library/windows/apps/mt270969)。
...
您可以使用觸控、 accelerometer、 Xbox 一個控制器或滑鼠來控制大理石迷宮。

-   使用控制器的方向鍵來變更現用功能表項目。
-   使用觸控 A 或開始控制器或挑選的功能表項目滑鼠按鈕。
-   使用觸控、加速計、左搖桿或滑鼠使迷宮傾斜。
-   使用觸控 A 或開始控制器或關閉功能表例如高滑鼠按鈕分數表格。
-   暫停或繼續遊戲控制器或鍵盤上的 P 鍵上使用 [開始] 按鈕。
-   使用控制器的 [返回] 按鈕或鍵盤的 Home 鍵來重新啟動遊戲。
-   高分數表格可見時，請使用 [上一頁] 按鈕控制站或鍵盤上的常用鍵來清除所有的分數。

##  <a name="code-conventions"></a>程式碼慣例


Windows 執行階段是一個程式設計介面，您可以使用它來建立只能在特定應用程式環境中執行的 UWP App。 這類的應用程式使用授權的功能、 資料類型和裝置，並分散式從 Microsoft 存放區。 Windows 執行階段在最低層級包含「應用程式二進位介面」(ABI)。 ABI 是讓 Windows 執行階段 API 可供多種程式設計語言 (例如 JavaScript、.NET 語言和 Visual C++) 存取的低階二進位合約。
...
Marble Maze 使用 **^** 並搭配 **Microsoft::WRL::ComPtr** 來管理堆積配置的物件，並使記憶體流失情況降到最低。 建議您依照 ^ 管理存留期 Windows Runtime 變數**ComPtr**管理 COM 變數 （例如，當您使用 DirectX），與**std::shared\_ptr**或**std::unique\_ptr**管理所有其他的存留週期堆積配置 c + + 物件。
...
-   當您呼叫 DirectX 常式傳回 **HRESULT** 時，請使用 **DX::ThrowIfFailed** 函式。 此函數是[DirectXHelper.h](https://github.com/Microsoft/Windows-appsample-marble-maze/blob/master/C%2B%2B/Shared/DirectXHelper.h)中定義。 如果提供的**HRESULT**錯誤代碼**ThrowIfFailed**會擲回例外狀況。 例如，**E\_POINTER** 會導致 **ThrowIfFailed** 擲回 [Platform::NullReferenceException](https://msdn.microsoft.com/library/windows/apps/hh755823.aspx)。
...
-   雖然建議您避免**HRESULT**用於未預期的錯誤，務必更避免使用的控制流程程式碼的例外處理。 因此，需要控制程式碼的流程時，最好使用 **HRESULT** 傳回值。
...
請考慮**BasicLoader::LoadMesh**方法，會在[BasicLoader.h](https://github.com/Microsoft/Windows-appsample-marble-maze/blob/e62d68a85499e208d591d2caefbd9df62af86809/C%2B%2B/Shared/BasicLoader.h)中宣告。 此方法會使用`_In_`來指定該*檔名*為輸入的參數 （與因此將只會從讀取）`_Out_`指定*vertexBuffer*和*indexBuffer*是輸出參數 （與因此將只會寫入）與`_Out_opt_`指定*vertexCount*和*indexCount*是選擇性的輸出參數 （和可能會寫入至）。 因為 *vertexCount* 和 *indexCount* 是選擇性輸出參數，所以允許為 **nullptr**。 C/C++ 程式碼分析工具會檢查這個方法的呼叫，以確定傳遞的參數符合這些準則。
...
若要執行程式碼分析您的應用程式、 的功能表列上，選擇 [**建置 > 解決方案上執行的程式碼分析**。 如需程式碼分析的詳細資訊，請參閱[使用程式碼分析進行 C/C++ 程式碼品質分析](https://docs.microsoft.com/visualstudio/code-quality/analyzing-c-cpp-code-quality-by-using-code-analysis)。
