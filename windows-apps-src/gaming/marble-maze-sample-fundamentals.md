...
若要執行的程式碼分析上您的應用程式，在功能表列上，選擇**建置 > 解決方案上執行的程式碼分析**。 如需程式碼分析的詳細資訊，請參閱[使用程式碼分析進行 C/C++ 程式碼品質分析](https://docs.microsoft.com/visualstudio/code-quality/analyzing-c-cpp-code-quality-by-using-code-analysis)。
