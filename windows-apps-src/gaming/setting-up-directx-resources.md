...
**目標：** 若要設定 c + + 通用 Windows 平台 (UWP) 應用程式中的 DirectX 資源，並顯示純色。
...
如果您宣告[ **ID3D11RenderTargetView** ](https://msdn.microsoft.com/library/windows/desktop/ff476582) ComPtr，您可以接著使用 ComPtr 的**GetAddressOf**方法來取得指標的位址**ID3D11RenderTargetView** (\*\*ID3D11RenderTargetView) 要傳遞給[ **ID3D11DeviceContext::OMSetRenderTargets**](https://msdn.microsoft.com/library/windows/desktop/ff476464)。 **OMSetRenderTargets** 會將轉譯目標繫結到[輸出合併階段](https://msdn.microsoft.com/library/windows/desktop/bb205120)，以將轉譯目標指定為輸出目標。

在範例應用程式啟動後，它會初始化並載入，然後準備執行。

### <a name="2-creating-the-direct3d-device"></a>2.建立 Direct3D 裝置

若要使用 Direct3D API 來轉譯場景，我們必須先建立一個代表顯示卡的 Direct3D 裝置。 若要建立 Direct3D 裝置，我們將呼叫 [**D3D11CreateDevice**](https://msdn.microsoft.com/library/windows/desktop/ff476082) 函式。 我們指定的層級 9.1 透過陣列中的 11.1 [ **D3D\_功能\_層級**](https://msdn.microsoft.com/library/windows/desktop/ff476329)值。 Direct3D 會依序巡覽陣列，然後傳回最高的支援功能層級。 因此，若要取得可用的最高功能層級，我們會列出**D3D\_功能\_層級**陣列項目，從最高到最低。 我們傳遞[ **D3D11\_建立\_裝置\_BGRA\_支援**](https://msdn.microsoft.com/library/windows/desktop/ff476107#D3D11_CREATE_DEVICE_BGRA_SUPPORT)旗標設為*旗標*參數設定Direct3D 資源交互操作使用 Direct2D 中。 如果我們使用偵錯組建，我們也會傳遞[ **D3D11\_建立\_裝置\_偵錯**](https://msdn.microsoft.com/library/windows/desktop/ff476107#D3D11_CREATE_DEVICE_DEBUG)旗標。 如需有關對應用程式偵錯的詳細資訊，請參閱[使用偵錯層對應用程式偵錯](https://msdn.microsoft.com/library/windows/desktop/jj200584)。
...
接著，我們會建立一個裝置用來轉譯和顯示的交換鏈結。 我們宣告並初始化[ **DXGI\_交換\_鏈結\_DESC1** ](https://msdn.microsoft.com/library/windows/desktop/hh404528)結構描述的交換鏈結。 然後，我們要設定交換鏈結成 flip 模型 (也就是已交換鏈結[ **DXGI\_交換\_效果\_FLIP\_循序**](https://msdn.microsoft.com/library/windows/desktop/bb173077#DXGI_SWAP_EFFECT_FLIP_SEQUENTIAL)設定的值**SwapEffect**成員)，並設定**格式**成員[ **DXGI\_格式\_B8G8R8A8\_UNORM**](https://msdn.microsoft.com/library/windows/desktop/bb173059#DXGI_FORMAT_B8G8R8A8_UNORM). 我們會設定**計數**隸屬[ **DXGI\_範例\_DESC** ](https://msdn.microsoft.com/library/windows/desktop/bb173072)結構**SampleDesc**成員指定為 1 而**品質**隸屬**DXGI\_範例\_DESC**為零，因為 flip 模型不支援多個範例消除鋸齒 (MSAA)。 我們會將 **BufferCount** 成員設定為 2，以便讓交換鏈結可以使用前端緩衝區呈現到顯示裝置，並且使用背景緩衝區做為轉譯目標。
...
1.  [**ID3D11DeviceContext::OMSetRenderTargets** ](https://msdn.microsoft.com/library/windows/desktop/ff476464)指定呈現目標做為輸出目標。
2.  [**ID3D11DeviceContext::ClearRenderTargetView** ](https://msdn.microsoft.com/library/windows/desktop/ff476388)清除呈現目標為純色。
3.  [**IDXGISwapChain::Present** ](https://msdn.microsoft.com/library/windows/desktop/bb174576)呈現視窗已轉譯的影像。
...
如果 app 視窗的大小發生變更，該 app 就必須調整交換鏈結緩衝區的大小、重新建立轉譯目標檢視，然後呈現已調整大小的轉譯影像。 為了調整交換鏈結緩衝區的大小，我們會呼叫 [**IDXGISwapChain::ResizeBuffers**](https://msdn.microsoft.com/library/windows/desktop/bb174577)。 在此呼叫中，我們保留緩衝區的數目和不變的緩衝區格式 ( *BufferCount*兩個參數並*NewFormat*參數[ **DXGI\_格式化\_B8G8R8A8\_UNORM**](https://msdn.microsoft.com/library/windows/desktop/bb173059#DXGI_FORMAT_B8G8R8A8_UNORM))。 我們讓交換鏈結的背景緩衝區大小與已調整大小的視窗相同。 在我們調整交換鏈結的緩衝區大小之後，我們會建立新的轉譯目標，並以類似初始化 app 時的方式呈現新的轉譯影像。
