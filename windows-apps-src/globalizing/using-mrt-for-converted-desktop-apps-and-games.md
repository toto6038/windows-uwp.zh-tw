...
description: "這個白皮書描述當封裝為 AppX 套件時，將 .NET 和 Win32 應用程式從舊版 Win32 資源移轉至 MRT 資源的技術。"
ms.author: ptorr
ms.date: 03/09/2017
ms.topic: article
ms.prod: windows
...
---
# <a name="migrating-legacy-resources-to-the-windows-10-resource-management-platform"></a>將舊版資源移轉至 Windows 10 資源管理平台

## <a name="executive-summary"></a>執行摘要

Win32 應用程式通常會當地語系化為不同語言，擴展他們的潛在市場範圍<a name="footnote1_ref" href="#footnote1"><sup>1</sup></a>。 當地語系化傳統 Win32 應用程式的方法有許多種，但 Windows 8 引進[新資源管理系統](https://msdn.microsoft.com/en-us/library/windows/apps/jj552947.aspx)(在本文件中稱為 "MRT"<a name="footnote2_ref" href="#footnote2"><sup>2</sup></a>)，其跨程式設計語言、跨應用程式類型運作，並且提供超簡單當的地語系化功能。 

MRT 結合 AppX 架構 (例如，從 Windows 市集) 的部署，自動傳送最適合指定使用者使用的資源 / 裝置，將應用程式的下載和安裝大小最小化。 對於具有大量當地語系化內容的應用程式而言，縮減約莫數個 *GB* 的 AAA 遊戲大小有絕對的重要性。 其他 MRT 好處包括 Windows Shell 和 Windows 市集的當地語系化清單，當使用者慣用的語言不符合可用資源時的自動後援邏輯。
...
在許多情況中，您可以繼續使用您的現有當地語系化格式與原始碼，同時整合 MRT 以在執行階段解析資源，並將下載大小最小化 - 這並不是全有或全無的方法。 下表總結每個階段的工作及預估成本 / 效益<a name="footnote3_ref" href="#footnote3"><sup>3</sup></a>：
...
<td>要在 Windows Shell 與 Windows 市集中顯示當地語系化的內容，需要最低工作</td>
<td>小型</td>
</tr>
<tr>
...
以下是簡單的應用程式範例，其有兩個上頭有文字標籤的按鍵 (`openButton`和`saveButton`)，以及一個當成標誌使用的 PNG 檔案 (`logoImage`)。 文字標籤當地語系化成英文和德文，而標誌針對正常桌面顯示 (100% 縮放比例) 和高解析度的手機 (300% 縮放比例) 進行最佳化<a name="footnote4_ref" href="#footnote4"><sup>4</sup></a>。 請注意，此圖表呈現模型的高階、概念視圖，並不完全對應實作︰
...
完成上述階段 0-3 後，您將會有可以傳送到「Windows 市集的應用程式「套件組合」，其可藉由省略不需要的資源將為使用者下載與安裝的程式大小最小化 (例如，非慣用語言)。 採取最後一個步驟以進一步改善應用程式的大小和功能。 
...
0. 在 XML 檔案中加入下列內容，其中<span style="background-color: yellow">反白文字</span>取代為您的預設語言的適當應用程式文字<a name="footnote5_ref" href="#footnote5"><sup>5</sup></a>
...
如果您直接編輯 XML，請開啟 `AppxManifest.xml` 檔案，對<span style="background-color: lightgreen">反白值</span>做下列變更 - 使用這個*確切*文字，而非應用程式的特定文字<a name="footnote6_ref" href="#footnote6"><sup>6</sup></a>。 這些名稱應該符合您在 `.resw` 檔案中建立的 `Names`，其以 `ms-resource:` 配置和 `Resources/` 命名空間做為首碼。 
...
0. 輸入下列命令，使用您專案適用的名稱取代 "contoso_demo.xml"，用您的應用程式適用的預設語言取代 "en-US" (如果適用則保留 en-US)。 請注意，xml 檔案會建立在上層目錄 (**不是**在專案目錄)，因為它不是應用程式的一部分<a name="footnote7_ref" href="#footnote7"><sup>7</sup></a>：
...
以反例來看，如果您的應用程式使用具單一 `Resources` 目錄的一般檔案結構 (包含檔案 `english_strings.dll` 和 `french_strings.dll`)，它**不會**和 MRT 對應得很好。 較好的結構是含子目錄及檔案 `en\strings.dll` 和 `fr\strings.dll`
<a name="footnote8_ref" href="#footnote8"><sup>8</sup></a> 的 `Resources` 目錄。 請注意，即使無法遵循此檔案命名慣例，您仍然可以使用 MRT 和 AppX 封裝的好處，只是需要做更多的工作而已。 未來的白皮書內容將涵蓋此案例。 
...
.NET 執行階段每當找不到資源 DLL 時，就會提升 `AssemblyResolve` 事件，此時所提供的事件處理常式會透過 MRT 尋找所需的檔案，並傳回組件<a name="footnote9_ref" href="#footnote9"><sup>9</sup></a>。
...
<a name="footnote1" href="#footnote1_ref">1.</a>本文不進一步推動和當地語系化作業相關的內容，假設讀者對於該項作業已經有所了解。

<a name="footnote2" href="#footnote2_ref">2.</a>過去此代表「現代化資源技術」，但「現代化」一詞已中止使用。 資源管理員也稱為 MRM (現代化資源管理員) 或 PRI (套件資源索引)。

<a name="footnote3" href="#footnote3_ref">3.</a>本表格不包含非當地語系化工作，例如提供高解析度或高對比的應用程式圖示。 如需有關提供動態磚、圖示等多項資產的詳細資訊，請參閱 MSDN。

<a name="footnote4" href="#footnote4_ref">4.</a>以下未涵蓋縮放比例的細節

<a name="footnote5" href="#footnote5_ref">5.</a>請注意，這些字串的有些長度有所限制，如需詳細資訊請參閱[有關 MSDN 的 **VisualElements** 主題](https://msdn.microsoft.com/en-us/library/windows/apps/br211471.aspx) 

<a name="footnote6" href="#footnote6_ref">6.</a>不需要使用這些確切的資源名稱，您可以自行選擇，而您的選擇必須完全符合 `.resw` 檔案內容。

<a name="footnote7" href="#footnote7_ref">7.</a>您可以選擇您需要的其他任何目錄，但是請務必汰換未來命令中的目錄

<a name="footnote8" href="#footnote8_ref">8.</a>也可以使用相同的基本檔名，但具有內嵌限定詞，例如 `strings.lang-en.dll` 和 `strings.lang-fr.dll`，而使用含語言代碼的目錄在概念上更加簡單，這便是我們著重的部分。

<a name="footnote9" href="#footnote9_ref">9.</a>請注意，如果您的應用程式已經具有 `AssemblyResolve` 處理常式供其他目的使用，您將需要使用現有的程式碼整合資源解析程式碼。
