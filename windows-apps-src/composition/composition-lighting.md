...
| [DistantLight](/uwp/api/windows.ui.composition.distantlight) | 大型無限遠方光源發出單一方向燈。 像是陽光。 |
...
當光線目標視覺效果 （新增到[目標](/uwp/api/windows.ui.composition.compositionlight.targets)清單），「 視覺效果和其所有子系已注意到，這個光源回應。 這可以是做為設定動畫效果的點狀光源方向應對 PointLight 來源根目錄的樹狀結構和所有的視覺效果，下列項目一樣簡單。

**ExclusionsFromTargets**可讓您移除的視覺效果或視覺效果的樹狀子目錄的光源以做為目標的新增類似的方式。 在樹狀目錄中的視覺項排除由 root 破解的子系不是如此一來亮起。
...
**座標空間** | 每個視覺效果具有隱含的 3D 座標空間。 X 方向是從左到右。 Y 方向是從上到下。 Z 方向是出在平面上的點。 此座標的原點是左上角的 「 視覺效果，並的單位是裝置獨立像素 (DIP)。 在這個座標中定義的光線位移。
**內錐和外錐** | 聚光燈發出的光線錐體分兩部分︰明亮的內錐和外錐體。 組合可讓您可以控制內錐和外錐角度和色彩。
**Offset** | 光源相對於其座標空間視覺效果的位移。

> [!NOTE]
> 當多個光線叫用的相同的視覺效果，或取得夠大，超過 1.0 的光線色彩值時，可能會因為鉗制光線色彩通道的變更光線的色彩。
...
- [選用]定義如何個別物件應對光線： 使用 SceneLightingEffect 搭配 EffectBrush 自訂顯示 SpriteVisual 光線反映。 反映的預設值支援的光線來源 CoordinateSpace 的子系的光源。  使用 SceneLightingEffect 繪製視覺效果會覆寫該視覺效果的預設光源。
...
[SceneLightingEffect](/uwp/api/Windows.UI.Composition.Effects.SceneLightingEffect)常用的材質建立。 SceneLightingEffect 是您想要達到更複雜，像是啟用在影像上的反射屬性和/或目的深度錯覺效果提供一般的對應項目時所使用的效果。 SceneLightingEffect 能夠藉由使用像金額反射和擴散光源屬性來自訂您的 UI。 您可以進一步自訂光源效果，以允許您個別混合和撰寫您的內容與不同的光源反應效果管線的其餘部分。

> [!NOTE]
> 場景光源不會產生陰影;它會將焦點放在 2D 轉譯效果。  它不會納入考量 3D 光源案例包含真實的光源模型，包括陰影。
...
**反射光源** | 鏡面反射的物件，使其看起來光亮建立反白顯示。 您可以控制鏡面反射的層級以及發亮的層級。  這些屬性被操作來建立像 shinny 金屬或光面紙張材質效果。
**擴散** | Diffused 的反映 scatters 中所有方向的光線。
**反射率模型** | [反射率模型](/uwp/api/windows.ui.composition.effects.scenelightingeffectreflectancemodel)可讓您[Blinn 鏡面](https://docs.microsoft.com/visualstudio/designers/how-to-create-a-basic-phong-shader)和實際基礎 Blinn 鏡面之間做出選擇。  您可以選擇實際基礎 Blinn 鏡面，當您想要有壓縮反射強光。
