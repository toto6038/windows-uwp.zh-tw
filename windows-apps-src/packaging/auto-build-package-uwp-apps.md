...
若要執行 UWP 單元測試，您必須執行下列動作：• 部署和啟動您的 App。 • 在互動模式中執行 VSTS 代理程式。 • 將您的代理程式設定為在重新開機之後自動登入。
...
如果您使用 Git 儲存機制，請將您專案的憑證檔案新增到儲存機制，這樣一來組建代理程式就可以簽署 appx 套件。 如果您沒有這麼做，Git 儲存機制會略過憑證檔案。 若要將憑證檔案新增到您的儲存機制，請以滑鼠右鍵按一下 [方案總管] 中的憑證檔案，然後在捷徑功能表中選擇 [將忽略的檔案加入原始檔控制] 命令。 
...
這個工作會將任何工作資料夾中的方案編譯為二進位檔，並產生輸出 AppX 檔案。 這個工作會使用 MSbuild 引數。  您必須指定那些引數的值。 使用下表做為指引。 
...
|**UapAppxPackageBuildMode**|StoreUpload|定義要產生的 appx 套件的類型。 (預設為不包含)|
...
因為我們已將 `UapAppxPackageBuildMode` 屬性設定為 `StoreUpload`，構件資料夾包含建議提交至市集的套件 (.appxupload)。 請注意，您也可以提交一般應用程式套件 (.appx) 或應用程式套件組合 (.appxbundle) 至市集。 根據本文的用途，我們將使用 .appxupload 檔案。
...
|將套件提交至市集的連續部署組建|發佈套件|隨選安裝|這種類型的組建會建立您可以發行至市集的套件。|
...
>注意︰如果您在同一個方案中組合多個應用程式，您可能會收到錯誤。 請參閱下列主題，以取得解決錯誤的說明︰[解決在同一個方案中組合多個 App 時出現的錯誤。](#bundle-errors) 
...
1. 首先，在您的電腦上啟用 [開發人員模式]。 請參閱＜啟用您的裝置以用於開發＞ 2. 啟用以互動式處理程序執行的服務。 請參閱＜在 Windows 上部署代理程式＞。 3. 將簽署憑證部署到代理程式。

若要這樣做，請按兩下 .cer 檔案、選擇 [本機電腦]，然後選擇 [受信任的人存放區]。
...
>注意。 使用 .NET 原生工具鏈仍然是工作流程中重要的一部分，因此您仍應該使用它來測試發行組建。 
...
完成這種類型的組建時，使用者可以從組建結果頁面的構件區段下載 appxbundle 檔案。 如果您想透過建立更完整的通訊群組對 App 進行 Beta 測試，您可以使用 HockeyApp 服務。 此服務提供進行 Beta 測試、使用者分析以及損毀診斷的進階功能。
...
首先，安裝 [HockeyApp](https://marketplace.visualstudio.com/items?itemName=ms.hockeyapp) Visual Studio 擴充功能。 

>注意︰您必須以 VSTS 系統管理員身分安裝此擴充功能。 
...
然後，您可以手動建立 HockeyApp App，或上傳現有的 appx 套件檔案。 若要深入了解，請參閱[如何建立新的 App](https://support.hockeyapp.net/kb/app-management-2/how-to-create-a-new-app)。  

若要使用現有的 appx 套件檔案，請新增建置步驟，並設定該建置步驟的二進位檔案路徑參數。 
...
>注意︰雖然 HockeyApp 工作允許您指定符號檔案的路徑，但最佳做法是在套件組合中包含符號 (appxsym 檔案)。

我們[稍後](#sideloading-best-practices)會在本指南中協助您安裝及執行側載套件。 

## <a name="set-up-a-continuous-deployment-build-that-submits-a-package-to-the-store"></a>設定將套件提交至市集的連續部署組建 

若要產生市集提交套件，請在 Visual Studio 中使用 [市集關聯精靈]，將您的 App 與市集產生關聯。

![關聯到市集](images/building-screen16.png) 

>注意︰此精靈會產生名稱為 Package.StoreAssociation.xml 的檔案，其中包含市集關聯資訊。 如果您在公用儲存機制 (例如 GitHub) 儲存您的原始程式碼，此檔案將包含該帳戶所有保留的應用程式名稱。 您可以在公開之前，先排除或刪除此檔案。

如果您沒有用來發行 App 之開發人員中心帳戶的存取權，您可以依照以下文件中的指示執行作業：[要針對協力廠商建置 App 嗎？如何封裝其市集應用程式](https://blogs.windows.com/buildingapps/2015/12/15/building-an-app-for-a-3rd-party-how-to-package-their-store-app/#e35YzR5aRG6uaBqK.97)。 
...
這將會產生可提交至市集的 .appxupload 檔案。


#### <a name="configure-automatic-store-submission"></a>設定自動市集提交

使用適用於 Windows 市集的 Visual Studio Team Services 擴充功能來整合市集 API，並將應用程式套件傳送到市集。

您需要將開發人員中心帳戶與 Azure Active Directory (AD) 連接，然後在您的 AD 中建立 App 以驗證要求。 您可以依照 [擴充功能] 頁面中的指示來完成該作業。 

一旦您設定好擴充功能，就可以新增建置工作，並使用您的應用程式識別碼以及 .appxupload 檔案的位置進行設定。
...
>注意。 您必須手動啟用此組建。 您可以使用它來更新現有的 App，但是您不能使用它來進行對市集的第一次提交。 如需詳細資訊，請參閱[使用 Windows 市集服務建立與管理市集提交](https://msdn.microsoft.com/windows/uwp/monetize/create-and-manage-submissions-using-windows-store-services)。
...
如果您想要在不發佈到市集的情況下發佈您的 App，只要這些裝置信任用來簽署應用程式套件的憑證，您就可以將您的 App 直接側載到裝置。 
...
如果您想要從網站 (例如 VSTS 或 HockeyApp) 發佈您的 appx 套件，您必須將該網站新增到您瀏覽器中信任的網站清單。 否則，Windows 會將檔案標示為已鎖定。 
