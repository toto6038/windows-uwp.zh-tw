...
- [一般用途功能](#general-use-capabilities)適用於大部分常見 app 案例。
...
如果您的應用程式宣告任何受限制的功能，您必須在[應用程式提交程序](../publish/app-submissions.md)提供資訊，才能將應用程式發行至 Microsoft Store 核准項目。 您提供這項資訊在[提交選項](../publish/manage-submission-options.md#restricted-capabilities)頁面上的 [您的提交，說明您的應用程式如何使用它宣告每個受限制的功能。
...
| **廣泛的檔案系統存取** | **broadFileSystemAccess** 功能可讓應用程式在沒有任何其他檔案選擇器樣式提示的情況下，於執行階段期間取得和目前正在執行應用程式的使用者同樣的檔案系統存取權。 請務必注意，不需要存取檔案，使用者已選擇使用 FilePicker 或 FolderPicker 這項功能。<br/><br/>這項功能適用於 [Windows.Storage](https://docs.microsoft.com/uwp/api/windows.storage) API。 請務必注意，在應用程式封裝資訊清單中宣告此功能，第一次使用任何 **Windows.Storage** API 時，將會觸發使用者可在其中授與或拒絕權限的使用者內容提示。 使用者也可以藉由切換 [設定]，隨時授與或拒絕權限。 同樣重要的是，請勿宣告任何使用這項功能的特殊資料夾功能，例如 **\[文件\]**、**\[圖片\]** 或 **\[影片\]**。 |
...
| **提高權限** | **AllowElevation**受限制的功能可讓您建立的 Microsoft 合作夥伴和企業，若要保留現有的傳統型功能需要自動-提高權限的應用程式生命週期期間或在啟動應用程式。<br/><br/>我們不建議在提交到 Microsoft Store 的應用程式中宣告這項功能。 大部分的開發人員都不會獲得核准使用這項功能。 它只會核准的特定業務的應用程式部署到私人市集透過商務用 Microsoft 網上商店的企業。  |
...
若要宣告自訂功能，修改您[的應用程式套件資訊清單](https://msdn.microsoft.com/library/windows/apps/BR211474)來源檔案 (`Package.appxmanifest`)。 新增**xmlns:uap4** XML 命名空間宣告，並宣告您的自訂功能時，請使用**uap4**前置詞。 這裡提供一個範例。
