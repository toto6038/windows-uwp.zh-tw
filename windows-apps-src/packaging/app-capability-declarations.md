...
如果您的應用程式宣告任何受限制的功能，您必須在[應用程式提交程序](../publish/app-submissions.md)提供資訊才能核准將應用程式發行至 Microsoft Store。 您提供這項資訊的提交，[提交選項](../publish/manage-submission-options.md#restricted-capabilities)\] 頁面上解釋您的應用程式如何使用它宣告每個受限制的功能。
...
| **提高權限** | **AllowElevation**受限制的功能讓 app 所建立的 Microsoft 合作夥伴和企業，若要保留現有的傳統型功能需要自動提高 app 生命週期期間或在啟動。<br/><br/>我們不建議在提交到 Microsoft Store 的應用程式中宣告這項功能。 大部分的開發人員都不會獲得核准使用這項功能。 只會針對特定業務的應用程式部署企業能夠透過商務用 Microsoft 網上商店它們的私人市集的核准。  |
...
上述的[受限制的功能](#restricted-capabilities)章節描述了相同的功能核准程序，您可以用來要求核准使用自訂的功能。 [內嵌的 sim 卡](/uwp/api/windows.networking.networkoperators.esim)Api 是需要自訂的功能的 Api 的範例。 如果您只想要在開發人員模式的本機執行您的應用程式，您不需要自訂的功能。 但是，您需要將您的應用程式發行至 Microsoft Store，或外部開發人員模式執行它。
