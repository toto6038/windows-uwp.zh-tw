...
ms.locfileid: "3822122"
---
# <a name="product-declarations"></a>產品宣告

[提交程序](app-submissions.md)的 [[屬性](enter-app-properties.md)] 頁面的**產品宣告**區段可協助確保您的 app 已適當顯示並提供給客戶及幫助他們了解如何使用您的應用程式的 「 正確設定。

下列各節將說明的宣告，以及您判斷每個宣告是否適用於您的應用程式時需要考量一些。 請注意，這些宣告其中兩個會檢查預設 （如下所述。）根據您的產品類別，您可能也會看到其他宣告。 請務必檢閱所有的宣告，並確保它們會正確反映您的提交。
...
> [!IMPORTANT]
> 除非您特別建置 app 並測試它針對該用途，否則不要清單您的 app 提供無障礙功能。 如果將您的 app 宣告為無障礙 app，但實際上不支援協助工具，則有面臨從社群收到負面意見反應的風險。
...
如果您想要防止 app 安裝至備用磁碟機或抽取式存放裝置，並只允許在內部的硬碟在其裝置上安裝，請取消選取核取此方塊。
