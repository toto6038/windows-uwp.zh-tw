...
>
>如果您先前已發行的應用程式支援舊的作業系統版本中，任何**停止擷取**您選取的日期不會套用到這些客戶，他們仍然能夠取得應用程式 (除非您提交的更新中的新選擇[可視性](choose-visibility-options.md#discoverability)區段中，或如果您選取**讓應用程式無法使用**從**應用程式概觀**頁面)。
...
請務必了解此選項，只需選取相同的影響**製作此應用程式，可探索，但無法**中[可視性](choose-visibility-options.md#discoverability)區段，然後選擇**停止擷取：任何具有直接連結的客戶可以看到產品的存放區清單，但可以只會下載它如果他們擁有的產品之前，或有促銷代碼，而且使用 Windows 10 裝置。** 若要完全停止為新客戶提供某個應用程式，請按一下 [應用程式概觀] 頁面的 [**停止提供應用程式**]。 如需詳細資訊，請參閱[從市集移除 app](guidance-for-app-package-management.md#removing-an-app-from-the-store)。
...
若要新增套用至多個市場的排程，您會建立 [*市場群組*]。 若要這樣做，請選取您想要包括的市場，然後輸入群組的名稱。 （這個名稱供您參考只是的而且將不會顯示任何客戶）。例如，如果您想要建立為 North America 市場群組，您可以選取**加拿大**，**墨西哥**，並**美國**，並將它命名**北美地區**或您選擇的另一個名稱。 當您完成建立市場群組時，按一下 [**儲存**]。 然後您會看到與上述相同的 [**發行**] 和 [**停止取得**] 選項，但您所做的選擇只會套用至該市場群組。
