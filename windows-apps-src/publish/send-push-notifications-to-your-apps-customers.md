...
2. 在 **\[通知\]** 頁面上，選取 **\[新通知\]**。
3. 在 [**選取範本**] 區段中，選擇[通知類型](#notification-template-types)您想要傳送再按一下 [**確定]**。
4. 在下一個頁面上，使用下拉式功能表選擇您想要產生通知的**\[單一應用程式\]** 或**\[多個應用程式\]**。 您只能選取已[設定為接收通知使用 Microsoft Store 服務 SDK](../monetize/configure-your-app-to-receive-dev-center-notifications.md)的應用程式。
...
2.  在表格中的現有的通知，選取進行**中**或**已完成**，然後再查看 [**傳遞率**和**應用程式啟動率**欄來查看每個通知的整體效能。
...
2.  在 [**通知**] 頁面上，按一下 [**分析**] 索引標籤。此索引標籤會顯示下列資料：
    * 圖形 toasts 和動作中心通知各種使用者動作狀態的檢視。
    * 按一下-透過-率 toasts 以及動作的 world 地圖檢視置中的通知。
3. 在頁面頂端附近，您可以選取您想要顯示資料的時間週期。 預設選項是 30D（30 天），但您可以選擇在 3、6 或 12 個月期間顯示資料，或指定自訂的日期範圍。 您也可以展開**篩選**來篩選所有應用程式及市場的資料。
