...
請依照下列步驟，在儀表板中建立推播通知，並傳送至特定的消費者區隔。

> **注意**：您必須先在 App 中呼叫 [RegisterNotificationChannelAsync](https://msdn.microsoft.com/library/windows/apps/mt771190.aspx) 方法註冊  App 以接收通知，您的 App 才能從開發人員中心收到確定目標的推播通知。 這個方法適用於 [Microsoft 市集服務 SDK](http://aka.ms/store-em-sdk)。 如需如何呼叫這個方法的詳細資訊 (包括程式碼範例)，請參閱[設定您的 App 以接收開發人員中心通知](../monetize/configure-your-app-to-receive-dev-center-notifications.md)。

1.    在 [Windows 開發人員中心儀表板](https://developer.microsoft.com/dashboard/overview)中，選取您的 app。
2.    在左邊的導覽功能表中，展開 **\[服務\]**，選取 **\[推播通知\]**。
3.    在 **\[特定對象的推播通知\]** 頁面上，選取 **\[新通知\]**。
...
8.    在 **\[通知內容\]** 區段的 **\[語言\]** 功能表中，選擇您希望通知以何種語言顯示。 如需詳細資訊，請參閱[翻譯您的通知](#translate-your-notifications)。
9.    在 **\[選項\]** 區段中，輸入文字並設定您想要的任何其他選項。 如果您使用範本開始，則預設會提供一部分，但您可以視需要變更。
...
10.    選取 **\[儲存為草稿\]** 以在稍後繼續處理通知，如果您已完成，請選取 **\[傳送\]**。
