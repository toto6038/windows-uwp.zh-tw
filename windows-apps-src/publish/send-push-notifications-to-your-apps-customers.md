---
Description: 瞭解如何將合作夥伴中心的通知傳送至您的應用程式，以鼓勵客戶群組進行行動，例如評等應用程式或購買附加元件。
...
[合作夥伴中心](https://partner.microsoft.com/dashboard) 提供資料導向的客戶參與平臺，您可以用來將通知傳送給您的所有應用程式客戶，或僅以符合您在 [客戶區段](create-customer-segments.md)中定義之準則的應用程式 Windows 10 客戶子集為目標。 您也可以建立通知，以傳送給一個以上的應用程式的客戶。

> [!IMPORTANT]
> 這些通知只能用於 UWP app。

考慮您的通知內容時，請牢記︰
- 通知中的內容必須符合Microsoft Store[內容原則](/legal/windows/agreements/store-policies#content_policies)。
...
1. **登錄您的 app 以接收推播通知。** 若要這麼做，您可以在應用程式中新增 Microsoft Store Services SDK 的參考，然後新增幾行程式碼，以便在合作夥伴中心和您的應用程式之間註冊通知通道。 我們將使用該管道傳遞您的通知給您的客戶。 如需詳細資訊，請參閱[設定您的應用程式以接收目標式推播通知](../monetize/configure-your-app-to-receive-dev-center-notifications.md)。
...
請遵循下列步驟，在合作夥伴中心中建立通知，並將其傳送至特定的客戶區段。

> [!NOTE]
> 您必須先在應用程式中呼叫 [RegisterNotificationChannelAsync](/uwp/api/microsoft.services.store.engagement.storeservicesengagementmanager.registernotificationchannelasync) 方法，以註冊應用程式以接收通知，應用程式才能接收來自合作夥伴中心的通知。 這個方法適用於 [Microsoft Store服務 SDK](https://marketplace.visualstudio.com/items?itemName=AdMediator.MicrosoftStoreServicesSDK)。 如需如何呼叫這個方法的詳細資訊 (包括程式碼範例)，請參閱[設定您的應用程式以接收目標式推播通知](../monetize/configure-your-app-to-receive-dev-center-notifications.md)。

1. 在 [合作夥伴中心](https://partner.microsoft.com/dashboard)中，展開 [ **參與** ] 區段，然後選取 [ **通知**]。
2. 在 **\[通知\]** 頁面上，選取 **\[新通知\]**。
3. 在 [ **選取範本** ] 區段中，選擇您要傳送的 [通知類型](#notification-template-types) ，然後按一下 **[確定]**。
4. 在下一個頁面上，使用下拉式功能表選擇您想要產生通知的**\[單一應用程式\]** 或**\[多個應用程式\]**。 您只能選取已 [設定為使用 Microsoft Store Services SDK 接收通知](../monetize/configure-your-app-to-receive-dev-center-notifications.md)的應用程式。
...
10. 在 [ **通知內容** ] 區段的 [ **語言** ] 功能表中，選擇您想要顯示通知的語言。 如需詳細資訊，請參閱[翻譯您的通知](#translate-your-notifications)。
11. 在 [ **選項** ] 區段中，輸入文字，並設定您想要的任何其他選項。 如果您使用範本開始，則預設會提供一部分，但您可以視需要變更。
    ...
    * **持續時間** (互動式快顯通知類型)。 您可以選擇 [ **短** ] 或 [ **長**]。
    * **案例** (互動式快顯通知類型)。 您可以選擇 **預設**、 **鬧鐘**、 **提醒**或 **來電**。
    * **基底 URI** (互動式快顯通知類型)。 如需詳細資訊，請參閱 [BaseUri](/uwp/api/windows.ui.xaml.frameworkelement.baseuri#Windows_UI_Xaml_FrameworkElement_BaseUri)。
    * **新增影像查詢** (互動式快顯通知類型)。 如需詳細資訊，請參閱 [addImageQuery](/uwp/schemas/tiles/toastschema/element-visual#attributes-and-elements)。
    * **視覺效果**。 影像、視訊或音效。 如需詳細資訊，請參閱[視覺效果](/uwp/schemas/tiles/toastschema/element-visual)。
    * **輸入** /**動作** /**選取** (互動式快顯通知類型) 。 可讓使用者與通知互動。 如需詳細資訊，請參閱 [自我調整和互動式](../design/shell/tiles-and-notifications/adaptive-interactive-toasts.md)快顯通知。
    * **繫結** (互動式磚類型)。 快顯通知範本。 如需詳細資訊，請參閱[繫結](/uwp/schemas/tiles/toastschema/element-binding)。

    > [!TIP]
    > 請嘗試使用[通知視覺化檢視](https://www.microsoft.com/store/apps/9nblggh5xsl1)應用程式來設計和測試您的調適型磚和互動式快顯通知。

12. 選取 [ **另存** 新檔] 以繼續稍後再處理通知，如果您已經完成，請選取 [ **傳送** ]。
...
-   **要求意見反應 (通知)。** 要求客戶為您的 app 提供意見的快顯通知。 當客戶選取通知時，會顯示應用程式的 [意見反應中樞] 頁面。
    > [!NOTE]
    > 如果您選擇這個範本類型，在 [**啟動**] 方塊中，記得將 {PACKAGE_FAMILY_NAME} 預留位置值取代為您的應用程式實際套件系列名稱 (PFN)。 您可以在應用程式的 [[識別](view-app-identity-details.md)] 頁面上找到應用程式的 PFN (**應用程式管理**  >  **應用程式識別**) 。
...
-   **交叉宣傳 (快顯通知)。** 宣傳您選擇的其他 app 的快顯通知。 當客戶選取通知時，會顯示其他應用程式的Microsoft Store清單。
    > [!NOTE]
    > 如果您選擇這個範本類型，在 [**啟動**] 方塊中，記得將 **{您想在此促銷的 ProductId}** 預留位置值取代為您想聯合促銷之項目的實際Microsoft Store識別碼。 您可以在 [應用程式[識別](view-app-identity-details.md)] 頁面上找到 [應用程式**管理**  >  **應用程式識別**] () 的 Store 識別碼。
...
1.  當您建立通知時，請在 [ **通知內容** ] 區段中，選取 [ **追蹤應用程式啟動率** ] 核取方塊。
2.  在您的應用程式中，呼叫 [ParseArgumentsAndTrackAppLaunch](/uwp/api/microsoft.services.store.engagement.storeservicesengagementmanager.parseargumentsandtrackapplaunch) 方法來通知合作夥伴中心您的應用程式已啟動，以回應目標通知。 這個方法由 Microsoft Store Services SDK 提供。 如需如何呼叫這個方法的詳細資訊，請參閱 [設定您的應用程式以接收合作夥伴中心通知](../monetize/configure-your-app-to-receive-dev-center-notifications.md)。


### <a name="to-view-notification-performance"></a>檢視通知效能

當您已設定通知，而您的應用程式以上述方式測量通知效能時，您可以查看通知的執行狀況。

若要查看每個通知的詳細資料：

1.  在合作夥伴中心中，展開 [ **參與** ] 區段，然後選取 [ **通知**]。
2.  在現有通知的表格中選取 [ **進行中** ] 或 [ **已完成**]，然後查看 [ **傳遞速率** ] 和 [ **應用程式啟動率** ] 資料行，以查看每個通知的高階效能。
3.  若要查看更詳細的效能詳細資料，請選取通知名稱。 在 **\[傳遞統計資料\]** 區段中，您可以檢視下列通知 **\[狀態\]** 類型的 **\[計數\]** 和 **\[百分比\]** 資訊︰
    * **失敗**︰通知因為某些原因未傳遞。 例如，如果 Windows 通知服務中發生問題，這個情況就會發生。
    * **通道到期失敗**：無法傳遞通知，因為應用程式與合作夥伴中心之間的通道已過期。 例如，如果客戶長時間未開啟您的 app，這個情況就會發生。
    * **傳送中**︰通知在待傳送的佇列中。
    * **已傳送**︰已傳送通知。
...
2.  在 [ **通知** ] 頁面上，按一下 [ **分析** ] 索引標籤。此索引標籤會顯示下列資料：
    * 快顯通知和控制中心通知的各種使用者動作狀態的圖表視圖。
    * 快顯通知和控制中心通知的「按比例」世界地圖視圖。
3. 在頁面頂端附近，您可以選取您想要顯示資料的時間週期。 預設選項是 30D（30 天），但您可以選擇在 3、6 或 12 個月期間顯示資料，或指定自訂的日期範圍。 您也可以展開 **篩選** 器，依應用程式和市場篩選所有資料。

## <a name="translate-your-notifications"></a>翻譯您的通知

為使通知發揮最大的影響力，請考慮將它們翻譯成您客戶偏好的語言。 合作夥伴中心可讓您輕鬆地利用 [Microsoft Translator](https://www.microsoft.com/translator/home.aspx) 服務的強大功能來自動翻譯您的通知。

1.  在您以預設語言撰寫通知之後，請選取 [**通知內容**] 區段中 [**語言**] 功能表底下的 [**新增語言** (]) 。
2.  在 [ **新增語言** ] 視窗中，選取您要顯示通知的其他語言，然後選取 [ **更新**]。
您的通知會自動轉譯為您在 [ **新增語言** ] 視窗中選擇的語言，而這些語言將會新增至 [ **語言** ] 功能表。
3.  若要查看您的通知轉譯，請在 [ **語言** ] 功能表中，選取您剛剛新增的語言。

關於翻譯，需注意幾件事︰
 - 您可以在該語言的內容方塊中輸入不同的 **內容** ，以覆寫自動翻譯。
