...
以特定客戶區隔為目標，透過有吸引力的個人化內容提高吸引力、留住客戶及創造營生。 有時候，針對性優惠會與 Microsoft 贊助的促銷相關聯，開發人員每次成功轉換時就會獲得紅利，讓您能賺取更多營收。
...
2. **實作應用程式內的購買選項體驗。** 在您應用程式的程式碼中使用 [*Windows 市集針對性優惠 API*] 擷取指定使用者可用的優惠，以及取得優惠 (如果與促銷相關)。 您也需要對針對性優惠建立應用程式內體驗。 如需詳細資訊，請參閱[使用市集服務管理針對性優惠](../monetize/manage-targeted-offers-using-windows-store-services.md)。
...
2.  在 **\[針對性優惠\]** 頁面中，檢閱可用的優惠。 針對您想要實作的任何優惠選取 **\[建立新優惠\]**。 
    ...
    > [!NOTE]
    > 您看見的可用優惠可能隨時間及依據帳戶準則而改變。 一開始您會看到一項或多項使用預先定義區隔準則的優惠。 不久之後，我們將讓您根據[您定義的客戶區隔](create-customer-segments.md)建立針對性優惠。
...
> 
> 考慮針對性優惠的內容時，請記住，就像所有應用程式內容一樣，優惠的內容必須符合市集[內容原則](https://msdn.microsoft.com/library/windows/apps/dn764944.aspx#content_policies)的規範。
> 
