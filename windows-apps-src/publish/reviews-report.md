---
author: jnHs
Description: "Windows 開發人員中心儀表板中的 \\[評論\\] 報告可讓您看到客戶在市集中對您 app 進行評分時所輸入的評分和意見。"
title: "評論報告"
...
Windows 開發人員中心儀表板中的 **\[評論\]** 報告可讓您看到客戶在市集中對您 app 進行評分時所輸入的評分和意見。 您可以在儀表板中檢視此資料，或[下載報告](download-analytic-reports.md)以便離線檢視。 或者，您可透過程式設計方式使用 [Windows 市集分析 REST API](../monetize/access-analytics-data-using-windows-store-services.md) 中的[取得應用程式評論](../monetize/get-app-reviews.md)方法來擷取此資料。

您也可以[直接從這個頁面](respond-to-customer-reviews.md)、[透過 Windows 市集評論 API](../monetize/submit-responses-to-app-reviews.md) 以程式設計方式，或使用[開發人員中心應用程式](https://www.microsoft.com/store/apps/dev-center/9nblggh4r5ws)回應客戶評論。
...
-   **評論內容**：預設設定為 **\[全部\]**，包括沒有加入評論文字的評分。 您可以選取 **\[包含評論內容的評分\]**，僅顯示包含已撰寫評論內容的評分。
...
-   **類別名稱**：預設篩選為 **\[全部\]**。 您可以選擇在特定[評論見解類別](#review-insights-categories)，只顯示與該類別關聯的評論。 
...
> [!TIP]
> 客戶在市集中看見的平均評分會將客戶的市場和裝置類型納入考量，因此，可能會與您在此報告中所看見的內容不同。 若要查看特定客戶的平均評分將顯示於市集的方式，您需要套用篩選來選取特定的市場和裝置類型。


## <a name="average-rating-over-time"></a>一段時間的平均評分

**\[一段時間的平均評分\]** 圖表顯示 app 平均評分在過去 12 個月內的變化情形。

**\[一段時間的平均評分\]** 圖表會顯示您的客戶在某週對您 app 的評分，而非在過去 12 個月計算所有獲得評分的平均數 (如 **\[評分明細\]** 圖表)。 這可協助您辨識出趨勢或判斷評分是否受更新或其他因素影響。
...
您可以對頁面上評論依照日期和/或評分、以遞增或遞減的順序來進行排序。 按一下 **\[排序方式\]** 連結來檢視依日期和/或評分進行排序的選項。 
...
## <a name="review-insight-categories"></a>評論見解類別

您可以檢視 **\[評論見解類別\]**，根據相關類別來分組檢視評論。
...
您可以使用 Windows 市集開發人員中心儀表板、[Windows 市集評論 API](../monetize/submit-responses-to-app-reviews.md) 或[開發人員中心應用程式](https://www.microsoft.com/store/apps/dev-center/9nblggh4r5ws)，將回應傳送到許多客戶的評論。 如需詳細資訊，請參閱[回應客戶評論](respond-to-customer-reviews.md)。
