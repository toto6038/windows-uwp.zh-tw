...
- **評論內容的**： 預設設定是**評等與評論內容**，這表示將會顯示只包含評論內容的評等。 您可以選取**所有**顯示所有的評等，甚至不包含任何評論的文字。 請注意，**評分明細**圖表一律會顯示所有的評論，無論您的選取項目。
...
此圖表也看過[評等報告](ratings-report.md)，以及更多有關您應用程式的評等詳細資料。
