---
Description: 合作夥伴中心的審查報告可讓您查看客戶在商店中分級應用程式時所輸入的評論（留言）。
title: 評論報告
ms.assetid: E50C3A4D-1D8A-4E5B-8182-3FAD049F2A2D
ms.date: 08/16/2018
ms.topic: article
keywords: windows 10，uwp，評論，批註，審核者
...
ms.locfileid: "78853235"
---
# <a name="reviews-report"></a>評論報告


[合作夥伴中心](https://partner.microsoft.com/dashboard)的**審查**報告可讓您查看客戶在商店中分級應用程式時所輸入的評論（留言）。

您可以在合作夥伴中心內查看此資料，或[下載報表](download-analytic-reports.md)以離線觀看。 或者，您可以使用[Microsoft Store 分析 REST API](../monetize/access-analytics-data-using-windows-store-services.md)中的「[取得應用程式評論](../monetize/get-app-reviews.md)」方法，以程式設計方式抓取此資料。

您也可以[直接從這個頁面](respond-to-customer-reviews.md)或透過[Microsoft Store 審查 API](../monetize/submit-responses-to-app-reviews.md)以程式設計方式回應客戶評論。
...
- **審查內容**：預設設定為 [**使用審核內容分級**]，這表示只會顯示具有審查內容的評等。 您可以選取 [**全部**] 以顯示所有分級，甚至是未包含任何書面審核文字的評等。 請注意，不論您的選擇為何，**評等明細**圖表一律會顯示所有評論。
-   **作業系統版本**：預設設定為 **\[全部\]** 。 如果您只想在此頁面上顯示該作業系統版本客戶所提供的評論，則可以選擇特定作業系統版本。
-   **套件版本**：預設設定為 [所有版本]。 如果您的 App 包含多個套件，您可以在這裡選擇一個特定的套件版本，只顯示評論您的 app 時具有該套件之客戶所提供的評論。
...
-   **類別名稱**：預設篩選為 **\[全部\]** 。 您可以選擇特定的 [[審查深入](#review-insight-categories)解析] 類別，只顯示我們與該類別相關聯的評論。 
...
**評等明細**圖表會顯示在這份報表的頂端，讓您可以快速查看下列各項： 
- app 的平均評分/星級評等。
- 在過去 12 個月 app 的評分總數。
- 每個星級評等的評分總數。
...
請注意，即使您在 [**審核內容**] 頁面篩選中選取 [**具有審核內容的評**等]，此圖表一律會包含您的所有評論。

此圖表也會出現在評等[報告](ratings-report.md)中，以及更多有關您應用程式評等的詳細資料。
...
-   評論的日期。
-   出現在 Microsoft Store 中的審核者名稱。
-   評論者的國家/地區。
-   留下評論時客戶裝置上的 app 套件版本。 （這項資訊不適用於線上提交或由客戶在 Windows 8.1 和更早版本提交的評論）。
-   客戶留下評論時所使用裝置的作業系統版本。
-   客戶留下評論時所使用裝置的名稱。 （這項資訊不適用於線上提交或由客戶在 Windows 8.1 和更早版本提交的評論）。
...
您可以對頁面上評論依照日期和/或評等、以遞增或遞減的順序來進行排序。 按一下 [**排序依據**] 連結以查看依**日期**和/或**分級**排序的選項。

您也可以使用 [搜尋] 方塊，在您的應用程式評論中搜尋特定單字或片語。 請注意，即使是以不同的語言撰寫評論，也只會搜尋客戶所撰寫的原始審核文字。 未搜尋轉譯的審查文字。
...
如上所述，搜尋您的評論時，只會搜尋客戶所剩下的原始文字（而不是任何翻譯的文字），即使您已核取 [**轉譯評論**] 方塊也一樣。


## <a name="responding-to-customer-reviews"></a>回應客戶評論

您可以使用[合作夥伴中心](https://partner.microsoft.com/dashboard)或[Microsoft Store 審核 API](../monetize/submit-responses-to-app-reviews.md) ，將回應傳送給許多客戶的評論。 如需詳細資訊，請參閱[回應客戶評論](respond-to-customer-reviews.md)。
