---
description: 合作夥伴中心中的 [評論] 報告可讓您查看客戶在商店中對您的應用程式評分時所輸入的評論 (批註) 。
title: 評論報告
ms.assetid: E50C3A4D-1D8A-4E5B-8182-3FAD049F2A2D
ms.date: 08/16/2018
ms.topic: article
keywords: windows 10、uwp、評論、留言、評論者
...
ms.locfileid: "93030301"
---
# <a name="reviews-report"></a>評論報告


[合作夥伴中心](https://partner.microsoft.com/dashboard)中的 [ **評論** ] 報告可讓您查看客戶在商店中對您的應用程式評分時所輸入的評論 (批註) 。
...
您也可以 [直接從這個頁面](respond-to-customer-reviews.md) 或透過 [Microsoft Store 評論 API](../monetize/submit-responses-to-app-reviews.md)，以程式設計方式回應客戶評論。
...
- **審核內容** ：預設設定是 **具有審核內容的評** 等，這表示只會顯示具有審核內容的評等。 您可以選取 [ **全部** ] 以顯示所有評等，甚至是未包含任何書面評論文字的評等。 請注意，不論您的選擇為何， **評等細目** 圖一律會顯示所有評論。
...
-   **類別名稱** ：預設篩選為 **\[全部\]** 。 您可以選擇特定的 [評論深入解析類別](#review-insight-categories) ，只顯示與該類別相關聯的評論。 
...
**評等細目** 圖表會出現在這份報表的頂端，讓您可以快速查看下列各項： 
- app 的平均評分/星級評等。
- 在過去 12 個月 app 的評分總數。
- 每個星級評等的評分總數。
...
請注意，此圖表一律會包含您所有的評論，即使您在 [ **審核內容** ] 頁面篩選中選取 **了 [與審核內容分級** ]。

您也可以在 [評等報告](ratings-report.md)中看到此圖表，以及更多關於應用程式評等的詳細資料。
...
**深入解析類別** 圖會根據我們判斷的類別，將您的評論分組，這些類別可能與審核相關聯。
...
您可以對頁面上評論依照日期和/或評分、以遞增或遞減的順序來進行排序。 按一下 [ **排序依據** ] 連結，以查看依 **日期** 和/或 **分級** 排序的選項。

您也可以使用 [搜尋] 方塊，在您的應用程式評論中搜尋特定單字或片語。 請注意，即使是以不同的語言撰寫評論，還是只會搜尋客戶所撰寫的原始評論文字。 未搜尋翻譯的評論文字。
...
根據預設，會幫您翻譯不是使用您慣用語言撰寫的評論。 如果您想要的話，可以在評論清單上方的右上方取消核取 [ **翻譯審核** ] 核取方塊，以停用評論轉譯。

請注意：評論是由系統自動翻譯，結果不一定正確。 我們也提供原文，供您與翻譯比較，或透過其他方式翻譯。

如上所述，搜尋您的評論時，只會搜尋客戶所留下的原始文字 (而非任何翻譯的文字) ，即使您已核取 [ **翻譯審核** ] 方塊也是一樣。


## <a name="responding-to-customer-reviews"></a>回應客戶評論

您可以使用 [合作夥伴中心](https://partner.microsoft.com/dashboard) 或 [Microsoft Store 評論 API](../monetize/submit-responses-to-app-reviews.md) ，將回應傳送給許多客戶的評論。 如需詳細資訊，請參閱[回應客戶評論](respond-to-customer-reviews.md)。

根據您所看到的評等和評論，以下是可以考慮採取的一些其他措施。

-   如果您發現有許多評論都建議推出新功能或變更功能，或抱怨某個問題，可以考慮發行新版本，解決特定的意見 (務必更新您的 app [描述](./create-app-store-listings.md)，指出問題已經修正)。
