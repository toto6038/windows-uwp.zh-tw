...
-   **使用清單和精簡的段落。** 潛在客戶很可能只會快速瀏覽您的 app 描述。 使用簡短的段落和清單將內容分段，可以讓使用者更容易閱讀。
    ...
    > [!NOTE]
    > 新增 [產品功能](create-app-store-listings.md#product-features) 清單也有助於快速顯示您應用程式的用途。 這個清單要顯示在 app 描述的正下方。
...
-   **不要包含連結或別處的資訊。** 在描述欄位中輸入的 URL 不是可按一下的連結，所以不要嘗試加入隱私權原則或支援網站之類的資訊連結。 相反地，請在提交內容頁面的指定區域中新增這些 **屬性** 。
