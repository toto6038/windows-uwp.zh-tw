...
在前置處理期間，當偵測到 Windows Phone 套件有問題時，您可能會看到**錯誤 2001**。 在大部分的情況下，您必須重建您的 app 套件來更正錯誤。 一旦完成，在您再次按一下 [提交至市集][](upload-app-packages.md) 前，請先在提交的 [**套件**] 頁面上使用新套件取代舊套件。
...
如果您看到「在套件中找到的名稱不是其中一個保留的應用程式名稱。請保留應用程式名稱和/或使用此語言的正確應用程式名稱更新您的套件」錯誤，可能是因為您在套件中輸入的名稱不正確。 如果您使用的應用程式名稱尚未在合作夥伴中心保留，也可能會發生此錯誤。 您通常可以依照下列步驟解決這個錯誤：
