...
ms.locfileid: "2918709"
---
# <a name="package-flights"></a>套件正式發行前小眾測試版

您可以使用新的提交套件，將一組有限的測試者的特定套件散佈。 您已經發佈到市集的套件將會用於其他客戶，因此將不會中斷他們的體驗。

使用新套件的提交，僅有套件是不同的。市集清單詳細資料可使用相同的所有客戶。 在您的正式群組中的任何人都將會收到不在正式群組中的客戶仍繼續收到標準 （非-發行前小眾測試） 的套件時套件傳輸中包含的套件。  如果您稍後決定您想要將套件從套件正式提供給您所有客戶，您可以輕鬆地使用這些相同的套件，在非正式的提交中。 請注意，套件正式必須通過[認證程序](the-app-certification-process.md)中，只是相同任何提交。
...
當您已指定套件並設定任何必要的選項時，按一下 **\[提交至市集\]**。 您的套件正式發行前小眾測試版將會進行 [app 認證程序](the-app-certification-process.md)。 請注意，套件包含在套件正式必須符合[Microsoft Store 原則](https://docs.microsoft.com/legal/windows/agreements/store-policies)，就像所有提交一樣。
...
若要建立新的提交的套件正式您已經發佈，按一下 [App 概觀] 頁面上的正式名稱旁邊的**更新**。 然後您可以上傳新的套件 (並移除不需要的套件)，就像在非正式發行前小眾測試版提交中所做的一樣。 進行任何其他需要的變更，然後按一下 **\[提交至市集\]** 來透過[應用程式認證程序](the-app-certification-process.md)傳送更新的套件正式發行前小眾測試版。
