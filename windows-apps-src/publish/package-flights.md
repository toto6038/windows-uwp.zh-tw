...
ms.locfileid: "2830124"
---
# <a name="package-flights"></a>套件正式發行前小眾測試版

您可以使用套件飛行散佈一組有限的測試的特定套件。 您已經發佈至存放區的套件將用於其他客戶，因此將不會中斷其經驗。

使用套件飛行僅套件是不同;列出詳細資料的存放區會與客戶的所有相同。 航班群組中的任何人都將會收到您納入套件航班，而不在 [航班] 群組中的客戶會繼續接收規則 (非 flighted) 套件的套件。  如果您稍後決定您想要從套件航班套件提供給您的客戶，您可以輕鬆地使用這些相同套件中非 flighted 送出。 請注意套件飛行必須相同任何提交通過[憑證處理程序](the-app-certification-process.md)。

當您設定好套件飛行時，您可以指定應將其新增至**已知使用者群組**（有時稱為航班群組） 取得特定套件的人員。 正式發行前小眾測試版群組中正在使用執行 Windows 10 某一版本且支援套件正式發行前小眾測試版 (Windows.Desktop build 10586 或更新版本；Windows.Mobile build 10586.63 或更新版本；或 Xbox One) 之裝置的任何人，都能夠從您為該特定群組指定的套件正式發行前小眾測試版取得套件。 (您的套件正式發行前小眾測試版可包含以任何作業系統版本 (包括 Windows 8.1/Windows Phone 8.1 或更新版本) 為目標的套件。) 尚未新增至您其中一個正式發行前小眾測試版群組，或正在使用的裝置不支援套件正式發行前小眾測試版的任何人員，則將從非正式發行前小眾測試版提交取得套件。
...
> [!NOTE]
> 如果您有多個套件正式發行前小眾測試版，您將需要為它們指派排名。 如需詳細資訊，請參閱[Add 和排名其他套件飛行](#add-and-rank-additional-package-flights)下。
...
您可以選取與之前發行的提交 (非正式發行前小眾測試版提交，或如果您有多個其他套件正式發行前小眾測試版，可選取其中之一) 相關聯的套件。 如果您需要將上傳要用於此套件航班新套件，您可以將其上傳此處 （使用[同一個處理序上傳至一般非 flighted 送出的應用程式套件時](upload-app-packages.md)）。 當您完成指定此套件正式發行前小眾測試版中要包含的套件時，按一下 **\[儲存\]**。
...
也請記得列出 info 您儲存區和裝置系列的可用性會根據您非 flighted 送出。 正式發行前小眾測試版群組中的客戶，只能將 app 下載到由您的非正式發行前小眾測試提交所支援的裝置系列。 如需詳細資訊，請參閱[裝置系列支援](#device-family-support)。 
...
> [!IMPORTANT]
> 當逐漸推出的套件航班套件，不包含在取得新的套件的百分比的人員會取得套件舊的套件航班提交 （除非有提供給他們的較高排名航班）。
...
當您已指定套件並設定任何必要的選項時，按一下 **\[提交至市集\]**。 您的套件正式發行前小眾測試版將會進行 [app 認證程序](the-app-certification-process.md)。 [Microsoft 存放區原則](https://docs.microsoft.com/legal/windows/agreements/store-policies)中，必須符合 packages 套件航班中包含的記事與所有送出相同。
...
若要建立新的送出套件航班您已經刊登，按一下 [應用程式概觀 」 頁面上的航班名稱旁的**更新**。 然後您可以上傳新的套件 (並移除不需要的套件)，就像在非正式發行前小眾測試版提交中所做的一樣。 進行任何其他需要的變更，然後按一下 **\[提交至市集\]** 來透過[應用程式認證程序](the-app-certification-process.md)傳送更新的套件正式發行前小眾測試版。

若要修改現有的正式發行前小眾測試版，但不建立和提交新的更新，按一下正式發行前小眾測試版名稱旁邊的 **\[修改\]**。 這可讓您變更詳細資料 (例如正式發行前小眾測試版群組、名稱及排名)，而套件正式發行前小眾測試版不需要再次經歷認證程序。 請注意，如果您有更新進度中或尚未尚未發佈套件航班，如果您將不會看到 [**修改**] 選項。 
...
在您已建立第一個套件正式發行前小眾測試版之後，您可依照上述概略說明的程序建立其他套件正式發行前小眾測試版。 唯一的差異是如果您已經建立一個套件正式發行前小眾測試版，您將需要在 **\[排名\]** 區段中指定所有套件正式發行前小眾測試版的優先順序。 這可讓決定要提供給任何個別的客戶若它們在多個航班群組中的套件的存放區。 正式發行前小眾測試版群組中的人員將永遠能夠取得提供給他們使用的最高排名套件正式發行前小眾測試版，即使較低排名的套件正式發行前小眾測試版包含版本號碼較大的套件也一樣。

依照預設，您的新套件正式發行前小眾測試版將取得最高排名。 如果您想要變更其排名，可以將它向下移動 (或向上移動) 來將它置於您其他套件正式發行前小眾測試版之間的正確位置。

請注意，您非 flighted 送出一定是依排名最低 (#1)。 也就是說，不在您任何正式發行前小眾測試版群組中的人員，只能透過市集從非正式發行前小眾測試版提交中取得套件。 航班] 群組中的人員將會一律取得套件提供最高排名套件航班它們 （但絕不非 flighted 送出作業，因為它具有最低的排名）。 這可以在您要決定如何將套件散發給可能是您多個正式發行前小眾測試版群組成員的人員時，提供您一些彈性。
