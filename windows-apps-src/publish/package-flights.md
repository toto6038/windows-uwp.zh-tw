...
ms.locfileid: "4499692"
---
# <a name="package-flights"></a>套件正式發行前小眾測試版

您可以使用新的提交套件，將一組有限的測試者的特定套件散佈。 將其他客戶，使用您已經發佈到市集的套件，因此將不會中斷他們的體驗。

使用套件正式僅有套件是不同的。市集清單詳細資料可使用相同的所有客戶。 在您的正式群組中的任何人都將會收到不在正式群組中的客戶仍繼續收到標準 （非-發行前小眾測試） 的套件時在封裝傳輸中包含的套件。  如果您稍後決定您想要將套件從套件正式提供給您所有客戶，您可以輕鬆地使用那些相同的套件，在非正式的提交。 請注意，套件正式必須通過[認證程序](the-app-certification-process.md)中，只是相同任何提交。
...
當您已指定套件並設定任何必要的選項時，按一下 **\[提交至市集\]**。 您的套件正式發行前小眾測試版將會進行 [app 認證程序](the-app-certification-process.md)。 請注意，套件包含在您的套件正式必須遵守 「 [Microsoft Store 原則](https://docs.microsoft.com/legal/windows/agreements/store-policies)中，就像所有提交一樣。
...
在您已建立第一個套件正式發行前小眾測試版之後，您可依照上述概略說明的程序建立其他套件正式發行前小眾測試版。 唯一的差異是如果您已經建立一個套件正式發行前小眾測試版，您將需要在 **\[排名\]** 區段中指定所有套件正式發行前小眾測試版的優先順序。 這可讓市集判斷哪一個套件提供給任何獨立客戶，如果它們位於多個其中一個正式群組。 正式發行前小眾測試版群組中的人員將永遠能夠取得提供給他們使用的最高排名套件正式發行前小眾測試版，即使較低排名的套件正式發行前小眾測試版包含版本號碼較大的套件也一樣。

依照預設，您的新套件正式發行前小眾測試版將取得最高排名。 如果您想要變更其排名，可以將它向下移動 (或向上移動) 來將它置於您其他套件正式發行前小眾測試版之間的正確位置。

請注意，您的非正式提交始終排在最低 (#1)。 也就是說，不在您任何正式發行前小眾測試版群組中的人員，只能透過市集從非正式發行前小眾測試版提交中取得套件。 該群組中的人員將會一律取得套件從可用的最高排名套件正式它們 （但永遠不會非正式的提交，因為它具有最低排名）。 這可以在您要決定如何將套件散發給可能是您多個正式發行前小眾測試版群組成員的人員時，提供您一些彈性。
