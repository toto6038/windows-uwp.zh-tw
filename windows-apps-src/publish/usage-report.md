---
Description: 合作夥伴中心內的使用方式報告可讓您瞭解客戶如何使用您的應用程式。
title: 使用方式報告
...
ms.locfileid: "75685114"
---
# <a name="usage-report"></a>使用方式報告


[合作夥伴中心](https://partner.microsoft.com/dashboard)內的**使用方式**報告可讓您查看 Windows 10 （包括 Xbox）上的客戶如何使用您的應用程式，並顯示您所定義之自訂事件的相關資訊。 您可以在合作夥伴中心內查看此資料，或[下載報表](download-analytic-reports.md)以離線觀看。
...
此圖表有不同的索引標籤可供您查看，其中顯示依日或周的使用量（視您選取的持續時間而定）。
...
- **Engagement**：顯示每位使用者的平均參與分鐘數（所有使用者會話的平均持續時間）。 
- **保留**：顯示在所選時段內的 **\[DAU/MAU\]** 總數（每日作用中使用者/每月作用中使用者）。
- 變換**預測**：根據最近的使用量，顯示我們所預測的使用者可能很快就會停止使用您的應用程式。

選取 [ **30D**時間週期] 時，您可能會在查看 [**使用者**]、[**裝置**] 或 [**持續時間**] 索引標籤時看到圓形標記。 這表示您想要瞭解的指定值有大幅增加或減少。 圓圈出現的日期代表我們偵測到一周的結束時間，而這是與之前的一周相比，明顯增加或減少。 若要查看已變更內容的更多詳細資料，請將滑鼠停留在圓形上。  
