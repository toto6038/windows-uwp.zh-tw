...
description: 合作夥伴中心的廣告行銷活動報告可讓您查看應用程式升級 Ad 活動的執行情況。
ms.date: 10/31/2018
ms.topic: article
...
ms.locfileid: "74260068"
---
# <a name="ad-campaign-report"></a>廣告行銷活動報告

[合作夥伴中心](https://partner.microsoft.com/dashboard)的**廣告行銷活動**報告可讓您查看應用程式升級[Ad 活動](create-an-ad-campaign-for-your-app.md)的執行情況。 若要查看報告，請展開左導覽功能表中的 [**吸引**地]，然後選取 [**廣告活動**]。
...
透過合作夥伴中心執行「安裝」廣告行銷活動，可提供廣告應用程式所需的更多風險。 廣告曝光會向最可能對應用程式感興趣的客戶顯示廣告，這些客戶就可能會點擊廣告並從市集安裝應用程式。 以前，我們很難區分使用者安裝應用程式時是因為廣告活動而安裝，或是從其他來源安裝。
...
3.  客戶需符合 [COPPA](https://www.ftc.gov/enforcement/rules/rulemaking-regulatory-reform-proceedings/childrens-online-privacy-protection-rule) 需求 (無法追蹤不符合 COPPA 需求的客戶)。

因此，app 安裝追蹤記錄到的數字可能低於因為廣告活動所產生的安裝次數。 請注意，您可以在合作夥伴中心的 [[收購](acquisitions-report.md)] 報告中，查看應用程式的安裝總數（透過行銷活動，否則為）。
