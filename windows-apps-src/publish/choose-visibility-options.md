---
Description: 設定如何探索和取得應用程式的限制，包括使用者是否可以在商店中尋找您的應用程式，或完全查看其商店清單。
title: 選擇可見度選項
ms.date: 10/31/2018
ms.topic: article
...
ms.locfileid: "79210434"
---
# <a name="choose-visibility-options"></a>選擇可見度選項


**\[價格與可用性\] 頁面**的 [\[可見度\]](set-app-pricing-and-availability.md) 區段可讓您設定探索及取得您應用程式之方式的限制。 這能讓您指定人員是否可以在 Microsoft Store 中找到您的應用程式或甚至是查看其 Store 清單。
...
- 您將無法在 [\[定價和可用性\] 頁面](configure-precise-release-scheduling.md)的 **\[排程\] 區段中設定發行日期**，因為您的應用程式無法發行給私人對象以外的客戶。
...
- 私人對象中的人員所撰寫的任何評論可供您檢視。 不過，即使您的提交移至 **\[公開對象\]** 後，這些評論仍不會發佈至您應用程式的 Store 清單。 您可以藉由觀看[評論報告](reviews-report.md)來閱讀私用物件所撰寫的評論，但無法下載此資料或使用[MICROSOFT STORE 的分析 API](../monetize/access-analytics-data-using-windows-store-services.md)以程式設計方式存取這些評論。
- 當您將應用程式從 **\[私人對象\]** 移動至 **\[公開對象\]** ，Microsoft Store 清單中顯示的 **\[發行日期\]** 將是它首次發行給公開對象的日期。

## <a name="discoverability"></a>發現性
...
此為預設選項。 如果您想要讓您的應用程式列于商店中，讓客戶透過應用程式的直接連結和/或其他方法尋找，包括搜尋、流覽和包含在策劃清單中，請將此選項保留為已選取狀態。 
...
- **僅限直接連結：任何具有產品清單直接連結的客戶都可以下載它，但 Windows 8.x 除外。** 透過直接連結前往您的應用程式清單的客戶可在執行 Windows 10 的裝置或執行 Windows Phone 8.1 和較舊版本的裝置 (但執行 Windows 8.x 的裝置除外) 上進行下載。
- **停止取得：任何具有直接連結的客戶都可以看到產品的商店清單，但只有在先前擁有產品或具有促銷代碼並使用 Windows 10 裝置的情況下，才可加以下載。** 即使客戶有直接連結，除非有[促銷碼](generate-promotional-codes.md)且使用 Windows 10 裝置，否則仍無法下載應用程式。 如果客戶有促銷碼，他們可以使用代碼免費取得您的應用程式 (僅限在 Windows 10 上)，即使您未提供給任何其他客戶。 除了使用促銷碼之外，任何人沒有其他方式可取得您的應用程式。

> [!TIP]
> 如果您想要完全停止為新客戶提供某個應用程式，您可選取其概觀頁面的 **\[停止提供應用程式\]** 。 在您確認想要停止提供該應用程式之後，該應用程式在數小時內便無法在 Microsoft Store 中讓別人看見，而所有的新客戶都將無法取得它 (除非他們有[促銷碼](generate-promotional-codes.md)且使用 Windows 10 裝置)。 這個動作會覆寫您提交中的 **\[可見度\]** 選取項目。 若要再次為新客戶提供該應用程式 (根據您的 **\[可見度\]** 選取項目)，您可隨時按一下概觀頁面中的 **\[提供 app\]** 。 如需詳細資訊，請參閱[從Microsoft Store移除應用程式](guidance-for-app-package-management.md#removing-an-app-from-the-store)。
