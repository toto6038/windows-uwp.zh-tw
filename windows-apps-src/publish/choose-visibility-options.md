---
Description: 設定如何探索和取得您應用程式的限制，包括使用者是否可以在存放區中找到您的應用程式，或完全查看其 Store 清單。
title: 選擇可見度選項
ms.date: 10/31/2018
ms.topic: article
...
如果您想在特定的日期和時間將應用程式提供給公開對象，您可以在建立提交時選取標示為 **\[在以下時間公開此產品\]** 方塊。 輸入您要讓產品公開可用的日期和時間 (UTC 時區)。 請記住下列事項︰
...
- 私人對象中的人員所撰寫的任何評論可供您檢視。 不過，即使您的提交移至 **\[公開對象\]** 後，這些評論仍不會發佈至您應用程式的 Store 清單。 您可以藉由查看 [評論報告](reviews-report.md)來讀取私用物件所撰寫的評論，但無法下載此資料或使用 [Microsoft Store 分析 API](../monetize/access-analytics-data-using-windows-store-services.md) 以程式設計方式存取這些評論。
...
這是預設選項。 如果您想要讓客戶在商店中列出您的應用程式，以透過應用程式的直接連結和/或其他方法（包括搜尋、流覽和包含在策劃清單中）來尋找，請將此選項保留為已選取。 
