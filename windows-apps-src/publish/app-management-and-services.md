...
如果您發佈遊戲，您可以啟用[Xbox Live 建立者程式](http://xbox.com/developers/creators-program)在此頁面上。 這可讓您開始設定和測試 Xbox Live 功能，以及最後發佈 Xbox Live 建立者程式遊戲。

如需詳細資訊，請參閱[Xbox Live 建立者程式快速入門](../xbox-live/get-started-with-creators/get-started-with-xbox-live-creators.md)並[建立新的 Xbox Live 建立者程式標題並發佈至測試環境](../xbox-live/get-started-with-creators/create-and-test-a-new-creators-title.md)。
...
若要使用之 Microsoft 存放區集合 API 和 Microsoft 存放區購買 API 來存取應用程式與附加元件的擁有權資訊，您需要輸入關聯 Azure AD 用戶端識別碼。 請注意，這些變更可能要花費最多 16 個小時才會生效。

如需詳細資訊，請參閱[管理服務的產品權利](../monetize/view-and-grant-products-from-a-service.md)。

## <a name="administrator-consent"></a>系統管理員同意

您的產品與 Azure AD 整合並呼叫要求任一[應用程式的權限或委派的權限](https://developer.microsoft.com/graph/docs/concepts/permissions_reference)需要系統管理員同意的 Api f 輸入您的 Azure AD 用戶端識別碼。 這可讓系統管理員取得代表租用戶中的所有使用者身分產品其組織授與同意的應用程式。

如需詳細資訊，請參閱[同意整個承租人的要求](https://docs.microsoft.com/en-us/azure/active-directory/develop/active-directory-v2-scopes#requesting-consent-for-an-entire-tenant)。
...
**WNS/MPNS**一節提供可協助您建立並將通知傳送給您的應用程式客戶的選項。 

> [!TIP]
> 對於 UWP 應用程式，我們建議使用儀表板中的 [**通知**] 選項。 此功能可讓您將通知傳送給所有的應用程式的客戶或 Windows 10 客戶符合條件者為目標子集您已定義的[客戶線段](create-customer-segments.md)。 如需詳細資訊，請參閱[傳送通知給您的應用程式客戶](send-push-notifications-to-your-apps-customers.md)。

依據您的應用程式套件類型和其特定需求，您也可以使用下列選項之一： 
...
-   **Microsoft 推播通知服務 (MPNS)** 可以與您適用於 Windows Phone 的 .xap 套件搭配使用。 儘管我們建議使用已授權的通知以避免發生節流限制，但您不需在此處進行任何設定，就能傳送有限數量的未授權通知。 如果您使用 MPNS，您需要將憑證上傳至**WNS/MPNS** ] 頁面上所提供的欄位。 如需詳細資訊，請參閱[設定已驗證的 Web 服務來傳送適用於 Windows Phone 8 的推播通知](http://go.microsoft.com/fwlink/p/?LinkId=528736)。
