...
keywords: windows 10, uwp, 名稱保留, app 名稱, 應用程式名稱, 名稱, 產品名稱, 命名
...
在 Windows 開發人員中心儀表板中建立新 app 的第一步，即為保留應用程式名稱。 了解如何保留應用程式名稱，並尋找關於[為您 app 選擇最佳名稱的建議](#choosing-your-apps-name)。 每個保留的名稱必須是整個 Microsoft Store 中唯一的名稱。

[上傳應用程式套件](upload-app-packages.md)時，[**Package/Properties/DisplayName**](https://docs.microsoft.com/uwp/schemas/appxpackage/appxmanifestschema/element-1-displayname) 值必須符合您為應用程式保留的名稱。 如果您使用 Microsoft Visual Studio 建立應用程式套件，則系統將會幫您填入此屬性。
...
保留名稱之後，您必須在一年內提交該應用程式。 如果您沒有在該年內提交應用程式，保留的名稱將會到期，而其他開發人員就能使用該名稱做為應用程式名稱。 如果您嘗試在名稱到期後送出應用程式，可能就會發生錯誤。
