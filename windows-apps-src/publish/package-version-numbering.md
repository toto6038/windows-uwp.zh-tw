...
如果您提供使用相同版本號碼的多個 Windows 10 套件，在考量要將哪個套件提供給指定的裝置時，架構 (依照 x64、x86、ARM、中性的順序) 將用來決定哪一個套件的等級較高。 當排名使用相同版本號碼的 app 套件組合時，會考量套件組合中最高架構等級：包含 x 64 套件的 app 套件組合比只包含 x 86 套件的套件組合具有較高的等級。
