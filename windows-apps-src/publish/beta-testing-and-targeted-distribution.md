---
description: 合作夥伴中心提供數個選項，讓測試人員先試用您的應用程式，再將它提供給大眾。
...
合作夥伴中心提供數個選項，讓測試人員先試用您的應用程式，再將它提供給大眾。
...
只有當您尚未將您的應用程式發佈給公用物件時，才能使用 [ **私用物件** ] 選項。 您可以使用此選項搭配以任何 OS 版本為目標的應用程式，但您的測試人員必須執行 Windows 10 1607 或更高版本 (包含 Xbox One) ，而且必須使用與您提供的電子郵件地址相關聯的 Microsoft 帳戶登入。
...
此選項提供另一種方式，將應用程式散發給特定的測試人員群組，並防止其他人在存放區中探索您的應用程式 (或在沒有促銷代碼) 的情況下取得。 不過，與私人對象選項不同的是，如果某人有直接連結，他們可以看得到您應用程式的清單。 如果機密性對您的提交很重要，我們建議改為發佈至私人對象。
