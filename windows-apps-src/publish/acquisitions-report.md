...
在頁面頂端附近，您可以選取您想要顯示資料的時間週期。 預設選項是 **30D**（30 天），但您可以選擇在 3、6 或 12 個月期間顯示資料，或指定自訂的日期範圍。 接近即時資料將會顯示的所有選項 （除非在**累積的應用程式**資料）。 **1 H**和**72h**時間期間僅適用於**下載數**圖表的 [**應用程式每日**] 索引標籤，以及**市場**圖表的 [**下載**] 索引標籤。 
