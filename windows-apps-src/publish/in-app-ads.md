...
2. 在**應用程式內廣告**頁面上，移至**流量分配設定**\] 區段並設定您的設定。
    ...
    * 根據預設，**\[讓 Microsoft 為您的應用程式選擇最佳流量分配設定\]** 核取方塊已選取。 建議您使用此選項。 這個選項會使用機器學習演算法為您的應用程式自動選擇廣告流量分配設定，幫助您在應用程式支援的所有市場獲得最大的廣告收益。 當您使用此選項時，您也可以選擇您想要在設定中使用廣告網路。 取消選取廣告網路，您不想要設定的過程中，並在我們的演算法可確保您的應用程式只從選取的廣告網路接收廣告。
