...
> [!TIP]
> 如果您想要在 .csv 檔案中以離線方式輸入清單資訊，而不是直接在合作夥伴中心內提供資訊和上傳檔案，您可以選擇匯[入和匯出存放區清單](import-and-export-store-listings.md)。 如果您有多種語言的清單版本，使用匯入及匯出選項就很方便，因為您可以一次進行多個更新。

如果您先前發佈的應用程式支援 Windows 8.x 和（或） Windows Phone 8.x 或更早版本，您可以[建立平臺特定的存放區清單](create-platform-specific-store-listings.md)，以向這些客戶顯示。
...
若要編輯商店清單，請從提交的 [總覽] 頁面選取語言名稱。 您必須分別編輯每個語言，除非您選擇匯出商店清單並離線工作，然後一次匯入所有清單資料。 如需其運作方式的詳細資訊，請參閱匯[入和匯出存放區清單](import-and-export-store-listings.md)。

可用的欄位如下所述。

## <a name="product-name"></a>產品名稱

此下拉式方塊可讓您指定要在商店清單中使用的名稱（如果您已為應用程式保留多個名稱）。

如果您上傳的套件與您正在處理的存放區清單相同，將會選取這些套件中使用的名稱。 如果您需要在[應用程式](manage-app-names.md#rename-an-app-that-has-already-been-published)已發佈之後將其重新命名，您可以在上傳使用新名稱的套件之後，于此處選取不同的保留名稱。

如果您尚未上傳您所使用之語言的套件，而且您已保留一個以上的名稱，則必須選取其中一個保留的應用程式名稱，因為該語言中沒有相關聯的套件可供提取名稱。

> [!NOTE]
> 您選取的**產品名稱**只會套用至您正在使用之語言中的商店清單。 它不會影響客戶安裝應用程式時所顯示的名稱;該名稱來自已安裝之套件的資訊清單。 為了避免混淆，我們建議每個語言的封裝和存放清單都使用相同的名稱。
...
如果這是您第一次提交應用程式，請將此欄位保留空白。 針對現有應用程式的更新，您可以在這裡讓客戶知道最新版本中的變更。 這個欄位有 1500 個字元的限制。 (此欄位之前稱為「**版本資訊**」)。
...
> [!NOTE]
> 這些功能會在您的商店清單中顯示為點符，因此請勿加入您自己的專案符號。
...
您可以為您的產品提交額外的資產，包括影片結尾和促銷映射。 這些都是選用，但我們建議您上傳越多越好。 這些影像有助於客戶更了解您的產品，製作更吸引人的清單。
...
為獲得最佳結果，請將簡短描述保留在270個字元之下。 欄位的限制為1000個字元，但在某些情況下，只會顯示前270個字元（具有可用來查看其餘簡短描述的連結）。
