...
9.  請檢閱您的帳戶資訊，並確認所有項目正確。 接著，請閱讀並接受[應用程式開發人員合約](https://msdn.microsoft.com/library/windows/apps/Hh694058)的條款及條件。 選取方塊以表示您已閱讀並同意這些條款。

10.  按一下 **\[完成\]** 來確認您的註冊。 我們會處理您的付款並將確認訊息傳送到您的開發人員電子郵件地址。
...
當建立公司帳戶時，建議遵循這些指導方針，特別是多人需要使用開啟該帳戶的 Microsoft 帳戶存取該帳戶時。 
...
-   一般來說，讓開發人員使用信任的裝置登入您的公司開發人員帳戶。 所有主要小組成員都必須擁有這些信任的裝置的存取權。 這樣可減少存取帳戶時必須傳送安全密碼的需求。 每週每個帳戶可產生的代碼數目有限。
...
您的 Microsoft 帳戶必須至少有兩個電子郵件地址和/或電話號碼。 建議您新增的越多越好。 請記住，部分安全性資訊需要經過確認才能生效。 同時，務必經常檢閱您的安全性資訊，以確保是最新的資訊。
...
當您登入 Windows 開發人員中心儀表板時，系統可能會要求您驗證身分。 如果使用電子郵件地址或電話號碼，我們會傳送一個安全密碼給您，您必須提供這個代碼才能完成登入程序。
