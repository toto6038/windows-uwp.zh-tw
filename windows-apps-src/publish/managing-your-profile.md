...
選取儀表板右上角的齒輪圖示，然後按一下 **\[帳戶設定\]**。 這裡會顯示有關您帳戶的詳細資料，以及其他頁面的連結，讓您可以檢視和/或更新資訊以及執行各種帳戶相關工作。

在 **\[帳戶詳細資料\]** 區段中，您可以查看帳戶的狀態和類型。 您也可以查看您的發行者識別碼資訊，包括賣方識別碼、Windows 發行者識別碼，以及 Windows Phone 發行者識別碼。 這些值是由 Microsoft 指派用以唯一識別您的開發人員帳戶。 您不能修改這些值。
...
- **驗證狀態**：當您的帳戶正在進行驗證程序（例如，當您第一次註冊或如果大幅更新連絡資訊）時，您可以在此檢閱您的驗證狀態。
- **開發人員中心測試人員計畫**：您可以從 **\[帳戶設定\]** 主頁選擇加入或退出[開發人員中心測試人員計畫](dev-center-insider-program.md)。
- **使用者**：將使用者新增到您的開發人員中心帳戶，並為他們指派角色和權限。 如需詳細資訊，請參閱[管理帳戶使用者](manage-account-users.md)。
- **兌換代碼**：部分計畫可能提供代碼讓您擴充帳戶功能。 如果您有這類代碼，您可以在此輸入。
...
- **解除鎖定 Windows Phone**：檢視任何[您已解鎖用於測試的 Windows Phone 8 或 Windows Phone 8.1 裝置](http://go.microsoft.com/fwlink/p/?LinkId=533897)的詳細資料。
- **Xbox One 開發主機**：檢視及管理[為開發啟用 Xbox 主機](../xbox-apps/devkit-activation.md)。 
