...
ms.locfileid: "63790983"
---
# <a name="getting-paid"></a>獲得報酬
以下是一些重要的資訊，關於接收付款，您的應用程式、 附加元件和廣告盈餘。

> [!IMPORTANT]
> 您可以從 Microsoft Store 中的應用程式銷售接收 money 之前，您需要[設定您的付款帳戶，並填妥必要的稅務表單](setting-up-your-payout-account-and-tax-forms.md)。
...
您可以存取有關您的付款資訊的詳細資訊和下載中的報表**支出摘要**的[合作夥伴中心](https://partner.microsoft.com/dashboard)。 如需此處所顯示的詳細資訊，以及我們將您所賺取金額分類的方式，請參閱[支付摘要](payout-summary.md)。
...
> [!NOTE]
> 您**支出保存狀態**選項亦適用於**所有**營收來源，並且會透過 Windows 開發人員計劃在合作夥伴中心 (Microsoft Store，通告，Azure Marketplace，依此類推。)。 您無法選取每個收入來源的不同保留狀態。
