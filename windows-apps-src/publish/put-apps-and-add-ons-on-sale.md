---
Description: 您可以在限定時間內降價銷售您的 App 或附加元件，以便在 Microsoft Store 進行促銷。
title: 促銷應用程式和附加元件
...
ms.locfileid: "63788356"
---
# <a name="put-apps-and-add-ons-on-sale"></a>促銷應用程式和附加元件

您可以在限定時間內降價銷售您的 App 或附加元件，以便在 Microsoft Store 進行促銷。 您可以選擇以較低價格區間或百分比計算的折扣提供產品。 此外，您可以選擇是否要提供銷售給所有人，或讓客戶擁有的其他產品的其中一項獨家優惠。
...
> [!IMPORTANT]
> 為您的客戶在 Windows 10 裝置，包括 Xbox One 上，才顯示銷售價格。 您提供只有給擁有者的其中一個其他產品的銷售只會顯示在 Windows 10 版本 1607年或更新版本上的客戶。
> 
...
4. 選擇下列其中一個選項指定折扣的類型︰
   - **價格**:使用此選項以選取您的應用程式將提供的較低的價格層。 您可以變更貨幣下拉式清單，選取以您偏好貨幣顯示的價格。 (價格將會轉換成每一種貨幣的對應區間。 如需詳細資訊，請參閱[價格](set-app-pricing-and-availability.md))。
   - **百分比**:使用此選項來選取將套用至您的應用程式的折扣百分比。 同一個折扣百分比會用於所有貨幣。
5. 在 **\[提供給\]** 列中，從下列其中一個選項選擇，包括：
   - **每個人都**:銷售會提供給所有客戶。
   - **擁有者**:已擁有您的應用程式的其中一個的客戶可以銷售。 您可以從出現的下拉式清單中選取發行的應用程式。 您必須有一或多個發行的應用程式，才能使用此選項。

  > [!IMPORTANT]
  > 如果您選取 **\[擁有者\]**，只有 Windows 10 1607 版或更新版本的客戶才會看到特價。

   - **已知的使用者群組**:中的人可以銷售[已知的使用者群組](create-known-user-groups.md)您選取。 您必須先建立已知的使用者群組，才能使用此選項。
   - **區段**:銷售會提供給您所選取的客戶區段中的人員。 您可以在此使用[您已建立的區隔](create-customer-segments.md)。 您也可以選擇 [**首次付款者**]，僅提供特價給從未在市集中購買過任何東西的客戶。 我們在這裡提供此區隔，是因為我們發現客戶第一次在市集購買之後，通常會繼續購買更多項目，所以運用特價吸引這群客戶會有很好的效果。
6. 輸入銷售期間開始和結束的日期與時間。 選擇下列其中一個時區選項：
   - **UTC**:您選取的時間會 Universal Coordinated Time (UTC) 時間，以便在所有位置的同時發生的銷售。
