...
如需有關套件內容及其建構方式的詳細資訊，請參閱[應用程式套件需求](app-package-requirements.md)。 您也將想要了解[如何版本號碼可能會影響哪些套件會傳遞至特定客戶](package-version-numbering.md)以及[如何將套件分散至不同的作業系統](guidance-for-app-package-management.md)。
...
如果我們時驗證其偵測錯誤套件，我們將顯示讓您知道錯誤訊息。 您需要移除封裝 （英文）、 修正問題，然後再試一次上載。 您也會看到警告，讓您知道可能會造成問題的相關資訊，但不會阻止您繼續提交。
...
上傳的套件會在這裡列出，依據目標作業系統。 系統將會顯示套件的名稱、版本及架構。 如需有關如每個套件支援的語言、App 功能，以及檔案大小的詳細資訊，可以按一下 **\[顯示詳細資料\]**。
