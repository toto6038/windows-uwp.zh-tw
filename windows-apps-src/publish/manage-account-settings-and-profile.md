...
title: 管理帳戶設定與設定檔資訊
description: 您可以在合作夥伴中心的 [帳戶設定] 區段中，查看及更新您的開發人員帳戶設定和設定檔資訊。
ms.date: 10/31/2018
ms.topic: article
...
ms.locfileid: "89161892"
---
# <a name="manage-account-settings-and-profile-info"></a>管理帳戶設定與設定檔資訊

您可以在[合作夥伴中心](https://partner.microsoft.com/dashboard)的 [**帳戶設定**] 區段中，查看及更新您的開發人員帳戶設定和設定檔資訊。 

> [!NOTE]
> 通知喜好設定 (例如要接收的通知類型以及如何接收它們) 可在 **訊息中心** (中進行管理，您可以選取接近合作夥伴中心) 頂端的訊息圖示加以存取。

## <a name="account-settings"></a>帳戶設定

若要查看您的帳戶設定，請按一下合作夥伴中心右上角的齒輪圖示，然後選取 [ **開發人員設定**]。 您可以在這裡找到有關帳戶的詳細資料，以及可供您查看資訊及執行各種帳戶相關工作的其他頁面連結。

在 **\[帳戶詳細資料\]** 區段中，您可以查看帳戶的狀態和類型。 您也可以在這裡看到您的賣方識別碼和其他發行者識別碼資訊。 Microsoft 會指派這些值來唯一識別您的開發人員帳戶，而且無法加以編輯。
...
- **驗證狀態**：當您的帳戶正在進行驗證程式時 (例如，當您第一次註冊，或在您大幅更新連絡人資訊) 之後，您可以在這裡查看您的驗證狀態。
...
- **使用者**：將使用者新增至您的帳戶，並指派他們的角色和許可權。 如需詳細資訊，請參閱[管理帳戶使用者](manage-account-users.md)。
- 租**使用者：管理**您的 Azure AD 租使用者關聯。 如需詳細資訊，請參閱 [將 Azure AD 與您的合作夥伴中心帳戶建立關聯](./associate-azure-ad-with-partner-center.md)。
- **計畫**：檢視您所參與之計畫的詳細資料，並了解其他感興趣的計畫。
- **合約**：檢視您的帳戶已接受的合約。
- **開發裝置**：查看和管理 [您已解除鎖定的 Windows Phone 8.x 裝置，以供](/previous-versions/windows/apps/dn614128(v=win.10)) 開發之用來測試和 [Xbox One 主控台](../xbox-apps/devkit-activation.md)。