---
Description: 附加元件取得報表在合作夥伴中心可讓您查看幾個附加元件，您已銷售，以及 人口統計與平台的詳細資料。
title: 附加元件下載數報告
...
**附加元件收購**中的報表[合作夥伴中心](https://partner.microsoft.com/dashboard)可讓您查看您已銷售，以及 人口統計的幾個附加元件平台的詳細資訊，以及顯示的 Windows 10 （客戶的轉換資訊包括 Xbox）。 您也可以檢視最後一小時或 seventy 兩小時期間的接近即時擷取的資料。

您可以檢視這項資料在合作夥伴中心，或[將報表下載為](download-analytic-reports.md)要離線檢視。 或者，您可透過程式設計方式使用 [Microsoft Store 分析 REST API](../monetize/access-analytics-data-using-windows-store-services.md) 中的[取得附加元件下載數](../monetize/get-in-app-acquisitions.md)方法來擷取此資料。

在此報告中，附加元件下載數表示客戶已經向您購買附加元件（或取得而不需付費，如果您免費提供附加元件）。 同一位客戶購買多個相同的消耗型附加元件會當作個別的附加元件下載數來計算。

> [!IMPORTANT]
> **附加元件收購**報表不包含有關退款、 低價、 退款等資料。若要預估您的應用程式會繼續進行，請瀏覽[支出摘要](payout-summary.md)。 在 \[**保留**\] 區段中，按一下 [\**下載保留的交易**\] 連結。


## <a name="apply-filters"></a>套用篩選

在頁面頂端附近，您可以選取您想要顯示資料的時間週期。 預設選項是 **30D**（30 天），但您可以選擇在 3、6 或 12 個月期間顯示資料，或指定自訂的日期範圍。 您也可以選取**1 小時**或是**72 小時**幾近即時顯示擷取資料可能是一小時或 seventy 兩小時; 這些時間週期只適用於**每日的附加元件** 索引標籤**附加元件收購**圖表以及**收購**索引標籤**市場**圖表。 

您也可以展開 **\[篩選條件\]**，依特定附加元件、市場和/或裝置類型篩選此頁面上的所有資料。

-   **附加元件**:預設篩選是**所有附加元件**，但您可以將資料限制為一或多個應用程式的附加元件。
