---
description: 合作夥伴中心中的附加元件收購報告可讓您查看已售出的附加元件數量，以及人口統計和平臺詳細資料。
title: 附加元件下載數報告
...
ms.locfileid: "98104499"
---
# <a name="add-on-acquisitions-report"></a>附加元件下載數報告


[合作夥伴中心](https://partner.microsoft.com/dashboard)中的 **附加元件收購** 報告可讓您查看已售出的附加元件數量，以及人口統計和平臺詳細資料，並顯示 Windows 10 (（包括 Xbox) ）的客戶轉換資訊。 您也可以在過去一小時或72小時的時間內，查看近乎即時的取得資料。

您可以在合作夥伴中心中查看這項資料，或 [下載報表](download-analytic-reports.md) 以離線查看。 或者，您可透過程式設計方式使用 [Microsoft Store 分析 REST API](../monetize/access-analytics-data-using-windows-store-services.md) 中的[取得附加元件下載數](../monetize/get-in-app-acquisitions.md)方法來擷取此資料。

在此報告中，附加元件下載數表示客戶已經向您購買附加元件（或取得而不需付費，如果您免費提供附加元件）。 同一位客戶購買多個相同的消耗型附加元件會當作個別的附加元件下載數來計算。

> [!IMPORTANT]
> **附加元件下載數** 報告不包含有關退款、解除安裝、信用卡退款等相關資料。若要評估您的應用程式收益，請造訪 [支付摘要](/partner-center/payout-statement)。 在 **\[保留\]** 區段中，按一下 **\[下載保留的交易\]** 連結。


## <a name="apply-filters"></a>套用篩選條件

在頁面頂端附近，您可以選取您想要顯示資料的時間週期。 預設選項為 **30D** (30 天) ，但您可以選擇要顯示3、6或12個月的資料，或是您指定的自訂資料範圍的資料。 您也可以選取 [ **1H** ] 或 [ **72H** ]，以近乎即時的方式顯示1小時或72小時的取得資料;這些時間週期僅適用于 **附加元件收購** 圖表的 [**每日附加** 元件] 索引標籤，以及 **市場** 圖表的 [**採購**] 索引標籤。 
...
附加元件的 **購買** 圖表會顯示所選時段內的附加元件每日或每週的購買次數。 (當您利用 **\[套用篩選\]** 顯示較長期間的資料時，下載數資料將會以週為單位分組)。
...
**\[客戶人口統計資料\]** 圖表顯示取得您附加元件對象的人數統計資訊。 您可以依特定年齡層和依性別來查看取得對象在選取時段內的下載數。
