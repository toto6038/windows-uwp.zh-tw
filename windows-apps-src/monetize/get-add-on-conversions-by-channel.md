---
author: mcleanbyron
description: "在 Windows 市集分析 API 中使用此方法，以針對特定日期範圍與其他選擇性篩選器，取得附加元件的依通道彙總轉換資料。"
title: "依通道取得附加元件轉換"
ms.author: mcleans
ms.date: 08/04/2017
ms.topic: article
ms.prod: windows
ms.technology: uwp
keywords: "Windows 10, UWP, 市集服務, Windows 市集分析 API, 附加元件轉換, 通道"
...
在 Windows 市集分析 API 中使用此方法，以針對特定日期範圍與其他選擇性篩選器，取得附加元件的依通道彙總轉換。
...
| 參數        | 類型   |  描述      |  必要  
|---------------|--------|---------------|------|
| applicationId | 字串 | 您想要擷取附加元件轉換資料之應用程式的[市集識別碼](in-app-purchases-and-trials.md#store-ids)。 舉例來說，市集識別碼可以是「9WZDNCRFJ3Q8」。 |  是  |
| inAppProductId | 字串 | 您想要擷取轉換資料之附加元件的[市集識別碼](in-app-purchases-and-trials.md#store-ids)。  | 是  |
...
| inAppProductId      | 字串  | 您正在擷取轉換資料之附加元件的[市集識別碼](in-app-purchases-and-trials.md#store-ids)。     |
