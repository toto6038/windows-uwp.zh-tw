...
這項資訊也會提供[附加元件取得報表](../publish/add-on-acquisitions-report.md#add-on-page-views-and-conversions-by-campaign-id)在合作夥伴中心。
...
| 參數        | 類型   |  描述      |  必要  
|---------------|--------|---------------|------|
| applicationId | 字串 | 您想要擷取附加元件轉換資料之應用程式的[ Store 識別碼](in-app-purchases-and-trials.md#store-ids)。 舉例來說，市集識別碼可以是「9WZDNCRFJ3Q8」。 |  是  |
...
| 值      | 類型   | 描述                  |
|------------|--------|-------------------------------------------------------|
| 值      | 陣列  | 物件陣列，內含附加元件的彙總轉換資料。 如需有關每個物件中資料的詳細資訊，請參閱下方的[轉換值](#conversion-values)一節。                     |
