...
| 值      | 類型   | 描述  |
|------------|--------|---------------|
| 值      | array  | 內含彙總廣告行銷活動績效資料的物件陣列。 如需有關每個物件中資料的詳細資訊，請參閱下方的[行銷活動績效物件](#campaign-performance-object)一節。          |
| @nextLink  | 字串 | 如果還有其他資料頁面，此字串包含可以用來要求下一頁資料的 URI。 例如，如果要求的 **top** 參數被設定為 5，但是查詢有超過 5 個資料項目，就會傳回此值。 |
...
| impressions           | long | 行銷活動的廣告曝光次數。        |
| installs              | long | 與行銷活動相關的 App 安裝次數。   |
| clicks            | long | 行銷活動的廣告點按次數。      |
| iapInstalls            | long | 與行銷活動相關的附加元件安裝數 (也稱為 App 內購買或 IAP)。      |
| activeUsers            | long | 按下行銷活動中廣告並返回應用程式的使用者人數。      |
