...
使用「Windows 市集分析 API」中的這個方法，以針對特定日期範圍及其他選擇性篩選，取得您應用程式促銷廣告行銷活動效益資料的彙總摘要。 這個方法會傳回 JSON 格式的資料。
...
若要建立、更新或擷取廣告活動的詳細資料，您可以使用 [Windows 市集促銷 API](run-ad-campaigns-using-windows-store-services.md) 中的[管理廣告活動](manage-ad-campaigns.md)方法。
