...
ms.locfileid: "8921797"
---
# <a name="get-error-reporting-data-for-your-app"></a>取得 App 的錯誤報告資料

使用「Microsoft Store 分析 API」中的這個方法，以針對特定日期範圍及其他選擇性篩選，取得您 App 的彙總錯誤報告資料 (JSON 格式)。 這個方法只能擷取過去 30 天內發生的錯誤。 這項資訊也會提供中的[健康情況報告](../publish/health-report.md)合作夥伴中心中的 [**失敗**] 區段中。
...
| 參數        | 類型   |  描述      |  必要  
|---------------|--------|---------------|------|
| applicationId | 字串 | 您想要擷取錯誤報告資料之 app 的 Store 識別碼。 在合作夥伴中心的[應用程式身分識別頁面](../publish/view-app-identity-details.md)上使用 「 市集識別碼 」。 舉例來說，Store 識別碼可以是「9WZDNCRFJ3Q8」。 |  是  |
