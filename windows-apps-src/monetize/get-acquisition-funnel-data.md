---
author: mcleanbyron
description: "在 Windows 市集分析 API 中使用此方法，以針對特定日期範圍與其他選擇性篩選器，取得應用程式的取得漏斗資料。"
title: "取得應用程式取得漏斗圖資料"
ms.author: mcleans
ms.date: 08/04/2017
ms.topic: article
ms.prod: windows
ms.technology: uwp
keywords: "Windows 10, uwp, 市集服務, Windows 市集分析 API, 取得, 漏斗"
...
在 Windows 市集分析 API 中使用此方法，以針對特定日期範圍與其他選擇性篩選器，取得應用程式的取得漏斗資料。 「Windows 開發人員中心」儀表板中的[下載數報告](../publish/acquisitions-report.md#acquisition-funnel)也有提供這項資訊。
...
| 參數        | 類型   |  描述      |  必要  
|---------------|--------|---------------|------|
| applicationId | 字串 | 您想要擷取取得漏斗資料之應用程式的[市集識別碼](in-app-purchases-and-trials.md#store-ids)。 舉例來說，市集識別碼可以是「9WZDNCRFJ3Q8」。 |  是  |
...
下列範例示範數個取得應用程式取得漏斗資料的要求。 將 *applicationId* 值取代為您應用程式的市集識別碼。
