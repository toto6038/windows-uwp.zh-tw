...
ms.locfileid: "86493103"
---
# <a name="get-app-reviews"></a>取得應用程式評論


使用「Microsoft Store 分析 API」中的這個方法，以針對特定日期範圍及其他選擇性篩選，取得評論資料 (JSON 格式)。 您也可以在合作夥伴中心的[評論報告](../publish/reviews-report.md)中取得這項資訊。
...
如需支援的欄位，以及每個欄位所支援的運算子清單，請參閱下列表格。 *篩選* 參數中的字串值必須由單引號括住。
...
| deviceType  | eq、ne  | 下列其中一個字串：<ul><li><strong>PC</strong></li><li><strong>來電</strong></li><li><strong>主控台-Xbox One</strong></li><li><strong>主控台-Xbox 系列 X</strong></li><li><strong>IoT</strong></li><li><strong>全息影像</strong></li><li><strong>Unknown</strong></li></ul>  |
...
| productFamily  | eq、ne  | 下列其中一個字串：<ul><li><strong>PC</strong></li><li><strong>平板電腦</strong></li><li><strong>來電</strong></li><li><strong>穿戴式裝置</strong></li><li><strong>Server</strong></li><li><strong>Collaborative</strong></li><li><strong>其他</strong></li></ul>  |
| deviceRAM  | eq, ne, gt, lt, ge, le  | 實體 RAM (以 MB 為單位)。  |
| deviceScreenResolution  | eq、ne  | 裝置螢幕解析度，格式為 &quot; <em>寬度</em>x<em>高度</em> &quot; 。   |
...
下列範例示範取得評論資料的數個要求。 將 *applicationId* 值取代為您 App 的 Store 識別碼。
...
| deviceRAM       | number  | 實體 RAM (以 MB 為單位)。    |
| deviceScreenResolution       | 字串  | 裝置螢幕解析度，格式為「*寬度*x*高度*」。    |
| deviceStorageCapacity | number | 主要存放磁碟的容量 (以 GB 為單位)。 |
| isTouchEnabled | 布林值 | **true** 值表示已啟用觸控功能，否則為 **false**。 |
| reviewerName | 字串 | 評論者名稱。 |
| rating | number | App 評分 (以星星為單位)。 |
| reviewTitle | 字串 | 評論的標題。 |
| reviewText | 字串 | 評論的文字內容。 |
| helpfulCount | number | 該評論被標記為「很有幫助」的次數。 |
| notHelpfulCount | number | 該評論被標記為「沒有幫助」的次數。 |
