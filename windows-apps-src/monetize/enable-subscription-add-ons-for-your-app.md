...
1. 在合作夥伴中心為您的訂用帳戶[建立附加元件提交](../publish/add-on-submissions.md)，併發布提交。 當您依照附加元件提交程序執行時，請密切注意下列屬性：
...
* 您已在合作夥伴中心內[建立應用程式提交](https://docs.microsoft.com/windows/uwp/publish/app-submissions)，並在商店中發佈此應用程式。 測試時您也可以選擇將應用程式設定為不可在市集中搜尋。 如需詳細資訊，請參閱[測試指導方針](in-app-purchases-and-trials.md#testing)。
* 您已在合作夥伴中心內[建立應用程式的訂用帳戶附加](../publish/add-on-submissions.md)元件。
...
* 客戶無法使用其 Microsoft 帳戶的 [https://account.microsoft.com/services](https://account.microsoft.com/services) 頁面切換訂閱期限。 若要切換至不同的訂用期間，客戶必須取消其目前的訂用帳戶，然後向您的應用程式購買不同訂用帳戶期間的訂用帳戶。
...
* 在設定訂閱附加元件的可見度之後，更新現有的訂閱以**停止**取得。 如需詳細資訊，請參閱[設定附加元件定價和可用性](../publish/set-add-on-pricing-and-availability.md)。
