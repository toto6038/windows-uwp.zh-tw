...
ms.locfileid: "89162362"
---
# <a name="get-xbox-live-club-data"></a>取得 Xbox Live 俱樂部資料

在 Microsoft Store 分析 API 中使用此方法取得[已啟用 Xbox Live 遊戲](/gaming/xbox-live/index.md) 的俱樂部資料。 這項資訊也可在合作夥伴中心的 [Xbox 分析報表](../publish/xbox-analytics-report.md) 中取得。
...
| metricType | 字串 | 指定要擷取之 Xbox Live 分析資料類型的字串。 對於此方法，請指定 **communitymanagerclub**值。  |  是  |
| startDate | date | 要擷取俱樂部資料之日期範圍的開始日期。 預設為目前日期的前 30 天。 |  否  |
| endDate | date | 要擷取俱樂部資料之日期範圍的結束日期。 預設值是目前的日期。 |  否  |
...
以下範例示範了用於已啟用 Xbox Live 遊戲取得俱樂部資料的要求。 將 *applicationId* 值取代為您的遊戲的 Store 識別碼。
...
| 值      | 類型   | 描述                  |
|------------|--------|-------------------------------------------------------|
| 值      | array  | 包含一個 [ProductData](#productdata) 物件的陣列，其中包含與您的遊戲相關的俱樂部的資料以及一個包含所有 Xbox Live 客戶的俱樂部資料的 [XboxwideData](#xboxwidedata) 物件。 包含這些資料是為了與遊戲資料作比較。  |
...
|  clubsWithTitleActivity               |    int     |  和您的遊戲有社交關聯的俱樂部數目。   |     
|  clubsExclusiveToGame               |   int      |  只與您的遊戲有社交關聯的俱樂部數目。   |     
|  clubFacts               |   array      |   包含一個或多個關於與您的遊戲有社交關聯的每個俱樂部的 [ClubFacts](#clubfacts) 物件。   |
...
|  clubsWithTitleActivity               |   int     |  平均而言，客戶與已啟用的 Xbox Live 遊戲有社交關聯的俱樂部數目。    |     
|  clubsExclusiveToGame               |   int      |  平均而言，只與已啟用的 Xbox Live 遊戲有社交關聯的俱樂部數目。   |     
|  clubFacts               |   物件 (object)      |  包含一個 [ClubFacts](#clubfacts) 物件。 此物件在 **XboxwideData** 物件的內容中沒有意義，並有預設值。  |
...
|  memberCount               |    int     | 在 **ProductData** 物件，這是俱樂部會員的數目，不包括只瀏覽俱樂部的非會員。 在 **XboxwideData** 物件，這總是 0。    |
|  titleSocialActionsCount               |    int     |  在 **ProductData** 物件，這是俱樂部成員所執行的與您的遊戲相關的社交動作次數。 在 **XboxwideData** 物件中，這一律為0   |
|  isExclusiveToGame               |    Boolean     |  在 **ProductData** 物件，這表示目前俱樂部是否只與您的遊戲有社交關聯。 在 **XboxwideData** 物件，這總是 True。  |
