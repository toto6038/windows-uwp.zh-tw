...
description: 了解如何新增應用程式識別碼和 ad 單元識別碼值從合作夥伴中心，您的應用程式提交至市集的應用程式之前。
...
完成測試之後，您的 UWP 應用程式，而且您已經準備好要將它提交至合作夥伴中心，您必須[建立即時廣告單元](#live-ad-units)從[應用程式內廣告](../publish/in-app-ads.md)在合作夥伴中心頁面，並更新您的應用程式程式碼，以使用應用程式識別碼和 ad 單元識別碼值的這個 ad 單位。
...
若要從合作夥伴中心取得即時廣告單元，並在您的應用程式中使用它：

1.  [建立 ad 單位](../publish/in-app-ads.md#create-ad-unit)上**應用程式內廣告**在合作夥伴中心內的頁面。 請務必指定您的應用程式中正在使用的正確廣告控制項的廣告單元類型。
    > [!NOTE]
...
2.  建立新的 ad 單位之後，擷取**應用程式識別碼**並**Ad 單元識別碼**可用的廣告單元，在資料表中的 ad 單位**Monetize** &gt; **應用程式內廣告**頁面。
    > [!NOTE]
