...
在您完成 UWP 應用程式的測試，並準備好將它提交至合作夥伴中心之後，您必須從合作夥伴中心的應用程式[內 ads](../publish/in-app-ads.md)頁面[建立即時 ad 單位](#live-ad-units)，並更新您的應用程式程式碼，以使用此 ad 單位的應用程式識別碼和 ad 單位識別碼值。
...
若要從合作夥伴中心取得即時 ad 單位，並在您的應用程式中使用它：

1.  在合作夥伴中心的**應用程式內 ads**頁面上[建立 ad 單位](../publish/in-app-ads.md#create-ad-unit)。 請務必指定您的應用程式中正在使用的正確廣告控制項的廣告單元類型。
    > [!NOTE]
