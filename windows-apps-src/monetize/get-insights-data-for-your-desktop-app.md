---
author: Xansky
description: 在 Microsoft Store 分析 API 中使用這個方法，以取得傳統型應用程式的深入解析資料。
...
下列範例示範取得深入解析資料的要求。 *ApplicationId*值取代為您的傳統型應用程式適當的值。
...
| 值               | 類型   | 描述                           |
|---------------------|--------|-------------------------------------------|
| FactName           | string | 表示目前的深入解析或目前的維度描述的衡量標準的字串。 目前，此方法僅支援值**叫用次數**。  |
