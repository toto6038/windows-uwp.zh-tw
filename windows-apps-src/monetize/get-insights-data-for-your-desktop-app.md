...
| 參數        | 類型   |  說明      |  必要  
|---------------|--------|---------------|------|
| applicationId | string | 您要取得的深入解析資料的傳統型應用程式的產品識別碼。 若要取得傳統型應用程式的產品識別碼，請開啟任何[傳統型應用程式的開發人員中心分析報告](https://msdn.microsoft.com/library/windows/desktop/mt826504) (例如**健康報告**)，並從 URL 擷取產品識別碼。 如果您未指定此參數，回應主體會包含針對登錄到您帳戶的所有應用程式的深入解析資料。  |  否  |
...
| 值               | 類型   | 說明                           |
|---------------------|--------|-------------------------------------------|
| FactName           | string | 表示為目前的深入解析或目前維度說明的衡量標準的字串。 目前，此方法僅支援值**叫用次數**。  |
