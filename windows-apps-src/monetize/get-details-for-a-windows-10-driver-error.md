...
description: 在 Microsoft Store 分析 API 中使用此方法，以取得 Windows 10 驅動程式錯誤的詳細資料。 這個方法僅供 IHV 使用。
...
ms.locfileid: "1663668"
---
# <a name="get-details-for-a-windows-10-driver-error"></a>取得 Windows 10 驅動程式錯誤的詳細資料

在 Microsoft Store 分析 API 中使用此方法，以取得特定 Windows 10 驅動程式錯誤的詳細資料 (JSON 格式)。 使用這個方法之前，您必須先使用[取得 Windows 10 驅動程式的錯誤報告資料](get-error-reporting-data-for-windows-10-drivers.md)方法，來擷取您要取得詳細資訊之錯誤的識別碼。
