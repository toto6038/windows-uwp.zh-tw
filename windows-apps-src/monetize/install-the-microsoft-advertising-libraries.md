...
若要在您的 Windows 應用程式中顯示廣告，請安裝下列其中一個 SDK：

* 針對適用於 Windows 10 的 UWP 應用程式，請安裝 [Microsoft Advertising SDK](http://aka.ms/ads-sdk-uwp)。 這個 SDK 是 Visual Studio 2015 和更新版本的擴充功能。
* 針對適用於 Windows 8.1 和 Windows Phone 8.x 的應用程式，請安裝[適用於 Windows 和 Windows Phone 8.x 的 Microsoft Advertising SDK](http://aka.ms/store-8-sdk)。 這個 SDK 是 Visual Studio 2015 和 Visual Studio 2013 的擴充功能。

> [!NOTE]
> 如果您正在開發 JavaScript/HTML UWP app 且已經安裝 Windows 10 SDK (14393) 或更新的版本，則也須安裝 WinJS 程式庫。 這個程式庫原本包含在舊版的 Windows10 SDK 中，但從 Windows10 SDK (14393) 開始，就必須另外安裝這個程式庫。 若要安裝 WinJS，請參閱[取得 WinJS](http://try.buildwinjs.com/download/GetWinJS/)。
...
3.  下載並安裝 [Microsoft Advertising SDK](http://aka.ms/ads-sdk-uwp) (適用於 Windows 10 的 UWP app) 或 [適用於 Windows 和 Windows Phone 8.x 的 Microsoft Advertising SDK](http://aka.ms/store-8-sdk) (適用於 Windows 8.1 和 Windows Phone 8.x 的 XAML 和 JavaScript/HTML app)。 可能需要幾分鐘的時間來安裝。 請務必等到程序完成為止。
...
## <a name="install-via-nuget-uwp-only"></a>透過 NuGet 安裝 (僅限 UWP)
...
3. 在 **\[參考管理員\]** 中，根據您的專案類型選取下列其中一項參考︰

    -   對於通用 Windows 平台 (UWP) 專案：展開 **\[通用 Windows\]**，按一下 **\[擴充功能\]**，然後選取 **\[適用於 XAML 的 Microsoft Advertising SDK\]** 旁邊的核取方塊 (適用於 XAML app) 或 **\[適用於 JavaScript 的 Microsoft Advertising SDK\]** 旁邊的核取方塊 (適用於使用 JavaScript 和 HTML 建置的應用程式)。

    -   對於 Windows 8.1 專案：展開 **\[Windows 8.1\]**，按一下 **\[擴充功能\]**，然後選取 **\[適用於 Windows 8.1 XAML 的 Ad Mediator SDK\]** 旁邊的核取方塊 (適用於 XAML 應用程式)或 **\[適用於 Windows 8.1 Native (JS) 的 Microsoft Advertising SDK\]** (適用於使用 JavaScript 和 HTML 建置的應用程式)。

    -   對於 Windows Phone 8.1 專案：展開 **\[Windows Phone 8.1\]**，按一下 **\[擴充功能\]**，然後選取 **\[適用於 Windows Phone 8.1 XAML 的 Ad Mediator SDK\]** 旁邊的核取方塊 (適用於 XAML 應用程式)或 **\[適用於 Windows Phone 8.1 Native (JS) 的 Microsoft Advertising SDK\]** (適用於使用 JavaScript 和 HTML 建置的應用程式)。
...
<span id="framework" />
## <a name="understanding-framework-packages-in-the-microsoft-advertising-sdk-uwp-only"></a>了解 Microsoft Advertising SDK 中的架構套件 (僅限 UWP)
