---
Description: 通用 Windows 平台 (UWP) 應用程式中執行實驗之前 / B 測試，您必須在合作夥伴中心來定義您的實驗。
title: 在合作夥伴中心定義您的實驗
...
ms.locfileid: "57624163"
---
# <a name="define-your-experiment-in-partner-center"></a>在合作夥伴中心定義您的實驗

之後您[建立專案，並在合作夥伴中心中定義遠端變數](create-a-project-and-define-remote-variables-in-the-dev-center-dashboard.md)並[應用程式進行測試的程式碼](code-your-experiment-in-your-app.md)，您已準備好在專案中建立實驗。 當您建立實驗時，您將會定義目標及使用者將接收的變化。
...
8.   請記下[專案識別碼](run-app-experiments-with-a-b-testing.md#terms)值。 當您[應用程式進行測試的程式碼](code-your-experiment-in-your-app.md)，因此您可以接收變化的資料，並向合作夥伴中心報告檢視 」 和 「 轉換 」 事件，您必須在程式碼中參考此識別碼。
...
> [!NOTE]
> 合作夥伴中心會回報為 24 小時制的時間週期內的每個使用者檢視僅限第一次轉換事件。 如果使用者在 24 小時內觸發您 App 中多個轉換事件，則只會回報第一個轉換事件。 這是為了防止當目標設為將執行轉換之使用者人數最大化時，單一使用者會扭曲一組範例使用者的實驗結果。
...
1. 中**遠端變數和變化** 區段中，您應該會看到兩個預設變化**變化 （控制）** 並**變化 B**。如果您想要更多的變化，請按一下**新增變化**。 或者，您亦可重新命名每個變化。
...
若您滿意實驗參數且已準備將其啟用，則可開始從您的 App 收集實驗資料，然後按一下 [啟用]。 啟用實驗時，您的應用程式可以擷取變化變數，並向合作夥伴中心報告檢視 」 和 「 轉換 」 事件。 如需詳細資訊，請參閱 <<c0> [ 執行和管理您的實驗，在合作夥伴中心](manage-your-experiment.md)。
...
合作夥伴中心和程式碼的實驗中定義您的實驗中您的應用程式之後，您已準備好[執行和管理您的實驗，在合作夥伴中心](manage-your-experiment.md)。
