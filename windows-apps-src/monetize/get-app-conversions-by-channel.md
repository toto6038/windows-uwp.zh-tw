---
author: mcleanbyron
description: "在 Windows 市集分析 API 中使用此方法，以針對特定日期範圍與其他選擇性篩選器，取得應用程式的依通道彙總轉換資料。"
title: "依通道取得應用程式轉換"
ms.author: mcleans
ms.date: 08/04/2017
ms.topic: article
ms.prod: windows
ms.technology: uwp
keywords: "Windows 10, UWP, 市集服務, Windows 市集分析 API, 應用程式轉換, 通道"
...
在 Windows 市集分析 API 中使用此方法，以針對特定日期範圍與其他選擇性篩選器，取得應用程式的依通道彙總轉換。

* *轉換*表示客戶 (使用 Microsoft 帳戶登入) 已經新獲得您 App 的授權 (不論是付費或免費取得)。
* *通道*是客戶到達您應用程式清單頁面的方法 (例如，透過市集或[自訂應用程式促銷活動](../publish/create-a-custom-app-promotion-campaign.md))。
...
| 參數        | 類型   |  描述      |  必要  
|---------------|--------|---------------|------|
| applicationId | 字串 | 您想要擷取轉換資料之應用程式的[市集識別碼](in-app-purchases-and-trials.md#store-ids)。 舉例來說，市集識別碼可以是「9WZDNCRFJ3Q8」。 |  是  |
...
| applicationId       | 字串 | 您正在擷取轉換資料之應用程式的[市集識別碼](in-app-purchases-and-trials.md#store-ids)。     |
...
| storeClient         | 字串 | 發生轉換之市集的版本。 目前唯一支援的值為 **SFC**。    |
