...
* 如果您尚未這樣做，請先完成 Windows 市集分析 API 的所有[先決條件](respond-to-reviews-using-windows-store-services.md#prerequisites)。
...
* 取得您想要查看的評論的識別碼，判斷您是否可回應。 評論識別碼是在 Windows 市集分析 API [取得 app 評論](get-app-reviews.md)方法的回應資料中，以及[評論報告](../publish/reviews-report.md)的[離線下載](../publish/download-analytic-reports.md)中。
...
| DefaultSupportEmail  | 字串 |  您的應用程式的[支援電子郵件地址](../publish/create-app-store-listings.md#support-contact-info)，如同您的應用程式市集清單中所指定。 如果您未指定支援電子郵件地址，這個欄位是空的。    |
...
* [使用 Windows 市集分析 API 提交評論的回應](submit-responses-to-app-reviews.md)
