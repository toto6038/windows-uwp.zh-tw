...
description: "在 Windows 市集中使用此方法促銷 API ，建立、編輯及獲得促銷廣告活動。"
title: "管理廣告行銷活動"
ms.author: mcleans
ms.date: 02/08/2017
ms.topic: article
...
當您使用這些方法建立或更新活動時，通常也呼叫下列一或多個管理方法*播送行*、*目標設定檔*及*廣告素材*相關的活動。 如需有關播送行、目標設定檔及廣告素材之間關聯性的詳細資訊，請參閱[使用 Windows 市集服務執行廣告行銷活動](run-ad-campaigns-using-windows-store-services.md#call-the-windows-store-promotions-api)。
