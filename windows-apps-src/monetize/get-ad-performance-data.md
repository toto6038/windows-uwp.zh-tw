...
ms.locfileid: "57627883"
---
# <a name="get-ad-performance-data"></a>取得廣告績效資料


使用「Microsoft Store 分析 API」中的這個方法，以針對特定日期範圍及其他選擇性篩選，取得應用程式的彙總廣告績效資料。 這個方法會傳回 JSON 格式的資料。

這個方法會傳回相同的資料所提供[廣告效能報告](../publish/advertising-performance-report.md)在合作夥伴中心。
...
| 參數     | 類型   | 描述     | 必要 |
|---------------|--------|-----------------|----------|
| applicationId   | 字串    | 您想要擷取廣告績效資料之 App 的 [Store 識別碼](in-app-purchases-and-trials.md#store-ids)。  |    否      |
| startDate   | date    | 要擷取的廣告績效資料之日期範圍的開始日期，格式為 YYYY/MM/DD。 預設為目前的日期減去 30 天。 |    否      |
| endDate   | date    | 要擷取的廣告績效資料之日期範圍的結束日期，格式為 YYYY/MM/DD。 預設為目前的日期減去一天。 |    否      |
...
| orderby   | 字串    | 將結果資料值排序的陳述式。 語法為 <em>orderby=field [order],field [order],...</em>。<em>field</em> 參數可以是下列其中一個字串：<ul><li><strong>日期</strong></li><li><strong>市場</strong></li><li><strong>裝置類型</strong></li><li><strong>adUnitId</strong></li></ul><p><em>order</em> 參數為選擇性，並可以是 <strong>asc</strong> 或 <strong>desc</strong>，以指定每個欄位的遞增或遞減順序。 預設為 <strong>asc</strong>。</p><p>下列為 <em>orderby</em> 字串的範例：<em>orderby=date,market</em></p> |    否      |
| groupby   | 字串    | 將資料彙總僅套用至指定欄位的陳述式。 您可以指定下列欄位：</p><ul><li><strong>applicationId</strong></li><li><strong>應用程式名稱</strong></li><li><strong>日期</strong></li><li><strong>accountCurrencyCode</strong></li><li><strong>市場</strong></li><li><strong>裝置類型</strong></li><li><strong>adUnitName</strong></li><li><strong>adUnitId</strong></li><li><strong>pubCenterAppName</strong></li><li><strong>adProvider</strong></li></ul><p><em>groupby</em> 參數可以搭配 <em>aggregationLevel</em> 參數使用。 例如：<em>&amp;groupby=applicationId&amp;aggregationLevel=week</em></p> |    否      |


### <a name="filter-fields"></a>篩選欄位

要求本體的 *filter* 參數包含在回應中篩選資料列的一或多個陳述式。 每個陳述式包含一個與 **eq** 或 **ne** 運算子關聯的欄位和值，而陳述式可以使用 **and** 或 **or** 結合。 以下是一個範例 *filter* 參數：

-   *篩選 = 市場 eq 'US' 和 deviceType eq '手機 」*
...
| deviceType    | 下列其中一個字串：<strong>PC/Tablet</strong>或是<strong>Phone</strong>。 |
...
下列範例示範數個取得廣告績效資料的要求。 將 *applicationId* 值以您應用程式的市集識別碼取代。
...
| 值               | 類型   | 描述                                                                                                                                                                                                                              |
|---------------------|--------|------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| date                | 字串 | 廣告績效資料之日期範圍中的第一個日期。 如果要求指定單一天數，此值便會是該日期。 如果要求指定一週、一個月或其他日期範圍，此值便會是該日期範圍的第一個日期。 |
| applicationId       | 字串 | 您要擷取廣告績效資料之 App 的「市集識別碼」。     |
| applicationName     | 字串 | 應用程式的顯示名稱。                         |
| adUnitId           | 字串 | 廣告單位的識別碼。        |
| adUnitName           | 字串 | Ad 單位，在合作夥伴中心開發人員所指定的名稱。              |
...
| pubCenterAppName       |  字串  |   在合作夥伴中心內的應用程式相關聯的 pubCenter 應用程式名稱。   |
