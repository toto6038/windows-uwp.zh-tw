...
Microsoft Store Services SDK 提供可協助您在「通用 Windows 平台」(UWP) app 中進行客戶業務開發的功能，例如將開發人員中心目標式通知傳送到您的 App，以及在您的 App 中執行 A/B 實驗。 此 SDK 是適用於 Visual Studio 2015 及後續 Visual Studio 版本的擴充功能。

> [!NOTE]
> 若要在您的 UWP app 中顯示廣告，請使用 [Microsoft Advertising SDK](http://aka.ms/ads-sdk-uwp) 而非 Microsoft Store Services SDK。 如需詳細資訊，請參閱[在您的應用程式中顯示廣告](display-ads-in-your-app.md)。
...
> [!NOTE]
> 若要在您的應用程式中顯示廣告，請使用 [Microsoft Advertising SDK](http://aka.ms/ads-sdk-uwp) 而非 Microsoft Store Services SDK。 如需詳細資訊，請參閱[在您的應用程式中顯示廣告](display-ads-in-your-app.md)。
...
* **NuGet 套件**&nbsp;&nbsp;您可以使用 NuGet 為特定的 UWP 專案安裝 SDK 程式庫。 使用此選項時，只會針對您已在其中安裝 NuGet 套件的專案安裝 SDK 程式庫。
...
5. 在搜尋方塊中，輸入 **Microsoft.Services.Store.SDK** 並安裝 Microsoft.Services.Store.SDK 套件。 套件完成安裝後，儲存您的方案。
    > [!NOTE]
...
3. 在 **\[參考管理員\]** 中，展開 **\[通用 Windows\]**、按一下 **\[擴充功能\]**，然後選取 **\[Microsoft Engagement Framework\]** 旁邊的核取方塊。 這可讓您使用 [Microsoft.Services.Store.Engagement](https://msdn.microsoft.com/library/windows/apps/microsoft.services.store.engagement.aspx) 命名空間中的 API 來進行客戶業務開發案例，例如[執行 A/B 實驗](run-app-experiments-with-a-b-testing.md)、[啟動意見反應中樞](launch-feedback-hub-from-your-app.md)、[接收來自開發人員中心的目標式推播通知](configure-your-app-to-receive-dev-center-notifications.md)，以及[將自訂事件記錄至開發人員中心](log-custom-events-for-dev-center.md)。

3. 按一下 **\[確定\]**。

> [!NOTE]
> 如果您已透過 NuGet 安裝 SDK 程式庫，您的專案將會包含 **Microsoft.Services.Store.SDK** 參考。 **Microsoft.Services.Store.SDK** 參考代表 NuGet 套件 (而不是它當中的程式庫)，您可以忽略它。
