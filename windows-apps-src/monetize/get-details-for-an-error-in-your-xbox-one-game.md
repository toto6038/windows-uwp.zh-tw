---
description: 在 Microsoft Store analytics API 中使用這個方法，以取得詳細的資料的特定錯誤為您的 Xbox One 遊戲。
title: 針對 Xbox One 遊戲中的錯誤取得詳細資料
ms.date: 11/06/2018
ms.topic: article
...
ms.locfileid: "57589963"
---
# <a name="get-details-for-an-error-in-your-xbox-one-game"></a>針對 Xbox One 遊戲中的錯誤取得詳細資料

Microsoft Store 在 analytics 中的 API 以取得詳細的資料的特定錯誤為您的 Xbox One 遊戲已擷取透過 Xbox 開發人員入口網站 (XDP)，而且可在 XDP Analytics 夥伴中心儀表板中使用這個方法。 這個方法只能擷取最近 30 天內發生的錯誤的詳細資料。

您可以使用這個方法之前，您必須先使用[取得錯誤報告資料，為您的 Xbox One 遊戲](get-error-reporting-data-for-your-xbox-one-game.md)方法來擷取您想要取得詳細的資訊錯誤的識別碼。
...
* 取得您想要取得詳細資訊之錯誤的識別碼。 若要取得這個識別碼，請使用[取得錯誤報告資料，為您的 Xbox One 遊戲](get-error-reporting-data-for-your-xbox-one-game.md)方法，並使用**failureHash**該方法的回應主體中的值。
...
| 參數        | 類型   |  描述      |  必要  
|---------------|--------|---------------|------|
| applicationId | 字串 | **存放區識別碼**的 Xbox One 遊戲，您要為其擷取錯誤詳細資料。 **存放區識別碼**位於在合作夥伴中心內的 [應用程式身分識別] 頁面。 舉例**存放區識別碼**是 9WZDNCRFJ3Q8。 |  是  |
| failureHash | 字串 | 您想要取得詳細資訊之錯誤的唯一識別碼。 若要取得此值，您有興趣的錯誤，請使用[取得錯誤報告資料，為您的 Xbox One 遊戲](get-error-reporting-data-for-your-xbox-one-game.md)方法，並使用**failureHash**該方法的回應主體中的值。 |  是  |
| startDate | date | 要擷取詳細錯誤資料之日期範圍的開始日期。 預設為目前日期的前 30 天。 |  否  |
| endDate | date | 要擷取詳細錯誤資料之日期範圍的結束日期。 預設為目前的日期。 |  否  |
| top | 整數 | 要在要求中傳回的資料列數目。 最大值及未指定的預設值為 10000。 如果查詢中有更多資料列，回應主體將會包含您可以用來要求下一頁資料的下一頁連結。 |  否  |
| skip | 整數 | 在查詢中要略過的資料列數目。 使用此參數來循頁瀏覽大型資料集。 例如，top=10 且 skip=0 將擷取前 10 個資料列的資料，top=10 且 skip=10 將擷取下 10 個資料列的資料，以此類推。 |  否  |
| filter |字串  | 一或多個篩選回應中資料列的陳述式。 每個陳述式包含一個與 **eq** 或 **ne** 運算子關聯的欄位名稱 (來自回應主體) 和值，而陳述式可以使用 **and** 或 **or** 結合。 *filter* 參數中的字串值必須由單引號括住。 您可以在回應本文中指定下列欄位：<p/><ul><li><strong>市場</strong></li><li><strong>日期</strong></li><li><strong>cabId</strong></li><li><strong>cabExpirationTime</strong></li><li><strong>裝置類型</strong></li><li><strong>deviceModel</strong></li><li><strong>osVersion</strong></li><li><strong>osRelease</strong></li><li><strong>PackageVersion</strong></li><li><strong>osBuild</strong></li></ul> | 否   |
| orderby | 字串 | 將結果資料值排序的陳述式。 語法為 <em>orderby=field [order],field [order],...</em>。<em>field</em> 參數可以是下列其中一個字串：<ul><li><strong>市場</strong></li><li><strong>日期</strong></li><li><strong>cabId</strong></li><li><strong>cabExpirationTime</strong></li><li><strong>裝置類型</strong></li><li><strong>deviceModel</strong></li><li><strong>osVersion</strong></li><li><strong>osRelease</strong></li><li><strong>PackageVersion</strong></li><li><strong>osBuild</strong></li></ul><p><em>order</em> 參數為選擇性，並可以是 <strong>asc</strong> 或 <strong>desc</strong>，以指定每個欄位的遞增或遞減順序。 預設為 <strong>asc</strong>。</p><p>下列為 <em>orderby</em> 字串的範例：<em>orderby=date,market</em></p> |  否  |


### <a name="request-example"></a>要求範例

下列範例會示範幾個要求，以取得詳細的錯誤資料的 Xbox One 遊戲。 取代*applicationId*值替換**存放區識別碼**為遊戲。
...
| 值           | 類型    | 描述     |
|-----------------|---------|----------------------------|
| applicationId   | 字串  | **存放區識別碼**的 Xbox One 的遊戲，您可以為其擷取詳細的錯誤資料。      |
...
| deviceModel           | 字串  | 其中一個下列字串，指定發生錯誤時，遊戲已執行的 Xbox One 主控台。<p/><ul><li><strong>Microsoft Xbox 其中一個</strong></li><li><strong>Microsoft Xbox 一個 S</strong></li><li><strong>Microsoft Xbox One X</strong></li></ul>  |
