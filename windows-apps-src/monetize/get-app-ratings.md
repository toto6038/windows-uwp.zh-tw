...
ms.locfileid: "86492923"
---
# <a name="get-app-ratings"></a>取得應用程式評分

使用「Microsoft Store 分析 API」中的這個方法，以針對特定日期範圍及其他選擇性篩選，取得彙總評分資料 (JSON 格式)。 您也可以在合作夥伴中心的[評論報告](../publish/reviews-report.md)中取得這項資訊。
...
| startDate | date | 要擷取評分資料之日期範圍的開始日期。 預設值是目前的日期。 |  否  |
| endDate | date | 要擷取評分資料之日期範圍的結束日期。 預設值是目前的日期。 |  否  |
...
| groupby | 字串 | 將資料彙總僅套用至指定欄位的陳述式。 您可以指定下列欄位：<ul><li><strong>date</strong></li><li><strong>applicationName</strong></li><li><strong>細分</strong></li><li><strong>osVersion</strong></li><li><strong>deviceType</strong></li><li><strong>isRevised</strong></li></ul><p>傳回的資料列將包含 <em>groupby</em> 參數中指定的欄位，以及下列項目：</p><ul><li><strong>date</strong></li><li><strong>applicationId</strong></li><li><strong>五星</strong></li><li><strong>四星</strong></li><li><strong>三星</strong></li><li><strong>兩星</strong></li><li><strong>一星</strong></li></ul><p><em>groupby</em> 參數可以搭配 <em>aggregationLevel</em> 參數使用。 例如： <em> &amp; Groupby = osVersion，市 &amp; aggregationLevel = 周</em></p> |  否  |
...
下列範例示範取得評分資料的數個要求。 將 *applicationId* 值取代為您 App 的 Store 識別碼。
...
| 值           | 類型    | 描述       |
|-----------------|---------|-------------------|
| date            | 字串  | 評分資料之日期範圍中的第一個日期。 如果要求指定單一天數，此值便會是該日期。 如果要求指定一週、一個月或其他日期範圍，此值便會是該日期範圍的第一個日期。 |
...
* [評分報告](../publish/reviews-report.md)
