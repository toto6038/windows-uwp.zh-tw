...
description: "在 Windows 市集分析 API 中使用此方法，以取得特定硬體錯誤的詳細資料。 這個方法僅供 OEM 使用。"
...
在 Windows 市集分析 API 中使用此方法，以取得特定 OEM 硬體錯誤的詳細資料 (JSON 格式)。 使用這個方法之前，您必須先使用[取得 OEM 硬體錯誤報告資料](get-oem-hardware-error-reporting-data.md)方法來擷取您要取得詳細資訊之錯誤的識別碼。
