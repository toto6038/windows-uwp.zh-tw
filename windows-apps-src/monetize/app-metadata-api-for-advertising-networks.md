...
| 值      | 類型   | 描述    |
|------------|--------|--------------------|
| storeId           | 字串  | 應用程式的市集識別碼。 範例「市集識別碼」如 9NBLGGH29DM8。     |  
| name           | 字串  | App 的名稱。   |
...
| downloadUrl           | 字串  | 「市集」中 App 清單的連結。 這個連結的格式為 ```https://www.microsoft.com/store/apps/<Store ID>```。  |
| isLive           | 布林值  | 如果 app 目前在 Microsoft Store 中可用，則為 true，否則為 false。  |
| iconUrls           | 陣列  |  一或多個字串的陣列，包含與 app 相關圖示 URL 的相對路徑。 若要擷取圖示，請在 URL 前面加上 *http* 或 *https*。  |
| type           | 字串  | 下列其中一個字串：**應用程式**或是**遊戲**。  |
| 裝置           |  陣列  | 包含一或多個下列字串，指定應用程式支援的裝置類型的陣列：**PC**，**電話**， **Xbox**， **IoT**， **Server**，以及**全像攝影版**。  |
| platformVersions           | 陣列  |  包含一或多個指定的應用程式支援的平台的下列字串的陣列：**Windows.Universal**， **Windows.Windows8x**，以及**Windows.WindowsPhone8x**。  |
| screenshotUrls           | 陣列  | 一或多個字串的陣列，包含與 app 相關螢幕擷取畫面 URL 的相對路徑。 若要擷取螢幕擷取畫面，請在 URL 前面加上 *http* 或 *https*。  |
