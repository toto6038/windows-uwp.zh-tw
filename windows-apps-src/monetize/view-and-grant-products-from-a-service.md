...
-   對於 *resource* 參數，請指定下列其中一個應用程式識別碼 URI (這些 URI 與您先前新增到應用程式資訊清單之 `"identifierUris"` 區段的 URI 相同)。 在此程序結束時，您應該會有三個存取權杖，且每個都與下列其中一個應用程式識別碼 URI 相關聯：
    -   **https://onestore.microsoft.com/b2b/keys/create/collections**：在稍後的步驟中，您將使用您搭配此 URI 所建立的存取權杖，來要求 Windows 市集識別碼金鑰，讓您能搭配 Windows 市集集合 API 來使用。
    -   **https://onestore.microsoft.com/b2b/keys/create/purchase**：在稍後的步驟中，您將使用您搭配此 URI 所建立的存取權杖，來要求 Windows 市集識別碼金鑰，讓您能搭配 Windows 市集購買 API 來使用。
    -   **https://onestore.microsoft.com**：在稍後的步驟中，您將使用您搭配此 URI 所建立的存取權杖，來直接呼叫 Windows 市集集合 API 或購買 API。
...
1.  將下列其中一個存取權杖，從您的服務傳遞給您的用戶端 app：
    ...
    -   如要取得可搭配 Windows 市集購買 API 使用的 Windows 市集識別碼金鑰，請傳遞您之前利用 **https://onestore.microsoft.com/b2b/keys/create/purchase** 對象 URI 所建立的 Azure AD 存取權杖。
