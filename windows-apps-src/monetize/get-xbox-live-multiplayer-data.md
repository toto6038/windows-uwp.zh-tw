...
ms.locfileid: "89171382"
---
# <a name="get-xbox-live-multiplayer-data"></a>取得 Xbox Live 多人遊戲資料


在 Microsoft Store 分析 API 中使用此方法，可以在每日或每月取得[已啟用 Xbox Live 遊戲](/gaming/xbox-live/index.md) 的多人遊戲資料。 這項資訊也可在合作夥伴中心的 [Xbox 分析報表](../publish/xbox-analytics-report.md) 中取得。
...
| endDate | date | 要擷取多人遊戲資料之日期範圍的結束日期。 預設值是目前的日期。 |  否  |
...
| filter | 字串  | 一或多個篩選回應中資料列的陳述式。 每個陳述式包含一個與 **eq** 或 **ne** 運算子關聯的欄位名稱 (來自回應主體) 和值，而陳述式可以使用 **and** 或 **or** 結合。 *篩選* 參數中的字串值必須由單引號括住。 您可以在回應本文中指定下列欄位：<p/><ul><li><strong>deviceType</strong></li><li><strong>packageVersion</strong></li><li><strong>市場</strong></li><li><strong>subscriptionName</strong></li></ul> | 否   |
| groupby | 字串 | 將資料彙總僅套用至指定欄位的陳述式。 您可以在回應本文中指定下列欄位：<p/><ul><li><strong>date</strong></li><li><strong>deviceType</strong></li><li><strong>packageVersion</strong></li><li><strong>市場</strong></li><li><strong>subscriptionName</strong></li></ul><p/>如果您指定一個或多個 *groupby* 欄位，則您未指定的任何其他 *groupby* 欄位將在回應本文中有 **All** 值。 |  否  |


### <a name="request-example"></a>要求範例

以下範例示範了用於已啟用 Xbox Live 遊戲取得多人遊戲資料的要求。 將 *applicationId* 值取代為您的遊戲的 Store 識別碼。
...
| 值      | 類型   | 描述                  |
|------------|--------|-------------------------------------------------------|
| 值      | array  | 包含多人遊戲資料的物件陣列，其中每個物件代表指定的每日或每月期間的一組資料，並按指定的 **篩選** 和 **groupby** 值整理。 如需每個物件中的資料的詳細資訊，請參閱[每日多人遊戲分析](#daily-multiplayer-analytics) 和[每月多人遊戲分析](#monthly-multiplayer-analytics) 部分。     |
...
| deviceType          | 字串 | 以下字串之一指定多人遊戲資料來自的裝置類型：<p/><ul><li><strong>主控台</strong></li><li><strong>Pc</strong></li><li>**Unknown**</li></ul>  |
| subscriptionName     | 字串 |  用於多人遊戲資料的訂閱名稱。 可能的值包括 **Xbox Game Pass** 和 **""** (沒有訂閱)。  |
| dailySessionCount     | number |  在指定日期，該遊戲中多人遊戲工作階段數目。  |
| engagementDurationMinutes     | number |  在指定日期，該遊戲中客戶參與多人遊戲工作階段的總分鐘數。  |
| dailyActiveUsers     | number |  在指定日期，該遊戲中使用中的多人遊戲使用者總數。  |
| dailyActiveDevices     | number |  在指定日期，該遊戲中玩過多人遊戲工作階段的使用中裝置總數。  |
| dailyNewUsers     | number |  在指定日期，該遊戲中新的多人遊戲使用者總數。  |
| monthlyActiveUsers     | number |  指定日期發生月份的使用中的多人遊戲使用者總數。  |
| monthlyActiveDevices     | number | 在指定日期發生的月份內，該遊戲中玩過多人遊戲工作階段的使用中裝置總數。   |
| monthlyNewUsers     | number |  在指定日期發生的月份，該遊戲中新的多人遊戲使用者總數。  |
...
| deviceType          | 字串 | 以下字串之一指定多人遊戲資料來自的裝置類型：<p/><ul><li><strong>主控台</strong></li><li><strong>Pc</strong></li><li>**Unknown**</li></ul>  |
| subscriptionName     | 字串 |  用於多人遊戲資料的訂閱名稱。 可能的值包括 **Xbox Game Pass** 和 **""** (沒有訂閱)。  |
| monthlySessionCount     | number |  在指定月份內，該遊戲中的多人遊戲工作階段數目。   |
| engagementDurationMinutes     | number |  在指定月份內，該遊戲中客戶參與多人遊戲工作階段的總分鐘數。  |
| monthlyActiveUsers     | number | 在指定月份內，使用中的多人遊戲使用者總數。   |
| monthlyActiveDevices     | number | 在指定月份內，該遊戲中玩過多人遊戲工作階段的使用中裝置總數。   |
| monthlyNewUsers     | number |  在指定月份內，該遊戲中新的多人遊戲使用者總數。  |
| averageDailyActiveUsers     | number |  在指定月份內，該遊戲中使用中的多人遊戲使用者的平均數量。  |
| averageDailyActiveDevices     | number |  在指定月份內，該遊戲中玩過多人遊戲工作階段的使用中裝置平均數量。  |
