...
| 參數        | 類型   |  描述      |  必要項  |
|---------------|--------|---------------|------|
| applicationId | 字串 | *ProductId*的 Xbox One 遊戲，您要為其擷取取得資料。 請注意，這是存放區識別碼和非 XDP 產品識別碼。 若要取得*productId*的遊戲，瀏覽至您的遊戲 XDP 分析程式，並擷取*productId*從 URL。 或者，如果您從合作夥伴中心分析報表中，下載您取得資料*productId*納入.tsv 檔案。 |  是  |
...
| applicationId       | 字串  | *ProductId*您要擷取附加元件取得資料的應用程式。 請注意，這是存放區識別碼和非 XDP 產品識別碼。                                                                                                                                                           |
