...
* 您必須與您的合作夥伴中心帳戶產生關聯的 Azure AD 應用程式、 擷取的租用戶識別碼和應用程式的用戶端識別碼，並產生金鑰。 Azure AD 應用程式代表您要呼叫 Microsoft Store 分析 API 的應用程式或服務。 您需要租用戶識別碼、用戶端識別碼和金鑰，才能取得傳遞給 API 的 Azure AD 存取權杖。
    > [!NOTE]
...
下列其他方法是適用於已內嵌透過 Xbox 開發人員入口網站 (XDP) 的 Xbox One 遊戲的開發人員帳戶使用，並可用於 XDP 分析儀表板。

| 案例       | 方法      |
|---------------|--------------------|
| 下載數 |  <ul><li>[取得 Xbox One 遊戲下載數](get-xbox-one-game-acquisitions.md)</li><li>[取得 Xbox One 附加元件下載數](get-xbox-one-add-on-acquisitions.md)</li></ul> |
| 錯誤 |  <ul><li>[取得 Xbox One 遊戲的錯誤報告資料](get-error-reporting-data-for-your-xbox-one-game.md)</li><li>[針對 Xbox One 遊戲中的錯誤取得詳細資料](get-details-for-an-error-in-your-xbox-one-game.md)</li><li>[針對 Xbox One 遊戲中的錯誤取得堆疊追蹤](get-the-stack-trace-for-an-error-in-your-xbox-one-game.md)</li><li>[針對 Xbox One 遊戲中的錯誤下載 CAB 檔案](download-the-cab-file-for-an-error-in-your-xbox-one-game.md)</li></ul> |
