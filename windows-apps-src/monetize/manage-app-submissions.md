...
> [!IMPORTANT]
> 如果您使用 Windows 市集提交 API 以建立應用程式的提交，請確定僅使用 API 變更提交，而不是開發人員中心儀表板。 如果您使用儀表板變更最初使用 API 所建立的提交，您將無法再使用 API 變更或是認可該提交。 有時候提交可能會處於錯誤狀態，而無法繼續提交過程。 若發生這種情形，您必須刪除提交並建立新的提交。
...
4. 如果您要新增提交的新套件、清單影像或預告片檔案，請[準備應用程式套件](https://msdn.microsoft.com/windows/uwp/publish/app-package-requirements)並[準備應用程式螢幕擷取畫面、影像與預告片](https://msdn.microsoft.com/windows/uwp/publish/app-screenshots-and-images)。 將所有這些檔案新增到 ZIP 封存。
    > [!NOTE]
    > 目前並非所有開發人員帳戶都能使用此 API 來提交 App 提交的預告片。 如果您的帳戶無法存取此資源，則當您取得或建立提交時，[App 提交資源](#app-submission-object)中的 *trailers* 陣列會是 null。

5. 以新提交的任何所需變更來修訂 [App 提交](#app-submission-object)資料，然後執行下列方法來[更新 App 提交](update-an-app-submission.md)。
...
某些提交可能無法使用兩組進階清單選項：[遊戲選項](#gaming-options-object)和[預告片](#trailer-object)。

這些清單選項是在 Windows 市集提交 API 首次發佈給開發人員後才新增的。 如果您是在這些清單選項推出之前就透過提交 API 建立 App 的提交，且這個提交仍在進行中，則除非您成功認可提交或將其刪除，否則您無法存取這些清單選項。

若要在使用提交 API 時判斷您是否可以存取特定 App 的這些清單選項，請使用[取得 App](get-an-app.md) 方法並檢查[應用程式資源](get-app-data.md#application_object)的 *hasAdvancedListingPermission* 欄位是否為 true。 如果您無法存取某個 App 的這些清單選項，當您[建立應用程式提交](create-an-app-submission.md)時，[App 提交資源](#app-submission-object)中的 *gamingOptions* 和 *trailers* 值會是 null。
...
| gamingOptions           |  陣列 |   包含一個[遊戲選項資源](#gaming-options-object)的陣列，其定義此 App 的遊戲相關設定。<br/><br/>**注意：**&nbsp;&nbsp;並非所有開發人員目前都能使用此 API 來設定遊戲選項。 如果您的帳戶無法存取此資源，則 *gamingOptions* 值會是 null。 若要判斷您是否可以設定 App 提交的 *gamingOptions*，請使用[取得 App](get-an-app.md) 方法並檢查[應用程式資源](get-app-data.md#application_object)的 *hasAdvancedListingPermission* 欄位是否為 true。      |   
...
| trailers           |  陣列 |   包含最多 15 個[預告片資源](#trailer-object)的陣列，代表 App 清單的預告影片。<br/><br/>**注意：**&nbsp;&nbsp;目前並非所有開發人員帳戶都能使用此 API 來提交 App 提交的預告片。 如果您的帳戶無法存取此資源，則 *trailers* 值會是 null。 若要判斷您是否可以設定 App 提交的 *trailers*，請使用[取得 App](get-an-app.md) 方法並檢查[應用程式資源](get-app-data.md#application_object)的 *hasAdvancedListingPermission* 欄位是否為 true。   |  
...
> [!IMPORTANT]
> **銷售**資源不再支援，目前您無法使用 Windows 市集提交 API 取得或修改應用程式提交的銷售資料。 我們將來會更新「Windows 市集提交 API」來導入新的方法，以程式設計方式存取應用程式提交的銷售資訊。
...
|  title               |     字串    |   應用程式清單的標題。   |  
|  shortDescription               |     字串    |  這個值僅適用於 Xbox 的清單。 這會指定清單的簡短描述，可協助使用者了解您的遊戲、遊戲類型，以及哪種玩家可能會喜歡您的遊戲。 輸入限制為 500 個字元。   |  
|  shortTitle               |     字串    |  這個值僅適用於 Xbox 的清單。 這會指定簡短版本的產品描述，以供一般描述太長時使用。 所有簡短標題版本的最大長度是 50 個字元，包括標準、當地語系化和覆寫。 覆寫版本會在使用替代語言的單一區域中顯示。    |  
|  sortTitle               |     字串    |   這個值僅適用於 Xbox 的清單。 這會指定清單的排序標題。 輸入限制為 255 個字元。   |  
|  voiceTitle               |     字串    |   這個值僅適用於 Xbox 的清單。 這會指定清單的語音標題。 輸入限制為 255 個字元。    |  
...
此資源包含應用程式清單的影像和圖示資料。 如需可用於列出之影像和圖示的詳細資訊，請參閱 [應用程式螢幕擷取畫面與影像](../publish/app-screenshots-and-images.md)。 此資源具有下列值。
...
|  id  |  字串  | 影像的識別碼。 此值由開發人員中心提供。  |
|  description  |  字串  | 影像的描述。  |
|  imageType  |  字串  | 指出影像的類型。 以下是目前支援的字串。 <p/>[螢幕擷取畫面影像](../publish/app-screenshots-and-images.md#screenshots)： <ul><li>Screenshot (此值適用於桌面螢幕擷取畫面)</li><li>MobileScreenshot</li><li>XboxScreenshot</li><li>SurfaceHubScreenshot</li><li>HoloLensScreenshot</li></ul><p/>[市集標誌](../publish/app-screenshots-and-images.md#store-logos)：<ul><li>StoreLogo9x16 </li><li>StoreLogoSquare</li><li>Icon (此值適用於 1:1 300 x 300 像素標誌)</li></ul><p/>[宣傳影像](../publish/app-screenshots-and-images.md#promotional-images)： <ul><li>PromotionalArt16x9</li><li>PromotionalArtwork2400X1200</li></ul><p/>[Xbox 影像](../publish/app-screenshots-and-images.md#xbox-images)： <ul><li>XboxBrandedKeyArt</li><li>XboxTitledHeroArt</li><li>XboxFeaturedPromotionalArt</li></ul><p/>[選用宣傳影像](../publish/app-screenshots-and-images.md#optional-promotional-images)： <ul><li>SquareIcon358X358</li><li>BackgroundImage1000X800</li><li>PromotionalArtwork414X180</li></ul><p/> <!-- The following strings are also recognized for this field, but they correspond to image types that are no longer for listings in the Store.<ul><li>PromotionalArtwork846X468</li><li>PromotionalArtwork558X756</li><li>PromotionalArtwork414X468</li><li>PromotionalArtwork558X558</li><li>WideIcon358X173</li><li>Unknown</li></ul> -->   |


<span id="gaming-options-object" />
### <a name="gaming-options-resource"></a>遊戲選項資源

此資源包含應用程式的遊戲相關設定。 此資源中的值對應至開發人員中心儀表板中提交的[遊戲設定](../publish/enter-app-properties.md#game-settings)。

> [!NOTE]
> 此資源[不一定可用於所有提交](#advanced-listings)。

如果您是在這些清單選項推出之前就透過提交 API 建立 App 的提交，且這個提交仍在進行中，則除非您成功認可提交或將其刪除，否則您無法針對 App 的任何提交存取這些清單選項。 若要在使用提交 API 時判斷您是否可以存取特定 App 的這些清單選項，請使用[取得 App](get-an-app.md) 方法並檢查[應用程式資源](get-app-data.md#application_object)的 *hasAdvancedListingPermission* 欄位是否為 true。
...
|  kinectDataForExternal               |   字串      |  下列其中一個值，指出遊戲是否可以收集 Kinect 資料並將其傳送到外部服務： <ul><li>NotSet</li><li>Unknown</li><li>Enabled</li><li>Disabled</li></ul>   |
...
> [!NOTE]
> 在呼叫[更新應用程式提交](update-an-app-submission.md)方法時，要求主體中只需要這個物件的 *fileName*、*fileStatus*、*minimumDirectXVersion* 及 *minimumSystemRam* 值。 其他值均是由開發人員中心所填入。

| 值           | 類型    | 描述                   |
|-----------------|---------|------|
| fileName   |   字串      |  套件的名稱。    |  
| fileStatus    | 字串    |  套件的狀態。 這可以是下列其中一個值： <ul><li>None</li><li>PendingUpload</li><li>Uploaded</li><li>PendingDelete</li></ul>    |  
| id    |  字串   |  唯一識別套件的識別碼。 此值由開發人員中心提供。   |     
...
### <a name="trailer-resource"></a>預告片資源

此資源代表應用程式清單的預告影片。 此資源中的值對應至開發人員中心儀表板中提交的[預告片](../publish/app-screenshots-and-images.md#trailers)選項。

您可新增高達 15 個預告片資源至 [App 提交資源](#app-submission-object)中的 *trailers* 陣列。 若要上傳提交的預告片影片檔案和縮圖影像，請將這些檔案新增至包含提交的套件和清單影像的同一個 ZIP 封存中，然後將此 ZIP 封存上傳到提交的共用存取簽章 (SAS) URI。 如需上傳 ZIP 封存至 SAS URI 的詳細資訊，請參閱[建立 App 提交](#create-an-app-submission)。
...
| 值           | 類型    | 描述        |
|-----------------|---------|------|
|  id               |    字串     |   預告片的識別碼。 此值由開發人員中心提供。   |
|  videoFileName               |    字串     |    ZIP 封存中預告片影片檔的名稱，包含提交的檔案。    |     
|  videoFileId               |   字串      |  預告片影片檔的識別碼。 此值由開發人員中心提供。   |     
|  trailerAssets               |   物件      |  機碼和值組的字典，其中每個機碼都是語言代碼，而每個值都是[預告片資產資源](#trailer-assets-object)，其中包含預告片的其他地區設定資產。 如需支援的語言代碼的詳細資訊，請參閱[支援的語言](https://msdn.microsoft.com/windows/uwp/publish/supported-languages)。    |     
