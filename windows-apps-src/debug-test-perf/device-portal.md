...
ms.locfileid: "98104529"
---
# <a name="windows-device-portal-overview"></a>Windows Device Portal 概觀

Windows 裝置入口網站 (WDP) 是 Windows 裝置隨附的網頁伺服器，可讓您透過網路或 USB 連線來設定及管理裝置的設定 (使用網頁瀏覽器的裝置上也支援本機連線)。

WDP 也提供進階診斷工具來進行疑難排解和檢視您 Windows 裝置的即時效能。

WDP 功能會透過 [REST API](device-portal-api-core.md) 集合以程式設計方式公開。

此文章提供 Windows 裝置入口網站的一般描述，並包含更具體說明每個 Windows 裝置系列的文章連結。
...
每個裝置系列都會提供 WDP 版本，但功能和安裝會根據裝置的需求而有所不同。
...
裝置系列 | 預設開啟？ | HTTP | HTTPS | USB | 指示 |
--------------|----------------|------|-------|-----|--------------|
桌上型| 在開發人員模式內啟用 | 50080\* | 50043\* | 不適用 | [在桌面裝置上設定 Windows 裝置入口網站](device-portal-desktop.md#set-up-windows-device-portal-on-a-desktop-device) |
Xbox | 在開發人員模式內啟用 | 停用 | 11443 | 不適用 | [Xbox 的裝置入口網站](../xbox-apps/device-portal-xbox.md) |
HoloLens | 是，在開發人員模式 | 80 (預設值) | 443 (預設值) | http://127.0.0.1:10080 | [HoloLens 的裝置入口網站](/windows/mixed-reality/develop/platform-capabilities-and-apis/using-the-windows-device-portal) |
IoT | 是，在開發人員模式 | 8080 | 透過登錄機碼啟用 | 不適用 | [IoT 的裝置入口網站](/windows/iot-core/manage-your-device/DevicePortal) |
電話 | 在開發人員模式內啟用 | 80| 443 | http://127.0.0.1:10080 | [行動裝置的行動裝置](device-portal-mobile.md) |

\* 情況並非總是如此，因為電腦上的裝置入口網站會宣告位於暫時性範圍內的連接埠 (>50,000)，以避免與裝置上現有的連接埠宣告相衝突。 若要深入了解，請在[桌上型裝置的 Windows 裝置入口網站](device-portal-desktop.md)中，參閱[以登錄為基礎的設定](device-portal-desktop.md#registry-based-configuration)。  
