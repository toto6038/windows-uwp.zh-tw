...
這裡描述各種裝置系列上常見的工具。 根據不同的裝置，可能提供其他選項。 如需詳細資訊，請參閱您的裝置類型的特定頁面。
...
- **已安裝應用程式**： 使用下拉式功能表中移除，或是開始在裝置安裝的應用程式。 按一下 [**新增**安裝新的應用程式。 這會起始安裝已封裝的應用程式從本機部署的 UX、 網路或 web 裝載並登錄鬆散檔案從網路共用。
...
2.  在裝置入口網站的應用程式管理員] 區段中，按一下 \ [**加入**，然後選取**安裝從本機存放區的應用程式套件**。
...
ETW 記錄頁面來管理裝置上的即時事件 Windows 追蹤 (ETW) 資訊。
...
- **篩選 / 事件**: [**事件**] 區段會列出來自以表格格式所選提供者的 ETW 事件。 此表格會即時更新。 使用**篩選器**功能表來設定將會顯示的事件的自訂篩選器。 按一下 [**清除**] 按鈕以刪除表格中的所有 ETW 事件。 這不會停用任何提供者。 您可以按一下來將目前收集的 ETW 事件匯出到本機的 CSV 檔案的 [**儲存到檔案**。
