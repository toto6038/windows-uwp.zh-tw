...
*條件式 XAML* 提供一種在 XAML 標記中使用 [ApiInformation.IsApiContractPresent](https://docs.microsoft.com/uwp/api/windows.foundation.metadata.apiinformation#Windows_Foundation_Metadata_ApiInformation_IsApiContractPresent_System_String_System_UInt16_) 方法的方式。 這可讓您在標記中根據 API 是否存在來設定屬性和具現化物件，而不必使用程式碼後置。 它會選擇性剖析元素或屬性以判斷這些項目是否可在執行階段使用。 條件陳述式在執行階段進行評估，如果評估為 **true**，則剖析條件式 XAML 標記所限定的元素，否則將其忽略。
...
> [!IMPORTANT]
> **發行前版本 | 需要秋季版 Creators Update**：您必須將目標設為 [Insider SDK 16225](https://www.microsoft.com/en-us/software-download/windowsinsiderpreviewSDK)，並且在執行[測試人員組建 16226](https://blogs.windows.com/windowsexperience/2017/06/21/announcing-windows-10-insider-preview-build-16226-pc/) 或更新版本，才能使用條件式 XAML。
...
若要使用條件式 XAML，您必須先在頁面頂端宣告條件式 [XAML 命名空間](../xaml-platform/xaml-namespaces-and-namespace-mapping.md)。 以下是條件式命名空間的虛擬程式碼範例：

```xaml
xmlns:myNamespace="schema?conditionalMethod(parameter)"
```

條件式命名空間可分成兩個部分 (以 '?' 分隔符號分隔)。 分隔符號之前的內容表示包含所參考之 API 命名空間或結構描述。 在 '?' 分隔符號之後的內容表示判斷條件式命名空間評估為 **true** 或 **false** 的條件式方法。
...
在此範例中，如果應用程式在 Windows Insider Preview (秋季版 Creators Update) 執行，您會顯示「Windows 測試人員您好」做為文字區塊的內容，如果在舊版執行，則依預設不顯示任何內容。

首先，定義以 'insider' 為前置詞的自訂命名空間，並使用預設 XAML 命名空間 (http://schemas.microsoft.com/winfx/2006/xaml/presentation) 做為包含 [TextBlock.Text](https://docs.microsoft.com/uwp/api/windows.ui.xaml.controls.textblock#Windows_UI_Xaml_Controls_TextBlock_Text) 屬性的結構描述。 若要讓這個命名空間變成條件式命名空間，請在結構描述後面加上 '?' 分隔符號。

接著定義會在執行 Windows Insider Preview 或更新版本之裝置傳回 **true** 的條件式。 您可以使用 ApiInformation method **IsApiContractPresent** 來檢查是否有第 5 版的 UniversalApiContract。 UniversalApiContract 版本 5 已隨 Windows Insider Preview (秋季版 Creators Update) 一起發行。
...
在 Insider Preview 上執行此範例時，將會顯示「我是 Windows 測試人員」，如果是在 Creators Update 中執行，則不顯示任何文字。
...
在上一個範例中，只有當應用程式在 Insider Preview 上執行時才會設定 Text 屬性。 但若是您要在執行於 Creators Update 時顯示不同的文字呢？ 您可以嘗試不使用條件式限定詞來設定 Text 屬性，就像這樣。
...
這在 Creators Update 上執行時會有作用，但在 Insider Preview 上執行時，就會收到錯誤訊息，指出已設定 Text 屬性一次以上。
...
若要使用反轉條件，請建立第二個使用 **IsApiContractNotPresent** 條件式的條件式 XAML 命名空間。 如下所示，其前置詞為 'notInsider'。
...
以下是另一個設定按鈕背景的範例。 從 Insider Preview (秋季版 Creators Update) 開始已提供[壓克力材質](../style/acrylic.md)功能，因此當應用程式在 Insider Preview 上執行時，您會使用壓克力做為背景。 舊版無法使用此功能，所以在這些情況下，請將背景設定為紅色。
...
在這裡，當應用程式執行於有 [ColorPicker](https://docs.microsoft.com/uwp/api/windows.ui.xaml.controls.colorpicker) 控制項可用的 Insider Preview 時，該控制項已具現化。 ColorPicker 無法在 Insider Preview 以前的版本中使用，因此當應用程式執行於舊版時，請使用 [ComboBox](https://docs.microsoft.com/uwp/api/windows.ui.xaml.controls.combobox) 來為使用者提供簡化的色彩選擇。
...
您可以透過不同形式的 [XAML 屬性語法](../xaml-platform/xaml-syntax-guide.md)來使用條件式限詞。 在這裡，矩形的 Fill 屬性針對 Insider Preview 使用屬性元素語法來設定，針對舊版則使用屬性語法來設定。
...
將屬性繫結到另一個相依於條件式命名空間的屬性時，您必須在這兩個屬性上使用相同的條件。 在這裡，`colorPicker.Color` 相依於 'insider' 條件式命名空間，因此您也必須將 'insider' 前置詞放在 SolidColorBrush.Color 屬性上 (或者，也可以將 'insider' 前置詞放在 SolidColorBrush，而不放在 Color 屬性)。如果沒有這麼做，您將會收到編譯時間錯誤。
...
當應用程式在 Insider Preview 上執行時，請使用 ColorPicker 來讓使用者設定色彩。 ColorPicker 無法在 Insider Preview 以前的版本中使用，因此當應用程式執行於舊版時，請使用下拉式方塊來為使用者提供簡化的色彩選擇。
