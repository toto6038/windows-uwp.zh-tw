...
-   [
            **UIElement.Opacity**](https://msdn.microsoft.com/library/windows/apps/BR208962) 屬性的動畫
...
-   以這些傳回值類型的子屬性為目標時，下列 [**UIElement**](https://msdn.microsoft.com/library/windows/apps/BR208911) 屬性的動畫：
    ...
    -   [**投影**](https://msdn.microsoft.com/library/windows/apps/BR208911-projection)
    -   [**裁剪**](https://msdn.microsoft.com/library/windows/apps/BR208911-clip)
...
-   執行元素 ([**CacheMode**](https://msdn.microsoft.com/library/windows/apps/BR208911-cachemode) 屬性設為 **BitmapCache**) 中被視為獨立式動畫的任何動畫。 這可視為相依式動畫，因為必須針對每個畫面點陣化快取。

### 不要讓 WebView 或 MediaElement 產生動畫

[
            **WebView**](https://msdn.microsoft.com/library/windows/apps/BR227702) 控制項內的網頁內容不是由 XAML 架構直接呈現，並且需要執行額外的工作才能與其他場景合成。 在畫面上產生控制項動畫會增加這項額外的工作，而這項工作可能會帶來同步化的問題 (例如，HTML 內容可能不會與頁面上的其餘 XAML 內容同步移動)。 當您需要讓 **WebView** 控制項產生動畫時，請改用 [**WebViewBrush**](https://msdn.microsoft.com/library/windows/apps/BR227702brush) 以取得持續的動畫。

讓 [**MediaElement**](https://msdn.microsoft.com/library/windows/apps/BR242926) 產生動畫同樣也不是很好的方法。 除了效能變差，還會造成播放的視訊內容產生裂紋或其他殘影。
...
例如，如果您的 app 使用 [**RandomAccessStream**](https://msdn.microsoft.com/library/windows/apps/BR241747) 或 [**IInputStream**](https://msdn.microsoft.com/library/windows/apps/BR241718) 物件，請確定您的 app 完成使用時呼叫物件的 close 方法，以釋放基礎物件。

### 盡可能顯示全螢幕視訊播放

在 UWP app 中，一律在 [**MediaElement**](https://msdn.microsoft.com/library/windows/apps/BR242926) 上使用 [**IsFullWindow**](https://msdn.microsoft.com/library/windows/apps/BR242926-isfullwindow) 屬性，以啟用和停用完整視窗轉譯。 這可確保在媒體播放時會使用系統層級最佳化。

當視訊內容是唯一要轉譯的內容時，XAML 架構可以最佳化顯示的視訊內容，因此可以使用較少的電力，並產生較高的畫面播放速率。 最有效的媒體播放，是將 [**MediaElement**](https://msdn.microsoft.com/library/windows/apps/BR242926) 物件的大小設為畫面的寬度和長度，並且不顯示其他 XAML 元素。

基於某些正當理由，您可以在佔據了螢幕全長和全寬的 [**MediaElement**](https://msdn.microsoft.com/library/windows/apps/BR242926) 上重疊 XAML 元素，例如隱藏式輔助字幕或短暫的傳輸控制項。 但是在不需要這些元素時，請務必隱藏它們 (例如設定 Visibility=”Collapsed”)，使媒體播放回到最有效狀態。
...
應用程式通常會提供在頁面中播放視訊的內嵌檢視。 由於 [**MediaElement**](https://msdn.microsoft.com/library/windows/apps/BR242926) 不是頁面大小，再加上其他繪製的 XAML 物件，您很明顯地無法使用全螢幕播放。 請注意不要在 **MediaElement** 周圍繪製框線，避免不慎進入這個模式。

當視訊是內嵌模式時，請勿在視訊上繪製 XAML 元素。 如果您這樣做，架構會強制執行一些額外工作來建構場景。 將傳輸控制項放在內嵌媒體元素下方 (而非視訊上)，是這個狀況最佳化的最佳範例。 在這個影像中，紅色列代表一組傳輸控制項 (播放、暫停及停止等)。

![含有重疊元素的 MediaElement](images/videowithoverlay.png)
請勿在不是全螢幕的媒體上放置這些控制項。 而是將這些傳輸控制項放在媒體轉譯區域外部的位置。 在下一個影像中，控制項放在媒體下方。

![含有相鄰元素的 MediaElement](images/videowithneighbors.png)

### 延遲設定 MediaElement 的來源

媒體引擎是耗費大量資源的物件，XAML 架構會盡可能延遲載入 dll 和建立大型物件。 在透過 [**Source**](https://msdn.microsoft.com/library/windows/apps/BR242926-source) 屬性或 [**SetSource**](https://msdn.microsoft.com/library/windows/apps/BR242926-setsource) 方法設定來源後，[**MediaElement**](https://msdn.microsoft.com/library/windows/apps/BR242926) 會被強制執行這個工作。 在使用者真的準備好播放媒體時設定這些就可以盡可能延遲與 **MediaElement** 關聯的大多數成本。

### 設定 MediaElement.PosterSource

設定 [**MediaElement.PosterSource**](https://msdn.microsoft.com/library/windows/apps/BR242926-postersource) 可以讓 XAML 釋放一些用於其他方面的 GPU 資源。 這個 API 可以讓 app 盡可能使用最少的記憶體。

### 改善媒體清除

為了使媒體平台可以立即回應，清除永遠是一項困難的工作。 使用者一般會透過變更滑桿的值來進行這項工作。 下列是一些如何讓這項工作更有效率的提示：

-   將 [**Slider**](https://msdn.microsoft.com/library/windows/apps/BR209614) 的值繫結到 [**MediaElement.Position**](https://msdn.microsoft.com/library/windows/apps/BR242926-position)，或依據計時器將它更新。 請執行其中一項動作。 如果選擇後者，請務必為您的計時器使用合理的更新頻率。 在播放期間，XAML 架構只會每 250 毫秒更新一次 **MediaElement.Position**。
-   滑桿上的步階頻率大小必須隨視訊的長度調整。
-   訂閱滑桿上的 [**PointerPressed**](https://msdn.microsoft.com/library/windows/apps/BR208911-pointerpressed)、[**PointerMoved**](https://msdn.microsoft.com/library/windows/apps/BR208911-pointermoved) 以及 [**PointerReleased**](https://msdn.microsoft.com/library/windows/apps/BR208911-pointerreleased) 事件，在使用者拖曳滑桿的指標時將 [**MediaElement.PlaybackRate**](https://msdn.microsoft.com/library/windows/apps/BR242926-playbackrate) 屬性設定為 0。
-   在 [**PointerReleased**](https://msdn.microsoft.com/library/windows/apps/BR208911-pointerreleased) 事件處理常式中，將媒體位置手動設定為滑桿位置值，在清除時獲得最佳效果。
...
包含簡短且低延遲的音訊效果時 (例如在遊戲中)，請使用含未壓縮的 PCM 資料的 WAV 檔案來減輕壓縮音訊格式特有的處理額外負荷。

### 硬體音訊卸載

若要自動套用硬體音訊卸載，[**MediaElement.AudioCategory**](https://msdn.microsoft.com/library/windows/apps/BR242926-audiocategory) 必須設定為 **ForegroundOnlyMedia** 或 **BackgroundCapableMedia**。 硬體音訊卸載可最佳化音訊表現，這可提升功能及電池使用時間。
...
[
            **DecodePixelWidth**](https://msdn.microsoft.com/library/windows/apps/BR243243) 和 [**DecodePixelHeight**](https://msdn.microsoft.com/library/windows/apps/BR243241) 的單位預設是實體像素。 [
...
-   使用 [**SetSourceAsync**](https://msdn.microsoft.com/library/windows/apps/JJ191522) 或 [**UriSource**](https://msdn.microsoft.com/library/windows/apps/BR243235-urisource) 設定內容之後，[**BitmapImage**](https://msdn.microsoft.com/library/windows/apps/BR243235) 連接到動態的 XAML 樹狀結構。
-   影像使用同步解碼 (例如 [**SetSource**](https://msdn.microsoft.com/library/windows/apps/BR243255)) 來解碼。
-   影像在主機影像元素或筆刷或任何父元素上透過將 [[**不透明度**](https://msdn.microsoft.com/library/windows/apps/BR208962)] 設定為 0，或將 [[**可見度**](https://msdn.microsoft.com/library/windows/apps/BR208911-visibility)] 設定為 [**收合**] 而隱藏。
...
快取最佳化適用於使用 [**UriSource**](https://msdn.microsoft.com/library/windows/apps/BR243235-urisource) 從應用程式套件或從網路載入內容的影像。 URI 是用來唯一識別相關的內容，因此就不需要內部多次下載 XAML 架構或解碼內容。 相反地會使用快取的軟體或硬體資源來多次顯示內容。

這個最佳化的例外是如果影像以不同解析度顯示多次 (這可以明確指定或透過自動以正確大小解碼)。 每個快取項目也會儲存影像的解析度，如果 XAML 找不到有來源 URI 的影像符合需要的解析度，則會以該大小解碼新的版本， 但是不會再下載編碼的影像資料。

因此，您必須在從應用程式套件載入影像時包含使用 [**UriSource**](https://msdn.microsoft.com/library/windows/apps/BR243235-urisource)，以及避免使用檔案串流和 [**SetSourceAsync**](https://msdn.microsoft.com/library/windows/apps/JJ191522) (如果不必要)。
...
按比例調整影像的一個使用案例是建立縮圖。 雖然您可以使用 [**DecodePixelWidth**](https://msdn.microsoft.com/library/windows/apps/BR243243) 與 [**DecodePixelHeight**](https://msdn.microsoft.com/library/windows/apps/BR243241) 來提供較小的影像版本，但 UWP 提供更有效的 API 供您擷取縮圖。 [
            **GetThumbnailAsync**](https://msdn.microsoft.com/library/windows/apps/BR227210) 會為已快取檔案系統的影像提供縮圖。 這樣做的效能比使用 XAML API 更好，因為不需要開啟或解碼影像。
