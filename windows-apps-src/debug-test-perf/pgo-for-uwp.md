...
description: 若要將設定檔指引最佳化 (PGO) 套用至通用 Windows 平台 (UWP) 應用程式的逐步指南。
ms.date: 02/08/2017
...
在本指南中的螢幕擷取畫面根據下列的新專案：![新增專案 對話方塊](images/pgo-001.png)
...
7. 系統會檢測現在執行的組建並產生 PGO 資料。 此時，您應該讓應用程式在一些您想要最佳化的最常見案例中從頭到尾執行一遍。 在程式於您想要的案例中從頭到尾執行一遍之後，請在您找到適當 `pgort140.dll` 版本的相同資料夾中尋找 pgosweep.exe 工具 。 或者，Visual Studio (x86/x64) Native Tools 命令提示字元的路徑中將會已經包含適當的版本。 若要收集 PGO 資料，請在應用程式仍在執行中時執行下列命令，以產生將包含分析資料的 .pgc 檔案︰
  ...
  也可能是進一步收集`App1!CoreScenario.pgc`，`App1!UseCase5.pgc`等等。如果以這種方式，並在組建輸出位置，以及.pgd 命名.pgc 檔案，它們會自動合併步驟 9 中的連結時。
 
8. 選擇性：根據預設，所有的.pgc 檔案，名為在步驟 7 中指定與.pgd 旁放置將會合併時連結和加權，同樣地，但您也可以有更大控制如何特定的執行進行加權。 為了這樣做，您將使用 **pgomgr.exe** 工具，此工具也位於您一開始找到 `pgort140.dll` 複本的相同資料夾中。 例如，若要將 `CoreScenario` 執行以其他執行的 3 倍優先順序合併，我可以使用下列命令：
