...
在**遊戲的事件資料**頁面上，您可以檢視即時圖形該資料流中的事件 Windows 追蹤 (ETW) 遊戲目前錄製您 Xbox One 上的事件數目。 如果沒有記錄系統上的遊戲事件，您也可以檢視詳細資料 （事件名稱、 事件發生，以及遊戲的標題） 描述資料表格下方資料圖形中的每個事件。 此表格僅適用於有事件記錄。
