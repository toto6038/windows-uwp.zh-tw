...
Windows 10 行動裝置版的 Microsoft 模擬器需要 Visual Studio 2015 或更新版本;它不是回溯相容於舊版的 Visual Studio。

適用於 Windows 10 Mobile 的 Microsoft 模擬器無法載入以 Windows Phone OS 7.1 之前的 Windows Phone OS 版本為目標的應用程式。

## <a name="installing-uninstalling-and-running-the-emulator"></a>安裝、 解除安裝和執行模擬器

### <a name="installing"></a>安裝
適用於 Windows 10 Mobile 的 Microsoft 模擬器隨附於 Windows 10 SDK。 安裝 Visual Studio 時也可以安裝 Windows 10 SDK 與模擬器。 請參閱 [Visual Studio 下載頁面](https://go.microsoft.com/fwlink/p/?LinkId=534785)。
...
若要在模擬器中執行的應用程式，只要選取 從模擬的裝置**執行**Visual Studio 中的下拉式選單。
...
> **重要**  當您第一次啟動 tapper 工具時，您會取得 Windows 防火牆提示。 您必須選取全部 3 個核取方塊，並允許該工具通過防火牆，否則該工具會失敗，而且不會顯示任何訊息。
...
> **附註**  FAT32 檔案系統格式化使用的電話因為的 sd 記憶卡，32 GB 的最大值。
...
### <a name="error-message-failed-while-removing-virtual-ethernet-switch"></a>錯誤訊息：「 同時移除虛擬乙太網路交換器失敗 」

在某些情況下，包括更新到新的 Windows 10 飛行之後, 的模擬器相關聯的虛擬網路交換器可以登入，則無法刪除透過使用者介面的狀態。

若要從此情況中復原，請從系統管理員命令提示字元執行 "netcfg -d"：`C:\Program Files (x86)\Microsoft XDE\<version>\XdeCleanup.exe`。 當命令完成執行時，需重新啟動電腦，才能完成修復程序。

**附註**  此命令會刪除所有的網路裝置，不只是那些模擬器相關聯。 當您的電腦重新啟動時，將會自動探索所有硬體網路裝置。
 
### <a name="unable-to-launch-the-emulators"></a>無法啟動模擬器

Microsoft 模擬器包含 XDECleanup.exe，這是一個工具，可刪除所有的 VM、差異磁碟及模擬器特定的網路交換器，而它已經隨附於模擬器 (XDE) 二進位檔中。 如果模擬器 VM 進入錯誤狀態，您應該使用這個工具來清除這些 VM。 從系統管理員命令提示字元中執行此工具：`C:\Program Files (x86)\Microsoft XDE\<version>\XdeCleanup.exe`

> **附註**  XDECleanup.exe 刪除所有的模擬器特定的 HYPER-V Vm，而且也會刪除任何 VM 檢查點，或儲存狀態。
...
在目前版本中，您接著必須手動刪除模擬器的 VHD 檔案。 如果您的預設路徑安裝模擬器，將 VHD 檔案位於 c:\\Program Files (x86)\\Windows 套件\\10\\模擬\\Mobile\\ <version> \\flash.vhd。
...
**注意：** 此登錄值設定之後，您必須刪除的組態，您在 Visual Studio 中啟動的 HYPER-V 管理員中的虛擬機器，並然後再重新啟動模擬器，軟體轉譯圖形。
