...
> [!IMPORTANT]
> 有時會因網路或相容性問題，致使開發人員模式無法正確安裝在您的裝置上。 如需協助進行這些問題的疑難排解，請參閱[＜啟用您的裝置以用於開發＞中的相關部分](https://docs.microsoft.com/windows/uwp/get-started/enable-your-device-for-development#failure-to-install-developer-mode-package)。
...
![設定應用程式的裝置入口網站區段](images/device-portal/device-portal-desk-settings.png) 

一旦啟用裝置入口網站，您會在區段的底端看見 Web 連結。 記下附加至所列 URL 結尾的連接埠號碼：啟用裝置入口網站時會隨機產生此號碼，但桌面重新開機時此號碼應保持一致。 

這些連結提供兩種連線至裝置入口網站的方式：透過區域網路 (包括 VPN) 或透過本機主機。

### <a name="connect-to-device-portal"></a>連線到裝置入口網站
...
如果您是在受保護的環境中使用裝置入口網站 (例如測試實驗室)，而在此環境中您信任區域網路上的每個人、裝置上沒有個人資訊、且有獨特的需求，則您可以停用「驗證」。 這會啟用未加密的通訊，並允許具有您電腦 IP 位址的任何人進行連線並對其進行控制。

## <a name="device-portal-content-on-windows-desktop"></a>Windows 桌面上的裝置入口網站內容

Windows 桌面上的裝置入口網站有一組標準頁面。 如需這些項目的詳細描述，請參閱 [Windows 裝置入口網站概觀](device-portal.md)。
...
- `-DeleteSSL` 
    - 這樣會重設用於 HTTPS 連線的 SSL 憑證快取。 如果您遇到 TLS 連線錯誤，且無法略過 (與預期的憑證警告不同)，此選項可修正問題。 
...
- `-Debug <various options for authentication, port selection, and tracing level>`
    - 執行具有特定組態並且會顯示偵錯訊息的單機版裝置入口網站。 這用來建置[已封裝的外掛程式](https://docs.microsoft.com/windows/uwp/debug-test-perf/device-portal-plugin)最實用。 
    - 如需如何以 System 身分執行此操作以完整測試已封裝外掛程式的詳細資料，請參閱 [MSDN Magazine 文章](https://msdn.microsoft.com/magazine/mt826332.aspx)。
