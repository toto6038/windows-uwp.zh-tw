...
2. 查閱您的裝置 IP 位址。 在裝置上找到的 IP 位址**設定 > 網路和網際網路 > Wi-fi > 硬體屬性**。 您也可以詢問：「嗨 Cortana，我的 IP 位址是什麼？」

3. 從您的電腦上網頁瀏覽器，移至 `https://<YOUR_HOLOLENS_IP_ADDRESS>`
...
> [!IMPORTANT]
> 如果您的電腦找不到裝置，請嘗試使用實際的網路 IP 位址的 HoloLens 裝置，而非`http://127.0.0.1:10080`。
...
2. 從 [安全性] 頁面上裝置入口網站-按一下連結圖示上方右側清單中的安全性，或瀏覽至下載此裝置的憑證： `https://<YOUR_HOLOLENS_IP_ADDRESS>/devicesecurity.htm`

3. 安裝的憑證。-從 [Windows] 功能表的 [型別，在 「 受信任的根憑證授權單位] 存放區：管理電腦憑證，並啟動小程式。
    - 展開 [可信任的根憑證授權單位] 資料夾。
    - 按一下 [憑證] 資料夾。
...
- **平移**：按下滑鼠右鍵 + 移動滑鼠；
- **縮放**：滑鼠滾輪。
- **追蹤選項**:藉由檢查強制 visual 追蹤開啟連續 visual 的追蹤。 暫停將會停止視覺追蹤。
- **檢視選項**:在 3D 檢視上設定選項:-追蹤：表示 visual 追蹤是否為作用中。
- **顯示 floor**:會顯示在棋盤式的 floor 平面。
- **顯示範圍**:顯示檢視範圍。
- **顯示穩定平面**:顯示 HoloLens 使用穩定影片的平面。
- **顯示網狀結構**:顯示介面對應網狀結構，表示周圍環境。
- **顯示詳細資料**:即時變更時，顯示會送位置 」、 「 前端旋轉的四元數和 「 裝置來源向量。
- **全螢幕按鈕**:以全螢幕模式顯示 3D 檢視。 按下 ESC 以離開全螢幕檢視。

- 介面的重新建構：按一下或點選 顯示最新的空間對應網狀結構，從裝置的更新。 可能需要花費一些時間 (最多幾秒鐘) 才能完成完整作業。 網格並不會在 3D 檢視中自動更新，您必須手動按一下 [更新] 以取得來自裝置的最新網格。 按一下 [儲存] 以將目前的空間對應網格在您的電腦上儲存為 obj 檔案。

### <a name="mixed-reality-capture"></a>混合實境擷取 

使用 [混合實境擷取] 頁面來儲存來自 HoloLens 的媒體串流。
- 設定：控制的媒體資料流，會擷取藉由檢查下列設定:-全像投影：擷取視訊資料流中全像攝影版的內容。 全像投影是以單聲道進行轉譯，而非立體聲。
...
允許您記錄並播放輸入資料以進行測試。
- **擷取聊天室**:用來下載包含如使用者的恍神空間對應網狀結構的模擬的空間檔案。 為房間命名，然後按一下 [擷取] 來將資料在您的電腦上儲存為 .xef 檔案。 此房間檔案可以載入 HoloLens 模擬器。
- **錄製**:請檢查記錄，名稱記錄，並按一下或點選 開始錄製的記錄資料流。 使用您的 HoloLens 執行動作，然後按一下 [停止] 來將資料在您的電腦上儲存為 .xef 檔案。 此檔案可以載入 HoloLens 模擬器或裝置。
- **播放**:按一下或點選 上傳記錄從您的電腦選取 xef 檔案，並將資料傳送至 HoloLens。
- **控制項模式**:從下拉式清單中，選取 預設 或 模擬並按一下或點選 設定 按鈕，選取 HoloLens 上的模式。 選擇 [模擬] 將會停用 HoloLens 上實際的感應器，並改為使用已上傳的模擬資料。 如果您切換到 [模擬]，您的 HoloLens 將不會對真實使用者做出回應，直到您切換回 [預設] 為止。
