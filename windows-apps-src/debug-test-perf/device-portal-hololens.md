...
2. 從 Device Portal 上的 [安全性] 頁面下載此裝置的憑證。您可以按一下右上方圖式清單中的 [安全性] 連結，或是瀏覽到：`https://<YOUR_HOLOLENS_IP_ADDRESS>/devicesecurity.htm`
...
- **錄製**：選取要錄製的串流、為錄製命名，然後按一下或點選 [錄製] 來開始錄製。 使用您的 HoloLens 執行動作，然後按一下 [停止] 來將資料在您的電腦上儲存為 .xef 檔案。 此檔案可以載入 HoloLens 模擬器或裝置。
