...
如需詳細資訊，請參閱[在模擬器中執行 Windows 市集應用程式](https://msdn.microsoft.com/en-us/library/hh441475.aspx)。
...
4. 在 Visual Studio 模擬器中執行應用程式。 按一下調色盤上的 [變更解析度]**** 按鈕，然後從清單中選取 Surface Hub 設定。
   ...
   > **提示**&nbsp;&nbsp;[開啟平板電腦模式](http://windows.microsoft.com/en-us/windows-10/getstarted-like-a-tablet)可模擬更貼近 Surface Hub 上的體驗。
...
如需詳細資訊，請參閱[部署和偵錯通用 Windows 平台 (UWP) 應用程式](https://msdn.microsoft.com/en-us/windows/uwp/debug-test-perf/deploying-and-debugging-uwp-apps)。
