...
-   [模擬器] 目標會開機，並以名稱中指定的設定將 App 部署到模擬器。 模擬器僅在執行 Windows 8.1 或更新版本且支援 Hyper-V 的電腦上才有提供。
...
UWP app 可在 Windows 8.1 或更新版本上開發及編譯，但需要 Windows 10 才能執行。 如果您在 Windows 8.1 電腦上開發 UWP app，而假設主機和目標電腦在相同的 LAN 上，您就可以遠端偵錯在另一部 Windows 10 裝置上執行的 UWP app。 若要這樣做，請在兩部電腦上都下載並安裝 [Visual Studio 遠端工具](http://aka.ms/remotedebugger)。 安裝的版本必須符合您已安裝的現有 Visual Studio 版本，且所選取的架構 (x86、x64) 也必須與您目標 App 的架構相符。   
  
## 封裝配置
在 Visual Studio 2015 Update 3 中，我們新增了可供開發人員指定其 UWP app 配置路徑的選項。 這會決定當您建置 App 時，要將封裝配置複製到磁碟上的哪個位置。 這個屬性預設會設定為專案根目錄的相對位置。 如果您沒有修改這個屬性，行為將會維持與它在舊版 Visual Studio 時相同。
...
若要將它變更為 [非驗證]，請在「遠端偵錯工具」 中，移至 [工具] -&gt; [選項]，然後將它設定為 [非驗證]。 設定遠端偵錯工具之後，您也必須確定已將主機裝置設定為 [開發人員模式](https://msdn.microsoft.com/windows/uwp/get-started/enable-your-device-for-development)。 之後，您便可以從您的開發電腦進行部署。

如需詳細資訊，請參閱 [Visual Studio 遠端工具](http://go.microsoft.com/fwlink/p/?LinkId=717039)下載頁面。
...
當您第一次從網路登錄配置時，系統會將您的認證快取在目標裝置上，讓您不需要重複地登入。 若要移除已快取的認證，您可以使用來自 Windows 10 SDK 的 [WinAppDeployCmd.exe 工具](https://msdn.microsoft.com/windows/uwp/packaging/install-universal-windows-apps-with-the-winappdeploycmd-tool) 搭配 **deletecreds** 命令。 
...
在 Windows 10 上，藉由使用稱為[預先啟動](https://msdn.microsoft.com/library/windows/apps/Mt593297)的技術，先主動啟動 App 再予以暫停，讓 UWP app 的啟動效能獲得提升。 許多 App 將不需要特別執行任何動作即可在此模式下工作，但某些 App 可能需要調整它們的行為。 若要協助針對這些程式碼路徑中的任何問題進行偵錯，您可以在預先啟動模式下，從 Visual Studio 開始針對 App 進行偵錯。 
...
- **允許區域網路回送**
  ...
  基於安全性考量，系統不允許以標準方式安裝的 UWP app 對其安裝所在的裝置進行網路呼叫。 根據預設，Visual Studio 部署會從這個已部署的應用程式規則建立免套用原則。 這個免套用原則可讓您在單一電腦上測試通訊程序。 將應用程式送出到 Windows 市集之前，您應該在沒有免套用原則的情況下測試您的應用程式。
...
您可以利用 `‘;’` 分隔符號，或使用 `.sympath+` 命令來新增更多路徑。 如需了解其他使用 WinDbg 的進階符號作業，請參閱[公用與專用符號](https://msdn.microsoft.com/library/windows/hardware/ff553493)。
