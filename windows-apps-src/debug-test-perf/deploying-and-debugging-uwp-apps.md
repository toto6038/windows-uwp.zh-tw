...
Microsoft Visual Studio 可讓您部署和偵錯您的通用 Windows 平台 (UWP) 應用程式，在各種不同的 windows 10 裝置上。 Visual Studio 會處理在目標裝置上建立及註冊 App 的處理程序。
...
截至 Visual Studio 2015 Update 3，我們新增了適用於開發人員指定其 UWP app 的版面配置路徑的選項。 這會決定當您建置 App 時，要將封裝配置複製到磁碟上的哪個位置。 這個屬性預設會設定為專案根目錄的相對位置。 如果您沒有修改這個屬性，行為將會維持與它在舊版 Visual Studio 時相同。
...
若要返回這個對話方塊，您可以開啟專案屬性，然後移至 [**偵錯**] 索引標籤。從該處選取**尋找**旁邊**遠端電腦：**
...
若要指定在 c + + 或 JavaScriptUWP 應用程式的遠端電腦目標：
...
- **允許區域網路回送**
  ...
  基於安全性考量，系統不允許以標準方式安裝的 UWP app 對其安裝所在的裝置進行網路呼叫。 根據預設，Visual Studio 部署會從這個已部署的應用程式規則建立免套用原則。 這個免套用原則可讓您在單一電腦上測試通訊程序。 之前提交到 Microsoft Store 應用程式，您應該測試您的應用程式沒有免套用原則。

  從 App 移除網路回送豁免：

  -   在 C# 和 Visual Basic**偵錯**] 屬性頁面上，清除 [**允許區域網路回送**] 核取方塊。
...
- **不啟動，但在我的程式碼啟動時進行偵錯 / 啟動應用程式**
  ...
  -   在 C# 和 Visual Basic**偵錯**] 屬性頁面上，選取 [**不啟動，但在我的程式碼啟動時進行偵錯**] 核取方塊。
