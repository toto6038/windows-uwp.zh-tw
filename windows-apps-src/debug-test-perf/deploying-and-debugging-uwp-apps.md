...
Microsoft Visual Studio 可讓您在各種 Windows 10 裝置上部署和偵測您的通用 Windows 平臺（UWP）應用程式。 Visual Studio 會處理在目標裝置上建立及註冊 App 的處理程序。
...
- **\[模擬器\]** 會將 App 部署到您目前開發電腦上的模擬環境。 只有當您 App 的 **「目標平台最小版本」** 小於或等於您開發電腦上的作業系統時，才能使用此選項。
...
- **\[模擬器\]** 目標會開機，並以名稱中指定的設定將 App 部署到模擬器。 只有在已啟用 Hyper-v 的電腦上執行 Windows 8.1 或以上版本時，才可使用模擬器。
...
從 Visual Studio 2015 Update 3，我們為開發人員新增了選項來指定 UWP 應用程式的版面配置路徑。 這會決定當您建置 App 時，要將封裝配置複製到磁碟上的哪個位置。 這個屬性預設會設定為專案根目錄的相對位置。 如果您沒有修改這個屬性，行為將會維持與它在舊版 Visual Studio 時相同。
...
若要返回此對話方塊，您可以開啟專案屬性，並移至 [**調試**] 索引標籤。從該處選取 [**遠端電腦**] 旁的 [**尋找**]：
...
若要將它變更為 **無驗證**，請在**遠端偵錯程式**中，移至 **工具** -&gt;**選項**，然後將它設定為 **無驗證**。 設定遠端偵錯工具之後，您也必須確定已將主機裝置設定為 [開發人員模式](https://docs.microsoft.com/windows/uwp/get-started/enable-your-device-for-development)。 之後，您便可以從您的開發電腦進行部署。
...
從 Visual Studio 2015 Update 3 的版本和 Windows 10 年度更新版中，某些 Windows 10 裝置有新的先進遠端部署選項。 您可以在專案屬性的 **\[偵錯\]** 功能表上找到進階遠端部署選項。
...
在遠端裝置上，版面配置會複製到下列預設位置： `\\MY-DEVKIT\DevelopmentFiles\PACKAGE-REGISTRATION-PATH`
...
在遠端裝置上，版面配置會根據裝置系列註冊到下列預設位置： `Xbox: \\MY-DEVKIT\DevelopmentFiles\XrfsFiles`-這是**封裝註冊路徑**的符號，電腦不會使用符號連結，而是直接註冊**套件註冊路徑**

## <a name="debugging-options"></a>偵錯選項

在 Windows 10 上，UWP 應用程式的啟動效能已藉由主動啟動，然後[以稱為預先](https://docs.microsoft.com/windows/uwp/launch-resume/handle-app-prelaunch)啟動的技術暫停應用程式來改善。 許多 App 將不需要特別執行任何動作即可在此模式下工作，但某些 App 可能需要調整它們的行為。 若要協助針對這些程式碼路徑中的任何問題進行偵錯，您可以在預先啟動模式下，從 Visual Studio 開始針對 App 進行偵錯。

從 Visual Studio 專案（**debug** -&gt;**其他 debug 目標** -&gt; **debug 通用 Windows 應用程式**預先啟動），以及已安裝在電腦上的應用程式（**debug** -&gt;**其他 Debug 目標** -&gt; **debug 已安裝的應用程式套件**，請選取 [**使用預先啟動應用**程式] 核取方塊），都支援進行調試。 如需詳細資訊，請參閱[偵錯 UWP 預先啟動](https://blogs.msdn.com/b/visualstudioalm/archive/2015/11/30/debug-uwp-prelaunch-with-vs2015.aspx)。

您可以在啟始專案的 **\[偵錯\]** 屬性頁面上，設定下列部署選項：

- **允許區域網路回送**

  基於安全性考量，系統不允許以標準方式安裝的 UWP app 對其安裝所在的裝置進行網路呼叫。 根據預設，Visual Studio 部署會從這個已部署的應用程式規則建立免套用原則。 這個免套用原則可讓您在單一電腦上測試通訊程序。 將您的應用程式提交至 Microsoft Store 之前，您應該先測試您的應用程式而不豁免。

  從 App 移除網路回送豁免：

  - 在C#和 Visual Basic **Debug**  屬性頁上，清除 **允許區域網路回送** 核取方塊。
  - 在 JavaScript 和 C++ 的 **\[偵錯\]** 屬性頁面上，將 **\[允許區域網路回送\]** 值設定為 **\[否\]** 。

- **不啟動，但在我的程式碼啟動/啟動應用程式時加以調試**

  將部署設定成在啟動 App 時自動啟動偵錯工作階段：

  - 在C#和 Visual Basic **Debug**  屬性頁上，選取 **不啟動，但在我的程式碼啟動時進行調試** 核取方塊。
