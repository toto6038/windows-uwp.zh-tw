---
彈性磚範本是 Windows 10 中的新功能，可讓您使用能夠適應不同螢幕密度的簡易靈活標記語言，設計專屬的磚通知內容。
建立彈性磚
...
(設計適用於 Windows 10 的通知時若有需要，您仍然可以使用 [Windows 8 磚範本資料目錄](https://msdn.microsoft.com/library/windows/apps/hh761491)中的預設範本。)
...
**安裝通知視覺化工具。** 這個免費的 UWP 應用程式可協助您透過在編輯磚時提供即時視覺預覽 (類似 Visual Studio 的 XAML 編輯器/設計檢視)，以設計彈性動態磚。 您可以讀取[此部落格文章](http://blogs.msdn.com/b/tiles_and_toasts/archive/2015/09/22/introducing-notifications-visualizer-for-windows-10.aspx)以取得詳細資訊，而且您可以在[這裡](https://www.microsoft.com/store/apps/notifications-visualizer/9nblggh5xsl1)下載通知視覺化工具。
...
每個磚大小的內容，會分別在 XML 承載內的不同 [&lt;binding&gt;](tiles-and-notifications-adaptive-tiles-schema.md) 元素中指定。 將範本屬性設定為下列其中一個值，以選擇目標大小：
...
針對單一磚通知 XML 承載，為您想要支援的每個磚大小提供 &lt;binding&gt; 元素，如此範例中所示：
...
您可以使用通知承載的品牌屬性，控制動態磚底部的品牌 (顯示名稱和角標誌)。 您可以選擇要顯示 "none"、僅顯示 "name"、僅顯示 "logo"，或使用 "nameAndLogo" 顯示兩者。
...
可以針對特定磚大小，以下列其中一種方式套用品牌：

1. 透過套用 [&lt;binding&gt;](tiles-and-notifications-adaptive-tiles-schema.md) 元素的屬性
2. 透過套用 [&lt;visual&gt;](tiles-and-notifications-adaptive-tiles-schema.md) 元素的屬性，這會影響整個通知承載
如果您沒有指定繫結的品牌，將使用針對視覺元素提供的品牌。
...
**注意：**這是來自 Windows 8.x 的變更，其預設品牌為 "logo"。
...
您可以輸入您使用 **displayName** 屬性選擇的文字字串，來覆寫通知的顯示名稱。 針對品牌，您可以為 [&lt;visual&gt;](tiles-and-notifications-adaptive-tiles-schema.md) 元素指定此顯示名稱 (這會影響整個通知承載)，或是針對 [&lt;binding&gt;](tiles-and-notifications-adaptive-tiles-schema.md) 元素指定此顯示名稱 (這只會影響個別的磚)。
...
[&lt;text&gt;](tiles-and-notifications-adaptive-tiles-schema.md) 元素用於顯示文字。 您可以使用此提示修改文字的顯示方式。
...
&lt;image&gt; 元素用於顯示磚通知上的影像。 影像可以內嵌於磚內容內 (預設值)、當做內容後方的背景影像，或當做以動畫方式從通知上方進入的預覽影像。
...
放置在 &lt;binding&gt; 根目錄或第一個群組中的影像也會伸展，以填滿可用的高度。
...
![提示移除邊界範例](images/adaptive-tiles-removemargin.png)

### <span id="Image_cropping"> </span> <span id="image_cropping"> </span> <span id="IMAGE_CROPPING"> </span>影像裁剪

使用 **hint-crop** 屬性可將影像裁剪成圓形，此屬性目前僅支援 "none" (預設) 或 "circle" 這些值。
...
若要設定背景影像，請將影像元素放在 &lt;binding&gt; 的根目錄中，並將 placement 屬性設定為 "background"。
...
您可以指定從磚上方「預覽」的影像。 預覽影像使用動畫，從磚的上方向下/向上滑動以進入檢視，之後再向後滑出，以顯示磚上的主要內容。 若要設定預覽影像，請將影像元素放在&lt;binding&gt; 的根目錄中，並將 placement 屬性設定為 "peek"。
...
您可以在預覽影像上使用 **hint-overlay** 增加不透明度，並讓磚的顯示名稱更為清楚。 若您在 &lt;binding&gt; 元素上指定 **hint-overlay**，則會在背景和預覽影像上套用重疊。
...
您也可以使用 [&lt;binding&gt;](tiles-and-notifications-adaptive-tiles-schema.md) 元素和 [&lt;subgroup&gt;](tiles-and-notifications-adaptive-tiles-schema.md) 元素上的 **hint-textStacking** 屬性，控制內容在磚上的垂直對齊方式。 根據預設，所有內容都會垂直靠上對齊，但是您也可以將內容靠下對齊或置中對齊。

### <span id="Text_stacking_on_binding_element"> </span> <span id="text_stacking_on_binding_element"> </span> <span id="TEXT_STACKING_ON_BINDING_ELEMENT"> </span>繫結元素上的文字堆疊
...
![繫結元素上的文字堆疊](images/adaptive-tiles-textstack-bindingelement.png)
