---
想想我們每天多常閱讀電子郵件、書籍、路標、菜單價格、胎壓標記，或是街道柱子上張貼海報中的文字。
文字控制項
...
- 輸入和編輯文字的控制項是：[TextBox](text-block.md)、[AutoSuggestBox](auto-suggest-box.md)、[PasswordBox](password-box.md) 和 [RichEditBox](rich-edit-box.md)。 
...
使用 **PasswordBox** 控制項，以收集密碼或其他私人資料，例如身分證號碼。 密碼方塊是一個文字輸入方塊，其中隱藏基於隱私權考量而輸入的字元。 密碼方塊看起來就像文字輸入方塊，只不過它會將已輸入的文字轉譯成項目符號。 您可以自訂項目符號字元。
...
您可運用眾多方式，取得使用者在您應用程式中的輸入內容。 下列問題有助於瞭解標準文字輸入方塊或另一個控制項何者最適合讓使用者輸入文字。

-   **有效率地列舉所有的有效值是否實際？**如果是，請考慮使用其中一個選取控制項，如[核取方塊](checkbox.md)、[下拉式清單](lists.md)、清單方塊、[選項按鈕](radio-button.md)、[滑桿](slider.md)、[切換開關](toggles.md)、[日期選擇器](date-and-time.md)或時間選擇器。
-   **是否有相對小的一組有效值？**如果是，請考慮使用[下拉式清單](lists.md)或清單方塊，特別是如果這些值只有幾個字元的長度。
-   **有效資料是否完全不受限制？有效資料是否只受格式限制 (限制的長度或字元類型)？**如果是，請使用文字輸入控制項。 您可以限制能輸入的字元數，此外還可驗證應用程式程式碼中的格式。
-   **值是否代表包含特殊通用控制項的資料類型？**如果是，請使用適當的控制項，而不要使用文字輸入控制項。 例如，使用 [**DatePicker**](https://msdn.microsoft.com/library/windows/apps/br211681) 接受日期輸入，不要使用文字輸入控制項。
-   如果資料只能是數值：
    -   **輸入的值是否是大約值和 (或) 相對於相同頁面上的其他數量？**如果是，請使用[滑桿](slider.md)。
    -   **使用者需要設定變更效果的立即回應嗎？**如果是，請使用[滑桿](slider.md)，也可以配合控制項一起使用。
...
您的應用程式可以在文字段落之間插入段落分隔字元。 使用此分隔字元可讓您建立純文字檔案，以在不同作業系統上使用各種不同的行寬度進行格式化。 目標系統僅會忽略位於段落分隔字元的任何行分隔字元與分段。
