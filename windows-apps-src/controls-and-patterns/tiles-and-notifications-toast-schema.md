...
如果您想要使用原始 XML，而不使用 [Notifications 程式庫](https://www.nuget.org/packages/Microsoft.Toolkit.Uwp.Notifications/)，請參閱 [XML 結構描述](tiles-and-notifications-toast-xml-schema.md)。
...
| 屬性 | 類型 | 必要 |描述 |
|---|---|---|---|
| **Text** | string 或 BindableString | false | 要顯示的文字。 Creators Update 新增的資料繫結支援，但僅適用於最上層文字元素。 |
...
| 屬性 | 類型 | 必要 | 描述 |
|---|---|---|---|
| **url** | uri | false | 要取代預設音效播放的媒體檔案。 僅支援 ms-appx 和 ms-appdata。 |
...
| **title** | 字串 | true | 標頭的標題。 |
| **action**| Action | false | 宣告使用者按一下快顯通知時要執行什麼動作。 |
