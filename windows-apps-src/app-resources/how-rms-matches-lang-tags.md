...
評分機制使用包含在 [BCP-47](https://go.microsoft.com/fwlink/p/?linkid=227302) 子標記登錄中的資料及其他資料來源。 該機制允許使用不同品質相符項目的評分梯度，當提供多個候選項目時，會選取分數最符合的候選項目。
...
語言和字集子標記的標記相符，但是它們在地區或某些其他子標記又不相同。 例如，EN-US 符合 en，或符合 EN-US en-\*。

#### <a name="macro-region-match"></a>宏觀地區相符

語言和字集子標記的標記相符；兩個標記都有地區子標記，其中一個代表包含其他地區的宏觀地區。 宏觀地區子標記一律都是數值，並且衍生自聯合國統計司 M.49 國家和區域代碼。 如需包含關係的詳細資訊，請參閱[宏觀地理 (大陸) 地區、地理子地區和所選取經濟和其他群組的組合](https://go.microsoft.com/fwlink/p/?LinkId=247929)。
...
- 資源：「en-US」、「pt-BR」。
- 較高分數的資源：「en-US」。
- 描述：比較 「 PT-PT"為開頭，但找不到完全相符。 因為使用者的語言清單中存在「pt-BR」，所以部分相符延遲到與「pt-BR」的比較。 下一個語言比較是具有完全符合的「en-US」。 因此，獲勝資源是「en-US」。
...
- 資源：「zh-Hans-CN」、「und」。
- 較高分數的資源：「und」。
- 描述：比較開頭為"EN-US"，但找不到相符項目"en"（部分或更好） 為基礎。 因為有標記「und」的資源，所以比對演算法會使用它。
...
- 資源：「zh-Hans-CN」、「und」。
- 較高分數的資源：「zh-Hans-CN」。
- 描述：比較會尋找完全相符的第一個項目，因此它並不會檢查標示為"und"的資源。
...
- 較高分數的資源：「und-Cyrl」。
- 描述：比較找不到 「 ru"的相符項目 （部分或更高），並因此比對語言標記"und"。 與「ru」關聯的隱藏字集值「Cyrl」與資源「und-Cyrl」相符。
...
進行全球化應用程式時，這些語言，與重大的區域性變化會呈現一些難題有待克服：「 應該支援幾個不同的區域變化？ 」 「哪幾個？」 「管理我的 App 的這些地區變體資產最具成本效益方式為何？」 如要回答所有這些問題，這已超出本主題的範圍。 不過，在 Windows 中的語言比對機制，確實提供可協助您處理地區變體的功能，。
...
英文需要特殊考量。 如果 App 新增兩個英文種類的當地語系化，這些可能會是美式英文和英國，或「國際」英文。 如前所述，美國以外特定地區遵照美國拼字規則，並且 Windows 語言比對會將其納入考量。 在本案例中，不建議對其中一個變體使用地區中立標記「en」；請改用「en-GB」和「en-US」。 （如果指定的資源不需要個別的變體，不過，"en"可用。）如果"en"取代"EN-GB"或"EN-US"，然後，將會干擾正視維繫同地區關係 Windows 所提供。 如果新增第三個英文當地語系化，則依需要為其他變體使用特定或宏觀地區子標記 (例如「en-CA」、「en-AU」或「en-053」），但是繼續使用「en-GB」和「en-US」。
