...
#### <a name="phase-3-build-resource-packs"></a>第 3 階段：組建資源套件

本節概述將您的資源分成不同的*資源套件*所需的最後變更，將下載 (和安裝) 的應用程式的大小縮到最小。

### <a name="not-covered-in-this-document"></a>未涵蓋於本文件中

完成階段 0-3 以上之後, 您會有應用程式 「 套件組合 」，可以提交給 Microsoft Store，而且會減少下載並安裝大小，讓使用者藉由略過不需要的資源 （例如，語言就不會說）。 採取最後一個步驟以進一步改善應用程式的大小和功能。

#### <a name="phase-4-migrate-to-mrt-resource-formats-and-apis"></a>第 4 階段：移轉至 MRT 資源格式與 API
...
* 如果您有複雜的安裝程式的大型桌面應用程式，或利用許多作業系統擴充點，您可以使用 Desktop App Converter 工具產生 UWP 檔案配置，並從您現有的應用程式安裝程式 (例如，MSI) 資訊清單資訊。
* 如果您有較小的桌面應用程式使用相對較少的檔案或簡單的安裝程式和任何擴充性攔截程序，您可以建立檔案配置，並以手動方式資訊清單資訊。
* 如果您從來源重建，並想要更新您的應用程式是純粹的 UWP 應用程式，您可以在 Visual Studio 中建立新的專案，並依賴 IDE，為您執行大部分的工作。

如果您想要使用[Desktop App Converter](https://aka.ms/converter)，請參閱[封裝傳統型應用程式使用 Desktop App Converter](https://aka.ms/converterdocs)如需有關轉換程序。 一組完整的桌面轉換器範例可於[傳統型橋接器至 UWP 範例 GitHub 存放庫](https://github.com/Microsoft/DesktopBridgeToUWP-Samples)。

如果您想要以手動方式建立封裝，您必須建立包含所有應用程式的檔案 （可執行檔和內容，但不是原始碼） 和封裝資訊清單檔案 (.appxmanifest) 的目錄結構。 中可以找到範例[Hello，World GitHub 範例](https://github.com/Microsoft/DesktopBridgeToUWP-Samples/blob/master/Samples/HelloWorldSample/CentennialPackage/AppxManifest.xml)，但執行名為 desktop 可執行檔的基本封裝資訊清單檔案`ContosoDemo.exe`如下所示，其中<span style="background-color: yellow">反白顯示文字</span>就是取代為您自己的值。
...
### <a name="step-11-update-strings--assets-in-the-manifest"></a>步驟 1.1:更新字串和資訊清單中的資產

您可以在階段 0 中建立基本封裝資訊清單 (.appxmanifest) 檔案來 （根據值提供給轉換器、 擷取自 MSI，或手動輸入資訊清單） 的應用程式，但它不會包含當地語系化的資訊，也會支援資產等，並排顯示額外的功能，例如高解析度的開始。

若要確保正確當地語系化您的應用程式名稱和描述，您必須定義一組資源檔案中的一些資源，並且更新封裝資訊清單參考它們。
...
1. 建立名為 `resources.resw` 的 XML 檔，並將它放置在您專案的 `Strings\en-us` 子資料夾中。 如果您的預設語言不是英文 （美國），請使用適當的 BCP-47 程式碼。
2. 在 XML 檔案中加入下列內容，其中<span style="background-color: yellow">醒目提示文字</span>部分會以您的預設語言的適當 App 文字取代。

> [!NOTE]
> 有一些這些字串的長度限制。 如需詳細資訊，請參閱 [VisualElements](/uwp/schemas/appxpackage/appxmanifestschema/element-visualelements)。
...
1. 建立`Strings\en-us`資料夾 （或其他語言適當地） 在您的專案，並新增**新的項目**到您的專案根資料夾，使用的預設名稱`resources.resw`。 請務必選擇**資源檔 (.resw)** 而非**資源字典**-資源字典是 XAML 應用程式所使用的檔案。
2. 使用設計工具，輸入下列字串 (使用相同 `Names` 但將 `Values` 取代為應用程式的適當文字)：

<img src="images\editing-resources-resw.png"/>

> [!NOTE]
> 如果您開始使用 Visual Studio 設計工具，您隨時都可以編輯 XML 直接藉由按下`F7`。 但是，如果您以最小的 XML 檔案啟動，*設計工具將無法辨識檔案*，因為檔案遺失許多額外的中繼資料。若要修正此問題，只需將重複使用的 XSD 資訊從設計工具產生的檔案，複製到您手工編輯的 XML 檔案即可。

#### <a name="update-the-manifest-to-reference-the-resources"></a>更新資訊清單以參考資源

中定義的值之後`.resw`檔案中下, 一個步驟是更新的資訊清單，以參考的資源字串。 您可以再次直接編輯 XML 檔案，或依賴 Visual Studio 資訊清單設計工具。
...
如果您使用 Visual Studio 資訊清單設計工具，開啟.appxmanifest 檔案，並將變更<span style="background-color: lightgreen">反白顯示的值</span>中的值 **應用程式*索引標籤和*封裝* 索引標籤：

<img src="images\editing-application-info.png"/>
<img src="images\editing-packaging-info.png"/>

### <a name="step-12-build-pri-file-make-an-msix-package-and-verify-its-working"></a>步驟 1.2:建置 PRI 檔案，讓 MSIX 的套件，並確認它是否運作
...
如果您要手動建置，請遵循下列步驟來建立的組態檔`MakePRI`工具，並產生`.pri`檔案本身 (中可以找到詳細資訊[手動應用程式封裝](https://docs.microsoft.com/en-us/windows/uwp/packaging/manual-packaging-root)):

1. 開啟開發人員命令提示字元**Visual Studio 2017**或是**Visual Studio 2019** [開始] 功能表中的資料夾。
2. 切換至專案根目錄 (包含.appxmanifest 檔案，**字串**資料夾)。
3. 輸入下列命令，使用您專案適用的名稱取代 "contoso_demo.xml"，用您的應用程式適用的預設語言取代 "en-US" (如果適用則保留 en-US)。 請注意 XML 檔案會建立父目錄中 (**不**專案目錄中) 因為它不是 （您可以選擇任何其他目錄，但務必替換成在未來的命令） 的應用程式的一部分。
...
您可以開啟對應檔案`..\resources.map.txt`來確認它包含您的專案 （包含 PRI 檔案，這不是專案的目錄的一部分） 所需的檔案。 重要的是，對應檔案將*不*包含 `resources.resw` 檔案的參考，因為該檔案的內容已經嵌入 PRI 檔案。 但是它將包含像是映像檔名之類的其他資源。

#### <a name="building-and-signing-the-package"></a>建置並登入套件 

現在建置了 PRI 檔案，接著您可以建置並登入套件︰

1. 若要建立的應用程式套件，執行下列命令取代`contoso_demo.appx`MSIX/應用程式的名稱取代檔案您想要建立，並確定選擇不同的目錄檔案 （此範例會使用父目錄; 它可以是任何位置但應該**不**是專案目錄)。
...
2. 建立封裝之後，它必須經過簽署。 若要取得簽章憑證最簡單的方式是在 Visual Studio 中建立的空白通用 Windows 專案和複製`.pfx`它所建立的檔案，但您可以建立一個使用手動`MakeCert`和`Pvk2Pfx`中所述的公用程式[如何建立應用程式套件簽署憑證](https://docs.microsoft.com/windows/desktop/appxpkg/how-to-create-a-package-signing-certificate)。

    > [!IMPORTANT]
    > 如果您以手動方式建立簽署的憑證，請確定將檔案放在不同的目錄中的比原始碼專案或封裝來源，否則可能會收到包含的封裝，包括私密金鑰過程 ！
...
1. 系統管理員身分執行 Visual Studio 2017 或 Visual Studio 2019 的命令提示字元。
...
3. 選擇`Local Machine`按一下 `Next`
4. 接受使用者帳戶控制系統管理員提高權限提示字元中，若出現，然後按一下 `Next`
5. 如果有的話，輸入為私用金鑰的密碼，然後按一下 `Next`
...
封裝資訊清單和初始進行完變更之後`resources.resw`檔案建立之後，新增其他語言很簡單。
...
#### <a name="update-the-package-manifest-to-list-supported-languages"></a>更新封裝資訊清單，列出支援的語言

封裝資訊清單必須更新為支援的語言清單，由應用程式。 Desktop App Converter 會新增預設的語言，但其他的語言則必須明確地新增。 如果直接編輯 `AppxManifest.xml` 檔案，請依下列方式更新 `Resources`，依您需要的數量新增元素，並且替換您支援的<span style="background-color: yellow">適當語言</span>，確保清單中的第一個項目為預設 (遞補) 語言。 在此範例中，預設值為英文 (美國)，同時也支援德文 (德國) 和法文 (法國)︰
...
如果您使用的是 Visual Studio，您應不需要做任何動作。如果查看 `Package.appxmanifest`，您應該會看到特殊的 <span style="background-color: yellow">x 產生</span>值，致使組建程序插入在您專案中找到的語言 (根據 BCP-47 代碼命名的資料夾)。 請注意，這不是有效的值，如實際封裝資訊清單;它僅適用於 Visual Studio 專案：
...
### <a name="step-14-localizing-more-parts-of-the-package-manifest-optional"></a>步驟 1.4:當地語系化的套件資訊清單 （選用） 的多個組件

您可以當地語系化封裝資訊清單中的其他章節。 舉例來說，如果您的應用程式處理檔案副檔名，資訊清單中就應具有 `windows.fileTypeAssociation` 副檔名，方法是完全依照所示來使用<span style="background-color: lightgreen">綠色反白文字</span> (因為它會參照資源)，然後將<span style="background-color: yellow">黃色反白文字</span>取代為應用程式特定資訊︰
...
本文假設您所有的當地語系化的資源具有相同的檔案名稱 (例如，`contoso_demo.exe.mui`或`contoso_strings.dll`或是`contoso.strings.xml`)，但位於不同的資料夾，BCP-47 名稱 (`en-US`， `de-DE`，依此類推。)。 它並不重要，您有多少的資源檔、 其名稱為何、 哪些其檔案格式 / 相關聯的 Api，等等。重要的是，每個唯一*邏輯*資源具有相同的檔案名稱 (但放在不同*實體*目錄)。 
...
>[!NOTE]
> 您仍可使用 MRT 及優點的封裝，即使您無法遵循命名慣例，此檔案只不過是需要更多工作。
...
本文假設在某個時間點在程式碼中您想要找出包含當地語系化的資源的檔案，載入它，並使用它。 用於載入資源的 API、用來擷取資源的 API 等並不重要。 在虛擬程式碼中，有三個基本步驟︰
...
因為 .NET 具有尋找與載入資源 (稱為「衛星組件」) 的內建機制，在上述綜合範例中並無可取代的明確程式碼 - 在 .NET 中，您只需要適當目錄的資源 DLL，而且會自動幫您找出。 當應用程式會封裝成 MSIX 或使用資源組件的 AppX，目錄結構會稍有不同層，而非具有資源目錄是主應用程式目錄的子目錄時，這些都是它的對等 （或不存在在 all 使用者不具有語言列在他們的喜好設定）。 
...
> [!NOTE]
> 如果您的應用程式已經有`AssemblyResolve`處理常式用於其他用途，您必須整合您現有的程式碼中的資源解析程式碼。
...
* 取用現有的肥包，並透過[套件組合產生器工具](https://aka.ms/bundlegen)執行以自動建立資源套件。 如果您有一個已經產生肥包的組建系統，而且想要後置處理以產生資源套件時，這是優先選用的方法。
...
如果您使用 Visual Studio，請參閱[確保不論裝置是否需要，在裝置上安裝資源](https://docs.microsoft.com/en-us/previous-versions/dn482043(v=vs.140))如需如何建置到主要套件的所有語言，藉由建立檔案`priconfig.packaging.xml`和`priconfig.default.xml`。
...
4. 在建立封裝之後，請使用下列命令來建立的組合，使用適當的目錄和檔案名稱：

    ```CMD
    BundleGenerator.exe -Package ..\contoso_demo.appx -Destination ..\bundle -BundleName contoso_demo
    ```

現在您可以移至最後一個步驟中，簽章 （如下所示）。
