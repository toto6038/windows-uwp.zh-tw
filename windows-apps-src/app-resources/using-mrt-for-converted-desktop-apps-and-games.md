...
結合 MSIX 型或 .appx 型部署 (例如，從 Microsoft Store) ，MRT.LOG 可以自動為指定的使用者/裝置提供最適用的資源，以將應用程式的下載和安裝大小降到最低。 對於具有大量當地語系化內容的應用程式而言，縮減約莫數個 *GB* 的 AAA 遊戲大小有絕對的重要性。 其他 MRT 好處包括 Windows Shell 和 Microsoft Store 的當地語系化清單，當使用者慣用的語言不符合可用資源時的自動後援邏輯。
...
如果使用者說的是英文或德文以外的語言，或具有100% 或300% 以外的顯示器縮放比例，則根據一組回溯規則，MRT.LOG 會挑選「最接近」的符合候選項 (請參閱 [資源管理系統](/previous-versions/windows/apps/jj552947(v=win.10)) 以取得更多背景) 。
...
完成上述的階段0-3 之後，您將會有可提交至 Microsoft Store 的應用程式「套件組合」，並藉由省略不需要的資源 (例如，它們) 不需要的語言來將下載和安裝大小降到最低。 採取最後一個步驟以進一步改善應用程式的大小和功能。
...
變更您的應用程式資源之前，必須先將目前包裝和安裝技術更換為標準 UWP 封裝及部署技術。 有三種方式可以執行此動作：

* 如果您的桌面應用程式具有複雜的安裝程式，或是您利用許多 OS 擴充點，您可以使用 Desktop App Converter 工具，從現有的應用程式安裝程式產生 UWP 檔案配置和資訊清單資訊 (例如 MSI) 。
* 如果您的桌面應用程式較少，或是簡單的安裝程式，而且沒有擴充性勾點，您可以手動建立檔案配置和資訊清單資訊。
* 如果您要從來源進行重建，並且想要將應用程式更新為純 UWP 應用程式，您可以在 Visual Studio 中建立新的專案，並依賴 IDE 來為您執行大部分的工作。

如果您想要使用 [Desktop App Converter](https://www.microsoft.com/store/p/desktopappconverter/9nblggh4skzw)，請參閱 [使用 Desktop App Converter 封裝桌面應用程式](/windows/msix/desktop/desktop-to-uwp-run-desktop-app-converter) ，以取得轉換程式的詳細資訊。 [傳統型橋接器轉 UWP 範例GitHub 存放庫](https://github.com/Microsoft/DesktopBridgeToUWP-Samples)上可找到一組完整的 Desktop Converter 範例。

如果您想要手動建立套件，您必須建立一個目錄結構，其中包含應用程式的所有檔案 (可執行檔和內容，但不是原始程式碼) 和套件資訊清單檔 (. package.appxmanifest) 。 您可以在 [Hello，World GitHub 範例](https://github.com/Microsoft/DesktopBridgeToUWP-Samples/blob/master/Samples/HelloWorldSample/CentennialPackage/AppxManifest.xml)中找到範例，但執行名為的桌面可執行檔的基本套件資訊清單檔案如下所示 `ContosoDemo.exe` ，其中醒目提示的 <span style="background-color: yellow">文字</span> 會以您自己的值取代。
...
如需套件資訊清單檔和套件配置的詳細資訊，請參閱 [應用程式套件資訊清單](/uwp/schemas/appxpackage/appx-package-manifest)。

最後，如果您使用 Visual Studio 建立新的專案，並將現有的程式碼遷移至整個，請參閱 [建立 "Hello，world" 應用程式](../get-started/create-a-hello-world-app-xaml-universal.md)。 您可以將現有的程式碼包含在新的專案中，但您可能必須進行大量的程式碼變更 (特別是在使用者介面) 中，才能以純 UWP 應用程式的形式執行。 這些變更不在本文件範圍內。

## <a name="phase-1-localize-the-manifest"></a>階段1：將資訊清單當地語系化

### <a name="step-11-update-strings--assets-in-the-manifest"></a>步驟1.1：在資訊清單中更新 & 資產的字串

在階段0中，您已根據提供給轉換器的值、從 MSI 解壓縮或手動輸入指令) 清單中的值，為您的應用 (程式建立了基本的套件資訊清單 ( package.appxmanifest) 檔案，但它不會包含當地語系化的資訊，也不會支援高解析度開始磚資產等其他功能。

為了確保您的應用程式名稱和描述已正確當地語系化，您必須在一組資源檔中定義一些資源，並更新套件資訊清單以參考它們。
...
1. 建立名為 `resources.resw` 的 XML 檔，並將它放置在您專案的 `Strings\en-us` 子資料夾中。 如果您的預設語言不是美國英文，請使用適當的 BCP-47 程式碼。
2. 在 XML 檔案中加入下列內容，其中<span style="background-color: yellow">醒目提示文字</span>部分會以您的預設語言的適當 App 文字取代。

> [!NOTE]
> 其中一些字串的長度有一些限制。 如需詳細資訊，請參閱 [VisualElements](/uwp/schemas/appxpackage/appxmanifestschema/element-visualelements)。
...
在檔案中定義值之後 `.resw` ，下一步是更新資訊清單以參考資源字串。 您可以再次直接編輯 XML 檔案，或依賴 Visual Studio 資訊清單設計工具。
...
如果您是以手動方式建立，請遵循下列步驟來建立工具的設定檔 `MakePRI` ，並產生檔案 `.pri` 本身 (詳細資訊可在 [手動應用程式封裝](/windows/msix/package/manual-packaging-root)) 中找到：

1. 從 [開始] 功能表中的 **Visual Studio 2017** 或 **Visual Studio 2019** 資料夾開啟開發人員命令提示字元。
2. 切換至專案根目錄 (其中包含 package.appxmanifest 檔案和 [ **字串** ] 資料夾) 。
3. 輸入下列命令，使用您專案適用的名稱取代 "contoso_demo.xml"，用您的應用程式適用的預設語言取代 "en-US" (如果適用則保留 en-US)。 請注意，XML 檔案會建立在父目錄中， (**不** 在專案目錄中) ，因為它不是應用程式的一部分 (您可以選擇想要的任何其他目錄，但請務必在未來的命令) 中加以取代。
    ...
    您可以輸入 `makepri createconfig /?` 來查看每個參數運作狀況，但請摘要說明︰
      * `/cf` 設定此命令的輸出 (設定檔案名) 
      * `/dq` 設定預設的限定詞，在此案例中為語言 `en-US`
      * `/pv` 設定平臺版本，在此案例中 Windows 10
      * `/o` 設定它以覆寫輸出檔（如果有的話）
...
4. 現在您擁有設定檔，請再次執行 `MakePRI` 以確實搜尋磁碟中的資源，並封裝到 PRI 檔案。 將 "contoso_demop.xml" 取代為您在上一個步驟中使用的 XML 檔名，並確實為輸入與輸出檔指定上層目錄： 
    ...
    您可以輸入 `makepri new /?` 來查看每個參數運作狀況，但請簡單地說明︰
      * `/pr` 設定專案根目錄 (在此案例中為目前的目錄) 
      * `/cf` 設定在上一個步驟中建立的設定檔案名
      * `/of` 設定輸出檔 
      * `/mf` 建立對應檔 (因此我們可以在稍後的步驟中排除封裝中的檔案) 
      * `/o` 設定它以覆寫輸出檔（如果有的話）
...
您可以開啟對應檔， `..\resources.map.txt` 確認它包含專案 (所需的檔案，包括不屬於專案目錄) 一部分的 PRI 檔案。 重要的是，對應檔案將*不*包含 `resources.resw` 檔案的參考，因為該檔案的內容已經嵌入 PRI 檔案。 但是它將包含像是映像檔名之類的其他資源。

#### <a name="building-and-signing-the-package"></a>建置並登入套件 

現在建置了 PRI 檔案，接著您可以建置並登入套件︰

1. 若要建立應用程式套件，請執行下列命令， `contoso_demo.appx` 並以您要建立的 msix/.appx 檔案的名稱取代，並確定為檔案選擇不同的目錄 (此範例會使用上層目錄; 它可以是任何位置，但 **不** 應該是專案目錄) 。
    ...
    您可以輸入 `makeappx pack /?` 來查看每個參數運作狀況，但請簡單地說明︰
      * `/m` 設定要使用的資訊清單檔
      * `/f` 設定要使用 (在上一個步驟中建立的對應檔)  
      * `/p` 設定輸出封裝名稱
      * `/o` 設定它以覆寫輸出檔（如果有的話）

2. 建立封裝之後，必須簽署。 取得簽署憑證最簡單的方式是在 Visual Studio 中建立空白的通用 Windows 專案，並複製它所建立的檔案 `.pfx` ，但您可以使用 `MakeCert` 和 `Pvk2Pfx` 公用程式（如 [如何建立應用程式套件簽署憑證](/windows/desktop/appxpkg/how-to-create-a-package-signing-certificate)中所述）手動建立一個專案。

    > [!IMPORTANT]
    > 如果您手動建立簽署憑證，請務必將檔案放在與來源專案或套件來源不同的目錄中，否則它可能會隨附于封裝的一部分，包括私密金鑰！
...
4. 接受 [使用者帳戶控制] 系統管理員提高許可權提示（如果有的話），然後按一下 `Next`
...
在對封裝資訊清單進行變更並建立初始檔案之後 `resources.resw` ，新增其他語言很簡單。
...
封裝資訊清單必須更新，以列出應用程式所支援的語言。 Desktop App Converter 會新增預設的語言，但其他的語言則必須明確地新增。 如果直接編輯 `AppxManifest.xml` 檔案，請依下列方式更新 `Resources`，依您需要的數量新增元素，並且替換您支援的<span style="background-color: yellow">適當語言</span>，確保清單中的第一個項目為預設 (遞補) 語言。 在此範例中，預設值為英文 (美國)，同時也支援德文 (德國) 和法文 (法國)︰
...
如果您使用的是 Visual Studio，您應不需要做任何動作。如果查看 `Package.appxmanifest`，您應該會看到特殊的 <span style="background-color: yellow">x 產生</span>值，致使組建程序插入在您專案中找到的語言 (根據 BCP-47 代碼命名的資料夾)。 請注意，這不是實際套件資訊清單的有效值;它只適用于 Visual Studio 專案：
...
### <a name="step-14-localizing-more-parts-of-the-package-manifest-optional"></a>步驟1.4：將封裝資訊清單的更多部分當地語系化 (選擇性) 
...
本文假設您的當地語系化資源都有相同的檔案名 (例如， `contoso_demo.exe.mui` 或 `contoso_strings.dll` `contoso.strings.xml`) ，但它們是放在具有 BCP-47 名稱的不同資料夾中 (、等） `en-US` `de-DE` ) 。 這無關於您有多少資源檔案、其名稱為何、其檔案格式 / 相關的 API 為何等等。唯一重要是每個*邏輯*資源都有相同的檔名 (各位於不同的*實體*目錄)。 
...
>[!NOTE]
> 即使您無法遵循此檔案命名慣例，仍然可以使用 MRT.LOG 和封裝的優點;它只需要更多工作。
...
本文假設您在程式碼中的某個時間點，要找出包含當地語系化資源的檔案，然後將它載入，然後使用它。 用於載入資源的 API、用來擷取資源的 API 等並不重要。 在虛擬程式碼中，有三個基本步驟︰
...
* [ResourceContext](/uwp/api/Windows.ApplicationModel.Resources.Core.ResourceContext)，這會封裝目前使用中的一組限定詞值 (語言、縮放比例等)
* [ResourceManager](/uwp/api/windows.applicationmodel.resources.core.resourcemanager) (WinRT 版本，不是 .NET 版本)，其允許從 PRI 檔案存取所有的資源
* [ResourceMap](/uwp/api/windows.applicationmodel.resources.core.resourcemap)，代表 PRI 檔案中的一組特定子集資源 (在此範例中，檔案型資源與字串資源)
* [NamedResource](/uwp/api/Windows.ApplicationModel.Resources.Core.NamedResource)，代表邏輯資源和其所有可能的候選項目
* [ResourceCandidate](/uwp/api/windows.applicationmodel.resources.core.resourcecandidate)，代表單一具體候選項目資源 
...
因為 .NET 具有尋找與載入資源 (稱為「衛星組件」) 的內建機制，在上述綜合範例中並無可取代的明確程式碼 - 在 .NET 中，您只需要適當目錄的資源 DLL，而且會自動幫您找出。 使用資源套件將應用程式封裝為 MSIX 或 .appx 時，目錄結構會有點不同，而不是讓資原始目錄成為主要應用程式目錄的子目錄，而是在使用者的喜好設定) 中沒有列出該語言的情況下 (或完全不存在。 
...
轉換至 .appx 之後，配置看起來會像這樣，假設 `en-US` 是預設語言，而使用者的語言清單中列出德文和法文：
...
> [!NOTE]
> 如果您的應用程式已經有 `AssemblyResolve` 其他用途的處理常式，您必須將資源解析程式代碼與現有的程式碼整合。
...
如果您使用 Visual Studio，請參閱，以 [確保不論裝置是否需要裝置都已在裝置上安裝資源](/previous-versions/dn482043(v=vs.140)) ，您可以建立檔案和，以取得如何將所有語言組建至主要套件的相關資訊 `priconfig.packaging.xml` `priconfig.default.xml` 。
...
4. 建立封裝之後，請使用下列命令來建立套件組合，並使用適當的目錄和檔案名：

    ```CMD
    BundleGenerator.exe -Package ..\contoso_demo.appx -Destination ..\bundle -BundleName contoso_demo
    ```

現在您可以移至最後一個步驟，簽署 (請參閱以下) 。
