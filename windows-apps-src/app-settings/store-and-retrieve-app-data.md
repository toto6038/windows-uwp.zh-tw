...
> [!IMPORTANT]
> App 資料的生命週期與 app 生命週期息息相關。 如果移除應用程式，所有應用程式資料也會隨之遺失。 請勿使用 app 資料來儲存使用者資料或使用者可能認為重要且無法取代的任何資料。 建議以使用者的媒體櫃和 Microsoft OneDrive 來儲存這類資訊。 app 資料適合儲存 app 特定的使用者喜好設定、各種設定值和我的最愛。
...
-   不要漫遊經常變更的資料。 例如，如果您的應用程式追蹤經常變更的資訊，例如追蹤歌曲每秒的位置，請勿將它儲存為漫遊的應用程式資料。 請改為挑選變化較少但仍提供良好使用者體驗的表示法，像是目前正在播放的歌曲。
...
* [App 設定的指導方針](guidelines-for-app-settings.md)
