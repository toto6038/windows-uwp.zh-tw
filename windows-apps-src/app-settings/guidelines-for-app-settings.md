...
屬於一般應用程式工作流程的命令 (例如，變更繪圖應用程式中的筆刷大小) 不應該放在 [設定] 頁面中。 若要深入了解命令的放置位置，請參閱[命令設計基本知識](../layout/commanding-basics.md)。
...
在桌面和行動裝置上，應用程式設定視窗應以全螢幕方式開啟，並填滿整個視窗。 如果您的應用程式設定功能表最多有四個最上層群組，這些群組應該重疊顯示下一欄。
...
* [命令設計基本知識](../layout/commanding-basics.md)
* [進度控制項的指導方針](../controls-and-patterns/progress-controls.md)
* [儲存和擷取 App 資料](store-and-retrieve-app-data.md)
