...
從 **MyType** 移至 **IStringable** 或 **IClosable** 物件，您可以使用或傳回做為您投影的一部分，您可以呼叫 [**winrt::make**](/uwp/cpp-ref-for-winrt/make) 函式範本。 [**進行**] 傳回實作類型的預設介面。
...
> [!NOTE]
> 不過，如果您正在從 XAML UI 參考您的類型，則在相同的專案中將會同時有執行類型和投影類型。 此時，[**進行**] 傳回投影類型的執行個體。 如需該案例的程式碼範例，請參閱 [XAML 控制項。繫結至 C++/WinRT 屬性](binding-property.md#add-a-property-of-type-bookstoreviewmodel-to-mainpage)。
