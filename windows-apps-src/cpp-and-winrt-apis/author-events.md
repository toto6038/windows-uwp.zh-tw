...
我們建議您做為函式參數傳遞事件，以及不委派。 [**Winrt::event**](/uwp/cpp-ref-for-winrt/event)的**新增**功能是一個例外，因為您必須在此情況下傳遞委派。 此指導方針的原因是因為委派可以採取不同形式跨不同的 Windows 執行階段語言 （而言，它們是否支援一部用戶端註冊或多個）。 事件，使用其多個訂閱者模型中，會造成更可預測且一致的選項。
