...
> [!IMPORTANT]
> 如果您想要逐漸移植您[C + + /CX](/cpp/cppcx/visual-c-language-reference-c-cx)程式碼為 C + + /winrt 中，則您可以。 C + + /CX 與 C + + /winrt 程式碼可以同時存在於相同的專案，除了 XAML 編譯器支援，以及 Windows 執行階段元件中。 對於這些例外狀況，您將需要為目標的 C + + /CX 或 C + + /winrt 在同一個專案中的。 但是，您可以使用因數的程式碼在 Windows 執行階段元件從您的 XAML 應用程式，因為您將其移植。 將移盡可能 C + + /CX 程式碼，因為您可以在元件中，並再將 XAML 專案變更為 C + + /winrt。 或其他留下的 XAML 專案為 C + + /CX，建立一個新的 C + + WinRT 元件，並開始移植 C + + /CX 程式碼不使用 XAML 專案和元件。 您可能也會有一個 C + + /CX 元件專案旁邊的 C + + 在同一個方案中，內的 WinRT 元件專案參考這兩個從您的應用程式專案，並逐漸移植到另一個。
...
當移植到對等 C + + /winrt 程式碼中，您基本上要移除控制帽和變更箭號運算子 (-&gt;) 為點運算子 （.），因為 C + + /winrt 投影類型是值，而不是指標。
...
## <a name="converting-from-a-base-runtime-class-to-a-derived-one"></a>轉換為衍生一個基本執行階段類別
它是類型的通常有參考-為基底您知道是類型的指衍生的物件。 在 C + + /CX，您使用`dynamic_cast`來*轉型*以基礎參考到參考衍生。 `dynamic_cast`是只[**QueryInterface**](https://msdn.microsoft.com/library/windows/desktop/ms682521)隱藏的呼叫。 以下是常見的範例&mdash;您所處理相依性屬性變更事件，而且您想要從**DependencyObject**轉換回實際，是相依性屬性的擁有者類型。
...
如果您正從在內部使用事件和委派的 C++/CX 程式碼基底進行移植 (並非所有二進位檔案)，則 [**winrt::delegate**](/uwp/cpp-ref-for-winrt/delegate) 可協助您在 C++/WinRT 中複寫該模式。 另請參閱[參數化委派、 簡單的訊號和在專案中的回呼](author-events.md#parameterized-delegates-simple-signals-and-callbacks-within-a-project)。
