...
> [!IMPORTANT]
> 如果您想要逐漸移植您[C + + /CX](/cpp/cppcx/visual-c-language-reference-c-cx)程式碼為 C + + /winrt，則您可以。 C + + /CX 與 C + + /winrt 程式碼可以同時存在於在同一個專案中，除了 XAML 編譯器支援，以及 Windows 執行階段元件。 對於這些例外狀況，您將需要為目標的 C + + /CX 或 C + + /winrt 相同的專案中。 但您可以使用 Windows 執行階段元件因素的程式碼不使用您的 XAML 應用程式，如將其移植。 將移動盡可能 C + + /CX 程式碼，因為您可以到元件，並接著將 XAML 專案變更為 C + + /winrt。 或其他留下的 XAML 專案為 C + + /CX，建立一個新的 C + + WinRT 元件，並開始移植 C + + /CX 程式碼不使用 XAML 專案和元件。 您可能也會有 C + + /CX 元件專案旁邊的 C + + 在同一個方案中，內的 WinRT 元件專案參考這兩個從您的應用程式專案，並逐漸移植從另一個。
...
它是通常有參考-為基底您知道是指衍生類型的物件。 在 C + + /CX，您使用`dynamic_cast`來*轉換*到基礎參考到參考衍生。 `dynamic_cast`是只[**QueryInterface**](https://msdn.microsoft.com/library/windows/desktop/ms682521)隱藏的呼叫。 以下是典型的範例&mdash;您所處理相依性屬性變更事件，而您想要從**DependencyObject**轉換回實際是相依性屬性的擁有者的類型。
