...
| 打開一個專案時，Visual Studio 產生此錯誤「*未安裝此專案的應用程式*」。|如果您尚未開始，您需要從 Visual Studio 的**新專案**對話方塊中安裝 **C++ 開發 Windows 通用工具**。 如果無法解決問題，則此專案可能取決於 C + + /winrt Visual Studio 擴充功能 (VSIX) (請參閱[Visual Studio 支援 C + + /winrt](intro-to-using-cpp-with-winrt.md#visual-studio-support-for-cwinrt-xaml-the-vsix-extension-and-the-nuget-package)。|
...
| C++ 編譯器產生「*錯誤 C2338：這僅適用於弱式參考資料支援*」。|您為將 **winrt::no_weak_ref** 標記結構做為範本引數傳遞至其基底類別的類型，要求一個弱式參考資料。 請參閱 <<c0>選擇退出弱式參考支援。|
...
| C + + 編譯器產生的投影類型的 「*沒有適當的預設建構函式*」。 | 如果您正嘗試延遲初始化執行階段類別物件，或使用，並在同一個專案中，實作執行階段類別，則您必須呼叫`nullptr_t`建構函式。 如需詳細資訊，請參閱 [使用 C++/WinRT 取用 API](consume-apis.md)。 |
...
| C + + 編譯器產生 「*錯誤 C2039: 'IUnknown': 不是成員的 '\'global 命名空間'*」。 |請參閱[如何重定您 C + + WinRT 更新版本的 Windows SDK 的專案](news.md#how-to-retarget-your-cwinrt-project-to-a-later-version-of-the-windows-sdk)。 |
| C + + 連結器會產生 「*錯誤 LNK2019： 無法解析的外部符號_WINRT_CanUnloadNow@0函式中參考_VSDesignerCanUnloadNow@0*」 | 請參閱[如何重定您 C + + WinRT 更新版本的 Windows SDK 的專案](news.md#how-to-retarget-your-cwinrt-project-to-a-later-version-of-the-windows-sdk)。 |
| 在建置程序會產生錯誤訊息*C + /winrt VSIX 不再提供專案建置的支援。 請新增 Microsoft.Windows.CppWinRT Nuget 套件的專案參考*。 | 安裝**Microsoft.Windows.CppWinRT** NuGet 套件到您的專案。 如需詳細資訊，請參閱[VSIX 擴充功能的較舊版本](intro-to-using-cpp-with-winrt.md#earlier-versions-of-the-vsix-extension)。 |

> [!NOTE]
> 如果本主題未能回答您的問題，也許則造訪[Visual Studio 的 c + + 開發人員社群](https://developercommunity.visualstudio.com/spaces/62/index.html)，或使用，您可能會發現說明[`c++-winrt`標記 Stack Overflow 上](https://stackoverflow.com/questions/tagged/c%2b%2b-winrt)。
