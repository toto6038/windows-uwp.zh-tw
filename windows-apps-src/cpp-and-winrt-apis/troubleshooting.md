...
| Windows 應用程式認證套件測試產生執行階段類別的錯誤之一「*不是從 Windows 的基底類別衍生。所有可組合類別必須最終從 Windows 命名空間中的類別衍生*」。|任何執行階段類別 （您可以在應用程式中宣告） 衍生自基底類別的項目稱為*可撰寫*類別。 可撰寫類別的最終基底類別必須是在 Windows.* 命名空間內; 源自的類型例如， [**Windows.UI.Xaml.DependencyObject**](/uwp/api/windows.ui.xaml.dependencyobject)。 請參閱[XAML 控制 ； 繫結至 C + + WinRT 屬性](binding-property.md)如需詳細資訊。|
...
| C + + 連結器會產生"*錯誤 LNK2019： 無法解析外部符號*"|請參閱[為何連結器給予我"LNK2019： 無法解析外部符號"錯誤？](faq.md#why-is-the-linker-giving-me-a-lnk2019-unresolved-external-symbol-error)|
| LLVM 和 Clang toolchain 會產生錯誤時搭配 C + + WinRT。|我們不支援 LLVM 和 Clang toolchain 的 C + + WinRT，但如果您想要模擬如何我們使用其內部，則您無法再試符合原先的假設等中所述的其中一個[可以使用 LLVM/Clang 編譯與 C + + WinRT？](faq.md#can-i-use-llvmclang-to-compile-with-cwinrt)。|
| C + + 編譯器會產生預計類型的 「*沒有適當的預設建構函式可用*"。 | 如果您嘗試要延遲 runtime 類別物件的初始化或取用及實作 runtime 類別在同一個專案中，則您將需要呼叫`nullptr_t`建構函式。 如需詳細資訊，請參閱 [使用 C++/WinRT 取用 API](consume-apis.md)。 |
| C + + 編譯器會產生"*錯誤 C3861: 'from_abi'： 找不到識別碼*"，及源自*base.h*中其他錯誤。 如果您使用 Visual Studio 2017，可能會看到此錯誤 (版本 15.8.0 或更高)，與目標 Windows sdk （英文） 版本 10.0.17134.0 (Windows 10、 版本 1803年)。 | 其中一個目標更新版本 （詳細一致） 版本的 Windows sdk （英文） 或設定專案屬性**C/c + +** > **語言** > **Conformance 模式： 否**(此外，如果 **/ 寬鬆-** 會出現在專案屬性**C/c + +**  > **語言** > [**其他選項**] 下的**命令列**予以刪除)。 |
