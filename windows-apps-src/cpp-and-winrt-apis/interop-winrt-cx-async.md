...
若因程式碼基底的大小或複雜度而必須逐步移植您的專案，您將需要適當的移植程序，讓 C++/CX 和 C++/WinRT 程式碼能夠在一段時間內並存於相同的專案中。 如果您有非同步程式碼，則您在逐步移植原始程式碼時，可能需要讓平行模式程式庫 (PPL) 工作鏈結和協同程式並存於您的專案中。 本主題著重於非同步 C++/CX 程式碼與非同步 C++/WinRT 程式碼之間的相互操作技術。 您可以個別或搭配使用這些技術。
...
> [!NOTE]
> 逐步移植有一些限制。 如果您有 Windows 執行階段元件專案，則無法進行逐步移植，而必須一次移植整個專案。 此外，針對 XAML 專案，您的 XAML 頁面類型無論何時都必須完全是 C++/WinRT *或*完全是 C++/CX。 如需詳細資訊，請參閱[從 C++/CX 移至 C++/WinRT](/windows/uwp/cpp-and-winrt-apis/move-to-winrt-from-cx)。

## <a name="the-reason-an-entire-topic-is-dedicated-to-asynchronous-code-interop"></a>用整個主題來闡述非同步程式碼相互操作的原因

從 C++/CX 移植到 C++/WinRT 通常不難，但從[平行模式程式庫 (PPL)](/cpp/parallel/concrt/parallel-patterns-library-ppl) 工作移至協同程式則屬例外。 兩者的模型不同。 從 PPL 工作到協同程式之間並沒有自然的一對一對應，且沒有簡單的方式可機械性地移植適用於所有案例的程式碼。

好消息是，從工作轉換到協同程式後，會產生顯著的簡化。 此外，開發小組會定期報告他們在移植其非同步程式碼方面克服的困難，而其餘的移植工作大多是機械性的。

演算法最初通常是針對同步 API 撰寫的。 然後，演算法轉變為工作和明確的接續&mdash;結果常會意外混淆了基礎邏輯。 例如，迴圈變成遞迴；if-else 分支變成工作的內嵌樹狀結構 (鏈結)；共用變數變成 **shared_ptr**。 若要解構 PPL 原始程式碼常見的非自然結構，建議您先回過頭了解原始程式碼的意圖 (也就是，探索原始的同步版本)。 然後，在適當之處插入 `co_await`。
...
為了對非同步程式設計的概念和術語有非常明確的參考，我們將先簡單描述 Windows 執行階段非同步程式設計，以及兩個語言投影如何以不同的方式在其上分層。

您的專案具有非同步執行工作的方法。 在許多情況下，您都會想要等到該工作完成後，再執行其他動作。 為了讓您能夠等待非同步方法完成，方法會傳回非同步作業物件。 但有時您不想要或不需要等待以非同步方式執行的工作完成。 在這種情況下，方法不需要傳回非同步作業物件。 您不會等待的非同步方法，稱為*射後不理*方法。
...
一般而言，非同步 C++/CX 方法會使用 Lambda 函式搭配 [**concurrency::create_task**](/cpp/parallel/concrt/reference/concurrency-namespace-functions#create_task) 和 [**concurrency::task::then**](/cpp/parallel/concrt/reference/task-class#then)，將 PPL 工作鏈結在一起。 這些 Lambda 函式會傳回一項工作，而此工作完成時將會產生一個值，傳入工作*接續*的 Lambda 中。
...
> [!NOTE]
> 如果非同步 C++/CX 方法傳回 **IAsyncXxx**\^，則 **TResult** 會限定為 Windows 執行階段類型。 例如，布林值是 Windows 執行階段類型；但 C++/CX 投影的類型 (例如 **Platform::Array<byte>** ^) 則不是。

### <a name="cwinrt-async"></a>非同步 C++/WinRT

C++/WinRT 會將 C++ 協同程式整合到程式設計模型中。 協同程式和 `co_await` 陳述式提供了一種自然的方式來合作等待結果。

每個 **IAsyncXxx** 類型皆投影到 **winrt::Windows::Foundation** C++/WinRT 命名空間中的對應類型。 我們將稱之為 **winrt::IAsyncXxx**。 C++/WinRT 協同程式的傳回類型是 **winrt::IAsyncXxx**，或稱 [**winrt::fire_and_forget**](/uwp/cpp-ref-for-winrt/fire-and-forget)。 協同程式會使用 `co_return` 關鍵字來傳回呼叫端實際需要的值 (可能是檔案、位元組陣列或布林值)，而不是使用 `return` 關鍵字傳回非同步物件。

如果方法至少包含一個 `co_await` 陳述式 (或至少一個 `co_return` 或 `co_yield`)，則該方法將因此成為協同程式。

如需詳細資訊，請參閱[使用 C++/WinRT 的並行和非同步作業](/windows/uwp/cpp-and-winrt-apis/concurrency)。

## <a name="the-direct3d-game-sample-simple3dgamedx"></a>Direct3D 遊戲範例 (**Simple3DGameDX**)

本主題包含數個逐步解說，說明如何逐步移植非同步程式碼。 為了作為案例研究，我們將使用 C++/CX 版的 [Direct3D 遊戲範例](/samples/microsoft/windows-universal-samples/simple3dgamedx/) (名為 **Simple3DGameDX**)。 我們將提供一些範例，說明如何取用該專案中的原始 C++/CX 原始程式碼，並將其非同步程式碼逐步移植到 C++/WinRT。

- 從上述連結下載 ZIP，並將其解壓縮。
- 在 Visual Studio 中開啟 C++/CX 專案 (位於名為 `cpp` 的資料夾中)。
- 接著，您必須將 C++/WinRT 支援新增至專案。 如需該作業所需的步驟，請參閱[取用 C++/CX 專案並新增 C++/WinRT 支援](/windows/uwp/cpp-and-winrt-apis/interop-winrt-cx#taking-a-ccx-project-and-adding-cwinrt-support)。 將 `interop_helpers.h` 標頭檔新增至專案的步驟非常重要，因為我們將仰賴本主題中的 Helper 函式進行操作。
- 最後，將 `#include <pplawait.h>` 新增至 `pch.h`。 這可為您提供協同程式對 PPL 的支援 (後續小節將詳細說明該支援)。

在建置時，您將收到關於**位元組**不明確的錯誤。 以下是解決此問題的方法。

- 開啟 `BasicLoader.cpp`，並註解 `using namespace std;`。
- 在相同的原始程式碼檔案中，接著您必須將 **shared_ptr** 限定為 **std::shared_ptr**。 您可以在該檔案中使用搜尋和取代來執行此動作。
- 將 **vector** 限定為 **std::vector**，並將 **string** 限定為 **std::string**。
...
簡言之，在進行移植時，我們會將 PPL 工作鏈結變更為 `co_await` 的呼叫。 我們會將 PPL 工作的方法傳回的值變更為 C++/WinRT **IAsyncXxx** 物件或 **IAsyncXxx**\^。 我們會將 **IAsyncXxx**\^ 變更為 C++/WinRT **IAsyncXxx**。

您應該記得，協同程式是任何會呼叫 `co_xxx` 的方法。 C++/WinRT 協同程式會使用 `co_return` 傳回其值。 端賴協同程式對 PPL 的支援 (`pplawait.h`)，您也可以使用 `co_return` 從協同程式傳回 PPL 工作。 此外，您也可以 `co_await` 工作和 **IAsyncXxx**。 但您無法使用 `co_return` 傳回 **IAsyncXxx**\^。 下表說明各種非同步技術之間的相互操作支援。

|方法|是否可以 `co_await`？|是否可從中 `co_return`？|
|-|-|-|
|方法傳回 task\<void\>|是|是|
|方法傳回 task\<T\>|否|是|
...
您可以使用上表直接跳到您感興趣的相互操作技術。

|非同步相互操作技術|本主題中的章節|
|-|-|
|使用 `co_await` 等待射後不理方法內的 **task\<void\>** 方法。|[ 等待射後不理方法內的 **task\<void\>** ](#await-taskvoid-within-a-fire-and-forget-method)|
...
即使有這些絕佳的相互操作選項，移植仍需仰賴我們精準地選擇變更，而不會影響系統的其餘部分。 我們要避免盲目徒勞，而應將整個專案理出頭緒。 為此，我們必須以特定順序執行操作。 讓我們看看一些進行這類非同步相關移植變更的範例。

## <a name="await-a-taskvoid-method-leaving-the-rest-of-the-project-unchanged"></a>等待 **task\<void\>** 方法，讓專案的其餘部分保持不變

傳回 **task\<void\>** 的方法會以非同步方式執行工作，但最終不會產生值。 我們可以依此方式 `co_await` 方法。

因此，在逐步移植非同步程式碼時，找出呼叫這些方法的位置可說是理想的起始點。 這些位置將牽涉到建立和/或傳回工作；和/或不會將任何值傳至其接續的工作鏈結。 在這類位置中，您可以直接將非同步程式碼取代為 `co_await` 陳述式，如下所示。
...
在 **Simple3DGameDX**中，您可以在方法 **GameMain::Update** 的實作中找到相符項目。 該項目位於原始程式碼檔案 `GameMain.cpp` 中。

#### <a name="gamemainupdate"></a>**GameMain::Update**

以下片段摘錄自 C++/CX 版的方法，其中顯示以非同步方式完成的兩個部分。
...
我們可以用相同的方式變更這兩個位置中的程式碼。 在下列清單後面會說明此程式碼。 我們可以在這裡討論如何以安全的方式在類別成員協同程式中存取 *this* 指標。 但我們將延後到後續小節再討論，因為此程式碼現在可以運作。
...
導入 `co_await` 會將方法轉換為協同程式，因此無法傳回 `void`。 這屬於射後不理方法，因此我們傳回 [**winrt::fire_and_forget**](/uwp/cpp-ref-for-winrt/fire-and-forget)。

您也必須在 `GameMain.h` 的宣告中，將 **GameMain::Update** 的傳回類型從 `void` 變更為 **winrt::fire_and_forget**。
...
**GameMain::ResetGame** 是另一個射後不理方法，也會呼叫 **LoadLevelAsync**。 因此，我們可以在該處進行相同的變更。
...
將任何 (會傳回工作的) `return` 變更為 `co_await`。 您將會看到一個未傳回任何項目的 `return`，請直接將其刪除。 完成後，無作業工作將會消失，而方法的非同步部分大致上會顯示如下。 同樣地，較不重要的同步程式碼會以省略符號表示。
...
但是，建構函式無法傳回 **winrt::fire_and_forget**，因此我們會將非同步程式碼移至新的 **GameMain::ConstructInBackground** 射後不理方法中、將程式碼簡化為 `co_await` 陳述式，然後從建構函式呼叫新的方法。
...
**GameMain** 中的所有射後不理方法&mdash;事實上，所有的非同步程式碼&mdash;都會轉換為協同程式。 如果您想要的話，或許您可以尋找其他類別的射後不理方法，並進行類似的變更。
...
您只能在衍生自 [**winrt::implements**](/uwp/cpp-ref-for-winrt/implements)的類別中 **implements::get_strong**。
...
下一個最簡單的案例是在本身會傳回 **task\<void\>** 的方法中等待 **task\<void\>** ，因為我們可以 `co_await` **task\<void\>** ，且可以從一個工作 `co_return`。
...
下列程式碼範例僅示範 **task\<void\>** 的簡單 `co_await`。 然後，為了滿足 **task\<T\>** 傳回類型，我們會藉由 co_awaiting Windows 執行階段方法以非同步方式傳回 **StorageFile\^** ，然後 `co_return` 產生的檔案。
...
下一個步驟是將方法的其餘部分移植到 C++/WinRT，如下所示。
...
我們已了解如何 `co_await` **task\<void\>** 。 您也可以 `co_await` 會傳回 **IAsyncXxx** 的方法，無論是您專案中的方法，還是非同步 Windows API (例如 [**StorageFolder.GetFileAsync**](/uwp/api/windows.storage.storagefolder.getfileasync))。

如需可在何處進行此程式碼變更的範例，可以查看 **BasicReaderWriter::ReadDataAsync** (您會發現其已在 `BasicReaderWriter.cpp` 中實作)。 以下是原始 C++/CX 版本。
...
我們不僅可以 `co_await` Windows API，還可以 `co_return` 方法以非同步方式傳回的值 (在此案例中為位元組陣列)。 第一個步驟說明如何僅進行這些變更；我們將在下一節實際將 C++/CX 程式碼移植到 C++/WinRT。
...
> [!NOTE]
> 在 **ReadDataAsync**中，我們已建構並傳回新的 C++/CX 陣列。 當然，我們這麼做是為了滿足方法的傳回類型 (如此，我們就不需要變更專案的其餘部分)。
>
> 您在自己的專案中可能會遇到其他範例，像是，在移植之後，您在方法結束後只獲得了 C++/WinRT 物件。 若要 `co_return`，請直接呼叫 **to_cx** 加以轉換。 以下是假設性範例。
...
您無法直接 `co_return` **IAsyncXxx**\^，但可以達到類似的目的。 如果您的工作使用 `co_return`，您可以用 [**concurrency::create_async**](/cpp/parallel/concrt/reference/concurrency-namespace-functions#create_async) 加以包覆。
...
這表示我們可以將該方法的傳回類型從 **task\<void\>** 變更為 **winrt::Windows::Foundation::IAsyncAction**。
...
現在，將該方法的其餘部分及其相依性移植到 C++/WinRT 的程序應該已相當機械性。
...
現在，我們要將 **BasicReaderWriter::ReadDataAsync** 完全移植到 C++/WinRT，以完成這個逐步解說。 在我們上一次查看時，([將 **ReadDataAsync** (大部分) 移植到 C++/WinRT，讓專案的其餘部分保持不變](#port-readdataasync-mostly-to-cwinrt-leaving-the-rest-of-the-project-unchanged))，大部分都已移植到 C++/WinRT。 但仍會傳回 **Platform::Array\<byte\>** ^ 的工作。
...
我們將改為以非同步方式傳回 C++/WinRT [**IBuffer**](/uwp/api/windows.storage.streams.ibuffer) 物件，而不會傳回工作，即使這表示會變更呼叫位置的程式碼亦然。
