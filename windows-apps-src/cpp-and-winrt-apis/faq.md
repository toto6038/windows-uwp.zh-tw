...
您可以移除這些`const`限定詞**ToString**上的應該您決定您需要修改它的實作中的某些物件狀態。 但讓每個您的成員函式 const 或非 const 不可兩者。 換句話說，不多載函式實作上`const`。

除了您實作的函式，另一個其他放置的位置 const 進入圖片是在 Windows 執行階段函式的投射。 請考慮此程式碼。
...
投影的函式是 const 不論您選擇以符合您的實作它們的方式。 在幕後投影呼叫應用程式二進位介面 (ABI)，哪些量，透過 COM 介面指標的呼叫。 投影的**ToString**互動的唯一狀態是該 COM 介面指標;而且，當然具備不需要修改該指標，因此函式是 const。 這讓您保證，它不會變更任何關於您正在呼叫，透過**IStringable**參考資料，以及它可確保您可以呼叫**ToString**即使有 const 參考到**IStringable**中。
...
在 Windows 執行階段世界中，編譯器就無法快取的值`c()`或透過間接取值呼叫每個方法的介面 ('。 ')。 除非您介入，這樣會造成更多虛擬呼叫和參考計數的額外負荷。 上述的模式無法輕鬆地產生倍為嚴格所需的程式碼。 相反地，偏好的模式，您可以在任何地方，如下所示。 它會產生很多較少的程式碼，以及它可以也可大幅改善您的執行的階段效能。
