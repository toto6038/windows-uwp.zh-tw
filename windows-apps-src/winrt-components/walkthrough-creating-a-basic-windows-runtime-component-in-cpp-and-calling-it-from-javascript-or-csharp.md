...
3. 在中央窗格中，選取 [ **Windows 執行階段元件**]，然後將專案\_命名為 WinRT CPP。
...
在 [方案總管] 中，開啟 MainPage.xaml.cs （檔案可能會內嵌在 MainPage 底下）。為 System.web 新增 using 指示詞，然後在 MainPage 類別中加入對數計算的事件處理常式。
...
2. 在物件瀏覽器的左窗格中，展開 [WinRT\_CPP] 節點，顯示在您的元件上定義的類型和方法。
