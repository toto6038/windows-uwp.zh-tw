...
> **請注意** ，參考 Windows 執行階段的錯誤訊息會使用舊的術語。 這現在稱為通用 Windows 平台 (UWP)。 例如，Windows 執行階段類型現在稱為 UWP 類型。

## <a name="missing-references-to-mscorlibdll-or-systemruntimedll"></a>遺失 mscorlib.dll 或 System.Runtime.dll 的參考

僅在從命令列使用 Winmdexp.exe 時，才會發生此問題。 建議您使用/reference 選項，同時包含 .NET Framework 核心參考元件（位於 "% ProgramFiles （x86）%\\reference 元件\\）中的mscorlib.dll和system.string的參考。 Microsoft\\Framework。\\NETCore\\v 4.5 "（"% ProgramFiles%\\... "在32位電腦上）。
...
> **請注意** ，在錯誤訊息中，運算子是以其中繼資料名稱來識別，\_例如 op 加法\_、op 乘法\_、op ExclusiveOr\_、op 隱含（隱含轉換）等等。
...
| 錯誤代碼 | 訊息文字                                                                                                                                            |
|--------------|---------------------------------------------------------------------------------------------------------------------------------------------------------|
| WME1099      | 類型 '{0}' 有多個具有 '{1}' 引數的函式。 Windows 執行階段類型不能有多個引數數目相同的建構函式。 |
...
| 錯誤代碼 | 訊息文字                                                                                                                                                                      |
|--------------|-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| WME1059      | ' {0} .'{2}的多重參數多載會以 windows.foundation.metadata.defaultoverloadattribute 裝飾裝飾。{1}                                                            |
| WME1085      | 的{0}- {1}參數{2}多載只能以 windows.foundation.metadata.defaultoverloadattribute 裝飾的方式，將一個方法指定為預設多載。 |
...
> **請注意**  ，這些限制僅適用于公用類型，而不適用於您的實作為執行所使用的私用類型。
...
> **請注意**  ，如果您直接呼叫 Winmdexp，但未使用/out 選項來指定 Windows 執行階段元件的名稱，則 Winmdexp 會嘗試產生一個包含元件中所有命名空間的名稱。 為命名空間重新命名，可變更元件的名稱。
...
| WME1044      | 公用類型有命名空間（'{1}'），不與其他命名空間（'{0}'）共用通用的前置詞。 Windows 中繼資料檔中的所有類型都必須存在檔名所隱含之命名空間的子命名空間內。                                                                                                                              |
| WME1067      | 命名空間名稱不能只有大小寫不同{0}： ' '{1}，' '。                                                                                                                                                                                                                                                                                                |
| WME1068      | 類型 '{0}' 不可以與命名空間 '{1}' 具有相同的名稱。                                                                                                                                                                                                                                                                                                 |
...
<td align="left">方法 '{0}' 有類型 '{1} {2}' 的參數 ' '。 '{2}' 不是有效的 Windows 執行階段參數類型。</td>
</tr>
<tr class="even">
<td align="left">WME1038</td>
<td align="left">方法 '{0}' 在其簽章中有{1}類型 ' ' 的參數。 雖然這種類型不是有效的 Windows 執行階段類型，但它實作了有效 Windows 執行階段類型的介面。 請考慮將方法簽章變更為使用下列其中一種類型：'{2}'。</td>
</tr>
<tr class="odd">
<td align="left">WME1039</td>
<td align="left"><p>方法 '{0}' 在其簽章中有{1}類型 ' ' 的參數。 雖然這種泛型類型不是有效的 Windows 執行階段類型，但此類型或其泛型參數實作了有效 Windows 執行階段類型的介面。 {2}</p>
> **** &lt;請注意  ，在中，Winmdexp 會附加替代專案清單，例如「請考慮將方法簽章中的類型 ' system.object. list&gt;T ' 變更為下列其中一項{2}改為類型：&gt; &lt;' System.string&lt;t，IReadOnlyListt&gt;，system.object&lt;t'..amt'&gt;. "."
</td>
</tr>
<tr class="even">
<td align="left">WME1040</td>
<td align="left">方法 '{0}' 在其簽章中有{1}類型 ' ' 的參數。 請勿使用 Managed 工作類型，應改用 Windows.Foundation.IAsyncAction、Windows.Foundation.IAsyncOperation，或其他 Windows 執行階段非同步介面其中之一。 標準的 .NET await 模式也適用於這些介面。 如需將 Managed 工作物件轉換成 Windows 執行階段非同步介面的詳細資訊，請參閱 System.Runtime.InteropServices.WindowsRuntime.AsyncInfo。</td>
</tr>
</tbody>
</table>
...
| 錯誤代碼 | 訊息文字                                                                                                                                                                                                                                                            |
|--------------|-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| WME1060      | 結構 '{0}' 具有類型 '{1} {2}' 的欄位 ' '。 '{2}' 不是有效的 Windows 執行階段欄位類型。 Windows 執行階段結構中的每個欄位只能是 UInt8、Int16、UInt16、Int32、UInt32、Int64、UInt64、Single、Double、Boolean、String、Enum，或本身是結構。 |
...
> **請注意** ，這項限制不適用於您在內部部署中使用的陣列。

 

| 錯誤代碼 | 訊息文字                                                                                                                                                     |
|--------------|--------------------|
| WME1034      | 方法 '{0}' 在其簽章中具有{1}類型 ' ' 的陣列，其具有非零的下限。 Windows 執行階段方法簽章中的陣列的下限必須是零。 |
| WME1035      | 方法 '{0}' 在其簽章中有類型 '{1}' 的多維陣列。 Windows 執行階段方法簽章中的陣列必須是一維陣列。                  |
| WME1036      | 方法 '{0}' 在其簽章中有類型{1}' ' 的嵌套陣列。 Windows 執行階段方法簽章中的陣列不能是巢狀陣列。                                    |
...
| 錯誤代碼 | 訊息文字         |
|--------------|----------------------|
| WME1101      | 方法 '{0}' 具有屬於陣列{1}的參數 ' '，其同時{2}具有和{3}。 在 Windows 執行階段中，內容陣列參數必須是可讀取或可寫入的。 請從 '{1}' 移除其中一個屬性。                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
| WME1102      | 方法 '{0}' 具有屬於陣列但具有{1} {2}的輸出參數 ' '。 在 Windows 執行階段中，輸出陣列的內容是可寫入的。 請從 '{1}' 移除屬性。                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
| WME1103      | 方法 '{0}' 具有屬於陣列{1}的參數 ' '，其具有 system.runtime.interopservices.outattribute. InAttribute 或 system.runtime.interopservices.outattribute. OutAttribute 中的。 在 Windows 執行階段中，陣列參數必須具有 {2} 或 {3}。 請移除這些屬性，或視需要以適當的 Windows 執行階段屬性取代。                                                                                                                                                                                                                                                                                                                                                                                          |
| WME1104      | 方法 '{0}' 有不是{1}陣列、 {2}且具有或的{3}參數 ' '。 Windows 執行階段不支援使用 {2} 或 {3} 標記非陣列參數。                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
| WME1105      | 方法 '{0}' 的參數 '{1}' 具有 system.runtime.interopservices.outattribute. InAttribute 或 system.runtime.interopservices.outattribute. OutAttribute。 Windows 執行階段不支援以 System.Runtime.InteropServices.InAttribute 或 System.Runtime.InteropServices.OutAttribute 標記參數。 請考慮移除 System.Runtime.InteropServices.InAttribute，並改以 'out' 修飾詞取代 System.Runtime.InteropServices.OutAttribute。 方法 '{0}' 的參數 '{1}' 具有 system.runtime.interopservices.outattribute. InAttribute 或 system.runtime.interopservices.outattribute. OutAttribute。 Windows 執行階段只支援以 System.Runtime.InteropServices.OutAttribute 標記 ByRef 參數，不支援這些屬性的其他用法。 |
| WME1106      | 方法 '{0}' 具有屬於陣列{1}的參數 ' '。 在 Windows 執行階段中，陣列參數的內容必須是可讀取或可寫入的。 請將 {2} 或 {3} 套用到 '{1}'。                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
...
> **請注意**  ，如果您變更傳回值的名稱，而新名稱與另一個參數的名稱衝突，您就會收到錯誤 wme1091 錯誤。
...
**請注意**  ，屬性存取子的預設名稱為 "returnValue"，而所有其他方法則為 "value"。
