...
多執行個體 UWP 應用程式 | UWP 應用程式可以選擇支援多個執行個體。 如果多執行個體 UWP 應用程式的其中一個執行個體正在執行，而且後續啟用要求成功時，平台將不會啟用現有的執行個體。 反而會建立執行於不同處理序中的新執行個體。 如需詳細資訊，請參閱[建立多執行個體通用 Windows 應用程式](../launch-resume/multi-instance-uwp.md)。
