...
機器學習 | Windows Machine Learning 可讓您建置透過 Windows 10 裝置本機預先訓練機器學習模型進行問題求解的應用程式。 若要深入了解此平台，請參閱 [Windows Machine Learning](https://docs.microsoft.com/windows/ai/)。 </br> [MachineLearning](https://docs.microsoft.com/uwp/api/windows.ai.machinelearning.preview) 命名空間包含可讓應用程式載入機器學習模型、繫結資料作為輸入以及評估結果的類別。
...
套件資源索引 API 和自訂組建系統 | 您可以使用[套件資源索引 (PRI) API](https://docs.microsoft.com/windows/uwp/app-resources/pri-apis-custom-build-systems)，開發適用於您的 UWP 應用程式資源的自訂組建系統。 建置系統可以依據 UWP 應用程式所需的任何複雜層級，建立 PRI 檔案並對這些檔案進行版本控制和傾印。 如果您的自訂建置系統目前使用 MakePri.exe 命令列工具，建議您改為呼叫 PRI API，因為它們提供更高的效能與控制。
PlayReady | Microsoft PlayReady 是一組保護數位內容免遭未經授權使用的技術。 PlayReady 可在各種裝置和應用程式上執行，以及跨所有作業系統執行。 [了解如何將 PlayReady 納入您的應用程式中。](https://docs.microsoft.com/playready/)
...
系統觸發程序 | [CustomSystemEventTrigger](https://docs.microsoft.com/uwp/api/windows.applicationmodel.background.customsystemeventtrigger) 可讓您在作業系統未提供您需要的系統觸發程序時定義系統觸發程序。 例如，當硬體驅動程式和 UWP 應用程式都屬於第三方，而且硬體驅動程式必須引發其應用程式處理的自訂事件時。 例如，需要通知使用者關於何時插入音訊插孔時的音訊卡。
...
Windows Mixed Reality 文件 | Windows Mixed Reality 開發人員指引[現在位於 docs.microsoft.com。](https://docs.microsoft.com/windows/mixed-reality/) 就如同在這些 UWP 文件中，您現在可以利用 GitHub Issues 提出意見反應，或透過提取要求提交自己的意見。
...
比以往更輕鬆地將您的傳統型應用程式攜入通用 Windows 平台。 [觀看影片](https://www.youtube.com/watch?v=fJkbYPyd08w)了解如何封裝您的 .NET 應用程式以供發佈，然後[查看這個頁面](../porting/desktop-to-uwp-packaging-dot-net.md)以取得詳細資訊。
