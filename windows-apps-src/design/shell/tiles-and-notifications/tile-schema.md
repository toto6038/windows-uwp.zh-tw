...
| **引數** | 字串 | false | 年度更新版的新功能：應用程式定義的資料，當使用者透過動態磚啟動您的應用程式，會透過 LaunchActivatedEventArgs 上的 TileActivatedInfo 屬性傳回至您的應用程式。 這可讓您知道當使用者點選動態磚時，看到哪個磚通知。 在沒有年度更新版的裝置上，這會被忽略。 |
...
| 屬性 | 類型 | 必要 |描述 |
|---|---|---|---|
| **圖示** | [TileBasicImage](#tilebasicimage) | true | 至少支援桌面和行動裝置版、小型和中型磚，提供正方形的長寬比例影像、解析度 200x200、PNG 格式、具有透明度，只使用白色。 如需詳細資訊，請參閱：[特殊磚範本](../tiles-and-notifications/special-tile-templates-catalog.md)。 |
...
| 屬性 | 類型 | 必要 |描述 |
|---|---|---|---|
| **映像** | [TileBasicImage](#tilebasicimage) | true | 要顯示的影像。 |
...
| 屬性 | 類型 | 必要 |描述 |
|---|---|---|---|
| **影像** | IList<[TileBasicImage](#tilebasicimage)> | true | 影像會以圓圈滾動。 |
...
| 屬性 | 類型 | 必要 |描述 |
|---|---|---|---|
| **影像** | IList<[TileBasicImage](#tilebasicimage)> | true | 可提供高達 12 個影像 (行動裝置版只會顯示最多 9 個)，將用於投影片放映。 新增超過 12 個會擲回例外狀況。 |
