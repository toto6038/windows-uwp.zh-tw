...
| **視覺化** | [ToastVisual](#toastvisual) | true | 描述快顯通知的視覺效果部分。 |
...
| **警示** | 鬧鐘通知。 這會以預先展開的方式顯示，並停留在使用者的螢幕上，直到關閉為止。 音效預設會重複播放，並使用鬧鐘音效。 |
...
| 屬性 | 類型 | 必要 | 描述 |
|---|---|---|---|
| **標題** | string 或 [BindableString](#bindablestring) | false | 取得或設定選用標題字串。 支援資料繫結。 |
| **值** | double 或 [AdaptiveProgressBarValue](#adaptiveprogressbarvalue) 或 [BindableProgressBarValue](#bindableprogressbarvalue) | false | 取得或設定進度列的值。 支援資料繫結。 預設為 0。 |
| **ValueStringOverride** | string 或 [BindableString](#bindablestring) | false | 取得或設定要顯示的選用字串，用於取代預設百分比字串。 如果未提供此項，將會顯示「70%」之類的內容。 |
| **狀態** | string 或 [BindableString](#bindablestring) | true | 取得或設定狀態字串 (必要)，它會顯示在左側進度列的下方。 這個字串應該反映作業的狀態，例如「正在下載...」或「正在安裝...」 |
...
| 屬性 | 類型 | 必要 | 描述 |
|---|---|---|---|
| **值** | double | false | 取得或設定值 (0.0 - 1.0)，代表完成百分比。 |
...
| 屬性 | 類型 | 必要 |描述 |
|---|---|---|---|
| **來源** | 字串 | true | 影像的 URL。 支援 ms-appx、ms-appdata 和 http。 Http 影像的大小必須是 200 KB 或更少。 |
...
| 屬性 | 類型 | 必要 | 描述 |
|---|---|---|---|
| **識別碼** | 字串 | true | Id 為必要項。 如果使用者選取此項目，這個 Id 將會傳回到 App 的程式碼，表示他們選擇了哪些選取項目。 |
| **內容** | 字串 | true | Content 為必要項，並且是顯示在選取項目上的字串。 |
...
| 值 | 意義 |
|---|---|
| **預設值** | 預設行為。 當使用者對快顯通知採取動作時，將會關閉快顯通知。 |
...
| **迴圈** | 布林值 | false | 如果快顯通知顯示多久就要重複音效多久，則設定為 true。false 表示只播放一次 (預設)。 |
| **無訊息** | 布林值 | false | true 表示將音效設定為靜音。false 表示允許播放快顯通知音訊 (預設)。 |
...
* [快速入門：傳送快顯通知及控制代碼的本機啟動](https://blogs.msdn.com/b/tiles_and_toasts/archive/2015/07/08/quickstart-sending-a-local-toast-notification-and-handling-activations-from-it-windows-10.aspx)
