...
> [!IMPORTANT]
> **需要年度更新版**:您必須執行組建 14393 或更新版本，才能看到通知鏡像的工作。 如果您想讓您的應用程式退出通知鏡像，您必須以 SDK 14393 為目標來存取鏡像 API。
...
開發人員從通知鏡像取得兩個好處：鏡像的通知會導致更多的使用者參與，與您的服務，同時還協助使用者探索 Microsoft Store 傳統型應用程式 ！ 您的使用者可能根本不知道您有適用於 Windows 10 桌面的絕佳 UWP app。 當使用者從他們的手機收到鏡像的通知時，使用者可以按一下快顯通知，即可前往 Microsoft Store，他們可以在其中安裝 UWP 傳統型應用程式。
...
**已知問題**:擷取鏡像屬性，透過`ToastNotificationHistory.GetHistory()`API 一律會傳回預設值 (**允許**) 而不是您所指定的選項。 請不要擔心，所有項目都能運作 - 它只是擷取已損壞的值。
...
某些開發人員可能會選擇將其應用程式完全退出通知鏡像。 雖然我們認為所有的應用程式受益於鏡像，我們讓您輕鬆選擇退出。只要一次，呼叫下列方法和您的應用程式將會選擇退出。比方說，您可以將此呼叫放在您的應用程式建構函式內`App.xaml.cs`...
