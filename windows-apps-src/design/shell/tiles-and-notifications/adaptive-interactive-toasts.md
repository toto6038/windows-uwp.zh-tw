...
快顯通知內容的核心元件為...
* **啟動**:這會定義哪些引數會傳遞給您的應用程式，當使用者按一下您的快顯通知，讓您能夠深層連結至已顯示快顯通知的正確內容。 若要深入了解，請參閱[傳送本機快顯通知](send-local-toast.md)。
* **visual**:快顯通知的視覺化部分，包括繫結的泛型包含文字和影像。
* **動作**:快顯通知，包括輸入和動作的互動式的部分。
* **音訊**:控制項的音訊播放時快顯通知會顯示給使用者。
...
**在年度更新版的新**:快顯通知可以顯示英雄影像，也就是精選[ **ToastGenericHeroImage** ](toast-schema.md#toastgenericheroimage)醒目顯示快顯橫幅中，並在 行動作業中心內。 100% 縮放比例的影像尺寸為 364x180 像素。
...
**在年度更新版的新**:如果您需要參考內容的來源，您可以使用屬性的文字。 此文字永遠和通知身分識別或通知時間戳記一起顯示在通知的底部。
...
**Creators Update 的新**:您現在可以覆寫的系統提供的時間戳記具有自己準確地呈現產生訊息/資訊/內容時的時間戳記。 此時間戳記可顯示在控制中心內。
...
**Creators Update 的新**:您可以在您的快顯通知來通知進度的作業，例如下載和多個使用者提供進度列。
...
**Creators Update 的新**:您可以群組在動作中心內的標頭的通知。 例如，您可以將來自某個群組聊天的群組訊息分組在一個標頭下，或將常見主題的通知分組在一個標頭下，以此類推。
...
**在年度更新版的新**:除了上述所指定的內容，您也可以顯示其他的自適性內容時會顯示快顯通知會展開。
...
**使用全新的 Fall Creators Update**:在背景啟用按鈕，您可以使用之後啟動的行為**PendingUpdate**在快顯通知建立多重步驟的互動。 當使用者按下您的按鈕時，便會啟用背景工作，快顯通知將進入「擱置中的更新」狀態，它會停留在螢幕上，直到背景工作使用新的快顯通知來取代快顯通知。
...
**在年度更新版的新**:您可以將額外的操作功能表動作加入現有的操作功能表出現在使用者以滑鼠右鍵按一下您從 行動作業中心內的快顯。 請注意，只有在控制中心內以滑鼠右鍵按一下時，才會出現這個功能表。 以滑鼠右鍵按一下快顯通知快顯橫幅並不會出現此功能表。
...
* **提醒**:通知會留在畫面中，直到使用者關閉為止，或採取行動。 在 Windows Mobile 上，快顯通知也會以預先展開的方式顯示。 將會播放提醒音效。
* **警示**:除了提醒行為中，警示會另外迴圈具有預設警示音效的音訊。
