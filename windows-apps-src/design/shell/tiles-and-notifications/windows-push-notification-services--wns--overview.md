...
ms.locfileid: "89494374"
---
# <a name="windows-push-notification-services-wns-overview"></a>Windows 推播通知服務 (WNS) 概觀 

Windows 推播 Notification Services (WNS) 讓協力廠商開發人員可以從自己的雲端服務傳送快顯、磚、徽章和原始更新。 這提供一種機制，用省電又可靠的方法，將最新的更新資訊傳送給使用者。
...
### <a name="step-1-register-your-app-with-the-dashboard"></a>步驟1：向儀表板註冊您的應用程式

您的應用程式必須先向合作夥伴中心的儀表板註冊，您才能透過 WNS 傳送通知。 這樣做會將您應用程式的認證提供給您，您的雲端服務向 WNS 進行驗證時要使用該認證。 這些認證由套件安全性識別碼 (SID) 與祕密金鑰組成。 若要執行此註冊，請登入 [合作夥伴中心](https://partner.microsoft.com/dashboard)。 建立您的應用程式之後，請參閱此頁面上的 [**線上服務網站**] 連結) ，以瞭解如何取得認證 (的 [[產品管理-WNS/MPNS](https://apps.dev.microsoft.com/) instrunctions]。
...
4.    藉由保留應用程式名稱來建立您的應用程式。 為您的應用程式提供唯一的名稱。 輸入名稱，然後按一下 [保留產品名稱] 按鈕。 如果名稱可供使用，則會保留給您的應用程式。 成功為您的應用程式保留名稱之後，如果您目前選擇這樣做，其他詳細資料就會變成可供修改。
...
當您為應用程式保留名稱時，Windows Store 會建立相關聯的認證。 它也指派了相關聯的識別值（name 和發行者），這些值必須存在於您應用程式的資訊清單檔中， (package.appxmanifest) 。 如果您已將應用程式上傳至 Windows 市集中，這些值就會自動新增至您的資訊清單。 如果您尚未上傳您的應用程式，則必須手動將身分識別值新增至您的資訊清單。
...
3.    在 [WNS/MPNS] 頁面上，按一下 [Windows 推播 Notification Services] (WNS) 和 Microsoft Azure 行動服務] 區段下找到的 [Live Services 網站] 連結。

![wns 即時服務](../images/wns-live-services-page.png)
 
4.    應用程式註冊入口網站 (先前的 [即時服務] 頁面) 頁面會提供您要包含在應用程式資訊清單中的身分識別元素。 這包括應用程式密碼 (s) 、封裝安全識別碼和應用程式識別碼。 在文字編輯器中開啟您的資訊清單，並在頁面指示的情況中加入該專案。    

> [!NOTE]
> 如果您使用 AAD 帳戶登入，則必須與註冊該應用程式的 Microsoft 帳戶擁有者取得相關聯的應用程式秘密。 如果您需要尋找此連絡人的協助，請按一下畫面右上角的齒輪，然後按一下 [開發人員設定]，並在該處顯示使用其 Microsoft 帳戶建立應用程式之人員的電子郵件地址。
 
5.    將 SID 和用戶端密碼上傳至您的雲端伺服器。

> [!Important]
> 您的雲端服務應該安全地儲存和存取 SID 和用戶端密碼。 此資訊的洩漏或遭竊可能會讓攻擊者在沒有您的許可或知識的情況下，傳送通知給您的使用者。

## <a name="requesting-a-notification-channel"></a>要求通知通道

執行可接收推播通知的應用程式時，該應用程式必須先透過 [**CreatePushNotificationChannelForApplicationAsync**](/uwp/api/Windows.Networking.PushNotifications.PushNotificationChannelManager#Windows_Networking_PushNotifications_PushNotificationChannelManager_CreatePushNotificationChannelForApplicationAsync_System_String_) 要求通知通道。 如需查看完整討論與程式碼範例，請參閱[如何要求、建立以及儲存通知通道](/previous-versions/windows/apps/hh465412(v=win.10))。 這個 API 可傳回以唯一方式連結至呼叫端應用程式及其磚的通道 URI，而所有的通知類型都可以透過它來傳送。
...
-   雲端服務務必確定通道 URI 使用「notify.windows.com」網域，這一點非常重要。 在任何情況下服務都不可以將通知推播至其他任何網域的通道。 如果應用程式的回呼遭到竄改，惡意攻擊者可能會提交通道 URI 來詐騙 WNS。 在不檢查網域的情況下，您的雲端服務可能會在不知情的情況下將資訊洩漏給攻擊者。
-   如果您的雲端服務嘗試將通知傳遞到已過期的通道，WNS 將傳回[回應碼 410](/previous-versions/windows/apps/hh465435(v=win.10))。 您回應這該代碼的方式為讓您的服務不繼續嘗試傳送通知到該 URI。
...
在向 WNS 進行驗證時，雲端服務會透過安全通訊端階層 (SSL) 提交 HTTP 要求。 參數使用「application/x-www-for-urlencoded」格式提供。 在 [用戶端識別碼] 欄位中提供您的套件 SID \_ ，並在 [用戶端密碼] 欄位中提供秘密金鑰， \_ 如下列範例所示。 如需語法詳細資訊，請參閱[存取權杖要求](/previous-versions/windows/apps/hh465435(v=win.10))參考。

> [!NOTE]
> 這只是一個範例，而非剪下和貼上的程式碼，您可以在自己的程式碼中順利使用。 
...
以下範例顯示成功的驗證回應，其中包括存取權杖。 如需語法詳細資訊，請參閱[推播通知服務要求和回應標頭](/previous-versions/windows/apps/hh465435(v=win.10))。
...
上述存取權杖可在多個通知要求重複使用；雲端伺服器不需要針對每個通知要求新的存取權杖。 如果存取權杖已經過期，通知要求會傳回錯誤。 如果存取權杖被拒絕，建議您不要嘗試多次重新傳送您的通知。 如果遇到這種錯誤，您必須要求新存取權杖並重新傳送通知。 如需確切的錯誤碼，請參閱[推播通知回應碼](/previous-versions/windows/apps/hh465435(v=win.10))。

1.  雲端服務向通道 URI 發出 HTTP POST。 這項要求必須透過 SSL 傳送，並且要包含必要的標頭與通知承載。 授權標頭必須包含取得的存取權杖以進行授權。

    以下提供一個要求範例。 如需語法詳細資訊，請參閱[推播通知回應碼](/previous-versions/windows/apps/hh465435(v=win.10))。

    如需製作通知承載的詳細資訊，請參閱[快速入門：傳送推播通知](/previous-versions/windows/apps/hh868252(v=win.10))。 磚、快顯通知或徽章推播通知的承載是以 XML 內容來提供的，它們遵守各自已定義的[彈性磚結構描述](adaptive-tiles-schema.md)或[傳統磚結構描述](/uwp/schemas/tiles/tiles-xml-schema-portal)。 原始通知的承載則沒有指定的結構。 它完全是由應用程式定義的。
...
-   當裝置離線時，WNS 預設將針對每個應用程式最多儲存五個磚通知 (已啟用佇列的情況，否則只儲存一個磚通知)，以及針對每個通道 URI 儲存一個徽章通知，而且不會有任何原始通知。 這個預設的快取行為可以透過 [X-WNS-Cache-Policy 標頭](/previous-versions/windows/apps/hh465435(v=win.10))來變更。 請注意，當裝置離線時，絕不會儲存快顯通知。
...
-   Windows 通知平臺會維護與 WNS 的定期資料連線，以保持通訊端保持運作狀態良好。 如果沒有任何應用程式要求或使用通知通道，則不會建立通訊端。
...
您的雲端服務可以藉由設定 X-WNS-TTL HTTP 標頭，指定在傳送通知之後該通知將維持有效狀態的時間 (秒)，以便設定每個通知的到期時間。 如需詳細資訊，請參閱 [推播通知服務要求和回應標頭](/previous-versions/windows/apps/hh465435(v=win.10))。
...
沒有任何方式檢查這兩個設定的狀態，無法您可以檢查省電模式的狀態。 在 Windows 10 中，使用 [**EnergySaverStatus**](/uwp/api/Windows.System.Power.PowerManager.EnergySaverStatus) 屬性檢查省電模式狀態。 您的應用程式也可以使用 [**EnergySaverStatusChanged**](/uwp/api/Windows.System.Power.PowerManager.EnergySaverStatusChanged) 事件接聽省電模式的變更。
...
> [!TIP]
> 當通知使用者省電模式設定時，建議您提供一種方式來抑制未來的訊息。 例如，以下範例中的 `dontAskMeAgainBox` 核取方塊會在 [**LocalSettings**](/uwp/api/Windows.Storage.ApplicationData.LocalSettings) 中保存使用者的喜好設定。

以下是如何檢查 Windows 10 中是否已開啟省電模式的範例。 此範例會通知使用者並啟動 [設定] 應用程式以進入**省電模式設定**。 如果使用者不想再收到通知，`dontAskAgainSetting` 可讓他們隱藏訊息。
...
這是此範例之 [**ContentDialog**](/uwp/api/Windows.UI.Xaml.Controls.ContentDialog) 的 XAML。
...
* [如何透過推播通知更新徽章](/previous-versions/windows/apps/hh465450(v=win.10))
