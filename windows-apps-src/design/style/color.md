...
## <a name="scoping-system-colors"></a>設定範圍系統色彩

除了在您的應用程式中定義您自己的色彩，您可以也將到所需的地區我們 systematized 的色彩在您的應用程式的整個範圍使用**ColorSchemeResources**標記。 此 API 可讓您不僅要以色彩標示和佈景主題大型的控制項群組，一次是藉由設定一些屬性，但也可讓您許多其他系統優點，您通常不會取得以手動定義您自己自訂的色彩：

- 使用**ColorSchemeResources**設定任何色彩不會作用高對比佈景主題
  * 這表示您的應用程式都可以存取更多人不需要任何額外的設計或開發人員成本
- 可以輕鬆地色彩為淺色、 深色或普遍跨兩個佈景主題上設定設定的一個屬性 API
- 色彩**ColorSchemeResources**上設定將會重疊顯示下所有相似的控制項也會使用該系統色彩
  * 這樣可確保您會有一致的色彩故事整個應用程式同時維持您的品牌的外觀
- 效果所有的視覺狀態、 動畫和不透明度變化，而不需要重新套用範本

### <a name="how-to-use-colorschemeresources"></a>如何使用 ColorSchemeResources

ColorSchemeResources 是 API，以告知正在進行哪些資源系統已限定範圍的位置。 ColorSchemeResources 必須採取一種[X:key](https://docs.microsoft.com/windows/uwp/xaml-platform/x-key-attribute)，可以是三個選項的其中一個：
- 預設值
  * 會顯示在[淺色](https://docs.microsoft.com/windows/uwp/design/style/color#light-theme)和[深色](https://docs.microsoft.com/windows/uwp/design/style/color#dark-theme)佈景主題中的色彩變更
- 光源
...
設定該 X:key 可確保您的色彩適當地變更系統或應用程式佈景主題中，您應該要在任一種佈景主題中的不同自訂外觀。

### <a name="how-to-apply-scoped-colors"></a>如何套用限定範圍的色彩

設定範圍透過**ColorSchemeResources** API 在 XAML 中的資源，可讓您充分任何系統色彩或筆刷，在我們的[佈景主題資源](https://docs.microsoft.com/windows/uwp/design/controls-and-patterns/xaml-theme-resources)庫，並重新定義它們的頁面或容器的範圍內。

例如，如果您定義兩個系統色彩- **SystemBaseLowColor**和**SystemBaseMediumLowColor**內 grid，並再放在頁面上的兩個按鈕： 在該格線內的第一個和一個外部：
...
不過，因為我們所有的系統色彩串聯到其他控制項，設定**SystemBaseLowColor**和**SystemBaseMediumLowColor**會影響不僅僅是按鈕。 在此情況下，會控制像**ToggleButton**、**選項按鈕**和**滑桿**會也會受到這些系統色彩會變更，應該這些控制項放上方 exampl 格線的範圍。
...
```
您基本上有完全相同的動作之前，但現在任何其他控制項新增至方格會被選取，色彩會變更。 這是因為這些系統色彩的僅**Button_A**範圍。

### <a name="nesting-scoped-resources"></a>巢狀結構已限定範圍，資源

巢狀系統色彩也是可能的時候，並會藉由在您的應用程式的版面配置的標記內的巢狀項目資源中放置**ColorSchemeResources**這樣做：
...
在此範例中， **Button_A**繼承**Grid_A**的資源中定義的色彩和**巢狀按鈕**繼承自**Grid_B**的資源的色彩。 透過擴充方式，這表示任何其他控制項放在**Grid_B**會檢查或檢查或套用**Grid_A**的資源之前先，套用**Grid_B**的資源，最後套用我們的預設色彩，如果不顯示任何資訊定義在頁面或應用程式層級。

這適用於任何數目的巢狀項目，將其資源有色彩定義。

### <a name="scoping-with-a-resourcedictionary"></a>設定範圍的 ResourceDictionary

您並不受限於容器或頁面的資源，並也可以合併在任何範圍的方式，您通常會合併字典 ResourceDictionary 中定義這些系統色彩。

#### <a name="mycustomthemexaml"></a>MyCustomTheme.xaml

首先，您會建立 ResourceDictionary。 然後放置內 ThemeDictionaries **ColorPaletteResources**並覆寫所需的系統的色彩：
...
現在，您可以是所有的資源、 佈景主題設定，以及自訂色彩放在單一**MyCustomTheme**資源字典中並已限定範圍，而不必擔心您的版面配置標記中的額外雜亂所需的位置。
