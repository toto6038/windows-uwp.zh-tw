...
**若要測試您的應用程式使用 [朗讀程式]，使用滑鼠與鍵盤，使用下列步驟：**
...
1.  熟悉 UI 並探索配置。
    ...
    * **巡覽 UI，使用單指撥動手勢。** 使用向左或向右撥動以在項目之間移動，並使用向上或向下撥動來變更瀏覽的項目類別。 類別包含所有項目、連結、表格、標頭等。 利用單指撥動手勢進行瀏覽類似於使用 _Caps Lock + 方向鍵_來瀏覽。
    * **您可以使用筆勢 索引標籤來瀏覽可焦點化項目。** 使用三指向右或向左撥動，就和使用鍵盤上的 _Tab_ 鍵與 _Shift + Tab_ 鍵來瀏覽一樣。
    * **在空間上調查單一手指的 UI。** 使用單指向上和向下拖曳，或是向左和向右拖曳，可以讓朗讀程式閱讀您手指下方的項目。 您可以使用滑鼠做為替代選項，因為滑鼠會使用和單指拖曳相同的點擊測試邏輯。
...
2.  與控制項互動，以測試它的主要和次要動作及其捲動行為。
    ...
    * 若要測試的主要動作：Double 點選，或一隻手指與按下，點選與另一個。
    * 若要測試的第二個動作：三倍點選，或按下一隻手指與另一個連點兩。
    * 若要測試捲動行為：您可以使用兩指 swipes 的所需的方向捲動。
