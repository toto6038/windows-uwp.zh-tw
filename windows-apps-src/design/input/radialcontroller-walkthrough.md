...
2. 尋找以這個步驟的標題標示的程式碼 ( " \<!-- Step 3: Add controls for wheel input --> " ) 。
3. 取消註解下列行。
...
5. 尋找以這個步驟的標題標示的程式碼 ( " \<!-- Step 6: Customize the device haptics --> " ) 。
...
9. 取消註解下列行：  
    ...
    - [Windows.Devices.Haptics](/uwp/api/windows.devices.haptics) 類型參考適用於後續步驟中的功能。  
