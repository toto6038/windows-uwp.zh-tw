...
![使用介面的 Studio 介面撥號的映像](images/radialcontroller/dial-pen-studio-600px.png)  
...
如需有關實作這些其他功能的詳細資訊，請參閱 [UWP app 中的 Surface Dial 互動](windows-wheel-interactions.md)。
...
> [!NOTE]
> 或者，您可以選取**偵錯** > **開始偵錯**功能表項目或選取**本機**執行如下所示的按鈕：![Visual Studio 建置專案的按鈕](images/radialcontroller/wheel-vsrun.png)
...
## <a name="step-3-add-controls-for-wheel-input"></a>步驟 3：新增滾輪輸入的控制項

首先，加入我們的 App 的 UI：

1. 請開啟 MainPage_Basic.xaml 檔案，
2. 尋找此步驟中的標題以標示的程式碼 ("\<！-步驟 3:新增滾輪輸入的控制項-->")。
3. 取消註解下列行。
...
## <a name="step-4-customize-the-basic-radialcontroller-menu"></a>步驟 4：自訂基本的 RadialController 功能表

現在我們來新增所需的程式碼，以啟用 **RadialController** 的控制項存取權。

1. 請開啟 MainPage_Basic.xaml.cs 檔案，
2. 尋找此步驟中的標題以標示的程式碼 ("/ / 步驟 4:基本 RadialController 功能表自訂 」）。
3. 取消註解下列行：
...
1. 請開啟 MainPage_Basic.xaml.cs 檔案，
2. 尋找此步驟中的標題以標示的程式碼 ("/ / 步驟 5:設定功能表在執行階段 」）。
...
## <a name="step-6-customize-the-device-haptics"></a>步驟 6：自訂裝置 haptics
...
5. 尋找此步驟中的標題以標示的程式碼 ("\<！-步驟 6:自訂 haptics--> 的裝置 」)。
...
8. 尋找此步驟中的標題以標示的程式碼 (「 步驟 6:Haptics 自訂 」）
9. 取消註解下列行：  
...
## <a name="step-7-define-on-screen-interactions-for-surface-studio-and-similar-devices"></a>步驟 7：螢幕上定義的介面 Studio 和類似的裝置互動
...
3. 開啟 App.xaml.cs 檔案
4. 尋找此步驟中的標題以標示的程式碼 (「 步驟 7:定義螢幕互動介面 Studio 和類似的裝置 」)
...
[介面的撥號互動](windows-wheel-interactions.md)
