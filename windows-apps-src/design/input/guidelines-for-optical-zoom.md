...
**視覺化縮放**:使用者選取區域後，，然後拉近到整個區域。

![將手指靠攏會縮小，將手指分開則會放大。](images/areazoom.png)

**調整大小**:使用者選取區域後中的物件，並調整大小，該物件。

![將手指靠攏會縮小物件，將手指分開會放大物件。](images/objectresize.png)

**附註**  光學縮放不應該與混淆[語意縮放](../controls-and-patterns/semantic-zoom.md)。 雖然這兩個互動使用相同的手勢，但語意式縮放是指呈現和瀏覽在單一檢視內 (例如，電腦的資料夾結構、文件庫或相簿) 組織的內容。
...
-   使用慣性物理。 包括以下各項：
    -   減速程度：發生於使用者停止進行捏合或延伸。 這類似於在光滑的表面上滑動到停止的現象。
    -   退件：界限傳遞或大小條件約束時，就會發生些微的退件後生效。
...
-   不要將可調整大小的物件放在可調整大小的內容區域中。 例外狀況包括：
    ...
    **附註**  在所有情況下，除非可調整大小的物件內的所有觸控點，調整內容區域。
