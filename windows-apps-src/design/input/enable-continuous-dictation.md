...
- 初始化語音辨識器。
- 編譯內建聽寫文法。
    **請注意**   語音辨識至少需要一個條件約束，才能定義可辨識的詞彙。 如果沒有指定任何限制式，則會使用預先定義的聽寫文法。 請參閱[語音辨識](speech-recognition.md)。
- 設定辨識事件的事件接聽程式。
