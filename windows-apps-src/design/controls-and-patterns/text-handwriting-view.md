...
針對 [TextBox](/uwp/api/windows.ui.xaml.controls.textbox)[RichEditBox](/uwp/api/windows.ui.xaml.controls.richeditbox) 之類的 Windows 文字控制項，以及可衍生自 [AutoSuggestBox](/uwp/api/windows.ui.xaml.controls.autosuggestbox) 等控制項所支援的筆跡轉換文字輸入，自訂內建的手寫檢視。
...
在此範例中，我們將 [TextBox](/uwp/api/windows.ui.xaml.controls.textbox) 控制項的 [IsHandwritingViewEnabled](/uwp/api/windows.ui.xaml.controls.textbox.ishandwritingviewenabled) 屬性設為 false，藉此停用手寫檢視。 支援手寫檢視的所有文字控制項都支援類似的屬性。
...
在此，我們示範如何使用 [TextBox](/uwp/api/windows.ui.xaml.controls.textbox)[HandwritingView](/uwp/api/windows.ui.xaml.controls.handwritingview) 的 [PlacementAlignment](/uwp/api/windows.ui.xaml.controls.handwritingview.placementalignment) 屬性，指定基礎文字控制項上的哪個錨點會用來對齊手寫檢視。
...
如果您的應用程式已經提供強大的自訂辨識功能，您可以使用 [AreCandidatesEnabled](/uwp/api/windows.ui.xaml.controls.handwritingview.arecandidatesenabled) 屬性來停用內建建議，如以下範例所示。
...
在此範例中，我們會接聽 [TextBox](/uwp/api/windows.ui.xaml.controls.textbox)的 [TextChanged](/uwp/api/windows.ui.xaml.controls.textbox.textchanged) 事件，如果文字變更源自 HandwritingView 則套用使用者選取的字型 (如果不是，則套用預設字型)。
...
使用 [TextBox](/uwp/api/windows.ui.xaml.controls.textbox) 或 [RichEditBox](/uwp/api/windows.ui.xaml.controls.richeditbox) 控制項的複合控制項 (例如 [AutoSuggestBox](/uwp/api/windows.ui.xaml.controls.autosuggestbox)) 也支援 [HandwritingView](/uwp/api/windows.ui.xaml.controls.handwritingview)。

若要存取複合控制項中的 [HandwritingView](/uwp/api/windows.ui.xaml.controls.handwritingview)，請使用 [VisualTreeHelper](/uwp/api/windows.ui.xaml.media.visualtreehelper) API。

下列 XAML 程式碼片段會顯示 [AutoSuggestBox](/uwp/api/windows.ui.xaml.controls.autosuggestbox) 控制項。
...
在對應的程式碼後置中，我們會示範如何停用 [AutoSuggestBox](/uwp/api/windows.ui.xaml.controls.autosuggestbox)上的 [HandwritingView](/uwp/api/windows.ui.xaml.controls.handwritingview)。
...
在某些情況下，您可能需要確保 [HandwritingView](/uwp/api/windows.ui.xaml.controls.handwritingview) 涵蓋它可能沒有的 UI 元素。
...
您也可以設定 [HandwritingView](/uwp/api/windows.ui.xaml.controls.handwritingview) 的大小，當您必須確保檢視不會遮蔽重要 UI 時，這很有用。
...
下列範例示範如何接聽 [HandwritingView](/uwp/api/windows.ui.xaml.controls.handwritingview)的 [Opened](/uwp/api/windows.ui.xaml.controls.handwritingview.opened)、[Closed](/uwp/api/windows.ui.xaml.controls.handwritingview.closed)和 [SizeChanged](/uwp/api/windows.ui.xaml.frameworkelement.sizechanged) 事件，以設定 [Popup](/uwp/api/windows.ui.popups) 的位置。
