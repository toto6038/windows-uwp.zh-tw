...
ms.locfileid: "9116280"
---
# <a name="text-input-with-the-handwriting-view"></a>文字輸入使用手寫檢視

![文字方塊在使用手寫筆點選時展開](images/handwritingview/handwritingview2.gif)

自訂 / [筆跡轉換[TextBox](https://docs.microsoft.com/en-us/uwp/api/windows.ui.xaml.controls.textbox)、 [RichEditBox](https://docs.microsoft.com/uwp/api/windows.ui.xaml.controls.richeditbox)，例如 UWP 文字控制項所支援的文字輸入的內建的手寫檢視和控制項衍生自這些例如[AutoSuggestBox](https://docs.microsoft.com/uwp/api/windows.ui.xaml.controls.autosuggestbox)。
...
手寫檢視是位於基礎的文字控制項的上方，並調整大小以容納使用者的手寫喜好設定 (請參閱設定 **-> 裝置-> 手寫筆 & Windows 筆跡-> 手寫-> 字型大小時直接寫入文字欄位**). 檢視也會自動對齊相對於文字控制項和其應用程式內的位置。
...
使用者可以從集合中選擇預先定義的手寫型時，要使用的字型根據筆墨辨識轉譯文字 (請參閱**設定-> 裝置-> Windows Ink 的手寫筆 &-> 手寫-> 字型時使用手寫**)。
...
在此範例中，我們會接聽的[TextBox](https://docs.microsoft.com/en-us/uwp/api/windows.ui.xaml.controls.textbox) [TextChanged](https://docs.microsoft.com/uwp/api/windows.ui.xaml.controls.textbox.textchanged)事件，並套用使用者選取的字型，如果文字變更來自 HandwritingView （或預設字型，如果不是）。
...
對應的程式碼後置中，我們會示範如何停用[HandwritingView](https://docs.microsoft.com/uwp/api/windows.ui.xaml.controls.handwritingview) [AutoSuggestBox](https://docs.microsoft.com/uwp/api/windows.ui.xaml.controls.autosuggestbox)上。
