...
ListView 和 GridView，例如 ComboBox 被衍生自[選取器](/uwp/api/windows.ui.xaml.controls.primitives.selector)，讓您可以取得使用者的選取項目中的相同的標準方式。

您可以取得或設定下拉式方塊選取項目使用[SelectedItem](/uwp/api/windows.ui.xaml.controls.primitives.selector.selecteditem)屬性，並取得或透過使用[SelectedIndex](/uwp/api/windows.ui.xaml.controls.primitives.selector.selectedindex)屬性以設定選取的項目索引。
...
在之前 RS5，SelectedItem 屬性的值 (因此 SelectedValue 和 SelectedIndex) 才能能在下拉式方塊的項目集合中。 使用上一個範例中，設定`colorComboBox.SelectedItem = "Pink"`會導致：

- SelectedItem = null
- SelectedValue = null
- SelectedIndex =-1

在 RS5 及更新版本，SelectedItem 屬性的值 (因此 SelectedValue 和 SelectedIndex) 並不必然是下拉式方塊的項目集合中。 使用上一個範例中，設定`colorComboBox.SelectedItem = "Pink"`會導致：
...
下拉式方塊可自動支援在其集合內的搜尋。 當焦點在一個已開啟或關閉的下拉式方塊上時，如果使用者在實體鍵盤上輸入字元，就會顯示與使用者的字串相符的候選項目。 在瀏覽長清單時，這項功能特別有幫助。 例如時與包含狀態清單的下拉式清單下互動，, 使用者可以按"w"鍵來顯示"Washington 」 以供快速選取。 文字搜尋不區分大小寫。
...
「 最近使用的名稱 「 選擇器可讓使用者輸入自訂字串。 'RecentlyUsedNames' 清單包含一些值，使用者可以選擇從，但是使用者也可以新增新的、 自訂的值。 'CurrentName' 屬性代表目前的輸入的名稱。
...
在此 examle，字型大小選擇包含一組值對應到字型大小坡形，但使用者可以輸入不是在清單中的字型大小。

當使用者將新增不在清單中，字型大小更新，但值的值不會新增到文字大小的清單。
...
在這裡，「 最愛的色彩選擇器 」 包含的最常見的喜愛色彩紅色、 藍色，綠色 （橘色），但是使用者可以輸入最愛的色彩不在清單中。 當使用者加入 （例如粉紅） 是有效的色彩時，新輸入的色彩已新增到清單，並設定為使用中的 「 最愛的色彩 」。
