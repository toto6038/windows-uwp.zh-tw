...
您可以建立樹狀檢視繫結到階層式資料來源的[ItemsSource](/uwp/api/windows.ui.xaml.controls.treeview.itemssource)或您可以建立並管理您自己的 TreeViewNode 物件。
...
您可以選擇設定不同 DataTemplate 根據項目類型的樹狀結構檢視項目。 例如，在檔案總管] app 中，您可以使用一個資料範本的資料夾，和另一個用於檔案。
