...
當您安裝包含當地語系化的語言資料的應用程式時，您可能會發現預設語言是適用於應用程式，即使您一開始包含多種語言的資源。 這是因為安裝程序最佳化，以將只能安裝語言資源符合目前的語言和文化特性的裝置。 因此，如果您的裝置已設定為 EN-US，只有 EN-US 語言資源會以您的應用程式安裝。

> [!NOTE]
> 不可能在初始安裝之後安裝您的應用程式的其他語言支援。 如果您變更預設語言安裝應用程式之後，應用程式會繼續使用原始的語言資源。

如果您想要確保所有的語言資源可供在安裝完成後，建立設定檔，指定特定的資源 （包括語言資源） 安裝期間所需的應用程式套件。 在封裝期間產生您的應用程式.appxbundle 時，會自動啟用此最佳化的安裝功能。 如需詳細資訊，請參閱[確保無論裝置是否需要它們在裝置上已安裝資源](https://docs.microsoft.com/en-us/previous-versions/dn482043(v=vs.140))。

或者，您可以停用.appxbundle 世代，當您封裝您的應用程式，以確保所有的資源會安裝 （而不只是子集）。 建議您不要不過因為它可以提高您的應用程式的安裝時間。
