...
不要在您的命令式程式碼、 XAML 標記中，或您的應用程式套件資訊清單中硬式編碼字串常值。 相反的，請將您的字串放入資源檔 (.resw)，使其可獨立於您應用程式的建置二進位檔之外，適應不同的本地市場。 如需詳細資料，請參閱[當地語系化您 UI 和應用程式封裝資訊清單中的字串](../../app-resources/localize-strings-ui-manifest.md)。
...
另一個範例是，假設有一個句子"提醒我在{0}minute。 」 使用 "minute(s)" (分鐘) 適用於英文，但其他語言可能會使用不同的詞彙。 例如，波蘭文會根據上下文使用 "minuta"、"minuty" 或 "minut"。
...
當您安裝應用程式，其中包含當地語系化的語言資料時，您可能會發現預設語言是適用於應用程式，即使您一開始包含多種語言的資源。 這是裝置的因為安裝程序最佳化，以將只能安裝語言資源符合目前的語言和文化特性。 因此，如果您的裝置已設定為 EN-US，只 EN-US 語言資源會以您的應用程式安裝。

> [!NOTE]
> 不可能在初始安裝之後安裝您的應用程式的其他語言支援。 如果您安裝 app 之後變更預設語言，應用程式會繼續使用原始語言資源。

如果您想要確保所有的語言資源可供在安裝完成後，建立設定檔，指定特定的資源 （包括語言資源） 安裝期間所需的應用程式套件。 在封裝期間產生您的應用程式.appxbundle 時，會自動啟用這項最佳化的安裝功能。 如需詳細資訊，請參閱[確保無論裝置是否需要它們在裝置上已安裝資源](https://docs.microsoft.com/en-us/previous-versions/dn482043(v=vs.140))。

或者，您可以停用.appxbundle 世代，當您封裝應用程式，以確保所有的資源會安裝 （而不只是子集）。 建議您不要不過因為它可能會增加應用程式的安裝時間。

「 產生應用程式套件組合 」 將屬性設定為 「 永遠不會 」 以停用自動產生.appxbundle:

1. 在 Visual Studio 中，以滑鼠右鍵按一下專案名稱
2. 選取 [**市集** -> **建立應用程式套件...**
3. 在**建立您的套件**] 對話方塊中，選取**我想要建立套件以上傳到 Microsoft Store 使用新的應用程式名稱**，然後按一下 [**下一步**。
4. 在 [**選取的應用程式名稱**] 對話方塊中，選取/建立能進行 app 命名為您的套件。
5. 在 [**選取並設定套件**] 對話方塊中，設定為**永不**的**產生應用程式套件組合**。
...
此範例中的格式化字串在英文 (美國) 中可正常運作。 但，例如，在德文 (德國) 中則不適當，因為日期和月份的顯示順序是相反的。 確保翻譯人員會知道每個參數的意圖，以便他們可以反轉格式化字串中格式化項目的順序 (例如，「{1} {0}」) 為目標語言適當的。
...
> [!NOTE]
> 當地語系化也可能是所需的其他資產，包括影像和音訊檔案。

您也應該考慮下列內容：

- **當地語系化工具**有許多當地語系化工具可供剖析資源檔案，並且讓僅可翻譯的字串來翻譯人員只能編輯。 這個方法可以降低翻譯人員不小心編輯到 XML 標記的風險。 但是它的缺點是要學習使用新的工具及當地語系化程序。 如果是含有大量字串但少量語言的專案，則適合使用當地語系化工具。 若要深入了解，請參閱[如何使用多語應用程式工具組](https://msdn.microsoft.com/en-us/library/windows/apps/xaml/jj572370.aspx)。
- **當地語系化廠商**請考慮使用當地語系化廠商，如果您的應用程式包含大量的字串，且需要翻譯為到大量的語言。 當地語系化廠商可以提供工具和程序，以及翻譯資源檔案的相關建議。 這是一個理想的解決方案，但也是最耗費成本的選項，而且可能會增加翻譯內容的往返時間。
...
- [應用程式資源和當地語系化範例](https://go.microsoft.com/fwlink/p/?linkid=254478)
