...
請勿在您的命令式程式碼、XAML 標記和您的應用程式套件資訊清單中，將字串常值硬式編碼。 相反的，請將您的字串放入資源檔 (.resw)，使其可獨立於您應用程式的建置二進位檔之外，適應不同的本地市場。 如需詳細資料，請參閱[當地語系化您 UI 和應用程式封裝資訊清單中的字串](../../app-resources/localize-strings-ui-manifest.md)。
...
可以取代各種單字，例如「約會」、「工作」 {0} 或「檔」。 雖然這個範例似乎對於英文沒有問題，但可能不見得適用於所有案例中的對應句子，例如德文。 請注意，在下列德文句子中，範本字串 ("Der"、"Die"、"Das") 中的部分字詞需要符合參數化的字詞：
...
另舉一個範例，請看一下「在幾分鐘內提醒我 {0} (s) 」。 使用 "minute(s)" (分鐘) 適用於英文，但其他語言可能會使用不同的詞彙。 例如，波蘭文會根據上下文使用 "minuta"、"minuty" 或 "minut"。
...
當您安裝包含當地語系化語言資料的應用程式時，您可能會發現應用程式只能使用預設語言，即使您一開始包含多種語言的資源也一樣。 這是因為安裝程式已優化，只會安裝符合裝置目前語言和文化特性的語言資源。 因此，如果您的裝置設定為 en-us，則只有 en-us 語言資源會與您的應用程式一起安裝。

> [!NOTE]
> 在初始安裝後，您無法為應用程式安裝其他語言支援。 如果您在安裝應用程式之後變更預設語言，則應用程式只會繼續使用原始語言資源。

如果您想要在安裝後確保所有語言資源都可供使用，請建立應用程式套件的設定檔，以指定安裝期間需要某些資源 (包括) 的語言資源。 當您的應用程式的 .appxbundle 在封裝期間產生時，就會自動啟用這項優化安裝功能。 如需詳細資訊，請參閱 [確保不論裝置是否需要，都已在裝置上安裝資源](/previous-versions/dn482043(v=vs.140))。

（選擇性）若要確保所有資源都已安裝 (而不只是子集) ，您可以在封裝應用程式時停用 .appxbundle 產生。 但不建議這麼做，因為它會增加應用程式的安裝時間。

將 [產生應用程式套件組合] 屬性設定為 [永不]，以停用 .appxbundle 的自動產生：

1. 在 Visual Studio 中，以滑鼠右鍵按一下專案名稱
2. 選取 [ **儲存**  ->  **建立應用程式套件** ]。
3. 在 [ **建立您的封裝** ] 對話方塊中，選取 [ **我要使用新的應用程式名稱來建立要上傳至 Microsoft Store 的封裝** ]，然後按 **[下一步]** 。
4. 在 [ **選取應用程式名稱** ] 對話方塊中，選取/建立套件的應用程式名稱。
5. 在 [ **選取和設定套件** ] 對話方塊中，將 [ **產生應用程式** 套件組合] 設定為 [ **永不** ]
...
此範例中的格式化字串在英文 (美國) 中可正常運作。 但，例如，在德文 (德國) 中則不適當，因為日期和月份的顯示順序是相反的。 確定 translator 知道每個參數的意圖，讓它們可以反轉格式字串中格式專案的順序 (例如，" {1} {0} " ) 適當的目的語言。
...
- **字串資源檔是 XML 或 Resources.resjson 文字格式，因此可以使用任何文字編輯器來遞交翻譯。轉譯後的檔案會複製回專案中。** 這個處理方式的風險在於翻譯人員可能會意外編輯 XML 標記，但是它讓翻譯人員可以在 Microsoft Visual Studio 專案以外的地方進行處理。 這個處理方式適合需要翻譯少數語言的專案。 XLIFF 格式是特別設計用於當地語系化的 XML 格式，一些當地語系化廠商或當地語系化工具應該都能支援。 您可以使用[多語應用程式工具組](/previous-versions/windows/apps/jj572370(v=win.10))，從其他資源檔案 (如 .resw 或 .resjson) 產生 XLIFF 檔案。

> [!NOTE]
> 其他資產（包括影像和音訊檔案）也可能需要當地語系化。

您也應該考慮下列事項：

- **當地語系化工具** 有一些當地語系化工具可用來剖析資源檔，並且只允許轉譯器編輯可翻譯的字串。 這個方法可以降低翻譯人員不小心編輯到 XML 標記的風險。 但是它的缺點是要學習使用新的工具及當地語系化程序。 如果是含有大量字串但少量語言的專案，則適合使用當地語系化工具。 若要深入了解，請參閱[如何使用多語應用程式工具組](/previous-versions/windows/apps/jj572370(v=win.10))。
...
*假名註解* 可解決此問題，方法是允許使用者或建立者為其使用的字元指定拼音。 如果您使用下列程序將假名註解加入應用程式名稱，可以確定它會在應用程式清單的正確位置進行排序。 如果應用程式名稱包含漢字字元，而且當使用者的 UI 語言或排序順序設定為日文時未提供假名註解，則 Windows 會盡最大的努力產生適當的發音。 不過，要排序的應用程式名稱可能會包含少見或獨特的讀法，而不是較常見的讀法。 因此，日文應用程式 (特別是名稱中包含漢字字元的應用程式) 的最佳做法是，在日文當地語系化程序期間提供假名註解版的應用程式名稱。
