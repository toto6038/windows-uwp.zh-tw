...
請不要在您的命令式程式碼，XAML 標記，也不在您的應用程式封裝資訊清單中的硬式編碼的字串常值。 相反的，請將您的字串放入資源檔 (.resw)，使其可獨立於您應用程式的建置二進位檔之外，適應不同的本地市場。 如需詳細資料，請參閱[當地語系化您 UI 和應用程式封裝資訊清單中的字串](../../app-resources/localize-strings-ui-manifest.md)。
...
無法取代各種不同的單字{0}，例如"約會 」、 「 工作 」 或 「 文件 」。 雖然這個範例似乎對於英文沒有問題，但可能不見得適用於所有案例中的對應句子，例如德文。 請注意，在下列德文句子中，範本字串 ("Der"、"Die"、"Das") 中的部分字詞需要符合參數化的字詞：
...
另舉一例，請考慮在句子"提醒我在{0}分鐘。 」 使用 "minute(s)" (分鐘) 適用於英文，但其他語言可能會使用不同的詞彙。 例如，波蘭文會根據上下文使用 "minuta"、"minuty" 或 "minut"。
...
當您安裝包含當地語系化的語言資料的應用程式時，您可能會發現只預設語言是可用的應用程式，即使您一開始所包含的多個語言資源。 這是因為安裝程序已經過最佳化，只安裝符合目前的語言和文化特性之裝置的語言資源。 因此，如果您的裝置設定為 EN-US，只有 EN-US 語言資源會以您的應用程式安裝。

> [!NOTE]
> 您不可以安裝在初始安裝後的應用程式的其他語言支援。 如果您變更預設語言，安裝應用程式之後，應用程式會繼續使用原始語言資源。

如果您想要確保安裝之後，所有語言資源都可供使用，建立應用程式套件，指定特定資源 （包括語言資源） 的安裝期間都所需的組態檔。 您的應用程式.appxbundle 封裝期間產生時，會自動啟用此最佳化的安裝功能。 如需詳細資訊，請參閱 <<c0> [ 確保不論裝置是否需要，在裝置上安裝資源](https://docs.microsoft.com/en-us/previous-versions/dn482043(v=vs.140))。

（選擇性） 以確保所有資源都都已安裝 （不只是部分），您可以停用.appxbundle 產生當您封裝應用程式。 建議您不要不過因為它可能會增加您的應用程式的安裝時間。

停用自動產生.appxbundle 」 產生的應用程式套件組合 」 將屬性設定為 「 從未 」:

1. 在 Visual Studio 中，以滑鼠右鍵按一下專案名稱
2. 選取 **存放區** -> **建立應用程式套件...**
3. 在 **建立您的套件**對話方塊中，選取**我想要建立套件以上傳到 Microsoft Store，使用新的應用程式名稱**，然後按一下 **下一步**。
4. 在 **選取 應用程式名稱**對話方塊中，選取/建立應用程式命名為您的封裝。
5. 在 **選取和設定套件**對話方塊中，將**產生應用程式套件組合**來**永不**。
...
此範例中的格式化字串在英文 (美國) 中可正常運作。 但，例如，在德文 (德國) 中則不適當，因為日期和月份的顯示順序是相反的。 確保當轉譯器，讓他們可以反轉資料的格式字串中的格式項目順序會知道每個參數的目的 (比方說，「{1} {0}") 為適用於目標語言。
...
- **資源檔可以轉譯直接在專案中開啟。** 這個方式適用於含有少量字串，且需要翻譯為兩到三種語言的專案。 如果開發人員了解多種語言，而且願意處理翻譯程序，則適用這個案例。 這個處理方式的好處是非常快速，不需要工具，而且可以將錯譯的風險降至最低。 但無法擴充。 特別是使用不同語言的資源很容易就變得不同步，因而產生不良的使用者經驗且不易維護。
- **字串資源檔是以 XML 或 ResJSON 文字格式，因此可以遞交給使用任何文字編輯器的翻譯。翻譯的檔案則會複製回專案。** 這個處理方式的風險在於翻譯人員可能會意外編輯 XML 標記，但是它讓翻譯人員可以在 Microsoft Visual Studio 專案以外的地方進行處理。 這個處理方式適合需要翻譯少數語言的專案。 XLIFF 格式是特別設計用於當地語系化的 XML 格式，一些當地語系化廠商或當地語系化工具應該都能支援。 您可以使用[多語應用程式工具組](https://docs.microsoft.com/previous-versions/windows/apps/jj572370(v=win.10))，從其他資源檔案 (如 .resw 或 .resjson) 產生 XLIFF 檔案。

> [!NOTE]
> 可能也需要其他資產，包括影像和音訊檔案的當地語系化。

您也應該考慮下列：

- **當地語系化工具**一些當地語系化工具可供剖析資源檔，並允許只可翻譯的字串轉譯程式編輯。 這個方法可以降低翻譯人員不小心編輯到 XML 標記的風險。 但是它的缺點是要學習使用新的工具及當地語系化程序。 如果是含有大量字串但少量語言的專案，則適合使用當地語系化工具。 若要深入了解，請參閱[如何使用多語應用程式工具組](https://docs.microsoft.com/previous-versions/windows/apps/jj572370(v=win.10))。
- **當地語系化廠商**請考慮使用當地語系化的廠商，如果您的應用程式包含要轉譯為語言一大堆的廣泛字串。 當地語系化廠商可以提供工具和程序，以及翻譯資源檔案的相關建議。 這是一個理想的解決方案，但也是最耗費成本的選項，而且可能會增加翻譯內容的往返時間。

## <a name="keep-access-keys-and-labels-consistent"></a>保持便捷鍵和標籤一致

「同步處理」協助工具中使用的便捷鍵與顯示當地語系化的便捷鍵是一項挑戰，因為這兩個字串資源分屬於兩個不同的區段。 請務必提供註解標籤字串，例如： `Make sure that the emphasized shortcut key  is synchronized with the access key.`
...
3. 在一般的 JA-JP 的 Resources.resw:Appname"希蒼 」 加入字串資源
4. 在 Resources.altform-msft-phonetic.resw 日文的假名資源：Appname"のあ"增值假名
