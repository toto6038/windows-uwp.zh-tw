...
[使用者設定檔語言] 清單是使用者在 [**設定** > **時間] & [語言** > **地區] & 語言** > ] 中所設定的清單**名稱。** 在程式碼中，您可以使用 [**GlobalizationPreferences.Languages**](/uwp/api/windows.system.userprofile.globalizationpreferences.Languages) 屬性將使用者設定檔語言清單作為唯讀字串清單存取，其中每個字串都是單一 [BCP-47 語言標記](https://tools.ietf.org/html/bcp47)，例如 "en-US" 或 "ja-JP"。
...
**注意**即使是應用程式預設語言中的資源，也必須指定語言辨識符號。 例如，如果您應用程式的預設語言是英文（美國），則請將您的資產`\Assets\Images\en-US\logo.png`限定為。
...
通常，[**Windows.Globalization**](/uwp/api/windows.globalization?branch=live) 命名空間中的 API 會使用應用程式執行階段語言清單來判斷語言。 如果沒有找到具有相符格式的語言，就會使用使用者地區設定。 這是用於系統時鐘的相同地區設定。 [**設定** > **時間] & [** > **&** > 語言] [其他日期]、[**時間]、[& 地區設定** > ] [地區]： [**變更日期]、[時間] 或 [數位] 格式**。 **Windows.Globalization** API 也具有指定要使用的語言清單的覆寫，可取代應用程式執行階段語言清單。
...
* [Windows. 全球化](/uwp/api/windows.globalization?branch=live)
