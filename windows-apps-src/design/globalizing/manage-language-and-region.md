---
Description: 本主題定義條款使用者設定檔的語言清單、 應用程式資訊清單的語言清單和應用程式執行階段的語言清單。 我們會在本主題及位於此功能區域的其他主題中使用這些術語，因此請務必了解他們的意義。
...
使用語言資源限定詞命名您的資源檔案或其資料夾。 若要深入了解資源限定詞，請參閱[針對語言、縮放比例、高對比及其他限定詞量身打造您的資源](../../app-resources/tailor-resources-lang-scale-contrast.md))。 資源檔可以是映像 （或其他資產），或它可以是資源容器檔案，例如 *.resw* ，其中包含文字字串。

**請注意**資源即使是在您的應用程式的預設語言中必須指定將語言限定詞。 比方說，如果您的應用程式的預設語言是英文 （美國），則請限定您的資產做為`\Assets\Images\en-US\logo.png`。

- Windows 會執行複雜的比對，包括跨區域的變數，例如 EN-US 和 en GB。 因此包含適當的區域子標記。 請參閱[資源管理系統如何比對語言標記](../../app-resources/how-rms-matches-lang-tags.md)。
- 沒有為語言定義隱藏指令碼的值時，請在辨識符號指定語言指令碼子標籤。 比方說，而不是 ZH-CN 或 ZH-TW，使用 Zh-hant、 Hant-ZH-TW 或-Zh-hans (如需詳細資訊，請參閱 < [IANA 語言子標記登錄](https://go.microsoft.com/fwlink/p/?linkid=227303))。
- 針對具有單一標準方言的語言，沒有需要包含區域辨識符號。 比方說，而不是 JA-JP 使用日本。
...
- 最小值，請確定所有的資源存在於的預設語言。
- 某些資源的子集可能會對密切相關的語言 （部分當地語系化） 已經足夠。 例如，若您的應用程式含有西班牙文的完整資源集合，則您可能並未將您應用程式所有的 UI 都當地語系化成卡達隆尼亞文。 若是說話卡達隆尼亞然後按一下 西班牙文的使用者，請以西班牙文顯示卡達隆尼亞中未提供的資源。
- 某些資源可能需要以特定語言，在大部分的其他資源要對應的一般資源時的例外狀況。 在此情況下，將標記可以用來將不明的語言標記 'und' 的所有語言的資源。 Windows 會解譯為萬用字元 'und' 語言標記 (類似於 '\*')，因為它符合任何其他特定相符項目之後的最上層應用程式語言。 例如，如果芬蘭文的少數資源不相同，但所有語言的其餘資源均相同，則芬蘭文資源應以芬蘭文語言標記標示，其餘則應以 'und' 標示。
- 針對資源為基礎的語言指令碼，例如字型或高度的文字，會將不明的語言標記使用指定的指令碼: ' und-&lt;指令碼&gt;'。 例如，拉丁字型使用 `und-Latn\\fonts.css`，斯拉夫字型則使用 `und-Cryl\\fonts.css`。
...
通常，[**Windows.Globalization**](/uwp/api/windows.globalization?branch=live) 命名空間中的 API 會使用應用程式執行階段語言清單來判斷語言。 如果沒有找到具有相符格式的語言，就會使用使用者地區設定。 這是用於系統時鐘的相同地區設定。 使用者地區設定是可從**設定** > **時間與語言** > **區域與語言** >  **額外日期、 時間和地區設定** > **區域：變更日期、 時間或數字格式**。 **Windows.Globalization** API 也具有指定要使用的語言清單的覆寫，可取代應用程式執行階段語言清單。
...
<td align="left">無</td>
<td align="left">法文 (奧地利)</td>
<td align="left">UI:法文 （法國） （來自 法文 （奧地利） 的後援）<br>日期/時間/數字：法文 (奧地利)</td>
</tr>
<tr>
...
<td align="left">無</td>
<td align="left">英文 (加拿大)；法文 (加拿大)</td>
<td align="left">UI:英文 （美國） （來自英文 （加拿大） 的後援）<br>日期/時間/數字：英文 (加拿大)</td>
</tr>
<tr>
...
<td align="left">UI:西班牙文 （西班牙） （使用預設因為沒有可用的後援英文）<br>日期/時間/數字：西班牙文 (西班牙)</td>
</tr>
<tr>
...
<td align="left">無</td>
<td align="left">卡達隆尼亞文；法文 (法國)</td>
<td align="left">UI:大部分卡達隆尼亞及一些法文 （法國） 因為並非所有的字串皆位於卡達隆尼亞<br>日期/時間/數字：卡達隆尼亞文</td>
</tr>
<tr>
...
<td align="left">UI:英文 (GB) （語言覆寫）<br>日期/時間/數字：英文 (英國)</td>
</tr>
</tbody>
</table>
