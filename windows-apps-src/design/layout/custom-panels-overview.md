...
配置系統得以運作，大部分是因為任何以 [**FrameworkElement**](https://msdn.microsoft.com/library/windows/apps/br208706) 為基礎的元素，在做為容器的子系時，本身已有一些繼承而來的行為。 例如，**FrameworkElement** 的 API 有些是通知配置行為，也些是配置運作時不可或缺者。 這些地方包括：
...
-   [**排列**](https://msdn.microsoft.com/library/windows/apps/br208914)並[**量值**](https://msdn.microsoft.com/library/windows/apps/br208952)方法： 這些項目必須在定義的原生實作[ **FrameworkElement**](https://msdn.microsoft.com/library/windows/apps/br208706)層級，處理項目層級配置動作
...
-   [**UpdateLayout**](https://msdn.microsoft.com/library/windows/apps/br208989)， [ **InvalidateMeasure**](https://msdn.microsoft.com/library/windows/apps/br208930)，以及[ **InvalidateArrange** ](https://msdn.microsoft.com/library/windows/apps/br208929)方法，起始配置傳遞。 **InvalidateArrange** 可能不會觸發度量階段，但其他兩種會。 永遠不要從配置方法覆寫內呼叫這些方法，因為幾乎可以確定它們會造成配置迴圈。 控制項程式碼通常也不需要呼叫它們。 大部分配置層面會藉由偵測架構定義的配置屬性 (例如 [**Width**](/uwp/api/Windows.UI.Xaml.FrameworkElement.Width) 等) 的變更而自動觸發。
-   [**LayoutUpdated** ](https://msdn.microsoft.com/library/windows/apps/br208722)是變更版面配置項目的某些層面時，就會引發的事件。 這並非面板特有；此事件是由 [**FrameworkElement**](https://msdn.microsoft.com/library/windows/apps/br208706) 所定義。
-   [**SizeChanged** ](https://msdn.microsoft.com/library/windows/apps/br208742)是引發之事件的配置階段完成，並指出之後，才[ **ActualHeight** ](https://msdn.microsoft.com/library/windows/apps/br208707)或是[ **ActualWidth** ](https://msdn.microsoft.com/library/windows/apps/br208709)結果已變更。 這是另一個 [**FrameworkElement**](https://msdn.microsoft.com/library/windows/apps/br208706) 事件。 有時候會觸發 [**LayoutUpdated**](https://msdn.microsoft.com/library/windows/apps/br208722)，但不會觸發 **SizeChanged**。 例如，內部內容可能已重新排列，但元素大小保持不變。
