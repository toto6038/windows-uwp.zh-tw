...
在 UWP 中的網路隔離功能可讓開發人員控制和限制 UWP app 的網路存取。 並非所有的 app 都需要存取網路。 不過對於那些需要存取網路的 app，UWP 提供對網路不同層級的存取權，這些存取權可透過選取適當的功能來啟用。

網路隔離可讓開發人員為每個 app 定義所需網路存取權的範圍。 沒有定義適當範圍的應用程式在於防止存取指定類型的網路，以及特定類型的網路要求 (對外用戶端起始的要求，或是對內未經同意的要求以及對外用戶端起始的要求)。 設定和強制網路隔離的功能可確保如果應用程式確實受到威脅，它只能存取已明確授與應用程式存取權的網路。 這將可大幅減少對其他應用程式和 Windows 的影響範圍。
...
建立 app 時，會在 app 資訊清單中設定 app 的網路功能。 開發 app 時，通常會使用 Microsoft Visual Studio 2015 來新增網路功能。 也可以使用文字編輯器在 app 資訊清單檔案中手動設定網路功能。

如需網路隔離和網路功能的詳細資訊，請參閱[網路功能基本知識](networking-basics.md)主題中的＜功能＞一節。

## 如何存取網頁摘要

本節說明如何在以 C# 或 Javascript 撰寫的 UWP app 中使用 [**Windows.Web.Syndication**](https://msdn.microsoft.com/library/windows/apps/br243632) 命名空間的類別來抓取和顯示網頁摘要。

**先決條件**

若要確保 UWP app 的網路可正常運作，您必須在專案 **Package.appxmanifest** 檔案中設定所需的所有網路功能。 如果 app 需要以用戶端的形式連線到網際網路上的遠端服務，則需要 **internetClient** 功能。 如需詳細資訊，請參閱[網路功能基本知識](networking-basics.md)主題中的＜功能＞一節。
...
如果無法與 HTTP 伺服器建立連線或 [**Uri**](https://msdn.microsoft.com/library/windows/apps/br226017) 物件未指向有效的 AtomPub 或 RSS 摘要，[**RetrieveFeedAsync**](https://msdn.microsoft.com/library/windows/apps/br243460) 方法會擲回例外狀況。 如果發生錯誤，Javascript 範例程式碼會使用 **onError** 函式來擷取例外狀況，然後列印出例外狀況中的詳細訊息。
