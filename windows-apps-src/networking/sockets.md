...
> [!IMPORTANT]
> 如程式碼範例中的意見所示，您的投影需要宣告 sharedUserCertificates 應用程式功能，以讓此程式碼正常運作。
...
如果傳遞的字串不是有效的主機名稱，[**HostName**](/uwp/api/Windows.Networking.HostName)建構函式會擲回例外狀況。 例如，它包含不允許的字元，如果使用者在您的應用程式中輸入主機名稱，可能發生此狀況。 在 try/catch 區塊內建構**HostName**。 如此一來，如果發生例外狀況，app 可通知使用者並要求新的主機名稱。
