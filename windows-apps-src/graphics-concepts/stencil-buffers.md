...
如果樣板測試通過，目前像素會寫入目標表面，否則會略過。 預設的比較行為是可寫入的像素，不論每位元運算方式開啟。您可以變更此行為變更列舉類型來識別所要的比較函數的值。
...
第一次轉譯會繪製面向前端的多邊形，並遞增樣板緩衝區的值。 第二次轉譯會繪製陰影錐面向後端的多邊形，並遞減樣板緩衝區的值。 一般而言，所有的遞增和遞減值有取消彼此。不過，場景已經被呈現導致某些像素呈現陰影磁碟區失敗 z 緩衝區測試的一般幾何。 留在樣板緩衝區的值將會對應到陰影中的像素。 這些剩餘的樣板緩衝區內容會作為遮罩使用，以將一個包括所有內容的巨大黑色四元組 Alpha 混合至場景中。 藉由將樣板緩衝區作為遮罩，陰影中的像素將會進一步加深。
