...
您的應用程式可利用樣板緩衝區將 2D 或 3D 影像組合為 3D 場景。 樣板緩衝區中的遮罩可用於遮蔽轉譯目標的部分表面區域。 儲存的 2D 資訊，例如文字或點陣圖，接著便可被寫入遮蔽區域。 或者，您的應用程式也可將其他 3D 基本類型轉譯至轉譯目標表面的樣板遮蔽區域。 甚至可以轉譯整個場景。

通常，遊戲會將多個 3D 場景組合在一起。 例如：競速遊戲通常都會顯示後視鏡。 後視鏡便包含了駕駛後方的 3D 場景。 該檢視基本上就是以駕駛前方檢視組合而成的第二個 3D 場景。
...
Direct3D 應用程式使用印花來控制從特定原始影像繪製到轉譯目標表面上的像素。 應用程式將印花套用至基本類型的影像，以正確地轉譯共面多邊形。

例如：將輪胎標記及黃線畫上道路之後，標記應該要直接出現在道路上方。 但是，標記和道路的 z 值是相同的。 因此，深度緩衝區不一定會在這兩者之間正常分離。 某些後方基本類型的像素可能會被轉譯至前方基本類型的上方，反之亦然。 結果影像可能會在影格間不斷閃爍。 此效果稱為「*z 衝突*」或「*影格閃爍*」。

若要解決這個問題，可使用樣板將後方基本類型上印花出現的區域遮蔽。 關閉 z 緩衝並將前方基本類型的影像轉譯至轉譯目標表面上已被遮蔽的區域。
...
如果樣板遮罩有著與基本類型相同的大小及形狀，結果影像將會在原先基本類型所在的位置留下一個洞。 您的應用程式可以再以黑色填滿該空洞，產生基本類型的剪影效果。
...
利用樣板緩衝區繪製陰影時將會使用到陰影錐。 應用程式透過計算剪影的邊緣，將其往光源的反方向突出並形成一組 3D 物體的集合，來計算遮蔽幾何物的陰影錐。 這些物體接著會在經過兩次的轉譯之後寫入樣板緩衝區。

第一次轉譯將會繪製面向前端的多邊形，並且增加樣板緩衝區的值。 第二次轉譯將會繪製面向後端的多邊形，並且減少樣板緩衝區的值。 一般而言，所有增加及減少的值會互相抵銷。 不過，當陰影錐被轉譯時，場景已經在一般幾何導致某些像素無法通過 z 衝突測試時轉譯完成。 留在樣板緩衝區的值將會對應到陰影中的像素。 這些剩餘的樣板緩衝區內容將會被作為遮罩使用，以將一個巨大的，包括所有內容的黑色四元組 Alpha 混合至場景之中。 藉由將樣板緩衝區作為遮罩，陰影中的像素將會被進一步加深。

這表示針對每個光源，陰影幾何都會被繪製兩次，因此對 GPU 的頂點輸送量形成了壓力。 雙面樣板便是為了避免這種情況而被設計出來的功能。 在這種方法之下，會有兩組樣板狀態（命名如下）：其中一組為針對每個面向前端的三角形設定的樣板狀態，另外一組則是針對面向後端的三角形。 如此一來，針對每個光源，每個陰影錐都只會進行一階段的繪製。
