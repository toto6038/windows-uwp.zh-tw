...
## <a name="span-idhal_devicespanspan-idhal_devicespanspan-idhal_devicespanhal-device"></a><span id="HAL_Device"></span><span id="hal_device"></span><span id="HAL_DEVICE"></span>HAL 裝置
...
## <a name="span-idreference_devicespanspan-idreference_devicespanspan-idreference_devicespanreference-device"></a><span id="Reference_Device"></span><span id="reference_device"></span><span id="REFERENCE_DEVICE"></span>參照裝置
...
## <a name="span-idhal_vs_refspanspan-idhal_vs_refspanspan-idhal_vs_refspanhal-vs-ref-devices"></a><span id="HAL_vs_REF"></span><span id="hal_vs_ref"></span><span id="HAL_VS_REF"></span>HAL 與 REF 裝置的比較
...
## <a name="span-idhardware_vs_softwarespanspan-idhardware_vs_softwarespanspan-idhardware_vs_softwarespanhardware-vs-software-vertex-processing"></a><span id="Hardware_vs_Software"></span><span id="hardware_vs_software"></span><span id="HARDWARE_VS_SOFTWARE"></span>硬體與軟體頂點處理


硬體與軟體頂點處理僅適用於 HAL 裝置。 當您透過管線推送頂點時，它們需要轉換 (輪流以世界、檢視、投影矩陣) 和照亮 (藉由 D3D 內建的照明) - 此處理階段稱為 T&L (代表轉換與光源)。 硬體頂點處理表示如果硬體支援，就會在硬體中完成這項作業;因此，軟體頂點處理是在軟體中完成。 一般方式是先嘗試建立硬體 T&L 裝置，如果失敗則嘗試混合，再失敗則嘗試軟體。 (如果軟體失敗，則放棄並在有錯誤的情況下結束)。
