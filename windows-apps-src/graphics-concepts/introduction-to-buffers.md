...
緩衝資源是一個完整輸入的資料集合，由元素所組成。 緩衝區儲存資料，例如紋理座標*頂點緩衝*、*索引緩衝*中的索引、*常數緩衝區*中的著色器常數資料、位置向量、法向向量或裝置的狀態。

緩衝元素由 1 到 4 個元件組成。 緩衝元素可以包含封裝資料值 (像是 R8G8B8A8 面值)、單一 8 位整數，或四個 32 位元浮點數值。

建立緩衝做為未建構的資源。 因為未建立，緩衝不能包含任何 mipmap 層次，當讀取時無法篩選，而且無法多重採樣。
...
索引緩衝包含整數位移至頂點緩衝區，可用於更有效率地呈現基本類型。 索引緩衝包含連續的 16 位元或 32 位元索引集，而每個索引用來識別頂點緩衝區中的頂點。 索引緩衝可以視覺化，如下圖所示。

![索引緩衝的圖例](images/d3d10-index-buffer.png)

儲存在索引緩衝中的循序索引，使用以下參數尋找︰

-   Offset - 索引緩衝的基底位址的位元組數目。
-   StartIndexLocation - 指定基底位址和位移的第一個索引緩衝元素。 代表呈現的第一個索引的起始位置。
-   IndexCount - 要呈現的索引數目。

索引緩衝的起始 = 索引緩衝基底位址 + 位移 (位元組) + StartIndexLocation \ * ElementSize (位元組)；
...
每個元素會儲存 1 到 4 個元件常數，由儲存的資料格式決定。
