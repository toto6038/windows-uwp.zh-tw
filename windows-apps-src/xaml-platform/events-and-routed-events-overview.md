---
description: 使用時，我們會說明在 Windows 執行階段應用程式中的事件的程式設計概念C#，Visual Basic 或 Visual c + + 元件擴充功能 (C + + /CX) 的程式設計語言，以及 XAML UI 定義。
title: 事件與路由事件概觀
...
使用時，我們會說明在 Windows 執行階段應用程式中的事件的程式設計概念C#，Visual Basic 或 Visual c + + 元件擴充功能 (C + + /CX) 的程式設計語言，以及 XAML UI 定義。 您可以在 XAML 中指派事件的處理常式，以做為 UI 元素宣告的一部分，或是在程式碼中新增處理常式。 Windows 執行階段支援「路由事件」：某些輸入事件與資料事件，可以由引發事件之物件以外的物件來處理。 當您定義控制項範本或是使用頁面或配置容器時，路由事件非常實用。
...
您是以用來撰寫所有 app 之程式碼和程式碼後置的程式設計語言，來撰寫實際的處理常式。 使用屬性 `Click="ShowUpdatesButton_Click"` 會建立一個協定，也就是當 XAML 是以標記編譯和剖析時，您的 IDE 建置動作的 XAML 標記編譯步驟和應用程式載入時的最終 XAML 剖析，都可以在應用程式的程式碼中找到名為 `ShowUpdatesButton_Click` 的方法。 `ShowUpdatesButton_Click` 必須是方法的實作 （以委派為基礎） 相容的方法簽任何的章處理常式[**按一下 ** ](https://msdn.microsoft.com/library/windows/apps/br227737)事件。 例如，下列程式碼會定義 `ShowUpdatesButton_Click` 處理常式。
...
**附註**  事件處理常式的 c + + 不取得在部分類別中定義，其宣告為私用類別成員的標頭中。 C++ 專案的建置動作會負責處理程式碼的產生，而這些程式碼可支援 C++ 的 XAML 類型系統和程式碼後置模型。
...
[**FrameworkElement.Unloaded** ](https://msdn.microsoft.com/library/windows/apps/br208748)或是[ **Page.NavigatedFrom** ](https://msdn.microsoft.com/library/windows/apps/br227507)具有適當的位置中的狀態管理和物件存留期這類可能的事件觸發程序您可以使用它們來移除其他事件處理常式。
...
**祕訣**  事件反昇輸入事件是特別有用，如果您要建立樣板化控制項。 控制項的取用者可以為含有範本的任一控制項套用新範本。 正嘗試重建工作範本的取用者，可能會不慎刪除在預設範本中宣告的某些事件處理。 您仍然可以提供控制項層級的事件處理，方法是將處理常式附加為類別定義中的 [**OnApplyTemplate**](https://msdn.microsoft.com/library/windows/apps/br208737) 覆寫的一部分。 接著，您可以攔截具現化時向上反昇至控制項根目錄的輸入事件。
...
- 元素的 **Background** 或 **Fill** 屬性值不是 **null**。 A **null** [**筆刷**](/uwp/api/Windows.UI.Xaml.Media.Brush)透明度值的結果，並點擊測試的可見度。 (若要讓元素變成透明但仍可以進行點擊測試，請使用 [**Transparent**](https://msdn.microsoft.com/library/windows/apps/hh748061) 筆刷而不要使用 **null**)。
...
部分控制項擁有特殊的點擊測試規則。 例如，[**TextBlock**](https://msdn.microsoft.com/library/windows/apps/br209652) 沒有 **Background** 屬性，但是在其尺寸的整個區域內仍然可以進行點擊測試。 [**映像**](https://msdn.microsoft.com/library/windows/apps/br242752)並[ **MediaElement** ](https://msdn.microsoft.com/library/windows/apps/br242926)控制項透過其定義的矩形的維度，不論透明的內容，例如在媒體中的 alpha 色頻進行點擊測試所顯示的原始程式檔。 [**WebView** ](https://msdn.microsoft.com/library/windows/apps/br227702)控制項有特殊的點擊測試行為，因為輸入可由裝載的 HTML 和火災指令碼事件。
...
* [快速入門：具備觸控輸入](https://msdn.microsoft.com/library/windows/apps/xaml/hh465387)
