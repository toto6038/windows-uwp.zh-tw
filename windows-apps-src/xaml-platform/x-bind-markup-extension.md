---
description: XBind 標記延伸是系結的高效能替代方式。 xBind-Windows 10 的新功能-以比系結更少的時間和較少的記憶體執行，並支援更佳的偵錯工具。
title: xBind 標記延伸
...
**請注意** 如需在您的應用程式中搭配 **{x:Bind}** 使用資料系結的一般資訊（以及 **{x:Bind}** 與 **{binding}** 之間的整體比較），請參閱[深入資料](https://docs.microsoft.com/windows/uwp/data-binding/data-binding-in-depth)系結。
...
若要系結至[附加的屬性](./attached-properties-overview.md)，您必須將類別和屬性名稱放在點後面的括弧中。 例如 **Text="{x:Bind Button22.(Grid.Row)}"** 。 如果屬性未在 Xaml 命名空間中宣告，您將需要以 xml 命名空間為它加上首碼，而此命名空間應對應於文件最前面的程式碼命名空間。
...
從 Windows 10 版本 1607 開始， **{x:Bind}** 支援使用函式作為繫結路徑的分葉步驟。 這是資料系結的強大功能，可在標記中啟用數個案例。 如需詳細資訊，請參閱[函數](../data-binding/function-bindings.md)系結。
...
使用 **{x:Bind}** 搭配資料範本時，您必須藉由設定**x:DataType**值來指定要系結的類型，如[範例](#examples)一節所示。 您也可以將類型設為介面或基底類別類型，然後在必要時使用轉換來編寫完整的運算式。
