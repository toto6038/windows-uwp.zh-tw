...
本節包含說明程式設計概念的主題，如果您使用 C#、Microsoft Visual Basic 或 Visual C++ 元件延伸 (C++/CX) 作為程式設計語言，並且使用 XAML 定義 UI，則這些概念一般適用於您撰寫的所有應用程式。 這包含基本的程式設計概念，例如使用屬性和事件，以及如何將它們套用到通用 Windows 平台 (UWP) 應用程式的程式設計。 通用 Windows 平台 (UWP) 會藉由新增相依性屬性系統，來延伸屬性及其值的 C#、Visual Basic 或 C++/CX 概念。 本節中的主題也會在 UWP 使用 XAML 語言時記錄該語言，並且涵蓋基本案例和進階主題，以說明如何使用 XAML 來定義適用於 UWP app 的 UI。
