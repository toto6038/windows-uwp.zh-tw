...
將附加的屬性定義為相依性屬性，藉由宣告**公開金鑰** **靜態** **readonly**屬性的型別[ **DependencyProperty**](https://docs.microsoft.com/uwp/api/Windows.UI.Xaml.DependencyProperty)。 使用 [**RegisterAttached**](https://docs.microsoft.com/uwp/api/windows.ui.xaml.dependencyproperty.registerattached) 方法的傳回值來定義這個屬性。 屬性名稱必須符合您指定作為附加的屬性名稱**RegisterAttached** *名稱*參數，"Property"新增至結尾的字串。 這是根據代表的屬性命名相依性屬性識別碼的既定慣例。
...
> [!NOTE]
> 它也可定義附加的屬性的預定使用方式是透過屬性元素語法的位置。 在該情況下，您不需要輸入值的轉換，但必須確保您想要的值可以在 XAML 中建構。 [**VisualStateManager.VisualStateGroups** ](https://docs.microsoft.com/dotnet/api/system.windows.visualstatemanager?view=netframework-4.8)是現有的附加屬性僅支援屬性項目使用方式的範例。
...
> [!NOTE]
> 如果您使用C++/WinRT，則請跳至下一節 ([設定您附加的自訂屬性，以命令方式與C++/WinRT](#setting-your-custom-attached-property-imperatively-with-cwinrt))。
...
> [!NOTE]
> 如果您正在撰寫 XAML UI，與C++//CX 中，則您必須包含標頭可定義附加的屬性，任何時間的自訂類型的 XAML 頁面會使用該類型。 每個 XAML 頁面具有相關聯的程式碼後置標頭 (。 xaml.h)。 這是您應該在此對話方塊包含 (使用 **\#包含**) 的附加的屬性的擁有者型別定義的標頭。

## <a name="setting-your-custom-attached-property-imperatively-with-cwinrt"></a>設定您附加的自訂屬性，以命令方式使用C++/WinRT

如果您使用C++/WinRT，則您可以存取自訂的附加的屬性受命令式的程式碼，而不是從 XAML 標記。 以下程式碼顯示如何。
...
在前面的附加屬性用法範例中，我們示範了設定 [**Canvas.Left**](https://docs.microsoft.com/dotnet/api/system.windows.controls.canvas.left?view=netframework-4.8) 附加屬性的不同方法。 但是，這對於 [**Canvas**](https://docs.microsoft.com/uwp/api/Windows.UI.Xaml.Controls.Canvas) 與您物件的互動方式有何改變，以及在何時發生？ 我們將進一步探討這個特定的範例，因為如果您實作附加屬性，看看當典型的附加屬性擁有者類別在其他物件上發現它的附加屬性值時，會對這些值做哪些其他處理，將是一件有趣的事。
...
> [!NOTE]
> 如需面板的運作方式的詳細資訊，請參閱[XAML 自訂面板概觀](https://docs.microsoft.com/windows/uwp/layout/custom-panels-overview)。
