...
將附加的屬性定義為相依性屬性，藉由宣告**公開金鑰****靜態** **readonly**屬性的型別[ **DependencyProperty**](https://msdn.microsoft.com/library/windows/apps/br242362)。 使用 [**RegisterAttached**](https://msdn.microsoft.com/library/windows/apps/hh701833) 方法的傳回值來定義這個屬性。 屬性名稱必須符合您指定作為附加的屬性名稱**RegisterAttached** *名稱*參數，"Property"新增至結尾的字串。 這是根據代表的屬性命名相依性屬性識別碼的既定慣例。

定義自訂附加屬性與自訂相依性屬性的主要差異在於定義存取子或包裝函式的方式。 而不是使用中所述的包裝函式技巧[自訂相依性屬性](custom-dependency-properties.md)，您也必須提供靜態 **取得 * * * PropertyName*和 **設定 * * * PropertyName*附加屬性存取子方法。 存取子主要是由 XAML 剖析器使用，不過任何其他呼叫者也可以使用它們在非 XAML 案例中設定值。

> [!IMPORTANT]
> 如果您未正確定義存取子，XAML 處理器無法存取您的附加的屬性，並嘗試使用它的人可能會取得 XAML 剖析器錯誤。 此外，設計和程式碼設計工具通常依賴"\*屬性 」 命名識別項，當遇到參考的組件中的自訂相依性屬性的慣例。
...
> [!NOTE]
> 它也可定義附加的屬性的預定使用方式是透過屬性元素語法的位置。 在該情況下，您不需要輸入值的轉換，但必須確保您想要的值可以在 XAML 中建構。 [**VisualStateManager.VisualStateGroups** ](https://msdn.microsoft.com/library/windows/apps/hh738505)是現有的附加屬性僅支援屬性項目使用方式的範例。
...
## <a name="setting-your-custom-attached-property-from-xaml-markup"></a>從 XAML 標記中設定您的自訂附加的屬性

> [!NOTE]
> 如果您使用 C + + /cli WinRT，然後跳至下一節 ([設定將附加的自訂屬性，以命令方式與 C + + /cli WinRT](#setting-your-custom-attached-property-imperatively-with-cwinrt))。
...
> [!NOTE]
> 如果您要撰寫 XAML UI，使用 C + + /CX 中，則您必須包含標頭可定義附加的屬性，任何時間的自訂類型的 XAML 頁面會使用該類型。 每個 XAML 頁面具有相關聯的程式碼後置標頭 (。 xaml.h)。 這是您應該在此對話方塊包含 (使用**\#包含**) 的附加的屬性的擁有者型別定義的標頭。

## <a name="setting-your-custom-attached-property-imperatively-with-cwinrt"></a>設定您附加的自訂屬性，以命令方式使用 C + + /cli WinRT

如果您使用 C + + /cli WinRT，則您可以存取自訂的附加的屬性受命令式的程式碼，而不是從 XAML 標記。 以下程式碼顯示如何。
...
> [!NOTE]
> 如需面板的運作方式的詳細資訊，請參閱[XAML 自訂面板概觀](https://msdn.microsoft.com/library/windows/apps/mt228351)。
