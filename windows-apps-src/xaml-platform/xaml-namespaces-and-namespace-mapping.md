...
在 XAML 命名空間建立執行階段物件表示方法時，XAML 處理器可以從與該 XAML 命名空間相關聯的支援組件查詢類型及成員。 這就是為什麼 XAML 是正規化及交換物件建構行為定義的有用方式，以及為什麼使用 XAML 做為 Windows 市集應用程式的 UI 定義技術的原因。

## 典型 XAML 標記使用方法中的 XAML 命名空間
