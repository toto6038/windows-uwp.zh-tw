...
Windows 執行階段也支援 [{x:Null} 標記延伸](x-null-markup-extension.md)。 您可以在 XAML 中使用這個標記延伸，將 [**Nullable**](T:System.Nullable%601) 值設定為 **null**。 例如，在 [**CheckBox**](https://msdn.microsoft.com/library/windows/apps/br209316) 的控制項範本使用這個標記延伸，會將 **null** 解譯為不確定的檢查狀態 (觸發 "Indeterminate" 視覺狀態)。
