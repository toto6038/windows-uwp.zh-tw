...
使用**ApplicationTrigger**個別處理程序中執行程式碼，從前景應用程式。 **ApplicationTrigger**是適用於您的 app 有需要執行的動作，在背景中： 即使在使用者關閉前景應用程式的工作。 如果背景工作應該停止當應用程式關閉時，或[延伸執行](run-minimized-with-extended-execution.md)應該改用，則應該與前景處理程序的狀態。

## <a name="create-an-application-trigger"></a>建立應用程式觸發程序

建立新的[ApplicationTrigger](https://docs.microsoft.com/uwp/api/Windows.ApplicationModel.Background.ApplicationTrigger)。 您無法將其儲存在欄位中為已完成下列程式碼片段中。 這是為了方便起見，因此，我們不需要稍後建立新的執行個體，當我們想要發出訊號觸發程序。 但是，您可以使用任何**ApplicationTrigger**執行個體發出訊號觸發程序。
...
在此範例中的條件設成**InternetAvailable** ，如此一次觸發，工作僅能執行一旦網際網路存取權是以可用。 如需可用條件的清單，請參閱 [**SystemConditionType**](https://msdn.microsoft.com/library/windows/apps/br224835)。
...
如果您正考慮使用來擴充您的前景處理程序的存留期的應用程式觸發程序，請考慮改為使用[延伸執行](run-minimized-with-extended-execution.md)。 應用程式觸發程序的設計是供建立分別裝載的處理程序來執行工作。 下列程式碼片段會註冊一個處理程序背景觸發程序。
...
觸發背景工作之前，使用[BackgroundTaskRegistration](https://docs.microsoft.com/uwp/api/Windows.ApplicationModel.Background.BackgroundTaskRegistration)來確認已登錄背景工作。 確認您的背景工作的所有已登錄的好時機是在應用程式啟動期間。
...
此外，只有一個 AppTrigger 可以執行一次。 如果您嘗試執行 AppTrigger 另一個正在執行時，此函式會傳回[CurrentlyRunning](https://docs.microsoft.com/uwp/api/windows.applicationmodel.background.applicationtriggerresult)。
...
- Memory： 調整您的應用程式的記憶體與能源使用量很重要，作業系統仍將允許您執行的背景工作。 若要查看您的背景工作正在使用多少記憶體中使用[的記憶體管理 Api](https://msdn.microsoft.com/library/windows/apps/windows.system.memorymanager.aspx) 。 您的背景工作會使用更多的記憶體，讓它執行另一個應用程式在前景時，作業系統會越難。 最終是由使用者控制您的應用程式可執行的所有背景活動，而且他也能看到您的應用程式對電池使用量的影響。  
...
從 windows 10 開始，以便在不再需要，讓使用者能夠將您的應用程式新增到鎖定畫面中，為了利用背景工作。
