...
> [!NOTE]
> 在 UWP app 中，特定的 Uri 和副檔名保留給使用內建 app 和作業系統。 如果嘗試以保留的 URI 或副檔名登錄 app，該嘗試將會被忽略。 請在[保留 URI 配置名稱和檔案類型](reserved-uri-scheme-names.md)中，參閱 URI 配置依字母排列的檔案類型清單，以了解遭保留或禁止而不能為 UWP app 登錄的檔案類型。
...
> [!NOTE]
> 如果您正在建立新的 URI 配置名稱為您的 app，請務必遵循[RFC 4395](http://go.microsoft.com/fwlink/p/?LinkID=266550)中的指引。 這樣可確保您的名稱符合 URI 配置的標準。
...
### <a name="complete-sample-app"></a>完整的範例應用程式
