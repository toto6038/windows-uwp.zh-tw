...
上述的宣告會登錄您的應用程式要從指定的主機來處理連結。 如果您的網站有多個位址 (例如： m.example.com、www \. example.com 和 example.com) 然後在 `<uap3:Host Name=... />` 中 `<uap3:AppUriHandler>` 為每個位址新增個別專案。
...
排除路徑都會先行檢查，如果有相符項目，將會使用瀏覽器開啟對應的頁面，而不是使用指定的應用程式。 在上述範例中，'/news/ \* ' 包含位於該路徑下的任何頁面，而 '/news \* ' (沒有正斜線軌跡 ' news ' ) 包含 ' news ' 下的任何路徑， \* 例如 ' newslocal/'、' newsinternational/' 等等。
...
-   檔案路徑：本機驗證的 JSON 檔案 (例如，C： \\ SomeFolder \\ windows-app-web-link) 
...
確認您的應用程式已經關閉。 按 **Windows 鍵 + R** 開啟 [ **執行** ] 對話方塊，並在視窗中貼上連結。 您的應用程式應要啟動，而不是網頁瀏覽器。

此外，您可以使用 [LaunchUriAsync](/uwp/api/windows.system.launcher.launchuriasync) API，測試從另一個應用程式來啟動您的應用程式。 您同樣可以使用這個 API，在手機上測試。
...
- 只要您的應用程式是利用 [LaunchUriAsync](/uwp/api/windows.system.launcher.launchuriasync) 啟動的 UWP 應用程式，或使用 [ShellExecuteEx](/windows/desktop/api/shellapi/nf-shellapi-shellexecuteexa) 啟動的 Windows 傳統型應用程式，此功能都能運作。 如果 URL 對應到已登錄的應用程式 URI 處理常式，會啟動該應用程式，而不是瀏覽器。

## <a name="see-also"></a>另請參閱

[Web 應用程式範例專案](https://github.com/project-rome/AppUriHandlers/tree/master/NarwhalFacts) 
[windows 通訊協定註冊](/uwp/schemas/appxpackage/appxmanifestschema/element-protocol) 
[控制碼 URI 啟用](./handle-uri-activation.md) 
[關聯啟動範例](https://github.com/Microsoft/Windows-universal-samples/tree/master/Samples/AssociationLaunching)說明如何使用 LaunchUriAsync ( # A1 API。
