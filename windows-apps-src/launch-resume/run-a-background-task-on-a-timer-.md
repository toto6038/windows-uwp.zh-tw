...
ms.locfileid: "89175132"
---
# <a name="run-a-background-task-on-a-timer"></a>在計時器上執行背景工作

了解如何使用 [**TimeTrigger**](/uwp/api/Windows.ApplicationModel.Background.TimeTrigger) 排程一次性的背景工作，或執行定期的背景工作。
...
建立一個新的 [**TimeTrigger**](/uwp/api/Windows.ApplicationModel.Background.TimeTrigger)。 第二個參數 *OneShot* 會指定背景工作將只執行一次，還是要繼續定期執行。 如果 *OneShot* 設定成 True，第一個參數 (*FreshnessTime*) 會指定排定背景工作之前要等待的時間 (以分鐘為單位)。 如果 *OneShot* 設定成 False，*FreshnessTime* 會指定背景工作的執行頻率。

以傳統型或行動裝置系列為目標之通用 Windows 平台 (UWP) app 的內建計時器會每隔 15 分鐘執行一次背景工作。 (計時器按照 15 分鐘的間隔執行，讓系統只需要每隔 15 分鐘喚醒一次來喚醒已要求 TimerTriggers 的 App，這樣也省節電力)。
...
這個範例中，條件是設成 **UserPresent**，因此觸發後工作只有在使用者作用中時才會執行一次。 如需可用條件的清單，請參閱 [**SystemConditionType**](/uwp/api/Windows.ApplicationModel.Background.SystemConditionType)。
...
> [!IMPORTANT]
> 針對在與應用程式相同的進程中執行的背景工作，請不要設定 `entryPoint` 。 對於在應用程式的個別進程中執行的背景工作，請將設定 `entryPoint` 為命名空間 '. '，以及包含您的背景工作執行之類別的名稱。
...
如果您已經先呼叫 [**RequestAccessAsync**](/uwp/api/windows.applicationmodel.background.backgroundexecutionmanager.requestaccessasync)，背景工作將只會使用 **TimeTrigger** 來執行。
