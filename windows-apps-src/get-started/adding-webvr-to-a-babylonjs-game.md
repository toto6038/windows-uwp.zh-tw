...
如果您正在使用 Windows Mixed Reality 不熟悉，且具備相容的圖形卡在電腦上安裝 Windows 10 Creators Update，請嘗試從 Windows 10 中的 [開始] 功能表開啟**混合實境入口網站**應用程式。

![混合的實境入口網站搜尋](images/mixed-reality-portal.png)

如果您滿足所有需求，您就可以再開啟開發人員功能，並模擬 Windows Mixed Reality 頭戴式裝置插入到您的電腦。 如果您真不足以有附近，實際頭戴式裝置插入它，並執行安裝程式。
...
此程式碼會新增內[**main.js**](https://github.com/Microsoft/Windows-tutorials-web/blob/master/BabylonJS-game-with-WebVR/before/js/main.js#L157-L168)。
```javascript
// GUI
...
此 UI 會顯示後建立，但是可以使用切換開啟或關閉`isVisible`根據遊戲中發生了什麼。
```javascript
...
使用 Babylon.js、 WebVR 可以新增快速藉由使用[`WebVRFreeCamera`](http://doc.babylonjs.com/classes/3.1/webvrfreecamera)。 這個相機可以使用鍵盤輸入，並可讓您使用 VR 耳機來控制您的 「 head 」 旋轉。


### <a name="step-1-checking-for-headsets"></a>步驟 1： 檢查耳機

我們後援的相機，我們會使用[`UniversalCamera`](https://doc.babylonjs.com/classes/3.1/universalcamera)，目前原始遊戲中使用。
...
在遊戲中的按一下現在會建立下列項目，例如提示或顯示遊戲頭戴式裝置中立即如果使用者已經接受之前的提示。
