...
如果您要建立一個需要使用者輸入大量資訊的應用程式，您可能需要建立一個表單供他們填寫。這篇文章將說明您需要知道的事項，以便建立有用且強固的表單。
...
您將需要各種 [XAML 控制項](../design/controls-and-patterns/controls-and-events-intro.md)來填入表單。 您可能對此很熟悉了，但若您需要複習，請隨時閱讀。 尤其是，您會需要控制項，可讓您的使用者輸入文字，或從值的清單中選擇。 這是您可以新增的選項基本清單 – 您不需要閱讀有關它們的一切，但已足夠讓您了解它們的外觀及運作方式。
...
* [ComboBox](https://docs.microsoft.com/uwp/api/Windows.UI.Xaml.Controls.ComboBox) 可展開以顯示可選取項目的清單。 您也可以在[這裡](../design/controls-and-patterns/lists.md#drop-down-lists)了解更多資訊
...
> [!IMPORTANT]
> 使用 StateTrigger 時，請一律確保會將 VisualStateGroups 附加到根目錄的第一個子項。 此處的 **Grid** 是根 **Page** 元素的第一個子項。
