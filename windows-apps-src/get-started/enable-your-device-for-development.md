...
設定側載應用程式通常會使用公司或學校需要受管理裝置上安裝自訂應用程式，而不必透過 Microsoft Store 中，或其他任何人需要來自非 Microsoft 來源執行的應用程式。 在此案例中，組織強制執行會停用 *UWP apps* 設定的原則相當常見，如先前設定頁面的圖中所示。 組織也會提供側載應用程式所需的憑證和安裝位置。 如需詳細資訊，請參閱 TechNet 文章[在 Windows 10 中側載 App](https://technet.microsoft.com/library/mt269549.aspx) 和[在 Microsoft Intune 中開始使用 App 部署](https://technet.microsoft.com/library/dn646955.aspx)。

裝置系列特定的資訊

-   在傳統型裝置系列上：您可以安裝應用程式套件 (.appx)，才能執行 Windows PowerShell 指令碼來執行應用程式的任何憑證以建立封裝 ("Add-AppDevPackage.ps1")。 如需詳細資訊，請參閱[封裝 UWP app](../packaging/packaging-uwp-apps.md)。
...
> [!NOTE]
> 如果不正確地安裝開發人員模式下，我們鼓勵您提出的意見反應要求。 在**意見反應中樞**應用程式，選取**新增意見反應**，然後選擇 **開發人員平台**類別目錄和**開發人員模式**子類別。 提交意見反應將協助 Microsoft 解決您遇到此問題。
...
3.  若要啟用側載功能，請編輯原則來啟用：
    ...
    -   **允許安裝所有信任的應用程式**
    -   **允許開發的 UWP 應用程式並將它們從整合式的開發環境 (IDE) 安裝。**
...
2.  若要啟用側載功能，請執行下列命令：
    ...
    -   **PS c:\\WINDOWS\\system32&gt; reg 新增"HKEY\_本機\_機器\\軟體\\Microsoft\\Windows\\CurrentVersion\\AppModelUnlock"/t REG\_DWORD /f /v"AllowDevelopmentWithoutDevLicense"/d"1"**
