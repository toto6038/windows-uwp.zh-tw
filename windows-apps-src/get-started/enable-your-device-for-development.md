...
-   *Microsoft 存放區的應用程式*是預設的設定。 如果您不開發應用程式，或使用您的公司所發行的特殊內部應用程式，請讓此設定保持作用中狀態。
...
> [!NOTE]
> 如果開發人員模式安裝不正確，我們建議您在檔案的意見反應要求。 **意見反應 Hub**應用程式中選取 [**新增新的意見反應**，然後選擇 [**開發人員平台**類別和**開發人員模式**子類別。 送出意見反應可協助 Microsoft 解決您遇到的問題。
