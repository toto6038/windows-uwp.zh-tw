...
-   *UWP app* 是預設設定。 如果您不開發應用程式，或使用您的公司所發行的特殊內部應用程式，請讓此設定保持作用中狀態。
...
## <a name="notes"></a>備註
在舊版 Windows 10 行動裝置版中，[損毀傾印] 選項顯示在 [開發人員設定] 功能表中。  此選項已移至 [Device Portal](../debug-test-perf/device-portal.md)，因此可以從遠端使用，而不只是透過 USB。  
...
-   您可以使用 [Device Portal](../debug-test-perf/device-portal.md)，從您的瀏覽器部署到執行 Windows 10 版本 1511 或更新版本的行動裝置。 使用 Device Portal 中的**[應用程式](../debug-test-perf/device-portal.md#apps-manager)** 頁面來上傳應用程式套件 (.appx)，並將它安裝在裝置上。
