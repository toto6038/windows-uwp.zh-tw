---
title: 建置複雜通用 Windows 平台 (UWP) 應用程式
...
**想推出什麼用途的應用程式？** 先從整體的概念開始，然後列出要透過 app 幫助使用者達成的所有事項。
...
良好的資訊架構不僅有助於擬定使用情景，也能協助您想像關鍵的開始畫面。 例如，[Audible](https://www.windowsphone.com/store/app/audible-for-windows-phone/bdc813dd-c20b-41f8-8646-de72fa0b365d) app 啟動後會直接進入中心，讓使用者存取使用者的媒體櫃、市集、新聞和統計資料等。這個應用程式提供明確的使用體驗，使用者可以迅速取得並聆聽有聲書。 較深的應用程式層級著重在更具體的工作上。
...
對新的開發人員，我們需要再三強調：製作絕佳的應用程式是反覆進行的程序。 建議您及早並經常製作原型。 如同絞盡腦汁發揮創意一樣，最好的應用程式是經過不斷的嘗試與錯誤之後產生的成果。
...
-   **檔案選擇器和檔案選擇器擴充功能**  ：讓使用者可以從本機檔案系統、連接的儲存裝置、家用群組，或甚至從其他應用程式載入和儲存檔案。 您也可以提供檔案選擇器延伸模組，讓其他 app 可以載入您 app 的內容。
