...
良好的資訊架構不僅有助於擬定使用情景，也能協助您想像關鍵的開始畫面。 [聲音](https://go.microsoft.com/fwlink/p/?LinkID=268089)的應用程式，例如，啟動直接進入中心，提供存取使用者的媒體櫃、 市集、 新聞和統計資料。焦點放在體驗，讓使用者可以取得並聆聽有聲。 較深的應用程式層級著重在更具體的工作上。
...
-   **共用**，可讓使用者與其他應用程式，透過其他人分享您的應用程式中的內容，並從其他人和應用程式，也收到可共用的內容。
-   **播放至**讓使用者享受音訊、 視訊或影像，從您的應用程式串流處理到家用網路中的其他裝置。
-   **檔案選擇器和檔案選擇器延伸模組**： 讓使用者可以載入和儲存其檔案從本機檔案系統、 連接的儲存裝置、 家用群組或甚至其他應用程式。 您也可以提供檔案選擇器延伸模組，讓其他 app 可以載入您 app 的內容。
...
**觸控優先**Windows 提供獨特的觸控經驗，不超過只是模擬滑鼠功能。
...
**引人注目和全新感受**確定您的應用程式耳目一新的感覺，並吸引使用者透過下列標準體驗：

-   **動畫**，為您的使用者，讓您快速且流暢的應用程式使用我們的動畫庫。 協助使用者了解內容變更，並將視覺轉換和使用者經驗緊密結合。 深入了解[讓 UI 產生動畫效果](../graphics/animations-overview.md)。
-   **快顯通知**讓使用者了解有時間敏感或個人相關的內容，透過快顯通知和邀請他們回到您的應用程式，即使您的應用程式處於關閉。 深入了解[磚、徽章及快顯通知](../design/shell/tiles-and-notifications/index.md)。
...
-   **近接手勢**，可讓使用者連接裝置與其他使用者實際近接，透過彼此實際輕觸裝置 （多人遊戲）。 深入了解[鄰近性和輕觸](https://msdn.microsoft.com/library/windows/apps/hh465229)。
-   **相機和外部存放裝置**您的使用者連線到他們的內建或插入入的相機，用於聊天和進行會議、 錄製影音部落拍攝設定檔格、 記錄周遭，或您的應用程式的強項任何活動。 深入了解[存取抽取式存放裝置上的內容](https://msdn.microsoft.com/library/windows/apps/hh465189)。
-   **加速計和其他感應器** 裝置都會隨附許多感應器現今。 您的應用程式可以根據環境光線將顯示器變暗或變亮，使用者旋轉顯示器時重新排列 UI，或是回應任何實體的移動。 深入了解[感應器](../devices-sensors/sensors.md)。
-   **地理位置**使用來自標準 web 資料或地理位置感應器的地理位置資訊可協助您如須克服、 尋找其位置，在地圖上的或人物、 活動及目的地鄰近的使用者。 深入了解[地理位置](https://msdn.microsoft.com/library/windows/apps/hh465139)。
...
**決定如何安排您的應用程式，每個檢視中。** Windows 支援直向和橫向方向和大小調整應用程式為任何寬度，從全螢幕，小至最小寬度的支援。 您希望應用程式在任何大小、任何螢幕以及任何方向都能相當美觀且運作順暢。 這表示您需要針對不同大小和檢視規劃 UI 元素的配置。 這樣做可讓您的應用程式 UI 流暢地變更，以符合使用者的需求與喜好。
...
**啟動顯示畫面**啟動顯示畫面應該儘速載入，並停留在畫面上，只要您需要初始化 app 狀態。 啟動顯示畫面應該表達您應用程式的特色。
...
在開發應用程式之前，請先確認您的設計或原型是否符合指導方針、使用者印象與需求，以免日後必須重做。 每個功能都有一組 UX 指導方針，協助您潤飾您的應用程式和一組市集需求，您必須符合 Microsoft Store 中發佈應用程式。 您可以使用 [Windows 應用程式認證套件](https://developer.microsoft.com/windows/develop/app-certification-kit)測試是否符合市集技術需求。 您也可以使用 Microsoft Visual Studio 中的效能工具，確認您在每個案例中都為使用者提供了絕佳的經驗。
