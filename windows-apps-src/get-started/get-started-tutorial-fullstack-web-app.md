---
author: libbymc
title: "入門教學課程 - 單頁模式 Web 應用程式與 REST API 後端"
description: "使用受歡迎的 Web 技術建置適用於 Windows 市集的託管 Web 應用程式"
...
---
# <a name="get-started-tutorial---single-page-web-app-with-rest-api-backend"></a>入門教學課程 - 單頁 Web 應用程式與 REST API 後端

**使用受歡迎的全端 Web 技術建置適用於 Windows 市集的託管 Web 應用程式**

![做為單頁 Web 應用程式的簡單記憶遊戲](images/fullstack.png)

這個兩部分教學課程提供快速導覽現代化的全端 Web 開發，讓您建置可在瀏覽器中運作以及做為 Windows 市集適用的託管 Web 應用程式的簡單記憶遊戲。 在部分 I 中，您將建置遊戲後端的簡易 REST API 服務。 藉由在做為 API 服務的雲端中裝載遊戲邏輯，您可保留遊戲狀態，讓您的使用者能夠跨不同裝置接續玩相同的遊戲執行個體。 在部分 II 中，您將建置前端 UI 為回應式配置的單頁 Web 應用程式。
...
如果您決定要放棄 (或延後) Azure 部分，只要略過部分 I 及 II 的最後區段，其中涵蓋 Azure 裝載和封裝 Windows 市集適用的應用程式。 您建置的 API 服務與 Web 應用程式仍會在您的電腦上本機執行 (分別從 `http://localhost:8000` 到 `http://localhost:3000`)。
...
Azure 文件將逐步引導您完成：
 ...
 - [向 Azure 入口網站註冊新的 *API App*](https://docs.microsoft.com/en-us/azure/app-service-api/app-service-api-nodejs-api-app#a-idcreateapiappa-create-a-new-api-app)
 - [設定適用於 API 應用程式的 Git 部署](https://docs.microsoft.com/en-us/azure/app-service-api/app-service-api-nodejs-api-app#set-up-your-new-api-app-for-git-deployment)，以及
 - [部署 API 應用程式的程式碼到 Azure](https://docs.microsoft.com/en-us/azure/app-service-api/app-service-api-nodejs-api-app#deploy-your-api-code-to-azure)
...
## <a name="part-ii-build-a-single-page-web-appl"></a>部分 II︰建置單頁 Web 應用程式
...
* [Pug](https://pugjs.org/)：(正式 Jade) 用於範本化引擎
...
2. 在您的瀏覽器中，移至 [http://localhost:3000/](http://localhost:3000/) 並套用遊戲！
...
您現在可以部署您的應用程式到 Azure (或任何其他雲端裝載服務)，跨不同的裝置外形規格 (例如行動裝置、平板電腦和桌上型電腦) 進行測試。 (別忘了也要跨不同的瀏覽器進行測試！) 一旦您的應用程式準備好進入生產，您可以輕鬆地將其封裝為*通用 Windows 平台* (UWP) 的*主控 Web 應用程式* (HWA)，以及從 Windows 市集進行散發。

在 Windows 市集中發佈的基本步驟如下︰
...
以下是一些可讓您更進一步的實用資源︰
 ...
 - [將您的應用程式開發專案部署至 Azure 網站](https://docs.microsoft.com/en-us/azure/documentdb/documentdb-nodejs-application#a-nametoc395783182aPart-7-deploy-your-application-development-project-to-azure-websites)
