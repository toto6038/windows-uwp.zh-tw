...
若要下載範例，請移至[存放庫](https://github.com/Microsoft/Windows-universal-samples "通用 Windows 平台應用程式範例 GitHub 存放庫") \(英文\) 並選取 [Clone or download]  \(複製或下載\)，然後選取 [Download ZIP]  \(下載 ZIP\)。 或者，請按一下[這裡](https://github.com/Microsoft/Windows-universal-samples/archive/master.zip "通用 Windows 平台應用程式範例壓縮檔下載")。
...
> [!NOTE]
> UWP 範例需 Visual Studio 2015 或更新版本和 Windows SDK 才能開啟、建置及執行。 您可以從[這裡](https://go.microsoft.com/fwlink/p/?LinkID=280676 "Windows 開發工具下載")取得免費的 Visual Studio Community，其支援 UWP 應用程式的建置。  
>
...
1.  將封存解壓縮之前，請以滑鼠右鍵按一下它，選取 [內容]   > [解除封鎖]   > [套用]  。 然後，將封存解壓縮到您電腦上的本機資料夾。
...
4.  選取您想要使用的語言 (例如適用於 C\# 的 CS)，您會看到可以在 Visual Studio 中開啟的 Visual Studio 方案檔。
