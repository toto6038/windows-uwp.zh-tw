...
當使用者拖曳項目至您的 app 上，但尚未將其放下時，會引發 [**DragOver**][DragOver] 事件。 在此處理常式中，您必須使用 [**DragEventArgs.AcceptedOperation**][AcceptedOperation] 屬性指定 app 支援何種類型的作業。 最常見的是複製。

[!code-cs[主要](./code/drag_drop/cs/MainPage.xaml.cs#SnippetGrid_DragOver)]

## 處理 Drop 事件

使用者在有效放下區域釋放項目時，會發生 [**Drop**][Drop] 事件。 使用 [**DragEventArgs.DataView**][DataView] 屬性來處理這些項目。
