...
本主題改編自 [利用 Cortana 語音命令啟動背景 app](launch-a-background-app-with-voice-commands-in-cortana.md)。 我們在這裡以 **Adventure Works** 這個行程安排和管理 app，繼續示範各項功能。
...
2.  語音命令的開頭不可以是 **PhraseTopic** 元素，通常是只有大型詞彙語音辨識才會用到。 它的前面至少要有一個單字。
    ...
    當您說出語音命令時，其中包含您的應用程式名稱或名稱的某個部分，就可以儘量避免 **Cortana** 啟動您的應用程式。
