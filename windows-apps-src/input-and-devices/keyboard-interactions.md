...
本主題中，我們會著重在針對電腦鍵盤的 UWP app 設計上。 不過，完善的鍵盤體驗對支援協助工具例如 Windows 朗讀程式，使用軟體鍵盤例如[觸控式鍵盤](#touch-keyboard)和[螢幕小鍵盤 (OSK)](#osk)，以及處理其他輸入裝置類型例如 Xbox 遊戲台與遙控器，也是很重要的。

本文討論的許多指導方針和建議，包括[對焦視覺效果](#focus-visual)、[便捷鍵](#access-keys)和 [UI 瀏覽](#navigation)，也適用於這些其他案例。
...
必要時，我們會識別您應該知道的關鍵差異，並描述任何應考慮的緩和措施。

以下是本主題中討論的裝置和工具：

| 裝置/工具                       | 描述     |
|-----------------------------------|-----------------|
|鍵盤 (硬體與軟體)   |除了標準硬體鍵盤，UWP app 支援兩個軟體鍵盤：[觸控式（或軟體鍵盤）](#touch-keyboard)和[螢幕小鍵盤](#osk)。|
|遊戲台與遙控器         |Xbox 遊戲台與遙控器是[10 英呎經驗](designing-for-tv.md)中的基本輸入裝置。
如需 UWP 支援遊戲台與遙控器的特定詳細資訊，請參閱[遊戲台與遙控器的互動](gamepad-and-remote-interactions.md)。|
|螢幕助讀程式（朗讀程式）          |「朗讀程式」是 Windows 內建螢幕助讀程式，提供唯一互動體驗和功能，但仍依賴基本鍵盤瀏覽和輸入。
如需朗讀程式詳細資訊，請參閱[開始使用朗讀程式](https://support.microsoft.com/help/22798/windows-10-narrator-get-started)。|
...
- 支援鍵盤快速鍵
    ...
    - 提供便捷鍵以瀏覽您應用程式的 UI (請參閱[便捷鍵](access-keys.md))

### <a name="focus-visuals-a-namefocus-visual"></a>焦點視覺效果<a name="focus-visual">
...
### <a name="tab-stops-a-nametab-stops"></a>定位停駐點 <a name="tab-stops">

若要搭配使用控制項 (包括瀏覽元素) 與鍵盤，控制項必須有焦點。 讓控制項接收鍵盤焦點的方式是，將它辨識為應用程式 Tab 順序的定位停駐點，可透過 Tab 瀏覽來存取它。

若要將控制項包含在 Tab 順序，[**IsEnabled**](https://msdn.microsoft.com/library/windows/apps/br209419) 屬性必須設定為 **true**，而且[**IsTabStop**](https://msdn.microsoft.com/library/windows/apps/br209422) 屬性也必須設定為 **true**。

若要將控制項明確地從 Tab 順序排除，設定 [**IsTabStop**](https://msdn.microsoft.com/library/windows/apps/br209422) 屬性為 **false**。

根據預設，Tab 順序反映 UI 元素建立的順序。 例如，如果 `StackPanel` 包含 `Button`、`Checkbox` 和 `TextBox`，Tab 順序就是 `Button`、`Checkbox` 和 `TextBox`。

您可以藉由設定 [**TabIndex**](https://msdn.microsoft.com/library/windows/apps/br209461)屬性覆寫預設 Tab 順序。
...
所有的互動式控制項都應該有定位停駐點 (除非它們在[群組](#control-group)中)，而非互動式控制項 (例如 `labels`) 則沒有。

避免讓焦點在應用程式中四處跳躍的 Tab 順序。 例如，像表單的 UI 中的控制項清單應該會有由上至下和由左至右的 Tab 順序。

如需定位停駐點的更多詳細資料，請參閱[鍵盤協助工具](../accessibility/keyboard-accessibility.md)頁面。

#### <a name="try-to-coordinate-tab-order-and-visual-order"></a>試著調整 Tab 順序和視覺順序

調整 Tab 順序和視覺順序（也稱為讀取順序或顯示順序）減少使用者瀏覽應用程式 UI 時的混淆。

試著先同時依 Tab 順序和視覺順序排列並呈現最重要的命令、控制項及內容。 不過，實際的顯示位置可能取決於上層配置容器，以及會影響配置之子元素的某些屬性。 特別的是，使用格線隱喻或表格隱喻的配置可以有與 Tab 順序完全不同的視覺順序。

**注意** 視覺順序也是視地區設定和語言而定。

### <a name="initial-focus-a-nameinitial-focus"></a>最初的焦點<a name="initial-focus">

最初的焦點指定當應用程式或網頁第一次啟動或啟用時取得焦點的 UI 項目。 使用鍵盤時，使用者從此項目開始與應用程式的 UI 互動。

對於 UWP app，最初的焦點設定為可以接收焦點、具有最高 [**TabIndex**](https://msdn.microsoft.com/library/windows/apps/br209461) 的元素。 忽略容器控制項的子元素。 在平局，視覺化樹狀結構上的第一個元素會接收焦點。
...
#### <a name="dont-set-initial-focus-on-an-element-that-exposes-a-potentially-negative-or-even-disastrous-outcome"></a>請勿在可能暴露負面或甚至悲慘結果的元素上設定最初的焦點。

這個功能層級應該是使用者的選擇。 最初的焦點設定到會產生重要結果的元素，可能會造成意外的資料遺失或系統存取。 例如，瀏覽到電子郵件時不將焦點設定到刪除按鈕。

如需覆寫 Tab 順序的詳細資料，請參閱[鍵盤協助工具](../accessibility/keyboard-accessibility.md)頁面。

### <a name="navigation-a-namenavigation"></a>瀏覽 <a name="navigation">
...
-   **方向鍵**公開控制項特定的「內部瀏覽」。當使用者進入「內部瀏覽」時，方向鍵不會瀏覽移出控制項。 一些範例包括：
    -   向上/向下鍵在下列控制項內部移動焦點：`ListView` 和 `MenuFlyout`
    -   修改下列控制項的目前選取值：`Slider` 和 `RatingsControl`
    -   在下列控制項內部移動插入號： `TextBox`
    -   在下列控制項內部展開/摺疊項目： `TreeView`

使用這些預設行為，最佳化您的 app 鍵盤瀏覽。

#### <a name="use-inner-navigation-with-sets-of-related-controls"></a>使用「內部瀏覽」搭配相關控制項組

提供方向鍵瀏覽到相關控制項組，強化應用程式 UI 整體組織內控制項的關係。

例如，此處顯示的 `ContentDialog` 控制項為水平列按鈕預設提供內部瀏覽 (對於自訂控制項，請參閱[控制項群組](#control-group)一節)。

![對話方塊範例](images/keyboard/dialog.png)

***透過使用方向鍵瀏覽，相關按鈕集合的互動變得更容易***

如果在單欄中顯示項目時，向上/向下鍵瀏覽項目。 在單列中顯示項目時，向左/向右鍵瀏覽項目。 如果是多欄項目，所有 4 個方向鍵瀏覽。

#### <a name="make-a-set-of-related-controls-a-single-tab-stop"></a>讓一組相關控制項成為一個定位停駐點

讓一組相關 (或補充) 控制項成為一個定位停駐點，您可以在應用程式減少整體定位停駐點數目。

例如，下列影像顯示兩個堆疊的 `ListView` 控制項。 在左側的影像會顯示搭配定位停駐點使用方向鍵在 `ListView` 之間瀏覽，右邊的影像會顯示，透過不使用 tab 鍵周遊父控制項，子元素之間的瀏覽變得更容易且更有效率。


<table>
  <td>![方向鍵與 Tab 鍵](images/keyboard/arrow-and-tab.png)</td>
  <td>![僅限方向鍵](images/keyboard/arrow-only.png)</td>
</table>

***兩個堆疊 ListView 控制項的互動，排除定位停駐點，只使用方向鍵瀏覽，可變得更容易且更有效率。***
...
-   [快速鍵](#accelerators)是應用程式命令的捷徑。 您的應用程式不一定會包含準確對應到命令的 UI。 快速鍵包含 Ctrl 鍵和一個字母按鍵。
-   [便捷鍵](#access-keys)是連至應用程式中某部分 UI 的捷徑。 便捷鍵包含 Alt 鍵和一個字母按鍵。

請瀏覽 [Windows 中的鍵盤快速鍵](https://support.microsoft.com/help/12445/windows-keyboard-shortcuts)完整清單以及 Microsoft 所開發[應用程式特定的鍵盤快速鍵](https://support.microsoft.com/help/13805/windows-keyboard-shortcuts-in-apps)。

#### <a name="accelerators-a-nameaccelerators"></a>加速鍵 <a name="accelerators">

快速鍵可協助使用者快速執行應用程式上存在的一般動作。 提供一致快速鍵，讓使用者可以輕鬆地記住及跨所有提供類似工作的應用程式使用，對有用且強大的快速鍵功能非常重要。

快速鍵的範例：
-   在郵件 app 的任何位置按下 Ctrl + N 鍵，會啟動新郵件項目。
-   在 Edge 和市集應用程式的任何位置按下 Ctrl + E 鍵，可讓使用者快速地在搜尋方塊中輸入文字。
...
#### <a name="access-keys-a-nameaccess-keys"></a>便捷鍵 <a name="access-keys">

便捷鍵提供有協助工具需求的使用者和進階鍵盤使用者有效率地瀏覽應用程式 UI 的方式。
...
-   它們使用 Alt 鍵和英數字元按鍵。
-   它們是主要的協助工具。
-   使用[按鍵提示](access-keys.md)，它們直接記錄在 UI 中，與控制項相鄰。
...
下表是常用鍵盤命令的小型樣本。 如需完整的鍵盤命令清單，請參閱 [Windows 鍵盤快速鍵](https://support.microsoft.com/kb/126449)。
...
### <a name="control-group-a-namecontrol-group"></a>控制項群組 <a name="control-group">
...
-   `AppBarButtons` 在下列控制項之中： `CommandBar`
-   `ListItems` 或 `GridItems` 在下列控制項內部：`ListView` 或 `GridView`
-   `Buttons` 在下列控制項內部： `ContentDialog`

UWP 控制項預設支援方向鍵瀏覽。 對於自訂配置和控制項群組，使用 `XYFocusKeyboardNavigation="Enabled"` 提供類似的行為。

當您有下列控制項時，請考慮增加方向鍵瀏覽支援：
...
<table>
  <tr>
    ...
    <td>
      ...
      <p>**AppBarButtons**</p>
      <p>![list 和 grid 項目](images/keyboard/list-and-grid-items.png)</p>
...
根據應用程式的功能與配置，控制項群組的最佳瀏覽選項可能是使用方向鍵瀏覽至子元素的單一定位停駐點、多個定位停駐點或某種組合。
...
範例包含：
-   `RadioButtons`
-   多個 `ListViews`，其外觀及操作像單一 `ListView`
...
-   [XYFocusKeyboardNavigation](custom-keyboard-interactions.md#xyfocuskeyboardnavigation) 可讓方向鍵在控制項之間瀏覽
-   [TabFocusNavigation](custom-keyboard-interactions.md#tab-navigation) 指示有多個定位停駐點或單一定位停駐點
-   [FindFirstFocusableElement 和 FindLastFocusableElement](managing-focus-navigation.md#findfirstfocusableelement) 使用 **Home** 鍵將焦點設定在第一個項目上，使用**End** 鍵將焦點設定在最後一個項目上
...
***使用鍵盤***，*使用者使用* ***Enter**** 鍵提交搜尋查詢*

<table>
  <tr>
    <td>
      <p>![自動建議朗讀程式焦點](images/keyboard/auto-suggest-narrator-1.png)</p>
      <p>**使用「朗讀程式」**，*使用者可以使用 Enter 鍵來提交搜尋查詢*</P>
    </td>
    <td>
      <p>![自動建議朗讀程式焦點在搜尋上](images/keyboard/auto-suggest-narrator-2.png)</p>
      <p>*使用者也可以由 Caps Lock + 向右鍵存取搜尋按鈕，然後按空格鍵*</p>
    </td>
  </tr>
</table>
...
如需 UWP 支援遊戲台與遙控器輸入的詳細資料，請參閱 [Xbox 和電視設計](designing-for-tv.md#gamepad-and-remote-control)。
...
-   焦點瀏覽不限於控制項群組，使用者可以自由瀏覽到 app 中任何可設定焦點的 UI 元素。
    ...
    **注意** 焦點可以移到按下按鍵方向中任何可設定焦點的 UI 元素，除非它是在重疊 UI 中或已指定[焦點佔用](designing-for-tv.md#focus-engagement)，這會防止焦點鍵入/離開區域，直到使用 A 按鍵佔用/離開。 如需詳細資料，請參閱[方向瀏覽](#directional-navigation)一節。
...
#### <a name="directional-navigation-a-namedirectional-navigation"></a>方向瀏覽 <a name="directional-navigation">
...
不同於鍵盤，當應用程式選擇不利用[滑鼠模式](designing-for-tv.md#mouse-mode)，在整個應用程式中，方向瀏覽會套用到遊戲台與遙控器。 請瀏覽[XY 焦點瀏覽和互動文章](designing-for-tv.md#xy-focus-navigation-and-interaction)，以取得遊戲台與遙控器方向瀏覽最佳化的詳細資訊。
...
<table>
  <tr>
    <td>
      <p>![方向瀏覽](images/keyboard/directional-navigation.png)</p>
      <p>***支援的方向瀏覽*** </br>*使用方向鍵（鍵盤方向鍵、遊戲台與遙控器方向鍵）時，使用者可以瀏覽不同的控制項。*</p>
    </td>
    <td>
      <p>![沒有方向瀏覽](images/keyboard/no-directional-navigation.png)</p>
      <p>***不支援的方向瀏覽*** </br>*使用者無法使用方向鍵在不同的控制項之間瀏覽。 不會影響控制項之間的瀏覽其他方法（tab 鍵）。*</p>
    </td>
  </tr>
</table>

### <a name="built-in-keyboard-optimization-a-namebuilt-in-keyboard-optimization"></a>內建的鍵盤最佳化 <a name="built-in-keyboard-optimization">
...
<table>
  <tr>
    <td>
      <p>![以列為主順序鍵盤瀏覽](images/keyboard/row-major-keyboard.png)</p>
      <p>***以列為主順序鍵盤瀏覽***</p>
    </td>
    <td>
      <p>![以欄為主順序鍵盤瀏覽](images/keyboard/column-major-keyboard.png)</p>
      <p>***以欄為主順序鍵盤瀏覽***</p>
    </td>
  </tr>
</table>

#### <a name="popup-ui-a-namepopup-ui"></a>快顯 UI <a name="popup-ui">

如上所述，您應該嘗試確保方向瀏覽對應於您的 app UI 中的控制項視覺順序。

某些控制項，例如 `ContextMenu`、`AppBarOverflowMenu` 和 `AutoSuggest`，包括快顯功能表，在相對於主要控制項（根據可用螢幕空間）的位置和方向中顯示。 例如，當空間不足以讓功能表向下開啟時（預設方向），它會向上開啟。 並不保證功能表每一次都會以相同方向開啟。

<table>
  <td>![使用向下鍵命令列向下開啟](images/keyboard/command-bar-open-down.png)</td>
  <td>![使用向下鍵命令列向上開啟](images/keyboard/command-bar-open-up.png)</td>
</table>
...
您應該嘗試在自訂控制項中模擬這些相同行為。 如何實作這個行為的程式碼範例位於[管理焦點瀏覽](managing-focus-navigation.md#popup-ui-code-sample)文件。
...
### <a name="software-keyboard-a-nametouch-keyboard"></a>軟體鍵盤 <a name="touch-keyboard">
...
![Windows 10 觸控式鍵盤](images/keyboard/kbdpcdefault.png)

***Windows 10 觸控式鍵盤***

![Windows 10 手機觸控式鍵盤](images/keyboard/kbdwpdefault.png)

***Windows Phone 10 觸控式鍵盤***
...
![預設配置模式的觸控式鍵盤](images/keyboard/touchkeyboard-standard.png)

***預設配置模式的觸控式鍵盤***

![展開配置模式的觸控式鍵盤](images/keyboard/touchkeyboard-expanded.png)

***展開配置模式的觸控式鍵盤***

![拇指配置模式的觸控式鍵盤](images/keyboard/touchkeyboard-thumb.png)

***預設拇指配置模式的觸控式鍵盤***

![數字拇指配置模式的觸控式鍵盤](images/keyboard/touchkeyboard-numeric-thumb.png)

***數字拇指配置模式的觸控式鍵盤***

成功的鍵盤互動讓使用者只利用鍵盤就可以完成基本的應用程式操作；也就是說，使用者可以使用所有的互動式元素以及啟動預設的功能。 有許多因素會影響成功的程度，包括鍵盤瀏覽、協助工具的便捷鍵，以及進階使用者的快速鍵。

**注意**  觸控式鍵盤不支援切換和大多數的系統命令。

#### <a name="on-screen-keyboard-a-nameosk"></a>螢幕小鍵盤 <a name="osk">
